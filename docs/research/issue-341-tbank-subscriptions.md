...
с доказуемой связью с Account. #128 закрыт; блокирующая задача #341 на момент отчёта ещё открыта. #337 сохраняет собственную
область; этот отчёт даёт вход для его контракта, но не реализует источник событий. Приоритет и
выбранный источник с тех пор изменились: действующее состояние — в
