...
Production deploy и миграционные записи не выполнялись. Snapshot production до изменения
не является подтверждением после deploy. Локальные PostgreSQL и browser проверки используют
изолированные данные и внешние provider fixtures. PR #427 и #422 пока не приняты; проверка
их итогового объединения с этой задачей остаётся перед merge.
