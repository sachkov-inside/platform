...
## Concurrent owner changes and import

`tribute-sources.test.ts` “multirow import and generic %s use account locks before source mutations” runs both `change_term` and `revoke`. PostgreSQL blocking PIDs establish the contested schedule with a reversed two-Account batch. Both cases failed with a technical deadlock before the repair and pass after all Account locks precede source mutations. The owner decision succeeds; stale import returns revision conflict without a receipt, the other Enrollment stays unchanged, and replay preserves the owner decision. Full owning PG verification after the repair: 468 tests across 65 files; focused final source suite: 16 tests. These regressions supplement the earlier paired runtime checkpoints; they do not claim that every paired browser scenario was rerun.
