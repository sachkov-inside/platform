...
builds use the Next TypeScript API checker, which filters stale development route artifacts before
checking. Keeping one Next-managed config was rejected because `next typegen` restores the dev glob
...
managed project stops extending it, or Next stops selecting the managed project and API checker.
