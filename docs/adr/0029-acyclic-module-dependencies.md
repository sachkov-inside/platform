...
Рёбра, которые ещё замыкают цикл, перечислены в `legacyCycleEdges` того же скрипта. У каждого указан
самый сильный разрешённый вид импорта: `type` или `dynamic`. Импорт сильнее разрешённого снова
участвует в проверке. Если ребро исчезло или ослабло, guardrail требует убрать или понизить запись.
Новых записей в список не добавляют: новое ребро, которое замыкает цикл, разворачивается через port
потребителя или переносом общего значения ниже по графу.
...
- Список `legacyCycleEdges` может только сокращаться. Разворот любого ребра из таблицы — отдельное
  изменение с собственной задачей.
