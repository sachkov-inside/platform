...
- The published transport contract enumerated the inline `video` block the document schema stopped
  accepting, declared once at the wire boundary as a legacy variant. That variant is gone: the wire
  boundary holds no hand-written block, so the published enumeration follows the registry without
  exception.
