...
Решения [#529](https://github.com/sachkov-inside/platform/issues/529) и
[#540](https://github.com/sachkov-inside/platform/issues/540) заменяют все фрагменты этого brief
про acquisition CTA и внешнее приобретение, в Account и на закрытых материалах, где
`Получить доступ` ведёт на Platform-configured Tribute URL или внешний acquisition destination:
призыв к покупке ведёт на внутреннюю витрину или страницу оплаты руководства, а внешнего адреса
покупки и его настройки у Platform нет.

Выбор верхней шапки и логотипа в #311, реализуемый через #313, заменяет desktop sidebar. После уточнения владельца mobile сохраняет прежнюю нижнюю
навигацию без верхней шапки. Текущую структуру и поведение задаёт
...
- Library имеет один общий поиск, который независимо сопоставляет Серии по их name/summary и
  ...
  заголовка Materials и фильтруют полный material query in-place; `q`/`topic`/`format`/`sort`
  каноничны для reload/back/share, cursor остаётся внутренним. Существующие `/topics/:slug`
  discovery routes и Reader return context сохраняются.
...
| Library / search | Все | Найти Series или published Material общим поиском и уточнить только Materials локальными filters | Один real-data экран: независимые Series по `q`, затем Materials по `q` + `topic`/`format`/`sort`; cursor internal; closed body отсутствует в public results |
...
| Material | Все с actor-dependent body state | Прочитать teaser/free/closed body и использовать image/file/video | Только current `published` state; public projection отделена от protected body; related request отсутствует |
...
| Recent history / reading state | Authenticated human | Вручную mark read/unread | Не входит в Home #271; auto-mark отсутствует |
...
- `/` server-render-ит bounded Home projection; Topic и Series открываются из real-data discovery
  sections и cards.
...
- Material принимает безопасный back context только для Home, Library, Topic, Series или Profile и
  иначе возвращает в Library. Отдельной related выдачи в Reader нет.
...
Owner decision #271 supersedes Home/global-navigation/Library/Reader fragments in the older
wireframes below. Текущий production contract описан в начале brief; frames ниже остаются только
историческим rationale и не определяют route, navigation, filters, history или related content.
