...
- **Главная** — public entry point `/` и bounded mobile-first витрина из текущих published данных:
  Руководства, компактные Темы, затем новые Видео → Гайды → Заметки. Выбор Темы на Главной
  открывает Базу знаний с фильтром `topic` (`/library?topic=<slug>`). Для подтверждённого отсутствия
  подписки витрина начинается с первого опубликованного непустого руководства с аватаром автора; название,
  описание и ссылка берутся из данных главной. Приглашение оформить доступ находится перед Видео,
