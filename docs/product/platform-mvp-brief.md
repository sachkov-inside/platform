...
- на закрытом материале при любой причине deny видит один призыв к покупке внутри платформы —
  оплату выбранного руководства. Подписка не используется как запасной вариант. Без однозначного
  руководства призыв не показывается; protected body и связанные ресурсы не загружаются.
  Пока цена не опубликована, страница покупки объясняет, что условия готовятся.
...
руководства читателя встречает одна кнопка «Оплатить практикум». Такая покупка не создаёт подписку,
...
3. **Guide.** Практикум — самостоятельный продукт с упорядоченным составом материалов.
   Решением владельца 13.09.2026 публичное название формата — «Практикум» вместо
   «Руководство»: на главной, в описании, программе, уроках, покупке и редакторе.
   Главный продукт — «AI-first разработка», формат — «Практикум с сопровождением».
   Внутренние Guide/Series, API и адреса `/guides/` сохраняются. «Гайд» как формат
   отдельного материала не переименовывается. Ниже прежнее слово «руководство»
   обозначает тот же продукт; пользовательские подписи следуют новому названию.
   ...
   Продажа появляется здесь одной кнопкой «Оплатить практикум» и только когда читателю чего-то не
   ...
   Общий порядок обозначен внутри карточек как «1 урок», «2 урок»; изученное — галочкой справа,
   текущее — широким действием «Продолжить» в карточке. Закрытый доступ обозначен замком без размытой обложки.
   Программа центрирована с шириной до 736 px: компактные строки с названием урока и временем
   видео, без описаний и результатов обучения. Превью уроков квадратные с центральным кадрированием
   и fallback без обложки; миниатюра скрывается при нехватке места. Номера глав не показываются. Увеличенный текст свободно переносится.
   Сверху — плашка руководства, количество глав и материалов, прогресс вошедшего читателя.
   Табы «Программа», «Дополнительно» и «Артефакты» видимы даже без наполнения.
   Обложки материала в ленте рекомендуются в формате 16:9. Формат не назначает последовательность
...
контекст. Единый нижний блок показывает позицию и название руководства, «Все материалы практикума»,
...
служит «Все материалы практикума».
...
- **Главная** — `/`: одно закреплённое автором руководство и общая лента всех опубликованных
  материалов. Руководство — главный продукт. Решением владельца 13.09.2026 оформляется
  «AI-first разработка» (`working-with-agents`): инженерная база, процесс работы с агентами
  и практики production-разработки, освоенные на проекте. Авторский выбор не заменяется
  ...
  проверки → приложение»; у AI-first разработки — «контекст и агенты → архитектура и функции →
  проверки и выпуск». Один проход за 4,8 секунды, без кнопки паузы, с reduced motion.
  Название и действие видимы постоянно.
...
- **страница руководства** — metadata, поля руководства для читателя, полный упорядоченный состав
  материалов и отдельные части «Дополнительно» и «Артефакты». Программа показывает сводку
  и прогресс над табами; derived-список тем не выводится. Locked Material сохраняет своё место.
...
сложность и короткий список «Чему научишься» — в самом уроке. Решением владельца 13.09.2026
программа использует компактные строки без этих подробностей; сверху показывает плашку руководства,
статистику и сохранённый прогресс. Табы «Программа», «Дополнительно», «Артефакты» видимы всегда,
а пустые разделы объясняют своё назначение.
