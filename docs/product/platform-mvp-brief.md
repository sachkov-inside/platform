...
[#313](https://github.com/sachkov-inside/platform/issues/313). Публичные страницы используют общую
...
«Войти». На mobile полное название, поиск и вход остаются видимыми; те же разделы открываются
кнопкой меню. Меню закрывается после перехода или Escape и возвращает фокус кнопке при закрытии.
Текущий раздел обозначен текстом и выбранным состоянием; Topic, Series и Reader относятся к Базе
знаний. После входа кнопка «Аккаунт» открывает Профиль и выход. Недоступный статус сессии даёт
явное действие завершения сессии. Редактор виден только при `materials:manage` и сохраняет свой
отдельный рабочий экран.

Шапка остаётся сверху при прокрутке. На desktop прокручивается содержимое, на mobile — страница.
Левый сайдбар, нижняя мобильная панель, повтор названия над страницей и строка якорей Главной
убраны. Карта остаётся доступной по прямому URL; будущие Roadmaps и Мастерская не добавляются в
навигацию до отдельного решения.
