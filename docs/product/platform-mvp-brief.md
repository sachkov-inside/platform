...
- **Главная** — public entry point `/` и bounded mobile-first витрина из текущих published данных:
  закреплённая автором Серия с его изображением, затем Серии, компактные Темы и новые Видео →
  Гайды → Заметки. Автор закрепляет одну Серию значком в общем списке серий; отдельные Materials
  ...
  на Серию берутся из текущей публичной проекции, body не загружается. Без выбора блок отсутствует,
  автоматической подстановки первой Серии нет. Архивная Серия или Серия без опубликованных
