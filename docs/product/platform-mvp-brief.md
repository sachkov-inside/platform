...
Контекстные переходы между материалами принадлежат страницам тем и плейлистов. Reader первой
версии не запрашивает и не показывает отдельный блок связанных материалов. Community и обсуждения
...
- **Главная** — public entry point `/` и bounded mobile-first витрина из текущих published данных:
  компактные темы, затем Видео → Плейлисты → Гайды → Заметки; history и `Продолжить` отсутствуют;
- **База знаний** — единый real-data экран тем, плейлистов и материалов; один поиск сопоставляет
...
Публичная navigation показывает Главную и Базу знаний на desktop, а mobile dock — Главную, Базу
знаний и Профиль. Карта остаётся доступной по прямому URL, но не является navigation item.
