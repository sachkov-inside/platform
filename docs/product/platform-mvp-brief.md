...
3. **Series.** Серия является упорядоченной последовательностью и в русском интерфейсе называется
   ...
   описание под заголовком на широком экране. На мобильном в маршруте скрыты описание, тема
   и текст продолжения: текущий материал выделен визуально. Статус просмотра обозначен галочкой
   в номере, закрытый доступ — размытым превью со значком замка; повторяющиеся текстовые статусы
   скрыты. Длительность видео показана бейджем под превью. Формат и последовательность шагов сохраняются. Нумерация шагов учитывает только опубликованный состав.
