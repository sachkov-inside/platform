...
- создаёт или открывает Account кодом из почты либо подтверждением в Telegram-боте,
  без отдельной формы регистрации; Telegram-вход доступен при серверном включении функции;
...
До регистрации через Telegram пользователь видит предложение войти в существующий Account по
почте и подключить Telegram в кабинете. Подтверждённая связь позволяет затем входить в тот же
