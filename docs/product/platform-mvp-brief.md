...
3. **Series.** Серия является упорядоченной последовательностью и в русском интерфейсе называется
   ...
   серии; эти связи сохраняются в редакторе, но не показываются отдельными «Шаг n из m» в публичных карточках.
...
служит «Все материалы серии».
Отметка видео подписана «Просмотрено» в обоих состояниях; состояние различается значком,
оформлением и `aria-pressed`.
Reader не показывает под текстом хэштеги и плашки принадлежности к сериям: эти метаданные
используются в поиске и каталоге. При прохождении серии сохраняются переходы к следующему
...
- **Главная** — public entry point `/` и bounded mobile-first витрина из текущих published данных:
  Серии, компактные Темы, затем новые Видео → Гайды → Заметки. Выбор Темы на Главной
  открывает Базу знаний с фильтром `topic` (`/library?topic=<slug>`). Для подтверждённого отсутствия
  подписки витрина начинается с первой опубликованной непустой Серии с аватаром автора; название,
  ...
  главной и пустые коллекции сохраняют свои состояния, фиктивной серии нет. [Personal Home](../specifications/personal-home.md)
...
В Базе знаний сначала видны первые три серии. Если их больше, кнопка «Показать все» раскрывает
оставшиеся на этой же странице; «Свернуть» возвращает компактный список. При новом поисковом
запросе список снова показывает первые три результата. При трёх и менее сериях кнопки нет.
...
Topic/Series представлены компактными карточками. Поля сохраняются автоматически; состав Series
...
Страница серии показывает прогресс по всему опубликованному составу, действие продолжения
