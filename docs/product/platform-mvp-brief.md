...
контекст. Единый нижний блок показывает позицию и название серии, «Все материалы серии»,
«Дальше» при наличии следующего шага и небольшую ссылку на предыдущий. Закреплённой верхней
кнопки возврата и повторного чипа выбранной серии с номером нет. Обычный возврат к источнику
всегда находится сверху. Вне серии нижний возврат появляется после прокрутки за верхнюю кнопку;
на странице, которая целиком помещается в экран, он не дублируется. В серии нижним возвратом
служит «Все материалы серии». Другие серии материала доступны без повторения номера шага.
Отметка видео подписана «Просмотрено» в обоих состояниях; состояние различается значком,
оформлением и `aria-pressed`.
Reader первой версии не выводит отдельный derived-блок связанных материалов. Community и обсуждения
...
- **Главная** — public entry point `/` и bounded mobile-first витрина из текущих published данных:
  Серии первыми, компактные Темы, затем новые Видео → Гайды → Заметки и переход в общий
  каталог. Плашки с предложением подписки на главной нет для всех посетителей; предложение
  оформить доступ показывается при открытии закрытого материала. [Personal Home](../specifications/personal-home.md)
  помещает начатую серию и недосмотренное видео первыми в обычных секциях. Продолжение серии
  открывает её маршрут с выделенным материалом;
...
- **страница серии** — metadata, полный упорядоченный состав материалов. Повторный счётчик и
