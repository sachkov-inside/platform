...
- **Главная** — public entry point `/` и bounded mobile-first витрина из текущих published данных:
  закреплённый автором Material с его изображением, затем Серии, компактные Темы и новые Видео →
  Гайды → Заметки. Автор выбирает один опубликованный материал в списке авторских материалов;
  замена и снятие закрепа сохраняются независимо от истории посетителя. Этот блок одинаков для
  гостей и участников; название, описание, формат и ссылка берутся из текущей публичной проекции,
  body не загружается. Без выбора блок отсутствует, автоматической подстановки Серии нет.
  Снятый с публикации материал скрывает блок; настройка сохраняется и снова действует после
  публикации, пока автор не заменит или не снимет закреп. Устаревшая вкладка получает конфликт
