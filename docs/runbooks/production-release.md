...
| `rabbitmq` (новый) | 150 MiB | 768 MiB и порог памяти брокера — в `compose.production.yaml` |
| **Всё приложение и брокер** | **≈1850 MiB** | |

Пределы (#688) — предохранитель: процесс с утечкой памяти или лавиной потоков упирается в свой
`mem_limit` и `pids_limit` (256, у брокера 512) и перезапускается, не забирая память PostgreSQL и
Logto. Это не бюджет: сумма пределов больше памяти сервера, одновременный рост всех процессов они не
