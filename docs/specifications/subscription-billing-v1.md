...
## Текущая поставка #410

Billing поставляет поводы служебных сообщений подписки и подключает их к общему
[Notifications](notifications-v1.md). Собственного отправителя, шаблонов, настроек каналов и
доставок здесь нет: Billing отвечает только за факт повода и за ответ «повод ещё актуален?».

Повод существует один раз на пару «назначение + источник», хранит своё название, сумму и дату из
подтверждённого снимка операции и растёт неизменяемыми revision. Повод и событие для outbox
фиксируются в той же транзакции, что и сам факт: подтверждение оплаты, отказ по расписанию, отмена
продления и конец оплаченного срока. Сбой канала поэтому не может отменить платёж или право, а
откат платежа не может оставить обещание сообщения.

| Повод | Факт-источник | Ключ | Сумма и дата |
|---|---|---|---|
| `payment_succeeded` | Первое проверенное CONFIRMED любой попытки | `payment:<attemptRef>` | Списанная сумма, конец оплаченного срока |
| `payment_failed` | Однозначный отказ попытки продления | `payment:<attemptRef>` | Сумма попытки |
| `renewal_reminder` | Календарь за три дня до списания | `subscription:<ref>:period:<next>` | Цена и дата следующего периода |
| `renewal_cancelled` | Команда отмены расписания | `subscription:<ref>:canceled:<revision>` | Конец оплаченного срока |
| `access_expired` | Завершение подписки любой причиной | `subscription:<ref>:ended` | Конец оплаченного срока |
| `refund_resolved` | Решение о возврате | — | Производителя пока нет: подключается поверх операций #409 |

Об отказе сообщаем только по списанию без покупателя: результат своей попытки в браузере он видит
сразу, а продление по расписанию иначе останется незамеченным. `refund_resolved` описан и
поддержан записью повода, но производителя не получает: решение о возврате появилось вместе с
операциями #409, и повод подключается к нему отдельным шагом. Неизвестный банковский исход не
выдаётся за исполненный возврат.

Срок сообщения считается от самого события, а не от момента, когда его заметили: сутки для
события жизненного цикла и момент списания для напоминания. Конец оплаченного срока поэтому
датируется этим концом, и замеченный с большим опозданием повод истекает вместо ложной новости.

Напоминание относится только к предстоящему списанию действующего расписания. Отмена, отзыв
привязки, согласованное изменение варианта и завершение подписки закрывают его до любой отправки;
изменение даты, суммы или состава выпускает следующую revision того же повода. Публичный ответ
источника подтверждает актуальность собственными фактами перед раскрытием аудитории и перед каждой
внешней отправкой: прежняя revision после отмены или смены условий получает `superseded`, а
временная недоступность — `unavailable` и повторный вопрос, но не отправку.

Календарь берёт только подписки внутри окна напоминания, у которых действующего повода ещё нет,
поэтому очередь длиннее одного пробега не оставляет последние подписки без напоминания.
Подтверждённое списание, отмена, отзыв привязки и согласованное изменение закрывают прежний повод
в собственной транзакции, а не ждут следующего пробега.

`billing-worker` получает очередь `billing.subscription-notices` с этим календарём. Кабинет
`currentBilling` возвращает собственные поводы Account; состояния доставок остаются за
Notifications и читаются его собственными endpoint. Прав на доступ поводы не выдают и не отменяют.
