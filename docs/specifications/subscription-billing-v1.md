...
Известное ограничение поставки: собственные независимые права покупатель списком не видит.
Чтение оснований есть только владельческое (`grants.read`), поэтому кабинет говорит о них честно —
ранее выданные права остаются в силе на своих условиях и подпиской не заменяются, — а пересечение
показывается там, где оно решает исход: `existing_access` при покупке. Ручная выдача нигде не
называется покупкой. Собственное чтение оснований и история списаний требуют нового endpoint и
вынесены в [#485](https://github.com/sachkov-inside/platform/issues/485).
