...
явного согласия на списания и проходит ту же проверку legacy, что и покупка. Служебные сообщения
ведут в раздел подписки: `BILLING_CABINET_PATH` остаётся `/account/subscription`. Раздел
«Подписка» показывается, когда каталог отдаёт хотя бы один вариант подписки или когда подписка
у человека есть.
