...
Платёжный кабинет `/account/subscription` объясняет действующий тариф, состав прав и их сроки,
оплаченный срок, сумму текущего периода, следующее списание, способ оплаты, запланированное
изменение и незавершённую попытку. Отсюда доступны отмена и возобновление продления, повышение и
запланированная смена варианта, отмена запланированного изменения, привязка другой карты и запрет
использования сохранённой. Возобновление требует нового явного согласия на списания и проходит ту
же проверку legacy, что и покупка. Служебные сообщения ведут в этот кабинет:
`BILLING_CABINET_PATH` теперь `/account/subscription`.
...
Форма `/account/email` подтверждает контакт; итоговые checkbox и checkout поставлены в #411 и
используют тот же серверный каталог документов. Она использует production BFF/API; Storybook
