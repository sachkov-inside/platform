...
Оформление идёт строго в одном порядке: сохранённый расчёт `quote` → раздельные согласия →
покупка. Ни один флажок не отмечен заранее, включая согласие на регулярные списания; до публикации
...
бессрочно: наследовать нечего, потому что оплаченного периода у такой покупки нет. Владелец может
назвать праву свой срок в месяцах, и тогда он считается от подтверждения оплаты.
...
`resolveCapabilities` выводит `community` из любого действующего права `guide:<id>` и складывает
...
перенаправляет туда же. Итоговые checkbox и checkout поставлены в #411 и используют тот же
серверный каталог документов. Она использует production BFF/API; Storybook
