...
Каталог, ручные права и классификация покупателя проверяют это же право (#523): классификация
решает, разрешены ли автосписания, поэтому она принадлежит владельческому набору billing, а не
отдельному `platform:admin`.
...
`grants.read|readClassification|classify|previewBatch|applyBatch|extend|revoke`.
`deliveries.read|resolve` остаются за
...
возврате и об отзыве требует видеть действующие основания и их историю. `grants.readClassification`
и `grants.classify` добавлены в #523: без явного решения владельца классификация остаётся
«неизвестно», при которой покупка подписки отклоняется кодом `legacy_review_required`. Массовая
классификация идёт строками того же `grants.previewBatch|applyBatch`, поэтому второго механизма
набора нет; устаревшая revision любой строки классификации отменяет применение целиком.
...
`refunds.decide` фиксирует сумму, основание возврата (`withdrawal|compensation`) и судьбу
автопродления (`keep|cancel`) с причиной, ничего не отправляя в банк. Основание решает доступ
(#648, решение владельца): отказ от договора (`withdrawal`) после подтверждённого возврата отзывает
права этой покупки, компенсация без отказа (`compensation`) доступ сохраняет. Решение хранит и
выведенный `access` (`revoke|keep`); у решений до #648 основание `null`, а доступ выбирал владелец.
Сумму вводит владелец: доли цены из оферты сервер пока не считает. Сумма ограничена остатком: подтверждённые
...
| `refund_resolved` | Подтверждённый банком возврат (#681) | `refund:<refundRef>` | Возвращённая сумма |

Об отказе сообщаем только по списанию без покупателя: результат своей попытки в браузере он видит
сразу, а продление по расписанию иначе останется незамеченным. `refund_resolved` записывается в
той же транзакции, что подтверждает попытку возврата: одна попытка — один повод, поэтому
повторное исполнение и сверка второго сообщения не создают, а каждый частичный возврат сообщается
своей суммой. Отказ банка и неизвестный исход поводом не становятся: неизвестный исход не
выдаётся за исполненный возврат. История списаний покупателя показывает у платежа
подтверждённую возвращённую сумму и дату последнего возврата.
...
## Текущая поставка #411

#411 поставляет покупателю и владельцу собственные страницы поверх уже принятых операций
#406–#410. Новых банковских правил, цен и коммерческих решений она не вводит: состав, сумма, срок
и доступность приходят с сервера, а страница только объясняет их и передаёт команду.

Витрина `/subscription` показывает действующие варианты каталога с их точной первой ценой,
ценой продления, длительностью и составом прав. Страница руководства ведёт сюда CTA, когда часть
материалов закрыта, и сохраняет исходный адрес: после входа покупатель возвращается на витрину с
тем же контекстом. Отдельная покупка незавершённого руководства и неактивные предложения не
выводятся; две карточки не зашиты как единственная модель каталога — витрина рендерит то, что
вернул `billing/offers`.

Оформление идёт строго в одном порядке: сохранённый расчёт `quote` → принятие документов нажатием
кнопки оплаты → покупка. Отметок нет (путь A, Workspace #185, #658): кнопка «Оплатить N ₽» или
«Оформить подписку и оплатить N ₽» принимает оферту, а у подписки и автопродление на сумме, дне
следующего списания и периоде, названных строкой под кнопкой; до публикации
документов #412 оформление честно сообщает, что принять оплату нельзя. Подтверждённый контакт
обязателен и подключён формой #406, поэтому временный маркер `#406/#411` снят: production и
Storybook используют один presentation interface `BillingContactFormProps`. `existing_access`
показывает уже имеющиеся права и требует явного подтверждения, `legacy_review_required` остаётся
блокировкой без кнопки оплаты.

Возврат из банка — отдельная страница `/subscription/return`, а не признак успеха. Терминал
настраивается одним `returnUrl`, поэтому он должен указывать на этот путь; страница читает
собственное состояние покупки у сервера и показывает банковское состояние, готовность доступа и
результат чека раздельно. Ссылка на начатую покупку хранится в `sessionStorage` браузера; её
отсутствие ведёт в кабинет, а не выдаёт неизвестный исход за успех.

Личный кабинет разделён в #494 на разделы с боковой навигацией. Раздел «Покупки»
`/account/purchases` объясняет действующие основания доступа и их сроки, историю собственных
списаний и чеков, подтверждённый email и способ оплаты; отсюда доступны привязка другой карты и
запрет использования сохранённой. Раздел «Подписка» `/account/subscription` объясняет действующий
тариф, оплаченный срок, сумму текущего периода, следующее списание, запланированное изменение и
незавершённую попытку; отсюда доступны отмена и возобновление продления, повышение и
запланированная смена варианта, отмена запланированного изменения. Возобновление требует нового
явного согласия на списания и проходит ту же проверку legacy, что и покупка. Раздел «Подписка»
показывается, когда каталог отдаёт хотя бы один вариант подписки или когда подписка у человека
есть; раздел «Покупки» виден всегда. Поэтому служебные сообщения ведут в «Покупки», где показана
их история и где чинят способ оплаты: `BILLING_CABINET_PATH` — `/account/purchases`.

Владельческая страница `/authoring/billing` выполняет операции #409 через те же API и полномочие
`billing:manage`: состав и версии предложений, вариантов и скидок, чтение и сверка платежей,
решение и исполнение возврата, отмена продления участника, чтение прав, продление и отзыв
основания, чтение и запись классификации покупателя, предпросмотр и применение партии. Каждая операция имеет собственный маршрут BFF и
собственный `operationId`, поэтому дискриминатор владельческой команды не превращается в
универсальный прокси браузера. Архивные позиции каталога здесь не перечисляются; выключенные из
продажи варианты остаются в списке, чтобы владелец мог вернуть их в продажу (#493).

Конверт `GET accounts/current/billing` расширен, как и предусматривал контракт #408: к подписке и
служебным поводам добавлены собственные основания доступа и история списаний. Путь не менялся.

Основание описывает состав, срок и то, чем оно выдано (`paid | manual | legacy`). Отозванные и
истёкшие основания не показываются, потому что вопрос кабинета — что доступно сейчас. Ручная
выдача называется выдачей, а не покупкой, и переживает окончание подписки: отзыв оплаченного
основания не трогает независимое. Операторские поля — причина, ссылка на источник, исполнитель и
история изменений — остаются за владельческим `grants.read`.

История списаний показывает последние 50 попыток Account: состав, сумму, состояние, дату
подтверждения, результат чека и ссылку на конкретную операцию для поддержки. Терминал, окружение и
идентификатор платежа провайдера остаются владельческими и покупателю не видны.

Поставка не включает production activation, реальный терминал, реальные возвраты и выдачи, а также
публикацию юридических страниц: они остаются за #402/#412/#413/#414 и отдельным разрешением
владельца.

## Текущая поставка #493

Продажа подписки становится включаемой опцией владельца, а не постоянным свойством каталога.
У предложения появляется обратимый признак `published`, независимый от `archived` и по умолчанию
выключенный. Пока ни один неархивный вариант не включён, подписка не предлагается: `GET
billing/offers` возвращает пустой список, блок на главной, CTA в закрытом материале и раздел
подписки в кабинете не показываются. Про входы, которые этим признаком тогда закрыты не были,
смотри «Текущая поставка #507»; последний из них, приглашение к тарифам в программе руководства,
закрывает #509. Архивация — окончательное снятие: с #648 она сбрасывает `published`, а
`offers.save` и `offers.publish` архивного предложения возвращают `not_found`.
...
поверхности `billing/admin`: `operationId`, проверка `expectedRevision`, audit и replay общие с
остальным каталогом. Выключение обратимо и не создаёт новое коммерческое предложение: предложение,
его состав и варианты не пересоздаются, а повторное включение возвращает продажу. Владельческий
`offers.list` (чтение, не пишет receipt) видит и выключенные варианты, поэтому
`/authoring/billing` может вернуть их в продажу; публичная витрина видит только включённые.

Выключение не отменяет действующие подписки, не отзывает права и не сокращает оплаченный срок.
Продление идёт по принятым условиям подписки, а не по текущему каталогу. Покупка нового варианта и
смена варианта на выключенный отклоняются сервером (`not_found`) в quote, reserve и change, а не
только прячутся интерфейсом. Разовая продажа руководства и изменение цен сюда не входят.
...
предложения. Отсюда следует «по умолчанию не продаётся ничего»: своего признака продажи разовой
продаже не нужно, она подчиняется тому же обратимому тумблеру предложения, что и подписка (#493),
а витрина и кнопка покупки появляются и исчезают вместе с ним.

Разовая покупка проходит тем же путём расчёта, согласий, оплаты и возврата, что и подписка, но:

- не требует согласия на регулярные списания и не принимает его: `consentEvidenceRefs` для
  `one_time` содержит только оферту;
- не проверяет классификацию старой подписки: она ограничивает recurring, которого здесь нет;
- не занимает место единственного subscription lifecycle Account и не создаёт подписку;
- просит банк об оплате без сохранения привязки (`OperationInitiatorType: 0`, без `Recurrent`);
- не имеет оплаченного срока: `periodEndsAt` подтверждённой разовой покупки — `null`, и повод
  `payment_succeeded` не обещает дату следующего списания.
...
открытое право остаётся одним: два бессрочных основания не удваивают доступ и не удлиняют его.

У сохранённого варианта способ продажи не меняется: он входит в принятые условия уже совершённых
покупок. Сколько способов у одного предложения — решает владелец; запрета смешивать их нет.
`GET billing/offers` принимает фильтры `mode` и `capability`, поэтому витрина подписки и витрина
руководства спрашивают один каталог о разном.

Читатель встречает продажу по одному маршруту, одинаковому для каждого продаваемого руководства:
страница продукта `/guides/<slug>` не называет цену и ведёт в программу, программа
`/guides/<slug>/programme` показывает одну кнопку «Оплатить сейчас», а цену, состав покупки и
оформление несёт страница оплаты `/guides/<slug>/buy`. Разделение намеренное: до цены читатель
...
Кнопка в программе появляется там, где читателю чего-то не хватает: предложение разрешает продажу,
но у кого руководство уже открыто, тому предлагать покупку нечего. Руководству без разового
варианта остаётся прежний путь — приглашение к тарифам подписки. Страница оплаты при этом
показывает цену по прямому адресу независимо от этого условия.

Страница оплаты не знает о разовости: она получает варианты предложения этого руководства и
показывает их составом и ценой, а при нескольких вариантах — выбором. Поэтому подписка на
руководство и несколько тарифов не потребуют второй страницы, когда владелец их заведёт.
...
Одна открытая попытка оплаты на Account сохраняется: разовая покупка и покупка подписки делят тот
же резерв цены, поэтому вторая одновременная попытка получает `payment_in_progress`.

## Текущая поставка #507

Витрина `/subscription` по прямому адресу отличает выключенную продажу от недоступного сервера.
Пустой каталог при успешном чтении — это решение владельца, а не сбой, поэтому страница не
показывает ни название тарифов, ни их состав, ни выбор варианта и не зовёт зайти позже за тем, что
не появится: она сообщает, что подписка сейчас не продаётся. Когда каталог прочитать не удалось,
сохраняется прежнее сообщение о временной недоступности.

Вход в платёжный кабинет вынесен из состояний каталога и зависит только от чтения самой подписки,
поэтому её владелец находит вход и при выключенной продаже, и при непрочитанном каталоге.

Кабинет тоже перестал звать на витрину, когда продавать нечего. Ссылка «Посмотреть тарифы» в
разделе «Покупки» у человека без действующих оснований доступа и такая же ссылка в разделе
«Подписка» у человека без действующей подписки появляются только при включённой продаже; там же
завершённая подписка больше не обещает новый срок «на витрине». Разделы берут адрес витрины от
своей страницы, а страницы решают по `useSubscriptionOffered`. Это то же правило и тот же каталог
кабинета, по которым навигация решает видимость раздела «Подписка»; рамка применяет правило к
своему каталогу напрямую, потому что провайдер стоит ниже неё. Раздел «Покупки» навигация не
прячет никогда, а раздел «Подписка» открывается по прямому адресу, поэтому каждый проверяет
состояние продажи сам. Непрочитанный каталог кабинет считает отсутствием продажи и ссылку не
показывает: в кабинете это безопасная сторона, а отличать сбой от выключенной продажи — работа
витрины.

Незакрытой на момент этой поставки оставалась страница руководства, и таких мест там было два:
у руководства без своей цены блок «Часть материалов открыта по подписке» вёл к тарифам, а у
руководства с ценой блок «Купите это руководство» нёс вторую ссылку «Посмотреть подписку». Оба
показывались независимо от состояния продажи и появились до этой поставки. Тот же блок переписывает
#509, переносящий покупку в программу руководства, и там же это место закрыто.

Не закрыто и описание маршрута `/subscription`: оно статично и в предпросмотре ссылки и поисковой
выдаче обещает подписку даже при выключенной продаже. Карточки ссылок и индексацию поставил #506,
но описание этого маршрута он не менял. Вывести его из каталога значит читать каталог на каждый
запрос ради мета-тега, поэтому отдельного решения владельца это место ещё ждёт.

После #509 незакрытым остаётся только описание маршрута, поэтому правило «при всех выключенных
вариантах ни одна страница не предлагает подписку» выполняется везде, кроме мета-тега.

## Текущая поставка #508

Оплата, выдача прав и проверка доступа проверяются одной матрицей, а не разрозненными наборами.
`apps/backend/test/integration/payment-access-matrix.test.ts` проходит путь покупки целиком на
реальном PostgreSQL: расчёт, согласия, ответ банка, выдача права и открытый материал руководства
вместе с его файлом, видео и артефактом. Денежные события воспроизводит двойник провайдера;
реальные платежи не выполняются.

Матрица связывает три слоя и опирается на уже поставленные наборы, а не повторяет их:

- оплата — повтор, опоздание и неизвестный ответ банка не создают второго права и второго
  списания, а отказ не выдаётся за успех. Расчёт цены, снимок условий, согласия, продление,
  отмена и возврат остаются в `billing-pricing`, `billing-payments`, `billing-subscriptions`,
  `billing-one-time-purchase` и `billing-operations`;
- выдача прав — область, источник, срок или явная бессрочность права; несколько оснований
  складываются, выдача владельческой операцией не становится покупкой, а выключение продажи не
  отзывает уже выданное. Независимость оснований и их история принадлежат `account-access`;
- проверка доступа — гость, участник, покупатель руководства, участник с истёкшей подпиской и
  покупатель с истёкшей подпиской, но действующим правом на руководство. Сюда же входят общий
  материал двух руководств, прямой адрес в обход витрины, закрытая глава программы и доставка
  файлов, видео и артефактов через ту же проверку.

## Текущая поставка #524

Решение владельца от 12.09.2026: продаются и руководства, и подписка, и любое купленное
...
отменяет и не удваивает.

Отзыв и возврат работают тем же durable-путём: снятие одного основания при другом действующем не
забирает участие, а снятие последнего — забирает. Возврат покупки руководства по отказу от договора
отзывает её оплаченное право и сопровождение, а вместе с ними участие, если другого основания нет.
Общую группу открывает и сопровождение (#648): сопровождение другого основания держит участие.

Проекция желаемого состояния и outbox `CommunityEntitlements` остаются без изменений по контракту:
меняется только то, что считается основанием. Проверка доступа к материалам и чтению не меняется:
право на руководство по-прежнему не открывает библиотеку.

Право на руководство уже открывает чат, поэтому предложение с `community` в составе пересекается
с ним: покупка старшего тарифа после покупки руководства требует того же подтверждения
пересекающихся прав, что и любое другое пересечение состава.
...
Архивирование предложения окончательно (#648): Save и publish архивного предложения отклоняются,
...
поля/operations отвергаются. `refunds.decide` содержит отдельные решения о сумме, основании возврата и
...
Форма контакта подтверждает адрес внутри раздела «Покупки»; прежний адрес `/account/email`
перенаправляет туда же. Checkout поставлен в #411 и использует тот же серверный каталог
документов; отметки согласий #411 заменены в #658 принятием кнопкой. Она использует production BFF/API; Storybook
подставляет только presentation.
...
условий, отдельно recurring consent и иные применимые согласия. С #658 запись журнала принятия
(`accounts.legal_acceptances`) хранит также экран, подпись нажатой кнопки и, для подписки и
возобновления, показанные условия продления такими, какими их показала страница; billing их не
пересчитывает, а согласие на списания без них не принимается. Публикация новой редакции не
...
Строка того же набора может вместо выдачи нести классификацию: rowKey, target Account, ожидаемую
revision классификации и её состав. Вид строки читается по наличию условий выдачи; один Account
классифицируется в наборе один раз. Применение проверяет revision всех таких строк под замками до
первой записи, поэтому устаревшая строка отменяет весь набор, а не часть его.
...
Classification хранит sourceRef, reason, verifiedAt и revision; её пишут одиночная владельческая
операция и строка набора, обе через одного writer. Только явный `confirmed_legacy`
