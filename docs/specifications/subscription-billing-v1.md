...
**Открытый вопрос владельцу:** стандартные карточные ответы банка не передают точное время оплаты.
Предложено закреплять первый период от первого проверенного сервером CONFIRMED. Пока правило
не принято, production composition не задаёт время подтверждения и admission недоступен.
Fixtures используют предложенное правило только для проверки транзакций и восстановления.
Это пока не заменяет принятое требование точного payment instant ниже.
