...
| `refund_resolved` | Решение о возврате | — | Ждёт операторских возвратов #409 |

Об отказе сообщаем только по списанию без покупателя: результат своей попытки в браузере он видит
сразу, а продление по расписанию иначе останется незамеченным. `refund_resolved` поставляется
вместе с решением о возврате: неизвестный банковский исход не выдаётся за исполненный возврат.
