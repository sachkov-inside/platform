...
остальным каталогом. Выключение обратимо и не создаёт новое коммерческое предложение: повторное
включение возвращает прежнюю редакцию продаваемой. Владельческий `offers.list` (чтение, не пишет
receipt) видит и выключенные варианты, поэтому `/authoring/billing` может вернуть их в продажу;
публичная витрина видит только включённые.
