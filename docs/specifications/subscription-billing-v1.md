...
Одно предложение продаётся одним способом, и у сохранённого варианта способ не меняется: он входит
в принятые условия уже совершённых покупок. `GET billing/offers` принимает фильтры `mode` и
`capability`, поэтому витрина подписки и витрина руководства спрашивают один каталог о разном.
