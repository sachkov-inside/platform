...
Статус: модель согласована владельцем 2026-09-06; реализация ещё не поставлена.
...
- Text: «Отметить прочитанным»; Video: «Отметить просмотренным»; Guide/остальные: «Отметить изученным».
  Это presentation mapping существующих Format, не новые состояния хранения. Unknown format имеет
  понятный fallback «Отметить изученным»; точный текст подтверждается визуальной приёмкой.
...
Account отклоняется. Race с unpublish/access change обрабатывается текущим version-bound
ContentAccess contract: stale authorization до commit не даёт записать mark-read.
