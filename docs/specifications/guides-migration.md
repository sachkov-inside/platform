...
Inventory до изменения содержит все production Series и их связи, а также три редакционных
комплекта. [Evidence](../evidence/issue-448/README.md) отделяет production read-only snapshot,
локальные проверки и неподтверждённую production-поставку.

PR #427 владеет закрепом и отдельным редактором; #360/#363 — историей PersonalHome,
#422 — доменом. Перед merge этой задачи нужно интегрировать принятые изменения этих PR
из main в собственный worktree и повторить затронутые проверки. Чужие ветки не изменяются.
