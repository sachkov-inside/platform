...
- #438 (поставлено): оба источника доходят до обоих каналов и возвращают результаты на реальных
  PostgreSQL и RabbitMQ. Platform и провайдер Telegram держат раздельные базы и делят только
  брокер; провайдер синтетический, но верен контракту и спрашивает preflight перед каждой попыткой.
  Проверены оба продукта продажи, жизненный цикл подписки двух тарифов, права библиотеки и
  отдельного руководства, отличие выдачи права от покупки, разрыв связи канала, отложенный ответ
  канала и независимость лент источников. Разбор видов доказательства —
  [evidence #438](../evidence/issue-438/README.md). Credentialed доказательство обоих каналов
  остаётся отдельным шагом и требует конкретного разрешения владельца. #413 использует эту приёмку
  для billing DEMO.
