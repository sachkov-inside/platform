...
Состав — Guide UUID; capability guide:* относится к отдельной покупке и не принимается как
состав назначаемого тарифа. Новый состав не принимает отдельные Material UUID; прежние снимки
...
- Стартовый тариф (курс, временный источник Tribute, ручное назначение) даёт `materials`, `support`
  и `community`. Миграция 0067 приводит к этому тариф миграции 0063, пока его никому не назначили,
  и убирает из его снимка отдельные материалы. Тариф с отдельным материалом или `reviews` не
