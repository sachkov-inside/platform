...
Публичное название Guide — «Практикум» (решение владельца 13.09.2026,
[product brief](../product/platform-mvp-brief.md)). Ранее использованное ниже слово
«руководство» обозначает ту же сущность; технические имена и маршруты сохраняются.
...
### Блоки материала и запись в Markdown

`@inside/material-blocks` описывает закрытый набор блоков: абзац, заголовок (H2–H4), списки,
цитата, код, разделитель, таблица, врезка, карточка ресурса, промпт, итоги, список терминов,
...
объявляет так `variantOption`: по одной ветке на режим прохождения, не больше двух, режим —
поле ветки со значениями `example` и `own`. Имена режимов принадлежат реестру, потому что их
печатает и читатель, и редактор, и подсказка. Адресуется сам шаг, а не ветка: адрес на ветку
сломался бы ровно в тот момент, когда читатель меняет режим.

Врезка `callout` расширяется, а не заменяется: к принятым `note`, `tip`, `warning` добавлены
`example`, `good`, `bad`, `definition`, и появилось необязательное название. Сохранённые врезки
...
Поле блока, чьё имя совпадает с настоящим HTML-атрибутом или чьё значение не строка, объявляет
собственный DOM-атрибут. Иначе оно не переживает копирование: буфер обмена восстанавливает узел из
его разметки, а не из сохранённого документа.

Запись блока в Markdown фиксируется сейчас, потому что от неё зависит будущий импорт из Obsidian;
сам импортёр в v1 не входит. Каждый блок записывается либо callout-синтаксисом Obsidian, либо
огороженным блоком кода, поэтому разбор однозначен:

| Блок | Запись |
|---|---|
| `callout` | `> [!note\|tip\|warning\|example\|success\|failure\|abstract] <название>` и тело в `>` |
| `keyPoint` | `> [!important]` с одной строкой |
| `takeaways` | `> [!summary] <заголовок>` и строки `> - <пункт>` |
| `labeledList` | `> [!terms]` и строки `> - **<метка>** <название> — <пояснение>` |
| `resourceCard` | `> [!link] <название>`, строка описания и строка адреса |
| `agentPrompt` | огороженный блок с языком `prompt` и названием в остатке строки |
| `variant` | по callout-блоку на ветку: `> [!variant-example]` и `> [!variant-own]` |

Виды врезки отображаются в типы Obsidian: `good` — `success`, `bad` — `failure`, `definition` —
`abstract`; остальные совпадают по имени. Имена веток вариантного шага совпадают с авторской
базой (`variant-example`, `variant-own`), поэтому импорту не нужна таблица соответствий, которая
однажды разъедется с источником.
...
сплошная внутри выбранной части. По уточнению владельца 13.09.2026 руководство без глав показывает
весь список в табе «Программа»; «Дополнительно» и «Артефакты» остаются видимыми.
Постранично адресуется только выбранная часть; переключение части начинает её с
первой страницы.
...
Для `working-with-agents` («AI-first разработка») решением владельца 13.09.2026 используется
авторская страница: AI-first процесс, аудитория, обещаемый результат с тремя визуальными карточками,
единый обзор программы с выбором агента и модели, компактная схема языков и проекта,
...
повторяющие темы программы, убраны. Основы программирования нужны до старта. Её тексты и диаграммы принадлежат
оформлению этого продукта; редакционная основа — Inside Content. Это не новая универсальная схема
введения и не отдельный путь чтения. Наличие руководства, опубликованные материалы, главы, количества
и доступ по-прежнему определяет Materials. Главное действие ведёт в `/guides/:slug/programme`.
Иллюстрация общей карточки и страницы использует одну реализацию; показывает темы, не реальные статусы
...
Владелец подтвердил доступ к приобретённым материалам и сообществу без ограничения срока и без
продлений; помощь автора доступна всё время участия в сообществе. Обсуждаются задачи и решения,
личные встречи — по договорённости. Опубликованные материалы открываются сразу после покупки.
На мобильном анимация предшествует всему тексту hero. Бесплатное количество показывается рядом
с первым действием и определяется Materials. Цена и сроки ответов ещё не определены.
Эта итерация меняет описание продукта, не редакцию оферты или механизм выдачи прав;
условия разовой покупки необходимо согласовать с сопровождением до запуска продаж.
...
Части состава — «Программа», «Дополнительно» и «Артефакты» — следуют решению владельца
13.09.2026: все три таба видимы, включая пустые разделы. Пустая часть объясняет, что появится здесь;
недоступный раздел артефактов показывает ошибку внутри своего таба. Это уточняет решение 10.09.2026
скрывать пустые части. Артефакты не входят в маршрут, поэтому их часть не нумеруется и
не постранична. Условия доступа и цены на странице не показываются: они приходят из #401/#411.

Программа начинается с компактной плашки руководства: обложка, название, количество глав и
опубликованных материалов, сводка открытого доступа. Для аккаунта показываются сохранённый прогресс
и его состояния загрузки/ошибки; гостю прогресс не придумывается. При закрытых уроках плашка
содержит «Оплатить практикум» с переходом на `/guides/:slug/buy`. Подписка не предлагается
вместо руководства, даже без опубликованной цены. В этом случае страница покупки показывает
готовящиеся условия. Декоративный блик проходит дважды и отключается при reduced motion.
Закрытый Reader ведёт на покупку того же руководства; без однозначного контекста не выбирает
руководство наугад. Главы и строки уроков уплотнены.
Строка содержит название, номер с подписью «урок» внутри карточки, длительность видео и состояние
доступа. Отметка чтения стоит справа и не заменяет номер. Текущий шаг имеет широкое действие
«Продолжить» внутри карточки. Глава обозначается только названием, без номера, «Глава n из m» и линий
между главами. Превью уроков квадратные, с центральным кадрированием; без обложки используется
общий fallback. Бесплатные доступные уроки отмечены плашкой «Бесплатно»; закрытые превью
размыты, название остаётся читаемым и рядом показан замок. На узкой строке миниатюра уступает
место тексту. Описания, сложность и результаты обучения остаются в Reader.

### Режимы прохождения и факты урока

Читатель проходит руководство одним из двух способов: на учебном проекте автора или на своём.
Выбор принадлежит читателю, а не материалу: одно значение покрывает все руководства. У вошедшего
оно хранится за аккаунтом в ReadingActivity, у гостя — в cookie этого браузера. Вошедшему cookie
не пишется, поэтому гостевое значение не может стать значением аккаунта. По умолчанию — «Учебный
проект». Недоступность сохранённого значения не ломает урок: он читается в режиме по умолчанию.

Режим меняет только вариантные шаги. Читатель видит ветку активного режима, а ссылка под шагом
раскрывает вторую ветку на месте и ничего не сохраняет. Шаг с одной веткой в чужом режиме не
показывается и ссылки не имеет. Обе ветки приезжают в разметку, поэтому переключение мгновенно,
а серверный рендер сразу отдаёт нужную ветку и не показывает чужую.

Оглавление урока в вариантные шаги не заходит. Набор заголовков не должен меняться от
переключения режима, а ссылка в скрытую ветку никуда не ведёт; заголовок внутри ветки остаётся
виден в тексте и участвует в поиске.

Переключатель принадлежит руководству и стоит в шапке урока. Он показывается, когда урок открыт
из руководства и у этого руководства есть хотя бы один вариантный шаг. Признак приезжает вместе с
составом: опубликованный материал несёт производное `has_mode_variants`, посчитанное при
сохранении из уже разобранного тела, а руководство отвечает по своему опубликованному составу.
Читать тела всех уроков на каждый показ урока для этого не нужно. При прямом входе в материал,
без руководства в адресе, контекста руководства нет — как нет там и навигации по руководству, —
поэтому нет и переключателя. У первого вариантного шага один раз на браузер показывается
подсказка о двух режимах и месте переключения.

`difficulty` (`basic | intermediate | advanced`) и `outcomes` — поля metadata материала, а не
руководства: значение принадлежит уроку и не должно различаться от руководства к руководству. Оба
необязательны. Черновик принимает от нуля до четырёх пунктов `outcomes`, чтобы автосохранение не
отказывало автору на втором пункте; публикация требует либо ноль пунктов, либо два–четыре и
возвращает `outcomes_too_few`. Значения показываются в начале урока до основного текста;
решением владельца 13.09.2026 они убраны из компактных строк программы. Их отсутствие страницу
не ломает и не дублируется в теле.
...
8. `Videos` хранит local identity, exact owning Material/access/project, opaque Kinescope API ID,
   ...
   загрузку Material, пока её исход не установлен, и сверяет её обычной reconciliation. Итог этой
   сверки: `ready` привязывается обычным Save, продолжающаяся обработка ждёт, а `uploading` без
