...
возвращает `outcomes_too_few`. Значения показываются в начале урока до основного текста и в
программе рядом с названием шага; их отсутствие страницу не ломает и не дублируется в теле.
