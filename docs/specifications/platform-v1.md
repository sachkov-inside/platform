...
`steps`, `list`, `trial`). Блок имеет устойчивый `id`, по которому оформление добавляет к нему свою
иллюстрацию; пустая строка означает ненаписанное поле. Оба поля и `slug` меняет только source-scoped
