...
`name` — точная строка после `trim()`, 1–120 UTF-16 code units. `summary` — авторское описание для
читателя после `trim()`, до 4000 UTF-16 code units; пустая строка означает отсутствие описания.
Порядок глав задаётся их позицией в сохранённом списке.
...
Guide reader показывает программу руководства по главам: номер, название, авторское описание и
число опубликованных материалов главы; глава без опубликованных материалов честно отмечается как
готовящаяся. Маршрут внутри страницы группируется заголовками глав; постраничность, прогресс
«N из M» и продолжение чтения не меняются. Черновики и неопубликованные материалы не попадают ни
в программу, ни в счётчики.
...
Страница руководства объединяет редактирование названия, описания, обложки и упорядоченного состава.
