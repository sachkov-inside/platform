...
Обязательное текстовое поле блока проверяется на присутствие строки, а не на непустоту: автор
заполняет поля после вставки, и автосохранение черновика не должно отказывать. Публикация
непустоты полей не требует; адрес карточки ресурса проверяется только когда он заполнен, и тогда
он обязан быть абсолютным `https`.
