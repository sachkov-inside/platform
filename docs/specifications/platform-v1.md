...
`example`, `good`, `bad`, `definition`, `task`, и появилось необязательное название. Сохранённые врезки
...
| Блок | Запись |
|---|---|
| `callout` | `> [!note\|tip\|warning\|example\|success\|failure\|abstract\|todo] <название>` и тело в `>` |
...
`abstract`, `task` — `todo`; остальные совпадают по имени. Название, совпадающее с именем вида
(`> [!todo] Задание`), в документ не пишется: читатель видит имя вида один раз. Имена веток
вариантного шага совпадают с авторской базой (`variant-example`, `variant-own`), поэтому импорту
не нужна таблица соответствий, которая однажды разъедется с источником.
...
сплошная внутри выбранной части. Руководство без глав показывает неразделённый список внутри вкладки «Программа»;
остальные вкладки сохраняют свои пустые состояния. Решением владельца 15.09.2026 выбранная часть показывается непрерывным списком: следующие
уроки добавляются при приближении к концу, без замены уже показанных. Доступна кнопка
«Показать ещё уроки» для клавиатуры и браузера без IntersectionObserver. Переключение части
начинает список с начала. Возврат из Reader восстанавливает урок по `at`; старые ссылки `page`
сохраняют совместимость. Состав уже получен целиком; это постепенное отображение на клиенте,
серверный cursor API этим изменением не вводится.

Глава в маршруте представлена своим названием и количеством материалов, без служебного номера. Глава без
...
иллюстрацию; внутри блока иллюстрации и значки идут по порядку пунктов. Пустая строка означает
ненаписанное поле. Границы описания: до 16 блоков, до 12 пунктов в блоке, короткое поле — 200,
длинное — 4000 символов после `trim()`, всё описание — до 32 КиБ; заголовок страницы один, поэтому
...
количества и доступ по-прежнему определяет Materials. Главное действие ведёт в `/guides/:slug/programme`.
...
входят уже опубликованные. Сроки автор пишет подстановками `{access_term}` и `{support_term}`, а
страница подставляет их из `@inside/legal/purchase-terms`, который тест пакета сверяет с текстом
действующей оферты. Опубликованные материалы открываются сразу после покупки.
...
Шапка программы содержит название, компактную обложку и прогресс вошедшего читателя.
Главы показаны без порядкового номера; номер урока находится внутри компактной карточки
с квадратным превью. Сложность и результаты обучения остаются на странице урока. Отметка
изученного и действие «Продолжить» относятся к конкретному уроку. Количества и доступ
берутся из текущих Materials, а не из демонстрационного набора оформления.

Части состава — «Программа», «Дополнительные материалы» и «Артефакты» — следуют решению владельца
10.09.2026 о переключаемых частях. В принятом виде программы, восстановленном 15.09.2026, все три вкладки видны постоянно;
пустая часть показывает короткое объяснение. Артефакты не входят в маршрут, поэтому их часть не нумеруется и
...
возвращает `outcomes_too_few`. Значения показываются в начале урока до основного текста; компактная карточка программы
их не дублирует. Их отсутствие страницу не ломает и не дублируется в теле.
...
1. Пока у Account нет записи о принятии действующей редакции `terms`, callback и серверные
   маршруты кабинета и покупки ведут на `/welcome` с локальным `returnTo`. Решением владельца
   17.09.2026 это модальное окно поверх размытой неактивной Главной: марка, заголовок «Добро
   пожаловать» (или «Условия обновились»), строка о принятии, о возрасте от 14 лет со ссылками на
   условия и политику и кнопка «Принять условия и продолжить» с фокусом. Сервер отдаёт окно
   открытым, чтобы текст был виден до загрузки скриптов. Окно не закрывается без решения; если
   условия не загрузились, вместо кнопки есть ссылка «На главную». Отметок нет, подтверждённая
   почта не требуется. Новая редакция
...
4. Publish допускает только ready MaterialAssets exact owning Material. Replacement создаёт новый
   ...
   Material advisory lock и в своей транзакции переводит только newly removed ready Assets из
   persisted referenced state на новый orphan boundary, от которого полностью отсчитывается cleanup
   grace; отменённый Save оставляет пометки прежними, а повторный Save с тем же reference set эту
   границу не сдвигает.
