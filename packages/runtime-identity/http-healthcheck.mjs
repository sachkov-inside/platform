...
const backendReportSchema = reportSchema.extend({
...
async function main() {
  ...
  if (!selected.success || process.argv.length !== 3)
  ...
  if (!report.success || report.data.process !== service)
    fail("invalid readiness report");
  if (report.data.release.release !== expected.data.release)
    fail("release version mismatch");
  if (report.data.release.sourceSha !== expected.data.sourceSha)
    fail("source SHA mismatch");
