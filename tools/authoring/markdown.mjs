...
parser.block.ruler.before("blockquote", "inside_callout", (state, start, end, silent) => {
  ...
  token.meta = { kind: match[1], title: match[2] ?? null, content: lines.join("\n") };
  state.line = next;
  return true;
});
...
export function convertMarkdown(markdown, { sourcePath, sourceId, link, image }) {
  const fail = (token, message) => { throw new Error(`${sourcePath}:${(token.map?.[0] ?? 0) + 1}: ${message}`); };
  const inline = (tokens, parent) => {
    const nodes = [];
    const marks = [];
    for (const token of tokens ?? []) {
      if (["text", "code_inline"].includes(token.type)) {
        if (token.content.length) nodes.push({ type: "text", text: token.content, ...(marks.length || token.type === "code_inline" ? { marks: [...marks, ...(token.type === "code_inline" ? [{ type: "code" }] : [])] } : {}) });
      } else if (token.type === "softbreak") {
        nodes.push({ type: "text", text: "\n", ...(marks.length ? { marks: [...marks] } : {}) });
      } else if (token.type === "hardbreak") nodes.push({ type: "hardBreak" });
      else if (["strong_open", "em_open", "s_open"].includes(token.type)) marks.push({ type: { strong_open: "bold", em_open: "italic", s_open: "strike" }[token.type] });
      else if (token.type === "link_open") marks.push({ type: "link", attrs: { href: link(token.attrGet("href")) } });
      else if (["strong_close", "em_close", "s_close", "link_close"].includes(token.type)) marks.pop();
      else if (token.type === "image") {
        if (marks.length) fail(parent, "linked/marked image requires an explicit supported block");
        nodes.push({ type: "assetImage", attrs: { assetId: image(token.attrGet("src")), alt: token.content, caption: token.attrGet("title") } });
  ...
  const blocks = (tokens) => {
    ...
    for (const token of tokens) {
      const types = { paragraph_open: "paragraph", heading_open: "heading", blockquote_open: "blockquote", bullet_list_open: "bulletList", ordered_list_open: "orderedList", list_item_open: "listItem", table_open: "table", tr_open: "tableRow", th_open: "tableHeader", td_open: "tableCell" };
      if (["thead_open", "thead_close", "tbody_open", "tbody_close"].includes(token.type)) continue;
      if (token.type in types) {
        const node = { type: types[token.type], content: [] };
        if (token.type === "heading_open") node.attrs = { level: Number(token.tag.slice(1)) };
        if (token.type === "ordered_list_open") node.attrs = { start: Number(token.attrGet("start") ?? 1) };
        append(node); stack.push(node);
      } else if (token.nesting === -1) {
        const node = stack.pop();
        if (["tableCell", "tableHeader"].includes(node.type)) node.content = [{ type: "paragraph", content: node.content }];
        if (node.type === "paragraph" && node.content.some((child) => child.type === "assetImage")) {
          if (node.content.some((child) => child.type !== "assetImage")) fail(token, "put an image on its own paragraph");
          const siblings = stack.at(-1).content;
          siblings.splice(siblings.indexOf(node), 1, ...node.content);
        }
      } else if (token.type === "inline") stack.at(-1).content.push(...inline(token.children, token));
      else if (["fence", "code_block"].includes(token.type)) append({ type: "codeBlock", attrs: { language: token.info.trim() || null }, content: token.content ? [{ type: "text", text: token.content }] : [] });
