...
test("unrecognized provider fields and escaping source paths fail closed", async (t) => {
  ...
  for (const altered of [
    { ...material, sourcePath: "../secrets.md" },
    {
      ...material,
      video: {
        kinescopeId: "06af55ae-3cb9-4868-b845-6a5cd7dff46b",
        token: "secret",
      },
    },
  ]) {
...
test("uncertain requests persist before transmission and retry the same key", async (t) => {
  ...
  await assert.rejects(
    withJournal(root, "http://127.0.0.1:3101", async (context) => {
  ...
  await withJournal(root, "http://127.0.0.1:3101", async (context) => {
    const result = await applyJournaled(context, request, async (_, key) => {
      keys.push(key);
      return { contentVersion: 3 };
    });
    assert.deepEqual(result, { contentVersion: 3 });
    await applyJournaled(context, request, () => {
      throw new Error("Applied entry must not be sent twice");
    });
  });
  assert.equal(keys[0], keys[1]);
  await assert.rejects(
    withJournal(root, "https://different.example", () => {}),
    /another environment/,
  );
});
