...
async function fixture(t) {
  ...
  const journal = { schemaVersion: 1, target: reviewOrigin, materials: { [sourceId]: { materialId, contentVersion: 2, digest: "a".repeat(64), access: "membership" } }, guides: {}, operations: {} };
  await writeFile(join(state, "journal.json"), canonical(journal));
  return { state, file, read: async () => JSON.parse(await readFile(join(state, "journal.json"), "utf8")) };
}

function providerApi({ endpoint = "https://uploads.invalid/provider-one", loseFirstInit = false } = {}) {
  const calls = [];
  ...
  return {
    ...
    async request(path, body, key) {
      ...
      if (path === `/authoring/materials/${materialId}/videos/uploads`) {
        ...
        assert.deepEqual(body, { access: "membership", byteSize: 15, filename: "final.mp4", title: "Финальная запись" });
        if (!attempts.has(key)) attempts.set(key, { providerVideoId: "provider-one", uploadEndpoint: endpoint, video: { videoId, materialId, state: "uploading" } });
        if (lose) { lose = false; throw new Error("Connection lost after the attempt was stored"); }
        return structuredClone(attempts.get(key));
      }
      if (path === `/authoring/videos/${videoId}/reconcile`) return { videoId, materialId, state: ++reconciles >= 2 ? "ready" : "processing", durationSeconds: 600 };
...
const upload = (setup, api, extra = {}) => uploadVideo({ stateDirectory: setup.state, sourceId, file: setup.file, title: "Финальная запись", request: api.request, sleep: async () => {}, ...extra });
...
test("a lost init response retries the persisted key and reaches a ready video once", async (t) => {
  ...
  assert.deepEqual(result, { sourceId, materialId, videoId, providerVideoId: "provider-one", durationSeconds: 600 });
  const keys = api.calls.filter((call) => call.path.endsWith("/uploads")).map((call) => call.key);
  assert.equal(new Set(keys).size, 1);
  assert.equal(api.attempts.size, 1);
  const journal = await setup.read();
  assert.deepEqual(journal.resources[`source-video:${sourceId}`].videoId, videoId);
  const before = api.calls.length;
  await upload(setup, api);
  assert.deepEqual(api.calls.slice(before).map((call) => call.path), ["/authoring/import/materials/environment"]);
});

test("a real provider endpoint is refused before any byte is sent", async (t) => {
  const setup = await fixture(t);
  const api = providerApi({ endpoint: "https://uploader.kinescope.io/v2/init/abc" });
  await assert.rejects(upload(setup, api), /separate owner approval/u);
  assert.equal(Object.values((await setup.read()).resources)[0].phase, "transfer");
...
test("an unknown upload outcome stops without a new key", async (t) => {
  ...
  const api = { async request(path) {
    ...
    throw Object.assign(new Error("409"), { body: { code: "upload_outcome_unknown" } });
  } };
  ...
  assert.equal(Object.values((await setup.read()).resources)[0].idempotencyKey, first);
});
