...
export async function withGitSnapshot(repository, ref, use) {
  ...
  const { stdout } = await execute("git", ["-C", root, "rev-parse", "--verify", "--end-of-options", `${ref}^{commit}`], commandOptions);
  ...
  try {
    ...
    await execute("git", ["-C", root, "archive", "--format=tar", `--output=${archive}`, commit], commandOptions);
