...
`callout-good`, `callout-bad`, `callout-definition`, `callout-task`. Токен задаёт только чернила
вида; поверхность и рамка выводятся из него через `color-mix`, поэтому собственных цветов у блока
нет. Значения живут в [`app/globals.css`](app/globals.css) для светлой темы, тёмной темы и
публичной оболочки.
