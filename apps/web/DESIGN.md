...
Вид врезки — единственная семантика, которой не хватало принятой палитры, поэтому у него есть
собственные токены: `callout-note`, `callout-tip`, `callout-warning`, `callout-example`,
`callout-good`, `callout-bad`, `callout-definition`. Токен задаёт только чернила вида; поверхность
и рамка выводятся из него через `color-mix`, поэтому собственных цветов у блока нет. Значения
живут в [`app/globals.css`](app/globals.css) для светлой темы, тёмной темы и публичной оболочки.

**The Block Tone Rule.** Эти токены принадлежат видам врезки и меткам блоков урока. Они не
становятся общей палитрой статусов, заливкой карточек или вторым акцентом интерфейса.
...
- **Status:** production-owned from issue #505; owner visual GO is not yet recorded.
- **Composition:** урок собирается из типизированных блоков: врезка с видом и необязательным
  названием, карточка внешнего ресурса, готовый промпт, итоги урока, список терминов и выделенная
  ключевая мысль. Вид блока задаёт платформа: у автора нет собственной вёрстки.
- **State:** вид врезки показан словом, значком и цветом одновременно. Незаполненный блок остаётся
  видимой рамкой, а не исчезает: автор должен увидеть, что блок пуст.
- **Copy:** названия видов приходят из реестра блоков, поэтому читатель и автор видят одно слово.
- **Surfaces:** читательский маршрут, предпросмотр авторинга и редактор используют одни и те же
  компоненты внешнего вида; редактор повторяет их токенами в своей таблице стилей.
...
- **Copy:** страницы не обещают неподключённую кассу: пока условия не опубликованы, оформление
  ...
  Суммы, сроки и состав берутся из снимка сервера; интерфейс их не пересчитывает и не додумывает
  недостающую дату.
