...
beforeAll(async () => {
  server = createServer((request, response) => {
    if (request.url === "/api/inside-telegram") {
      ...
      response.setHeader("Content-Type", "text/html; charset=utf-8");
      response.end(
        "<title>Fixture destination</title><h1>Возврат ко входу или callback</h1>",
      );
    }
  });
...
async function poll(page: Page) {
  const answered = page.waitForResponse((response) =>
    response.url().endsWith("/api/inside-telegram/status"),
  );
  ...
  for (const width of [1440, 320]) {
    ...
    for (const status of statuses) {
      ...
      if (status === "pending") {
        ...
        expect(await page.locator("#alternative").getAttribute("href")).toBe(
          "/sign-in",
        );
      }
...
it("keeps keyboard focus and control geometry across polls, reconnects, and returns from decline", async () => {
  ...
  await page
    .getByRole("status")
    .filter({ hasText: "Вы отменили вход" })
    .waitFor();
  expect(await focusedId(page)).toBe("alternative");
...
it("automatically completes in the original tab after approval and respects reduced motion", async () => {
  ...
  state = {
    status: "approved",
    callback: `${origin}/callback?fixture=approved`,
  };
  await page.clock.runFor(clockStepMs);
  ...
  expect(
    await page
      .locator(".inside-telegram-progress")
