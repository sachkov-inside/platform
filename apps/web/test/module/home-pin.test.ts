...
import { handleHomePinReadRequest, handleHomePinWriteRequest } from "@/features/series-order/api/home-pin-route.server";
const seriesId = "10000000-0000-4000-8000-000000000001";
...
it("authenticates selection and removal and keeps both reads and writes private", async () => {
  fakes.load.mockResolvedValue({ ok: true, body: { seriesId: null, version: 1 }, response: new Response() });
  const read = await handleHomePinReadRequest();
  expect(await read.json()).toEqual({ kind: "ready", pin: { seriesId: null, version: 1 } });
  expect(read.headers.get("cache-control")).toBe("private, no-store");
  for (const id of [seriesId, ""]) {
    fakes.save.mockResolvedValue({ ok: true, body: { seriesId: id || null, version: 2 }, response: new Response() });
    const response = await handleHomePinWriteRequest(request({ seriesId: id, expectedVersion: "1", actor: "untrusted" }));
    expect(await response.json()).toEqual({ kind: "ready", pin: { seriesId: id || null, version: 2 } });
    expect(fakes.save).toHaveBeenLastCalledWith({ seriesId: id || null, expectedVersion: 1 }, "trusted-token");
...
it("rejects cross-origin and malformed writes before backend access", async () => {
  ...
  expect(await (await handleHomePinWriteRequest(request({ seriesId: "bad", expectedVersion: "0" }))).json()).toEqual({ kind: "invalid_input" });
...
it.each([[403, "forbidden", "forbidden"], [409, "stale_home_pin", "conflict"], [422, "invalid_reference", "invalid_input"], [503, "dependency_unavailable", "unavailable"]])("maps a %s failure without reporting success", async (status, code, kind) => {
  ...
  expect(await (await handleHomePinWriteRequest(request({ seriesId, expectedVersion: "1" }))).json()).toEqual({ kind });
});
it("does not accept a malformed success body", async () => {
  fakes.save.mockResolvedValue({ ok: true, body: { seriesId, version: 0 }, response: new Response() });
  expect(await (await handleHomePinWriteRequest(request({ seriesId, expectedVersion: "1" }))).json()).toEqual({ kind: "unavailable" });
});
