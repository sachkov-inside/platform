...
describe("Guide product page", () => {
  test("keeps a description this site can draw", () => {
    ...
    expect(
      readGuideProductPage(
        { presentation: "ai-first-process", page: aiFirstProductPage },
        "Guide x",
        warn,
      ),
    ).toEqual({
  ...
  test("falls back to the default template for a presentation it does not know", () => {
    const warn = vi.fn();
    expect(resolveGuidePresentation("neon-hero", "Guide x", warn)).toBe(
      "default",
    );
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining('unknown presentation "neon-hero"'),
    );
  });
  ...
  test("shows no description when the stored one does not match this site", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining("does not match this site"),
    );
  });
  ...
  test("a product without a description is not a warning: the default template uses the editor fields", () => {
    ...
    expect(
      readGuideProductPage(
        { presentation: "default", page: null },
        "Guide x",
        warn,
      ),
    ).toEqual({
  ...
  test("substitutes only the offer terms the author may write", () => {
    ...
    expect(
      fillOfferTerms("Доступ — {access_term}, помощь — {support_term}.", terms),
    ).toBe("Доступ — 2 года, помощь — 6 месяцев.");
