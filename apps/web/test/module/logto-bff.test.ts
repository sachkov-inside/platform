...
describe("Logto BFF configuration", () => {
  ...
  it("permits HTTP only on loopback for the production-mode full-stack harness", () => {
    ...
    expect(parseLogtoBffConfig({ ...production, WEB_BASE_URL: "http://127.0.0.1:3000" }).baseUrl)
      .toBe("http://127.0.0.1:3000");
    expect(() => parseLogtoBffConfig({ ...production, WEB_BASE_URL: "http://inside.example.test" }))
