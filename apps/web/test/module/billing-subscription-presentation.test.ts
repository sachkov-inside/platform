...
describe("состав доступа", () => {
  ...
  it("наследует период подписки, а явный null выдаёт право без даты окончания", () => {
  ...
  it("называет отдельное право на продукт и не даёт ему даты окончания", () => {
    const [line] = benefitLines(guideOnlyOffer);
    expect(line?.label).toBe("Отдельный продукт");
    expect(line?.term).toBe("без даты окончания");
  });
  ...
  it("называет общий чат, который открывает сам купленный продукт", () => {
    expect(
      ...
      ["Отдельный продукт", "без даты окончания"],
      ["Вопросы автору и эфиры", "3 месяца"],
      ["Общий чат", "без даты окончания"],
    ]);
  });
  ...
  it("разовая покупка открывает право без объявленного срока без даты окончания", () => {
...
describe("состояния и ошибки", () => {
  ...
  it("объясняет ожидаемые исходы billing словами покупателя", () => {
    ...
    // Согласие на прежнюю редакцию не принимается: покупатель читает действующую и отмечает снова.
    expect(billingErrorMessage("document_changed")).toBe(
      "Условия покупки обновились. Прочитайте действующую редакцию и отметьте согласие снова.",
    );
  });
});
