...
describe("состав доступа", () => {
  ...
  it("называет отдельное право на практикум и сохраняет его бессрочный срок", () => {
    const [line] = benefitLines(guideOnlyOffer);
    expect(line?.label).toBe("Отдельный практикум");
    expect(line?.term).toBe("бессрочно");
  });

  it("называет общий чат, который открывает само купленное практикум", () => {
    expect(
      benefitLines(guideWithSupportOffer).map((line) => [line.label, line.term]),
    ).toEqual([
      ["Отдельный практикум", "бессрочно"],
      ["Вопросы автору и эфиры", "3 месяца"],
      ["Общий чат", "бессрочно"],
    ]);
  });

  it("даёт выведенному чату срок самого долгого практикума предложения", () => {
    const lines = benefitLines({
      offer: {
  ...
  it("держит чат по самому долгому сроку, когда оба основания срочные", () => {
    ...
    expect(lines.map((line) => [line.label, line.term])).toEqual([
      ["Отдельный практикум", "6 месяцев"],
      ["Общий чат", "6 месяцев"],
    ]);
  });

  it("не обещает чат там, где практикум не продаётся", () => {
...
describe("состав предложения", () => {
  it("называет практикум с сопровождением по-русски", () => {
    expect(offerCompositionLabel(guideWithSupportOffer.offer)).toBe(
      "Практикум с сопровождением и общим чатом",
    );
  });
  ...
  it("называет чат и тогда, когда его открывает само практикум", () => {
    expect(offerCompositionLabel(guideOnlyOffer.offer)).toBe(
      "Практикум с общим чатом",
    );
  });
  ...
  it("удлиняет предлог перед стечением согласных", () => {
    ...
    expect(offerCompositionLabel(offer)).toBe(
      "Практикум со всеми материалами, сопровождением и общим чатом",
    );
  });
