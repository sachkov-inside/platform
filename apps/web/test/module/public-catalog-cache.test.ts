...
const body = {
  ...
  blocks: [
    {
      kind: "paragraph",
      content: [{ kind: "text", text: "Тело урока.", marks: [] }],
    },
  ],
} as const;
...
describe("guest Material read kept in the shared cache", () => {
  ...
  it("reads without a token and keeps a body the backend marked public", async () => {
    const fetchBackend = backendAnswers({
      kind: "available",
      cacheScope: "public",
      projection: { ...projection, access: "free" },
      body,
      primaryVideo: null,
    });
  ...
  it("passes a missing Material on so that the cache can give it a short life", async () => {
    ...
    await expect(getGuestMaterial("lesson")).resolves.toEqual({
      kind: "not-found",
    });
  });
});
