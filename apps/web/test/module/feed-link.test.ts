...
describe("public feed link", () => {
  it("uses the editor link without trimming valid destination punctuation", () => {
    expect(
      feedLink("Документация", "https://example.com/search?q=what?")?.href,
    ).toBe("https://example.com/search?q=what?");
  ...
  it("recognizes a prose link without taking trailing punctuation into the destination", () => {
    expect(
      feedLink("Пример: https://www.example.com/docs?q=hello#start. Ещё текст"),
    ).toEqual({
      href: "https://www.example.com/docs?q=hello#start",
      label: "example.com",
    });
  });
  ...
  it("leaves unsafe schemes and malformed links as text", () => {
    expect(
      feedLink("javascript:alert(1) data:text/html,test https://"),
    ).toBeUndefined();
  });

  it("does not show credential-bearing links and can use the next public address", () => {
    expect(
      feedLink("https://user:secret@example.com https://docs.example.com/page"),
    ).toEqual({
