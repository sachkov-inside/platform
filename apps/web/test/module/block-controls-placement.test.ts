...
function nodeType(name: string) {
  ...
  if (type === undefined)
    throw new Error(`В схеме документа нет узла «${name}»`);
  return type;
}
...
describe("Может ли блок под контролами сдвинуться", () => {
  it("не может, когда документ не менялся", () => {
    expect(
      blockMayHaveMoved(document, document, start(document, afterCard)),
    ).toBe(false);
  });

  it("не может, когда автор печатает внутри самого блока", () => {
    const next = edit((state) =>
      state.tr.insertText("!", start(document, afterCard) + 3),
    );
    expect(blockMayHaveMoved(document, next, start(next, afterCard))).toBe(
      false,
    );
  });
  ...
  it("может, когда знак набран в блоке выше: строка там могла перенестись", () => {
    const next = edit((state) =>
      state.tr.insertText("!", start(document, heading) + 3),
    );
    expect(blockMayHaveMoved(document, next, start(next, afterCard))).toBe(
      true,
    );
  ...
  it("может, когда перед блоком появился новый блок", () => {
    const next = edit((state) =>
      state.tr.insert(0, nodeType("paragraph").create()),
    );
    expect(blockMayHaveMoved(document, next, start(next, afterCard + 1))).toBe(
      true,
    );
  });
