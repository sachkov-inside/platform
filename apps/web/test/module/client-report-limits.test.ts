...
function stubPage() {
  ...
  vi.stubGlobal("navigator", {
    sendBeacon: (route: string, body: Blob) =>
      beacons.push({ body, route }) > 0,
  });
  return {
    beacons,
    hide() {
...
it("обычная страница шлёт один отчёт на скрытие вкладки и не попадает ни под один предел", async () => {
  ...
  for (const name of ["TTFB", "FCP", "LCP", "CLS"])
    queueWebVital({ id: `v5-${name}`, name, value: 1 });
  page.hide();
  page.show();
  page.hide();
  ...
  expect(page.beacons.map((beacon) => beacon.route)).toEqual([
    "/api/web-vitals",
    "/api/web-vitals",
  ]);

  const { handleWebVitalsReport } =
...
it("загруженная минута обычных посещений со всей площадки проходит потолок журнала", async () => {
  ...
  const { handleRenderErrorReport, handleWebVitalsReport } =
    await import("@/features/client-telemetry.server");
  const limiter = createEntryRateLimiter();
  const loadMetrics = ["TTFB", "FCP", "LCP", "CLS", "INP"].map((name) => ({
    id: `v5-${name}`,
    name,
    value: 1,
  }));
