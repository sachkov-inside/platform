...
describe("an authoring write expires the public catalog cache", () => {
  beforeEach(() => { fakes.expirePublicCatalog.mockReset(); });

  it("expires the catalog after a catalog authoring write", async () => {
    const { handleAuthenticatedMutation } = await import("@/shared/auth/authenticated-mutation-handler.server");

    const response = await handleAuthenticatedMutation(mutation("/api/authoring/guides/order", "PUT"), () => Promise.resolve({ ok: true }));
  ...
  it("answers a failed catalog write with a response, so that the queued expiry still runs", async () => {
    ...
    const response = await handleAuthenticatedMutation(mutation("/api/authoring/guides/order", "PUT"), () => Promise.reject(failure));
  ...
  it("lets a failed write outside the catalog fail as before", async () => {
    ...
    await expect(
      handleAuthenticatedMutation(mutation("/api/reading-progress/state"), () => Promise.reject(new Error("response lost"))),
  ...
  it.each([
    ...
    ["a billing command: prices never enter the shared cache", "/api/authoring/billing/offers/save"],
  ])("keeps the catalog after %s", async (_name, path) => {
    const { handleAuthenticatedMutation } = await import("@/shared/auth/authenticated-mutation-handler.server");

    await handleAuthenticatedMutation(mutation(path), () => Promise.resolve({ ok: true }));
  ...
  it("keeps the catalog when the request never reached the write", async () => {
    const { handleAuthenticatedMutation } = await import("@/shared/auth/authenticated-mutation-handler.server");
    const crossOrigin = new Request("https://inside.example.test/api/authoring/guides/order", { body: new FormData(), headers: { origin: "https://elsewhere.example.test" }, method: "PUT" });

    const response = await handleAuthenticatedMutation(crossOrigin, () => Promise.resolve({ ok: true }));
