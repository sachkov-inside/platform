...
beforeEach(async () => {
  ...
  vi.resetModules();
  ({ handleRenderErrorReport, handleWebVitalsReport } =
    await import("@/features/client-telemetry.server"));
...
it("отклоняет неизвестную метрику и тело, которое не JSON", async () => {
  const unknownMetric = await handleWebVitalsReport(
    report(
      "/api/web-vitals",
      JSON.stringify({ metrics: [{ ...lcp, name: "Custom" }], route: "/" }),
    ),
  );
  const notJson = await handleWebVitalsReport(
    report("/api/web-vitals", "metrics=LCP"),
  );
...
describe("общий потолок журнала для отчётов браузера", () => {
  ...
  it("сверх потолка отвечает 429 с Retry-After и не пишет отчёт в журнал", async () => {
    ...
    for (let index = 0; index < 60; index += 1)
      accepted.push((await send()).status);
  ...
  it("считает метрики строками журнала и не пишет часть отчёта, который не помещается", async () => {
    ...
    const send = (count: number) =>
      handleWebVitalsReport(
        report(
          "/api/web-vitals",
          JSON.stringify({ metrics: metrics(count), route: "/" }),
        ),
      );

    const statuses = [];
    for (let index = 0; index < 14; index += 1)
      statuses.push((await send(20)).status);
  ...
  it("поток метрик не вытесняет отчёты об ошибках", async () => {
    ...
    for (let index = 0; index < 301; index += 1) {
      lastVitals = (
        await handleWebVitalsReport(report("/api/web-vitals", vitals))
      ).status;
    }
    const errorReport = await handleRenderErrorReport(
      report("/api/render-errors", JSON.stringify(renderError)),
    );
