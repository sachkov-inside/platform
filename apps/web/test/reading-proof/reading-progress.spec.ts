...
for (const scenario of [
  ...
  test(`${scenario}: responsive proof and accessibility`, async ({
    page,
  }, testInfo) => {
    await page.goto(
      `/iframe.html?id=pages-reading-progress--${scenario}&viewMode=story`,
    );
    await expect(page.getByRole("main")).toBeVisible();
    await page.evaluate(() => document.fonts.ready);
    await page.addStyleTag({
      content: "[data-agentation-root] { visibility: hidden !important; }",
    });
    expect(
      await page.evaluate(
    ...
    if (scenario === "text") {
      ...
      await expect(
        page.getByRole("button", { name: "Прочитано", exact: true }),
      ).toHaveAttribute("aria-pressed", "true");
    }
    const readingAction = page.locator("[data-reading-action-state]");
    if ((await readingAction.count()) > 0)
      await readingAction.scrollIntoViewIfNeeded();
    await page.screenshot({
      fullPage: true,
      animations: "disabled",
      path: evidencePath(
        "issue-328",
        `${scenario}-${testInfo.project.name}.png`,
      ),
    });
  });
}

test("loading, ready and pending keep the minimal toggle footprint", async ({
  page,
}) => {
