...
function projection(lesson, entitled) {
  return {
    ...lesson,
    availability: lesson.access === "free" || entitled ? "available" : "locked",
  };
}
...
function body(lesson, entitled) {
    ...
    content: [
      {
        kind: "text",
        marks: [],
        text: `${entitled ? protectedBodyMarker : "Открытый текст"} · ${lesson.title} · абзац ${String(index + 1)}. Текст нужен, чтобы страница была выше экрана и подвал не поднимался к скелету.`,
      },
    ],
  }));
  ...
  const step = {
    kind: "variant",
    options: ["example", "own"].map((mode) => ({
      content: [
        {
          kind: "paragraph",
          content: [
            { kind: "text", marks: [], text: `ШАГ-ДЛЯ-РЕЖИМА-${mode}` },
          ],
        },
      ],
      mode,
    ...
    blocks: [
      {
        kind: "heading",
        level: 2,
        content: [{ kind: "text", marks: [], text: "Раздел урока" }],
      },
      ...written,
      ...paragraphs,
    ],
...
function readerProjection(lesson) {
  ...
  return {
    ...base,
    seriesMemberships: seriesMemberships.map(({ ordinal, series }) => ({
      ordinal,
      series,
    })),
  };
...
function route(method, url, entitled) {
  ...
  if (path === "/library/home") {
    return json({
      ...
      pinnedSeries: {
        ...
        card: {
          action: "Открыть продукт",
          eyebrow: "Продукт",
          subtitle: "Синтетический продукт проверок.",
        },
      },
  ...
  if (discovery !== null) {
    ...
    if (kind === "topics") {
      ...
      return json({
        ...
        reference: {
          ...
          name: topic.name,
          slug: topic.slug,
          summary: "Тема для проверки переходов.",
        },
        relatedSeries: [],
        topics: [],
      });
    ...
    return json({
      ...
      relatedSeries: [],
      topics: [
        { cover: null, id: topic.id, name: topic.name, slug: topic.slug },
      ],
    });
  }
  ...
  if (material !== null) {
    ...
    if (lesson.access === "free" || entitled) {
      return json({
        body: body(lesson, lesson.access !== "free"),
        cacheScope: lesson.access === "free" ? "public" : "private-no-store",
        kind: "available",
        primaryVideo: null,
        projection: readerProjection(lesson),
      });
    }
    return json({
      ...
      : json(
          {
            ...
            type: "urn:inside:problem:invalid_proof",
          },
          401,
        );
  }
  ...
  if (delivery !== null) {
    const rendition =
      delivery[1] === coverId
        ? cover.renditions.find(({ width }) => String(width) === delivery[2])
        : undefined;
      ...
      ? json(
          {
            ...
            type: "urn:inside:problem:cover_not_found",
          },
          404,
        )
      : image(rendition);
...
const server = createServer(async (request, response) => {
  ...
  if (result === undefined) {
    send(404, {
      ...
      status: 404,
      title: "Not found",
      type: "urn:inside:problem:not_found",
    });
    return;
  }
