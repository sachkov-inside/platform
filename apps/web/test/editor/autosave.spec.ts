...
test("series picker shows materials before typing and saves composition on the same page", async ({
  ...
  await page.reload();
  await expect(
    page.getByRole("list", { name: "Материалы практикума" }),
  ).toContainText(title ?? "");
});
...
test("image size survives reload and is used in preview; series are searchable without expanding", async ({
  ...
  await page.getByLabel("Поиск практикумов").fill("Нет такой практикума");
  await expect(page.getByText("Практикумы не найдены")).toBeVisible();
  await page.getByLabel("Поиск практикумов").fill("Demo");
  const series = page.getByLabel("Выбор практикумов", { exact: true });
