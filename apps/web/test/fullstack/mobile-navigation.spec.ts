...
test("mobile navigation keeps real catalog context and public canvas", async ({
  page,
}, testInfo) => {
  test.skip(!testInfo.project.name.startsWith("mobile"));
  await page.goto("/?format=guide");
  await expect(
    page.getByRole("list", { name: "Материалы, страница 1" }),
  ).toBeVisible();
  await page.evaluate(() => {
...
test("mobile navigation public route evidence", async ({ page }, testInfo) => {
  ...
  await testInfo.attach("library-public-route", {
    body: await page.screenshot(),
    contentType: "image/png",
  });
});

test("mobile navigation stays mounted without fading the document during tab changes", async ({
  page,
}, testInfo) => {
  test.skip(!testInfo.project.name.startsWith("mobile"));
  await page.goto("/");
    ...
    await new Promise<void>((resolve) => {
      const sample = () => {
        ...
        if (style.visibility !== "visible" || Number(style.opacity) < 1)
          failures.add("navigation hidden");
        if (Math.abs(navigation.getBoundingClientRect().top - top) > 1)
          failures.add("navigation moved");
        for (const animation of document.getAnimations()) {
          if (
            animation instanceof CSSAnimation &&
            /public-page-fade|view-transition/u.test(animation.animationName)
          ) {
...
test("home notes are readable posts with dates and a format filter", async ({
  page,
}, testInfo) => {
  await page.goto("/?format=note");
  const notes = page.getByRole("region", { name: "Материалы", exact: true });
  await expect(
    page.getByRole("button", { name: "Заметки", exact: true }),
  ).toHaveAttribute("aria-pressed", "true");
  await expect(notes.locator("time").first()).toHaveAttribute("datetime", /T/u);
  await expect(notes.locator("[inert]")).toHaveCount(0);
  await expect(
    notes.getByRole("article").first().locator(".home-feed-post-copy"),
  ).not.toBeEmpty();
  expect(
    await page.evaluate(() => document.documentElement.scrollWidth),
  ).toBeLessThanOrEqual(page.viewportSize()?.width ?? 0);
  await page.screenshot({
    path: testInfo.outputPath("home-notes.png"),
    animations: "disabled",
  });
});

test("mobile navigation displays a cold destination before the server responds", async ({
  page,
}, testInfo) => {
  test.skip(!testInfo.project.name.startsWith("mobile"));
...
test("the retired knowledge base has no route or navigation entry", async ({
  page,
  request,
}) => {
  expect((await request.get("/library", { maxRedirects: 0 })).status()).toBe(
    404,
  );
  await page.goto("/");
  await expect(
    page.getByRole("link", { name: "База знаний", exact: true }),
  ).toHaveCount(0);
  await expect(
