...
test("author edits series metadata on a dedicated page and returns to the list after autosave", async ({ context, page }) => {
  ...
  await page.getByRole("button", { name: "Создать практикум" }).click();
  ...
  await expect(page.getByRole("heading", { name: "Практикум пока пуст" })).toBeVisible();
  await page.getByRole("textbox", { name: "Название практикума" }).fill(`${title} · Обновлена`);
  ...
  await expect(page.getByRole("textbox", { name: "Название практикума" })).toHaveValue(`${title} · Обновлена`);
  ...
  await page.reload();
  await expect(page.getByRole("list", { name: "Материалы практикума" }).getByRole("listitem")).toHaveCount(1);
...
test("trusted author reorders a PostgreSQL series with keyboard controls", async ({
  ...
  const items = page
    .getByRole("list", { name: "Материалы практикума" })
    .getByRole("listitem");
