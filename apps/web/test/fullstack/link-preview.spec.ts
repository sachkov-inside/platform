...
test("ссылка на практикум и на материал показывает название, описание и картинку", async ({
  baseURL,
  page,
  request,
}) => {
  ...
  expect(title).toContain("практикум");
...
test("совместимый адрес `/series/` указывает на канонический адрес практикума", async ({
  baseURL,
  page,
}) => {
...
test("карта сайта перечисляет опубликованные практикума, темы и материалы", async ({
  baseURL,
  request,
}) => {
