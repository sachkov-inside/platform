...
test("reading progress persists for a free non-member, reconciles lost responses and isolates Accounts", async ({
  page,
  context,
  browser,
}, testInfo) => {
  ...
  await button.click();
  await expect(
    page.locator("[data-reading-action-state]:visible").getByRole("alert"),
  ...
  await page.screenshot({
    path: resolve(directory, `${testInfo.project.name}-reader.png`),
  });
  ...
  await page.screenshot({
    path: resolve(directory, `${testInfo.project.name}-library.png`),
  });
  ...
  try {
    ...
    await second.reload();
    await expect(
      second.locator("[data-reading-action-state]:visible"),
    ).toHaveAttribute("data-reading-action-state", "anonymous");
    expect(await second.locator("[data-material-reading-status]").count()).toBe(
      0,
    );
  } finally {
    await other.close();
...
test("reading progress reconciles stale windows and account changes without reloading the app", async ({
  page,
  context,
  browser,
}) => {
...
test("reading progress appears on Home and Topic for video and other formats", async ({
  page,
  context,
}) => {
  ...
  for (const material of [
    ...
    {
      slug: "demo-chto-proverit-pered-peredachey-sekretov",
      label: "Прочитано",
      title: "Demo · Что проверить перед передачей секретов",
    },
  ]) {
...
test("reading progress counts a shared material in both real Series", async ({
  page,
  context,
}, testInfo) => {
  await signInFullStack(context, "EXPIRED_MEMBER");
  const button = await openReader(
    page,
    "demo-podgotovka-prilozheniya-k-relizu",
  );
  if ((await button.getAttribute("aria-pressed")) !== "true")
    await button.click();
  await expect(button).toHaveAttribute("aria-pressed", "true");
  ...
  for (const slug of ["demo-series-release", "demo-series-release-shared"]) {
    ...
    await expect(row.locator('[data-series-marker-read="true"]')).toHaveCount(
      1,
    );
    await expect(
    ...
    if (slug.endsWith("shared")) {
      await page.screenshot({
        path: resolve(directory, `${testInfo.project.name}-series.png`),
      });
    }
  }
...
test("reading progress supports Note and lets an expired member remove a protected mark", async ({
  page,
  context,
}) => {
  await signInFullStack(context, "EXPIRED_MEMBER");
  const text = await openReader(
    page,
  ...
  const protectedAction = await openReader(
    page,
  ...
  if ((await protectedAction.getAttribute("aria-pressed")) === "true")
  ...
  await expect(
    page.getByRole("region", { exact: true, name: "Telegram" }),
  ).toBeVisible();
  ...
  await page.waitForURL(
    (url) =>
      !url.pathname.startsWith("/account") && !url.pathname.startsWith("/auth"),
  );
