...
async function resetSeries(page: Page) {
  for (const [index, slug] of materialSlugs.entries()) {
    ...
    await unmark(
      page,
      ["Прочитано", "Просмотрено", "Изучено"][index] ?? "Изучено",
    );
  }
}
...
async function screenshot(page: Page, project: string, surface: string) {
  const directory = await prepareEvidenceDirectory("issue-332");
  ...
  if (captureViewport && surface === "series")
    await page.evaluate(() => {
...
test("profile continuation opens the real series, persists marks and reconciles a lost visible open", async ({
  ...
  await resetSeries(page);
  await page.goto(`/materials/${materialSlugs[0]}`);
  const otherMark = await unmark(page, "Прочитано");
  await otherMark.click();
  await expect(otherMark).toHaveAttribute("aria-pressed", "true");
  ...
  const commandId = (body: string) =>
    /name="commandId"\r\n\r\n([^\r]+)/u.exec(body)?.[1];
  expect(commandId(commands[0] ?? "")).toBeTruthy();
  expect(commandId(commands[1] ?? "")).toBe(commandId(commands[0] ?? ""));
  await button.click();
  await expect(button).toHaveAttribute("aria-pressed", "true");
  await page.goto("/account");
  const resumeSeries = page.getByRole("link", {
    name: "Продолжить продукт Demo · Прогресс обучения",
  });
  ...
  await current
    .locator('a[href^="/materials/video-pro-developer-pipeline"]')
    .click();
  await expect(
    page.getByText("Материал 2 из 3", { exact: true }),
  ).toBeVisible();
  ...
  await page.goto("/account");
  await expect(resumeSeries).toHaveCount(0);
  await page.goto(`/materials/${materialSlugs[0]}`);
  await unmark(page, "Прочитано");
  await page.goto("/account");
...
test("profile resumes real Video progress and excludes playback end", async ({
  page,
  context,
}, testInfo) => {
  ...
  await page.route("https://kinescope.io/**", async (route) => {
    await route.fulfill({
      contentType: "text/html",
      body: "<title>Local video provider</title>",
    });
  });
  await page.addInitScript(() => {
    Object.defineProperty(window, "Kinescope", {
      value: {
        IframePlayer: {
          version: "1.0.0",
          create: (mount: HTMLElement, options: { url: string }) => {
  ...
  await page.goto("/materials/video-pro-developer-pipeline");
  const mark = await unmark(page, "Просмотрено");
  await opened;
  ...
  if (materialId === null || videoId === null)
    throw new Error("Missing real video identity");
  ...
  const card = videos
    .getByRole("link")
    .filter({ hasText: "Видео про Developer Pipeline" });
  await expect(card).toContainText("Продолжить с 2:03");
  await expect(card).toHaveCount(1);
  await card.scrollIntoViewIfNeeded();
  await screenshot(page, testInfo.project.name, "video");
  await card.click();
  await expect(
    page.locator("[data-video-player-mount] iframe"),
  ).toHaveAttribute("data-seek-seconds", "123");
  await save("628");
  await page.goto("/account");
  await expect(
    videos
...
test("profile continuation preserves the account form through errors and excludes denied, prefetched and SSR opens", async ({
  page,
  context,
}) => {
  await page.clock.install();
  ...
  page.on("request", (request) => {
    if (request.url().endsWith("/api/reading-progress/open"))
      opens.push(request.url());
  });
  await page.goto("/account");
  await (
    await fullStackPageRequest(page)
  ).get("/materials/demo-podgotovka-prilozheniya-k-relizu", {
  ...
  await expect(
    page.getByRole("link", { name: /^Продолжить продукт/u }),
  ).toHaveCount(0);
  await page.unroute("**/api/personal-home");
  await page.goto("/");
  await expect(
    page.getByRole("region", { name: "Материалы", exact: true }),
  ).toBeVisible();
});

test("personal Home waits for document visibility before recording an available Reader", async ({
  page,
  context,
}) => {
  await signInFullStack(context);
  ...
  await page.addInitScript(() => {
    ...
    window.addEventListener("test-reader-visible", () => {
      hidden = false;
  ...
  await page.evaluate(() =>
    window.dispatchEvent(new Event("test-reader-visible")),
  );
  await opened;
  expect(requests).toHaveLength(1);
});

test("personal Home retries an already visible open with the same command after the tab is hidden", async ({
  page,
  context,
}) => {
  await signInFullStack(context);
