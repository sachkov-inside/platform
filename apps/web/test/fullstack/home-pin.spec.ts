...
test("author Home pin persists for guests and members, replaces and removes through the real BFF", async ({ page, context, browser, baseURL }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole("textbox", { name: "Название продукта" })).toHaveValue(releaseName);
    ...
    await expect(page.getByText("Продукт закреплён на главной.", { exact: true })).toBeVisible();
    ...
    for (const viewer of [guestPage, memberPage]) {
      ...
      await expect(viewer.getByRole("link", { name: "Открыть продукт", exact: true })).toHaveAttribute("href", "/guides/demo-series-release?from=%2F");
    ...
    await guestPage.getByRole("link", { name: "Открыть продукт", exact: true }).click();
