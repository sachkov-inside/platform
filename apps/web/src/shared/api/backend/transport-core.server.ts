...
export async function executeGeneratedRequest<T>(
  ...
  try {
    ...
    if (cause instanceof ApiError && request.response !== undefined) {
      return { ok: false, problem: cause.body, response: request.response };
    }
    if (cause instanceof BackendConnectionError) throw cause;
...
function parseAccountResponse(result: BackendTransportResult): AuthenticatedAccount {
  ...
  if (!parsed.success) {
    throw new BackendConnectionError(
      "invalid-response",
      "Account response does not match the contract",
    );
  }
