...
export function requestContinueMaterials(accessToken: string) {
  return executeGeneratedRequest(
    (request) => new PersonalHomeService(request).getContinueMaterials(),
    200,
    { accessToken },
  );
}
export function requestRecordMaterialOpen(
  requestBody: {
    materialId: string;
  ...
  return executeGeneratedRequest(
    (request) =>
      new PersonalHomeService(request).recordMaterialOpen({ requestBody }),
    200,
    { accessToken },
  );
}
...
export function requestLearningHome(accessToken: string) {
  return executeGeneratedRequest(
    (request) => new PersonalHomeService(request).getLearningHome(),
    200,
    { accessToken },
  );
}
