...
export function requestReadingStates(
  materialIds: string[],
  accessToken: string,
) {
  return executeGeneratedRequest(
    (request) =>
      new ReadingActivityService(request).getMaterialReadingStates({
...
export function requestSetReadingState(
  input: {
    materialId: string;
    commandId: string;
  ...
  return executeGeneratedRequest(
    (request) =>
      new ReadingActivityService(request).setMaterialReadingState({
        materialId,
        requestBody,
      }),
    200,
