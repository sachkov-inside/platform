...
export const BackNavigationFlow: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole("button", { name: "Открыть руководство AI-first работа" }));
    ...
    await expect(canvas.getByRole("button", { name: "Назад: К руководству" })).toBeVisible();

    await userEvent.click(canvas.getByRole("button", { name: "Назад: К руководству" }));
...
export const TopicMobile: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const playlistsHeading = canvas.getByRole("heading", { name: "Руководства" });
