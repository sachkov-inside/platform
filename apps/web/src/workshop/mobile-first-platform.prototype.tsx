...
const materials = [
  ...
  },
  {
    date: "20 августа",
    duration: "3 мин",
    excerpt: "Руководство становится маршрутом, когда порядок материалов отвечает на вопрос: что делать после этого шага.",
    format: "note",
    icon: List,
    id: "playlist-is-a-route",
    label: "Начало → порядок → результат",
    tags: ["learning", "structure"],
    title: "Руководство — это маршрут",
    tone: "blue",
    topic: "AI-first",
  },
...
function MagazineHome({ audience, openLibrary, openMaterial, openPlaylist, openTopic }: HomeProps) {
  return (
    <div className="mx-auto max-w-[66rem] px-4 pb-16 pt-5 sm:px-7 md:px-10 md:pb-20 md:pt-9" data-page-frame>
      ...
      <SectionHeading action="Все руководства" onAction={() => { openLibrary(); }} title="Руководства" />
  ...
  return (
    <div className="mx-auto max-w-[66rem] px-4 pb-16 pt-5 sm:px-7 md:px-10 md:pb-20 md:pt-9" data-page-frame>
      ...
      <div className="mt-7 flex min-h-14 items-center gap-3 rounded-2xl bg-[#f3f1ed] px-4">
        ...
        <input className="min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-[#5f5e59]" id="library-search" onChange={(event) => { setQuery(event.target.value); }} placeholder="Материал, Руководство, Тема или тег" type="search" value={query} />
      ...
      {hasResults ? <>
        ...
        {visiblePlaylists.length > 0 ? <section aria-labelledby="library-playlists-heading"><CatalogSectionHeading count={visiblePlaylists.length} id="library-playlists-heading" title="Руководства" /><div className="mt-4 grid gap-4 md:grid-cols-2">{visiblePlaylists.map((playlist) => <PlaylistCard audience={audience} key={playlist.id} onOpen={openPlaylist} playlist={playlist} />)}</div></section> : null}
...
function backDestinationLabel(screen: Exclude<PrototypeScreen, "material">): string {
  switch (screen) {
    ...
    case "playlist": return "К руководству";
