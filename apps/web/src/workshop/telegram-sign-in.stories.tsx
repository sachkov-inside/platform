      ...
      <div className="inside-telegram" lang="ru">
        <main
          dangerouslySetInnerHTML={{
            __html: renderTelegramSignInContent(view),
          }}
        />
      </div>
    </>
...
const meta = {
  ...
  args: {
    status: "pending",
    deepLink: "https://t.me/inside_storybook_bot?start=fixture",
  },
  parameters: {
    layout: "fullscreen",
    docs: {
      description: {
        component:
          "Экран Logto из #303. Одна реализация для Storybook и входа; состояния здесь заданы примерами. Кнопка бота в примерах не начинает настоящий вход. Оформление с голубым акцентом принято владельцем 2026-09-06 (#303).",
      },
    },
  },
...
export const Pending: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByRole("link", { name: "Открыть бота" }),
...
export const Unavailable: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByRole("link", { name: "Вернуться ко входу" }),
    ).toHaveAttribute("href", "/sign-in");
    await expect(
      canvas.queryByRole("link", { name: "Открыть бота" }),
