...
export const TopicDesktop: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole("heading", { level: 2, name: "Руководства по теме" })).toBeVisible();
    ...
    await expect(canvas.queryByText(/Руководство может продолжаться/u)).not.toBeInTheDocument();
...
export const TopicEmpty: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByText("Руководств по теме пока нет")).toBeVisible();
...
export const PlaylistDesktop: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole("navigation", { name: "Темы руководства" })).toBeVisible();
    await expect(canvas.queryByText(/Руководство открывается целиком/u)).not.toBeInTheDocument();
    await expect(
...
export const PlaylistEmpty: Story = {
  ...
  render: (args) => <PlaylistPagePrototype {...args} />,
  play: async ({ canvasElement }) => {
    await expect(within(canvasElement).getByText("Руководство пока пусто")).toBeVisible();
  },
};
