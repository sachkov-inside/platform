...
export function EnrollmentsPanel() {
  ...
  const admissionText = admission.data?.admissionRestriction === "moderation" ? "Вступление в сообщество ограничено модерацией. Доступ к материалам сохраняется."
    ...
    : admission.data?.state === "no_access" ? "Действующего права на сообщество нет." : "Проверяем возможность вступления в сообщество…";
  return <section className="rounded-2xl border border-border bg-card p-6">
    <p role="status" className="mb-4 text-sm">{admissionText}</p>
    {admission.isError ? <Button onClick={() => void admission.refetch()}>Повторить проверку сообщества</Button> : null}
    <h2 className="mb-4 text-xl font-semibold">Ваши тарифы</h2>
    {query.isPending ? <p role="status">Загружаем назначения…</p> : query.isError ? <div role="alert"><p>{query.error.message}</p><Button onClick={() => void query.refetch()}>Повторить загрузку</Button></div> : <EnrollmentList items={query.data} />}
  </section>;
}
