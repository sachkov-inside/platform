  ...
  return (
    ...
    >
      ...
      {subscription.inFlightPayment === null ? null : (
        <p
          className="mt-4 rounded-xl border border-border bg-muted/50 p-4 text-sm leading-6"
          role="status"
        >
          Незавершённая операция:{" "}
          {attemptStateLabel(subscription.inFlightPayment.state)}. Новую оплату
          начинать не нужно.
        </p>
      )}
      ...
      {subscription.pendingChange === null ? null : (
        <div className="mt-4 rounded-xl border border-accent/35 bg-accent/6 p-4 text-sm leading-6">
          ...
          <p className="mt-1 text-muted-foreground">
            Согласовано{" "}
            {formatBillingDateTime(subscription.pendingChange.acceptedAt)}. Цена
            следующего периода:{" "}
            {formatKopecks(
              subscription.pendingChange.snapshot.renewalPriceKopecks,
            )}
