...
export function PlaylistCard({
  ...
  return (
    <Link
      aria-label={`${playlist.continuation === undefined ? "Открыть" : "Продолжить"} практикум ${playlist.name}`}
    ...
    >
      <span className="flex items-start justify-between gap-3">
        <span className={cn("inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold", playlist.continuation === undefined ? "bg-white/10 text-white/75" : "bg-accent text-accent-foreground")}>
          ...
          <span>{playlist.continuation === undefined ? `Практикум · ${playlist.countLabel}` : `Продолжить · изучено ${String(playlist.continuation.read)} из ${String(playlist.continuation.total)}`}</span>
        </span>
        <span
          aria-hidden="true"
