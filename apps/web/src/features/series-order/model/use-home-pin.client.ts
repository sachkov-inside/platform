...
function homePinMessage(result: HomePinResult | undefined, saved: boolean): string {
  if (result?.kind === "ready") return saved ? result.pin.seriesId === null ? "Закреп снят с главной." : "Практикум закреплён на главной." : result.pin.seriesId === null ? "Выберите практикум для первого блока главной." : "Авторский закреп сохранён.";
  if (result?.kind === "conflict") return "Закреп изменился в другой вкладке. Состояние обновлено; закрепите практикум ещё раз.";
  ...
  if (result?.kind === "invalid_input") return "Практикум недоступен или в нём нет опубликованных материалов. Обновите состав и повторите действие.";
