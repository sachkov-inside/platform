...
import {
  materialResumeSchema,
  seriesContinuationProjectionSchema,
  continuationLabel,
} from "../model/series-continuation-contract";
...
const projection = z
  .object({
    video: z
      .object({
        material: publishedMaterialProjectionSchema,
        lastOpenedAt: z.iso.datetime(),
        resume: materialResumeSchema,
      })
      .strict()
      .nullable(),
  ...
  .strict();
export async function getPersonalHome(
  accessToken: string,
): Promise<PersonalHomeResult> {
  try {
    const response = await requestLearningHome(accessToken);
    if (!response.ok)
      return {
        kind: response.response.status === 401 ? "hidden" : "unavailable",
      };
    ...
    return {
      ...
      continuation: {
        ...(video === null
          ...
          : {
              video: {
                material: toMaterialPreview(video.material),
          ...
          : {
              series: {
                collection: {
                  ...series.collection,
                  previewItems:
                    series.collection.previewItems.map(toMaterialPreview),
                },
                read: series.read,
                total: series.total,
              },
