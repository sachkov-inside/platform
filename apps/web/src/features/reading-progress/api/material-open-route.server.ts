...
export function handleMaterialOpen(request: Request) {
  return handleAuthenticatedMutation(request, async (form, token) => {
    if (
      [...form.keys()].some(
        (key) => !["materialId", "contentVersion", "commandId"].includes(key),
      )
    )
      return { kind: "invalid_input" };
    const command = materialOpenCommandSchema.safeParse({
      materialId: form.get("materialId"),
      contentVersion: Number(form.get("contentVersion")),
    ...
    try {
      ...
      const result = z
        .object({ openedAt: z.iso.datetime(), replayed: z.boolean() })
        .strict()
        .safeParse(response.body);
      return result.success
        ? { kind: "saved", ...result.data }
        : { kind: "unavailable" };
    } catch {
