...
import {
  materialOpenResultSchema,
  type MaterialOpenCommand,
} from "../model/material-open-contract";
export async function recordMaterialOpen(command: MaterialOpenCommand) {
  const form = new FormData();
  form.set("materialId", command.materialId);
  form.set("contentVersion", String(command.contentVersion));
  form.set("commandId", command.commandId);
  const result = await requestSameOriginMutation(
    "/api/reading-progress/open",
    "POST",
    form,
  );
  if (!result.ok)
