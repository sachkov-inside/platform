...
import { z } from "zod";
import {
  requestReadingStates,
  requestSetReadingState,
...
export function handleSetReadingState(request: Request) {
  return handleAuthenticatedMutation(request, async (form, token) => {
    const parsed = readingCommandSchema.safeParse({
      materialId: form.get("materialId"),
      commandId: form.get("commandId"),
      expectedVersion: Number(form.get("expectedVersion")),
      isRead:
        form.get("isRead") === "true"
          ? true
          : form.get("isRead") === "false"
            ? false
            : null,
    });
    if (!parsed.success || form.get("expectedVersion") === null)
      ...
      if (result.ok) {
        ...
        return saved.success
          ? {
              ...
              replayed: saved.data.replayed,
            }
          : { kind: "unavailable" };
      }
      if (result.response.status === 409) {
        const conflict = z
          .object({ current: readingStateSchema.optional() })
          .safeParse(result.problem);
        return {
          kind: "conflict",
          current: conflict.success ? (conflict.data.current ?? null) : null,
        };
      }
      return {
        kind:
