...
export function VisibleMaterialOpen({
  materialId,
  contentVersion,
  children,
}: {
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: async (input: {
      ...
      if (currentAccount.current === null)
        throw new Error("open_identity_pending");
  ...
  useEffect(() => {
    ...
    if (command.current?.accountId !== accountId)
      command.current = {
        accountId,
        value: { materialId, contentVersion, commandId: crypto.randomUUID() },
        sent: false,
      };
