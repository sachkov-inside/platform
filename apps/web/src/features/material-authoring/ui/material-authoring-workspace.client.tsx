...
import { MaterialAssetFileNode, MaterialAssetImageNode } from "../model/material-asset-nodes";
import {
  ...
  return (
    <div className="mt-5 overflow-clip rounded-xl border border-border bg-card focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/30">
      <div aria-label="Форматирование" className="flex flex-wrap gap-1 border-b border-border bg-muted/65 p-2" role="toolbar">
        ...
        <MaterialAssetUploadButtons controller={assetUploads} disabled={disabled || materialId === null} />
      </div>
      <MaterialAssetUploadQueue controller={assetUploads} />
      {materialId === null ? (
        <p className="border-b border-border bg-muted/40 px-4 py-2 text-xs text-muted-foreground">Сначала создайте черновик, затем добавляйте файлы и изображения.</p>
      ) : null}
      <EditorContent
