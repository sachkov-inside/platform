...
import { assignSubscriptionEnrollment, changeSubscriptionEnrollment, listSubscriptionTiers, readSubscriptionEnrollments } from "../api/enrollments.browser";
...
export function EnrollmentAdminPanel() {
  ...
  const [account, setAccount] = useState(""); const [target, setTarget] = useState("");
  ...
  const assignments = useQuery({ queryKey: ["owner-enrollments", target], enabled: target !== "", queryFn: async () => {
    const result = await readSubscriptionEnrollments({ operationId: crypto.randomUUID(), accountId: target });
    if (!result.ok) throw new Error(billingErrorMessage(result.code)); return result.value.result.items;
  } });
  ...
  return <AdminSection title="Тарифы и назначения" description="Назначение не требует продажи или оплаты. Разовые покупки и другие основания остаются независимыми.">
    ...
    <form className="grid gap-4" onSubmit={event => { event.preventDefault(); setTarget(account); }}>
      <AdminField label="Аккаунт получателя" name="enrollment-account" value={account} onChange={event =>{  setAccount(event.target.value); }} required placeholder="Идентификатор подтверждённого аккаунта" />
      <Button type="submit">Показать назначения</Button>
    </form>
    ...
    {assignments.data ? <><EnrollmentList items={assignments.data} />{assignments.data.map(enrollment => <form key={enrollment.id} className="grid gap-3 border-b border-border pb-5" onSubmit={event =>{  changeExisting(event, enrollment); }}>
      ...
      <label className="grid gap-1 text-sm">Действие<select name="action" className="min-h-11 rounded-xl border border-input bg-background px-3" defaultValue={enrollment.state === "revoked" ? "restore" : "change_term"}>
    ...
    <form className="grid gap-4 border-t border-border pt-5" onSubmit={event => {
      ...
      const command = { accountId: account, origin, sourceRef: formText(data.get("sourceRef")), tierId: tier.tier.id, tierRevision: tier.tier.revision,
        ...
        billingRef: null, reason: formText(data.get("reason")), ...(origin === "course" ? { courseSource: { policyRef: formText(data.get("sourceRef")), verifiedIdentityRef: formText(data.get("identityRef")) } } : {}) };
      ...
      <Button type="submit" disabled={pending || tiers.isPending}>Назначить без оплаты</Button>
    </form>
  </AdminSection>;
}
