...
import {
  assignEnrollmentInputSchema,
  changeEnrollmentInputSchema,
...
export function EnrollmentAdminPanel() {
  ...
  const assign = useMutation({
    ...
    onSuccess: (result) => {
      ...
      announceEnrollmentChange();
      repeat.completeOperation("assign-tier");
      setNotice("Тариф назначен. Платёж и расписание списаний не создавались.");
      setError(undefined);
      setTarget(result.value.result.value.accountId);
  ...
  const change = useMutation({
    ...
    onSuccess: (result) => {
      ...
      announceEnrollmentChange();
      repeat.completeOperation("change-tier");
      setError(undefined);
      setNotice("Назначение обновлено. Остальные права сохранены.");
      void cache.invalidateQueries({ queryKey: ["owner-enrollments"] });
    },
  });
  const lookup = useMutation({
    mutationFn: lookupSubscriptionRecipient,
    onSuccess: (result) => {
      ...
      if (value.state !== "found") {
        setError(
          value.state === "ambiguous"
            ? "Найдено несколько связей. Назначение недоступно до проверки связи."
    ...
    onError: () => {
      setError("Не удалось найти аккаунт. Повторите запрос.");
    },
  });
  ...
  function changeExisting(
    ...
    if (action !== "restore" && action !== "revoke" && action !== "change_term")
      return;
    ...
    const parsed = changeEnrollmentInputSchema.safeParse({
      ...command,
      operationId: repeat.operationId("change-tier", command),
    });
    if (!parsed.success) {
      setError("Проверьте срок и причину изменения.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        <AdminField
          label="Подтверждённая Telegram identity получателя"
          ...
          required
          hint="Точный идентификатор подтверждённой связи, не username."
        />
      ...
      {recipient ? (
        <div className="grid gap-2 rounded-xl border border-border p-4 text-sm [overflow-wrap:anywhere]">
          <p>Подтверждённая связь: {recipient.identityRef}</p>
          <p>Аккаунт: {recipient.accountId}</p>
          <Button
            variant="outline"
            onClick={() => {
              setTarget(account);
              if (target === account) void assignments.refetch();
            }}
          >
            Выбрать получателя и показать назначения
          </Button>
        </div>
      ) : null}
      ...
      {assignments.data ? (
        <>
          <EnrollmentList items={assignments.data} />
          {assignments.data.map((enrollment) => (
            <form
              key={enrollment.id}
              className="grid gap-3 border-b border-border pb-5"
              onSubmit={(event) => {
                changeExisting(event, enrollment);
              }}
            >
              <h3 className="font-semibold">
                Изменить «{enrollment.tier.name}»
              </h3>
              ...
              <AdminField
                label="Окончание по Москве (пусто — без даты)"
                name={`endsAt-${enrollment.id}`}
                type="datetime-local"
                defaultValue={
                  enrollment.endsAt === null
                    ? ""
                    : new Date(
                        new Date(enrollment.endsAt).getTime() +
                          3 * 60 * 60 * 1000,
                      )
                        .toISOString()
                        .slice(0, 16)
      ...
      {assignments.data && tiers.data ? (
        <EnrollmentExpansionPanel items={assignments.data} tiers={tiers.data} />
      ) : null}
      <form
        ...
        onSubmit={(event) => {
          ...
          const command = {
            accountId: target,
            origin,
            sourceRef: formText(data.get("sourceRef")),
            tierId: tier.tier.id,
            ...
            ...(origin === "course"
              ? {
                  courseSource: {
                    policyRef: formText(data.get("sourceRef")),
                    verifiedIdentityRef: recipient?.identityRef ?? "",
                  },
                }
              : {}),
          };
          const parsed = assignEnrollmentInputSchema.safeParse({
            ...command,
            operationId: repeat.operationId("assign-tier", command),
          });
          if (!parsed.success) {
            setError("Проверьте аккаунт, источник, причину и даты назначения.");
            return;
          }
      ...
      >
        ...
        <label className="grid gap-1 text-sm">
          ...
          >
            ...
            {tiers.data
              ?.filter((row) => row.availableForAssignment && !row.archived)
              .map((row) => (
                <option value={row.tier.id} key={row.tier.id}>
                  {row.tier.name}
                  {row.published ? "" : " · не продаётся"}
                </option>
              ))}
          </select>
        </label>
