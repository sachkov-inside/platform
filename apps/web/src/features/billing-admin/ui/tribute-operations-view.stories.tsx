...
] } }, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await expect(canvas.getByText("Выбранная строка сократит ранее подтверждённый доступ. Проверьте начало, окончание и режим подтверждения.")).toBeVisible(); await expect(canvas.getByRole("checkbox", { name: /Нужна проверка/u })).toBeDisabled(); } };
