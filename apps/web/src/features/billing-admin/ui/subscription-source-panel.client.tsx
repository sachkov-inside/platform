...
export function SubscriptionSourcePanel() {
 ...
 const [message, setMessage] = useState<string>(); const repeat = useRepeatableOperations();
 ...
 return <AdminSection title="Подтверждение до регистрации" description="Сохраните проверенный курс для известного пользователя источника. Tribute переносится в разделе «Перенос доступа из Tribute». Запись не создаёт аккаунт, платёж или право сама по себе.">
 {message ? <p role="status">{message}</p> : null}<form className="grid gap-4" onSubmit={event => { event.preventDefault(); const form = new FormData(event.currentTarget); const now = (checkedAt.current ??= new Date().toISOString()); const command = { origin: "course", sourcePolicyRef: formText(form.get("policy")), identityRef: formText(form.get("identity")), checkedAt: now, startsAt: now, endsAt: null, reason: formText(form.get("reason")) }; const parsed = registerSourceInputSchema.safeParse({ ...command, operationId: repeat.operationId("source", command) }); if (!parsed.success) { setMessage("Проверьте источник, пользователя и срок."); return; } mutation.mutate(parsed.data); }}>
