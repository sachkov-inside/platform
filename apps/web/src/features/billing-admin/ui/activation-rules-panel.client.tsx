...
import { formText, AdminField, AdminSection, AdminSelect } from "./admin-form.client";
export function ActivationRulesPanel() {
  ...
  return <AdminSection title="Ссылки активации курса и Tribute" description="Код выбирает правило, но не подтверждает покупку. Право выдаётся только после проверки источника и текущей связи аккаунта.">
    {error || rules.isError ? <p role="alert">{error ?? rules.error?.message}</p> : null}{notice ? <p role="status">{notice}</p> : null}
    {rules.data?.map(rule => <div key={rule.id} className="grid gap-2 border-b border-border pb-4"><p className="font-semibold">{rule.name} · {rule.published ? "Опубликовано" : "Приостановлено"}</p><p className="text-sm [overflow-wrap:anywhere]">Код: {rule.code} · источник: {rule.sourceRef} · {rule.verificationMode === "tribute_registry" ? "Подтверждённый реестр Tribute" : "Проверка курса"}</p><Button variant="outline" disabled={save.isPending} onClick={() => { const { revision, ...value } = rule; const command = { expectedRevision: revision, value: { ...value, published: !rule.published }, reason: rule.published ? "Приостановка владельцем" : "Публикация владельцем" }; save.mutate({ ...command, operationId: repeat.operationId("save-rule", command) }); }}>{rule.published ? "Приостановить" : "Опубликовать"}</Button>
      <form className="grid gap-3" onSubmit={event => {
        event.preventDefault(); const form = new FormData(event.currentTarget);
        const tier = tiers.data?.find(item => item.tier.id === form.get("tier"));
        if (!tier) { setError("Выберите доступный тариф."); return; }
        const { revision, ...value } = rule;
        const command = { expectedRevision: revision, value: { ...value, tierId: tier.tier.id, tierRevision: tier.tier.revision }, reason: formText(form.get("reason")) };
        save.mutate({ ...command, operationId: repeat.operationId("save-rule", command) });
      }}>
        <p className="text-sm">Закреплена редакция тарифа {rule.tierRevision}. Обновление применяется только к новым активациям; прежние назначения сохраняют свой состав.</p>
        <label className="grid gap-1 text-sm">Новая редакция тарифа<select name="tier" defaultValue={rule.tierId} required className="min-w-0 w-full min-h-11 rounded-xl border border-input bg-background px-3"><option value="">Выберите тариф</option>{tiers.data?.filter(item => item.availableForAssignment && !item.archived).map(item => <option value={item.tier.id} key={item.tier.id}>{item.tier.name} · редакция {item.tier.revision}</option>)}</select></label>
        <AdminField label="Причина обновления правила" name="reason" required />
        <Button type="submit" disabled={save.isPending}>Обновить привязку тарифа</Button>
      </form></div>)}
    <form className="grid gap-4" onSubmit={event => {
      ...
      const command = { value: { id: formText(form.get("id")), code: formText(form.get("code")), name: formText(form.get("name")), tierId: tier.tier.id, tierRevision: tier.tier.revision, sourceRef: formText(form.get("source")), verificationMode: formText(form.get("verificationMode")), published: form.get("published") === "on", startsAt: new Date(`${formText(form.get("start"))}+03:00`).toISOString(), endsAt: null }, reason: formText(form.get("reason")) };
      ...
      <AdminSelect label="Способ подтверждения" name="verificationMode" options={[{ value: "course_membership", label: "Проверка утверждённого источника курса" }, { value: "tribute_registry", label: "Подтверждённый реестр Tribute на Platform" }]} /><AdminField label="Подтверждаемый источник" name="source" required hint="Для курса — правило проверки Telegram; для Tribute — обозначение разрешённого источника из реестра ниже. Режим после создания неизменен." /><AdminField label="Начало по Москве" name="start" type="datetime-local" required />
