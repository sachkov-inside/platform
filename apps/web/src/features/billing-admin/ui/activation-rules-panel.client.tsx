...
export function ActivationRulesPanel() {
  ...
  const rules = useQuery({
    queryKey: ["owner-activation-rules"],
    queryFn: async () => {
      const result = await listActivationRules({
  ...
  const save = useMutation({
    mutationFn: saveActivationRule,
    onSuccess: (result) => {
      if (!result.ok) {
        setError(billingErrorMessage(result.code));
        return;
      }
      repeat.completeOperation("save-rule");
      setError(undefined);
      setNotice("Правило сохранено.");
      void cache.invalidateQueries({ queryKey: ["owner-activation-rules"] });
    },
    onError: () => {
      setError("Не удалось сохранить правило. Повторите запрос.");
    },
  });
  return (
    <AdminSection
    ...
    >
      ...
      {rules.data?.map((rule) => (
        <div key={rule.id} className="grid gap-2 border-b border-border pb-4">
          ...
          <Button
            ...
            onClick={() => {
              ...
              const command = {
                ...
                value: { ...value, published: !rule.published },
                reason: rule.published
                  ? "Приостановка владельцем"
                  : "Публикация владельцем",
              };
              save.mutate({
                ...command,
                operationId: repeat.operationId("save-rule", command),
              });
            }}
          >
            {rule.published ? "Приостановить" : "Опубликовать"}
          </Button>
          <form
            ...
            onSubmit={(event) => {
              ...
              const tier = tiers.data?.find(
                (item) => item.tier.id === form.get("tier"),
              );
              if (!tier) {
                setError("Выберите доступный тариф.");
                return;
              }
              const { revision, ...value } = rule;
              const command = {
          ...
          >
            <p className="text-sm">
              Закреплена редакция тарифа {rule.tierRevision}. Обновление
              применяется только к новым активациям; прежние назначения
              сохраняют свой состав.
            </p>
            <label className="grid gap-1 text-sm">
              Новая редакция тарифа
              <select
                name="tier"
                defaultValue={rule.tierId}
                required
              ...
              >
                ...
                {tiers.data
                  ?.filter(
                    (item) => item.availableForAssignment && !item.archived,
                  )
                  .map((item) => (
                    <option value={item.tier.id} key={item.tier.id}>
                      {item.tier.name} · редакция {item.tier.revision}
                    </option>
                  ))}
              </select>
            </label>
            <AdminField
              label="Причина обновления правила"
              name="reason"
              required
            />
            <Button type="submit" disabled={save.isPending}>
              Обновить привязку тарифа
            </Button>
          </form>
        </div>
      ))}
      <form
        ...
        onSubmit={(event) => {
          ...
          const command = {
            value: {
              id: formText(form.get("id")),
              code: formText(form.get("code")),
          ...
          const parsed = saveRuleInputSchema.safeParse({
            ...command,
            operationId: repeat.operationId("save-rule", command),
          });
          if (!parsed.success) {
            setError("Проверьте код, источник и даты.");
            return;
          }
      ...
      >
        <AdminField
          label="Идентификатор нового правила"
          name="id"
          required
          hint="Новый UUID. Код и источник после создания неизменны."
        />
        <AdminField label="Название правила" name="name" required />
        <AdminField
          label="Код активации"
          name="code"
          required
          pattern="[A-Za-z0-9_-]{1,40}"
          hint="До 40 латинских букв, цифр, дефисов или подчёркиваний."
        />
        <label className="grid gap-1 text-sm">
          Тариф
          <select
        ...
        <AdminField
          ...
          required
        />
        <AdminField label="Причина" name="reason" required />
        <label className="flex gap-2 text-sm">
          <input type="checkbox" name="published" /> Опубликовать правило
        </label>
        <Button type="submit" disabled={save.isPending}>
          Создать правило
        </Button>
