...
export function handleSaveActivationRule(request: Request): Promise<Response> { return handleAuthenticatedMutation(request, (form, accessToken) => executeBillingCommand(form, saveRuleInputSchema, ruleOutcomeSchema, input => { const { expectedRevision, value: ruleValue, ...command } = input; const { verificationMode, ...rule } = ruleValue; return requestManageBilling({ ...command, value: { ...rule, ...(verificationMode === undefined ? {} : { verificationMode }) }, ...(expectedRevision === undefined ? {} : { expectedRevision }), operation: "activationRules.save" }, accessToken); })); }
