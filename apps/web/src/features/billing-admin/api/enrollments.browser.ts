import {
  type registerSourceInputSchema,
  sourceOutcomeSchema,
...
export async function listSubscriptionTiers(
  input: z.infer<typeof listTiersSchema>,
) {
...
export async function listActivationRules(
  input: z.infer<typeof listRulesInputSchema>,
) {
  return billingCommandResult(
    await requestSameOriginMutation(
...
export async function saveActivationRule(
  input: z.infer<typeof saveRuleInputSchema>,
) {
  return billingCommandResult(
    await requestSameOriginMutation(
...
export async function previewEnrollmentExpansion(
  input: z.infer<typeof previewExpansionInputSchema>,
) {
  return billingCommandResult(
    await requestSameOriginMutation(
...
export async function applyEnrollmentExpansion(
  input: z.infer<typeof applyExpansionInputSchema>,
) {
  return billingCommandResult(
    await requestSameOriginMutation(
...
export async function readContentCatalog() {
  return billingCommandResult(
    await requestSameOriginMutation(
      ...
      "POST",
      billingCommandPayload({ operationId: crypto.randomUUID() }),
    ),
    contentCatalogOutcomeSchema,
  );
}

export async function registerSubscriptionSource(
  input: z.infer<typeof registerSourceInputSchema>,
) {
  return billingCommandResult(
    await requestSameOriginMutation(
      "/api/authoring/billing/sources/register",
      "POST",
      billingCommandPayload(input),
    ),
    sourceOutcomeSchema,
...
import {
  type lookupRecipientInputSchema,
  recipientOutcomeSchema,
} from "../model/enrollment-operations";
export async function lookupSubscriptionRecipient(
  input: z.infer<typeof lookupRecipientInputSchema>,
) {
  return billingCommandResult(
    await requestSameOriginMutation(
