...
export const readEnrollmentsInputSchema = z.object({
  operationId,
  accountId: z.uuid(),
});
...
export const assignEnrollmentInputSchema = z.object({
  ...
  courseSource: z
    .object({
      ...
      verifiedIdentityRef: z.string().min(1),
    })
    .optional(),
});
export const changeEnrollmentInputSchema = z.object({
  operationId,
  enrollmentId: z.uuid(),
...
export const enrollmentsOutcomeSchema = z.object({
  operationRef: z.uuid(),
  result: z.object({
...
export const enrollmentOutcomeSchema = z.object({
  operationRef: z.uuid(),
  result: z.object({
    outcome: z.literal("enrollment"),
    value: enrollmentSchema,
  }),
});
...
export const saveRuleInputSchema = z.object({
  ...
  value: activationRuleSchema.omit({ revision: true }),
  reason: z.string().min(1).max(1000),
});
export const rulesOutcomeSchema = z.object({
  operationRef: z.uuid(),
  result: z.object({
...
export const previewExpansionInputSchema = z.object({
  operationId,
  tierId: z.uuid(),
  tierRevision: z.int().positive(),
  targets: z
...
export const expansionPreviewSchema = z.object({
  previewRef: z.uuid(),
  expiresAt: z.iso.datetime(),
  targets: previewExpansionInputSchema.shape.targets,
  tier: tierSchema,
});
export const previewExpansionOutcomeSchema = z.object({
  operationRef: z.uuid(),
  result: z.object({
...
export const applyExpansionInputSchema = z.object({
  operationId,
  previewRef: z.uuid(),
});
export const applyExpansionOutcomeSchema = z.object({
  operationRef: z.uuid(),
  result: z.object({
...
export const contentCatalogOutcomeSchema = z.object({
  operationRef: z.uuid(),
  result: z.object({
...
export const registerSourceInputSchema = z.object({
  operationId,
  origin: z.enum(["course", "tribute"]),
  sourcePolicyRef: z.string().min(1).max(256),
  identityRef: z.string().min(1).max(256),
...
export const lookupRecipientInputSchema = z.object({
  operationId,
...
export const recipientOutcomeSchema = z.object({
  ...
  result: z.object({
    ...
    value: z.discriminatedUnion("state", [
      z.object({ state: z.literal("found"), recipient: recipientSchema }),
      z.object({ state: z.literal("not_found") }),
      z.object({ state: z.literal("ambiguous") }),
    ]),
  }),
});
