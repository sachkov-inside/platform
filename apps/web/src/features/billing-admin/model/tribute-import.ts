...
export const tributeCsvColumns = [
  "rowRef",
  "policyRef",
  "subscriptionId",
  "identityRef",
...
export function parseTributeCsv(text: string) {
  if (text.length > 262144)
    throw new Error("Файл должен быть не больше 256 КБ.");
  ...
  for (let index = 0; index < source.length; index++) {
    ...
    if (quoted) {
      ...
      throw new Error("Неверные кавычки в CSV.");
    else cell += char;
  }
  if (quoted) throw new Error("В CSV не закрыты кавычки.");
  if (cell !== "" || record.length > 0 || closed) {
    record.push(cell);
    records.push(record);
  }
  const header = records.shift();
  if (JSON.stringify(header) !== JSON.stringify(tributeCsvColumns))
    throw new Error("Названия и порядок столбцов должны совпадать с шаблоном.");
  if (records.length === 0 || records.length > 100)
    throw new Error("В одном импорте должно быть от 1 до 100 строк.");
  return records.map((cells, index) => {
    if (cells.length !== tributeCsvColumns.length)
      throw new Error(
        `Строка ${String(index + 2)}: неверное количество столбцов.`,
      );
    const input = Object.fromEntries(
      tributeCsvColumns.map((key, column) => [
        key,
        cells[column]?.trim() ?? "",
      ]),
    );
    ...
    if (!parsed.success)
      throw new Error(
        `Строка ${String(index + 2)}: проверьте ${parsed.error.issues.map((issue) => issue.path.join(".")).join(", ")}. Даты указываются в UTC, например 2030-01-01T00:00:00.000Z.`,
      );
    return parsed.data;
  });
}
