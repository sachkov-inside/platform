...
const meta = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(Math.abs(illustration.width - container.width)).toBeLessThan(
      1,
    );
    await expect(Math.abs(illustration.height - container.height)).toBeLessThan(
      1,
    );
  },
  render: (args) => (
...
export const ReducedMotion: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "При системном prefers-reduced-motion показывается сцена 5. Здесь проверяем и изменение настройки во время показа.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    await waitFor(async () => {
      await expect(canvasElement.querySelector(".stage")).toHaveAttribute(
        "data-scene",
        "5",
      );
    });
    await expect(canvasElement.querySelector(".stage")).toHaveClass(
      "is-static",
    );
  },
};
