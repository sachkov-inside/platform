...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "Оплата практикума одной страницей: что входит, сколько стоит, куда придёт чек и одна кнопка. Цену покупатель видит только здесь — программа лишь приглашает оплатить.",
      },
    },
  },
...
export const Ready: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByText("Практикум с сопровождением и общим чатом"),
    ).toBeInTheDocument();
...
export const ExistingAccess: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByText("Этот практикум у вас уже открыт."),
    ).toBeInTheDocument();
    await expect(canvas.getByRole("button", { name: /Купить за/u })).toBeDisabled();
  },
};
