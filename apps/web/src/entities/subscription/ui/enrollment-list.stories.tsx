...
const course: Enrollment = {
  ...
  accountId: "62000000-0000-4000-8000-000000000002",
  origin: "course",
...
const meta = {
  title: "Components/Billing/Enrollment list",
  component: EnrollmentList,
  args: { items: [course] },
  parameters: { layout: "padded" },
} satisfies Meta<typeof EnrollmentList>;
export default meta;
type Story = StoryObj<typeof meta>;
export const Course: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.queryByText("Срок")).not.toBeInTheDocument();
    await expect(
      canvas.queryByText("Без даты окончания"),
    ).not.toBeInTheDocument();
    await expect(
      canvas.getByText("Следующего списания нет"),
    ).toBeInTheDocument();
  },
};
export const IndependentSources: Story = {
  args: {
    items: [
      course,
      {
  ...
  play: async ({ canvasElement }) => {
    await expect(
      within(canvasElement).getByText("Одинаковый состав · 2 основания"),
    ).toBeInTheDocument();
  },
};
export const Revoked: Story = {
  args: { items: [{ ...course, state: "revoked" }] },
};
export const Expired: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByText("Срок")).toBeInTheDocument();
    await expect(canvas.getByText("До 14 августа 2026 г.")).toBeInTheDocument();
  },
};
