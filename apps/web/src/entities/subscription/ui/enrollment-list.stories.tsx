...
const course: Enrollment = { id: "62000000-0000-4000-8000-000000000001", accountId: "62000000-0000-4000-8000-000000000002", origin: "course", startsAt: "2026-09-14T10:00:00Z", endsAt: null, endPolicy: "fixed", revision: 1, state: "active", renewal: "not_applicable", nextChargeAt: null,
 tier: { id: "62000000-0000-4000-8000-000000000003", revision: 1, name: "Подписка Inside", benefits: ["materials", "community"], contentScope: { guideIds: ["62000000-0000-4000-8000-000000000004"], materialIds: [] } },
...
export const Course: Story = { play: async ({ canvasElement }) => { const canvas = within(canvasElement); await expect(canvas.queryByText("Срок")).not.toBeInTheDocument(); await expect(canvas.queryByText("Без даты окончания")).not.toBeInTheDocument(); await expect(canvas.getByText("Следующего списания нет")).toBeInTheDocument(); } };
...
export const Expired: Story = { args: { items: [{ ...course, origin: "tribute", endPolicy: "temporary_membership", endsAt: "2026-08-14T10:00:00Z", state: "expired" }] }, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await expect(canvas.getByText("Срок")).toBeInTheDocument(); await expect(canvas.getByText("До 14 августа 2026 г.")).toBeInTheDocument(); } };
