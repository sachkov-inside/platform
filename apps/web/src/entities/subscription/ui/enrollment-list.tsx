...
export function EnrollmentList({ items }: { readonly items: readonly Enrollment[] }) {
  const groups = Map.groupBy(items, item => JSON.stringify([item.tier.benefits.toSorted(), item.tier.contentScope.allGuides === true, item.tier.contentScope.guideIds.toSorted(), item.tier.contentScope.materialIds.toSorted()]));
  return <div className="grid gap-5">{items.length === 0 ? <p className="text-muted-foreground">Назначенных тарифов пока нет. Разовые покупки сохраняются в разделе «Покупки».</p> : [...groups.entries()].map(([key, group]) => <section key={key} className="grid gap-5">{group.length > 1 ? <h2 className="font-semibold">Одинаковый состав · {group.length} основания</h2> : null}{group.map((item, index) =>
    <article key={item.id} className="grid gap-3 border-b border-border pb-5 last:border-0">
      ...
      <dl className="grid grid-cols-[auto_1fr] gap-x-5 gap-y-2 text-sm"><dt>Начало</dt><dd>{formatBillingDate(item.startsAt)}</dd>
        {item.endsAt === null ? null : <><dt>Срок</dt><dd>{`До ${formatBillingDate(item.endsAt)}`}</dd></>}
        <dt>Материалы</dt><dd>{contentScopeSummary(item.tier.contentScope)}</dd>
        <dt>Поддержка</dt><dd>{item.tier.benefits.includes("support") ? "Входит" : "Не входит"}</dd>
        <dt>Сообщество</dt><dd>{accessComposition(item.tier.benefits).includes("community") ? "Входит в тариф" : "Не входит в тариф"}</dd>
        <dt>Списания Inside</dt><dd>{item.origin === "platform_payment" ? "Условия оплаты — в разделе «Покупки»" : "Следующего списания нет"}</dd>
      </dl>
      {item.benefitTerms && item.origin === "platform_payment" ? <ul className="grid gap-1 text-sm">{item.benefitTerms.filter(term => !isWithheldCapability(term.capability)).map(term => <li key={`${benefitLabel(term.capability)}:${term.startsAt}`}>{term.revoked ? `${benefitLabel(term.capability)}: отозвано` : term.endsAt === null ? benefitLabel(term.capability) : `${benefitLabel(term.capability)}: до ${formatBillingDate(term.endsAt)}`}</li>)}</ul> : null}
      ...
      {accessComposition(item.tier.benefits).includes("community") ? <p className="text-xs leading-5 text-muted-foreground">Участие в сообществе также зависит от правил модерации. Тариф сохраняет право на материалы.</p> : null}
    </article>)}</section>)}</div>;
}

function benefitLabel(capability: string): string { switch (capability) { case "materials": return "Материалы"; case "community": return "Сообщество"; case "support": return "Поддержка"; default: return capability; } }
