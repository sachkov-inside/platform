...
export const enrollmentSchema = z.object({ id: z.uuid(), accountId: z.uuid(), tier: tierSchema,
  ...
  state: z.enum(["scheduled", "active", "expired", "revoked"]), content: z.array(contentScopeEntrySchema).optional(), history: z.array(z.object({ kind: z.string(), reason: z.string(), recordedAt: z.iso.datetime() })).optional(), renewal: z.enum(["not_applicable", "billing_agreement"]), benefitTerms: z.array(z.object({ capability: z.string(), startsAt: z.iso.datetime(), endsAt: z.iso.datetime().nullable(), revoked: z.boolean() })).optional(), nextChargeAt: z.iso.datetime().nullable().optional() });
...
export const enrollmentStateLabels = { scheduled: "Начнётся позже", active: "Действует", expired: "Срок завершён", revoked: "Отозвано" } as const;
