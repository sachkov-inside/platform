...
export function capabilityLabel(capability: AccessCapability): string {
  switch (capability) {
    case "materials":
      return "Все опубликованные материалы и продукты";
    case "support":
 ...
 * наследует период варианта оплаты, у разовой покупки такого периода нет и право выдаётся без даты окончания.
 * Явный `null` означает право без даты окончания в обоих случаях. Договорный срок покупки называет оферта, а не право (#648): «бессрочно» здесь не обещается. Общий чат, который открывают сами
...
export function benefitLines(conditions: {
  ...
  return accessComposition(offer.benefits).map((capability) => {
    ...
    return {
      ...
      term: term === null ? "без даты окончания" : formatMonths(term),
    };
  });
}

/**
 * Чем открыто основание. Оплатой открывается и подписка, и отдельно купленное руководство,
 * поэтому основание названо доступом. Ручная выдача названа выдачей: покупкой она не является,
 * даже когда открывает тот же состав.
 */
...
export function billingErrorMessage(code: BillingFailureCode): string {
  switch (code) {
    ...
    case "payment_in_progress":
      return "Оплата уже прошла или ещё выполняется. Откройте в кабинете раздел «Покупки» — новую покупку начинать не нужно.";
