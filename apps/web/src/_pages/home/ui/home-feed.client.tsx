...
export function HomeFeedView({ initialQuery, createQueryOptions, syncLocation = false }: {
  ...
  return <section className="home-feed" aria-label="Материалы" id="materials">
    <div className="home-feed-toolbar">
      ...
      <FeedFilters query={catalog.searchQuery} topics={page?.kind === "ready" ? page.facets.topics : []} onQueryChange={changeQuery} />
    </div>
...
function FeedFilters({ query, topics, onQueryChange }: {
  readonly query: LibrarySearchQuery;
  readonly topics: readonly { readonly id: string; readonly slug: string; readonly name: string }[];
  ...
  return <div className="home-feed-filters">
    ...
    {topics.length === 0 ? null : <>
      ...
      {topics.length > inlineTopicLimit
        ? <Select value={query.topicSlug ?? "all"} onValueChange={(value) => { chooseTopic(value === "all" ? null : value); }}>
          <SelectTrigger aria-label="Тема" className="home-feed-topic-menu" data-active={query.topicSlug !== null}><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Все темы</SelectItem>
            {topics.map((topic) => <SelectItem key={topic.id} value={topic.slug}>{topic.name}</SelectItem>)}
          </SelectContent>
        </Select>
