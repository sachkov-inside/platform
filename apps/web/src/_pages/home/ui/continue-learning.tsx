...
export function ContinueLearning({ view, readingActions, onRetry }: {
  ...
  return (
    <section aria-labelledby="home-continue" className="mb-10 md:mb-12" data-personal-home-state={view.kind}>
      ...
      {view.kind === "ready" && view.items.length === 0 ? (
        ...
        <ul aria-label="Незавершённые материалы" className="mt-4 grid gap-3 md:grid-cols-2" role="list">
          {view.items.map((item) => <li key={item.id}><ContinueCard item={item} readingAction={readingActions?.get(item.id)} /></li>)}
        </ul>
      ) : view.kind === "loading" ? (
        <div role="status" aria-label="Загружаем ваши материалы" aria-busy="true" className="mt-4 grid gap-3 md:grid-cols-2">
          {[0, 1].map((key) => <div aria-hidden="true" className="h-40 rounded-2xl bg-muted motion-safe:animate-pulse" key={key} />)}
        </div>
      ) : (
