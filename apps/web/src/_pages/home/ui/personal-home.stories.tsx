...
export const Unavailable: Story = { args: { view: { kind: "unavailable" } }, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await expect(canvas.getByRole("heading", { name: "Серии" })).toBeVisible(); await expect(canvas.getByRole("status")).toHaveTextContent("Не удалось загрузить ваши материалы"); } };
