...
const meta = {
  ...
  parameters: { layout: "fullscreen", docs: { description: { component: "Прежний вариант, отклонён владельцем 2026-09-07. Текущая проверка: Pages/Progress walkthrough — продолжение внутри существующих секций руководств и видео." } } },
...
export const CompletedExclusion: Story = { args: { view: { kind: "ready", items: [ended, text] } }, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole("button", { name: /^Просмотрено$/u })); await expect(canvas.queryByText(ended.title)).not.toBeInTheDocument(); await expect(canvas.getByRole("heading", { name: "Руководства" })).toBeVisible(); } };
...
export const Unavailable: Story = { args: { view: { kind: "unavailable" } }, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await expect(canvas.getByRole("heading", { name: "Руководства" })).toBeVisible(); await expect(canvas.getByRole("status")).toHaveTextContent("Не удалось загрузить ваши материалы"); } };
