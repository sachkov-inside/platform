...
const collection = {
  ...
  summary:
    "Учебный пример серии: собираем приложение, готовим окружение и проверяем первый релиз.",
  version: 1,
} as const;
const items = [
...
const meta = {
  ...
  title: "Pages/Authoring/Редактор серии",
...
export const Desktop: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByRole("textbox", { name: "Название серии" }),
    ).toHaveValue(collection.name);
    await expect(
      canvas.getByRole("list", { name: "Материалы серии" }),
    ).toBeVisible();
...
export const KeyboardReorder: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByRole("list", { name: "Материалы серии" }).querySelector("li"),
