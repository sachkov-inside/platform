...
function form(input: object) {
  ...
  value.set("input", JSON.stringify(input));
  return value;
}
...
export const readBroadcasts = (cursor?: string) =>
  read(
    `/api/communications/broadcasts?${query({ cursor })}`,
    broadcastListSchema,
  );
export const readBroadcast = (broadcastId: string) =>
  read(
    `/api/communications/broadcast?${query({ broadcastId })}`,
    broadcastResultSchema,
  );
export const readFunnels = (cursor?: string) =>
  read(`/api/communications/funnels?${query({ cursor })}`, funnelListSchema);
export const readStatistics = (scope: {
  broadcastId?: string;
  funnelId?: string;
  cursor?: string;
}) =>
  read(
    `/api/communications/statistics?${query(scope)}`,
    statisticsResultSchema,
  );
export const readDeliveries = (scope: {
  broadcastId?: string;
  funnelId?: string;
  cursor?: string;
}) =>
  read(`/api/communications/deliveries?${query(scope)}`, deliveryListSchema);
export const readEntries = (contactId: string, cursor?: string) =>
  read(
    `/api/communications/entries?${query({ contactId, cursor })}`,
    entryListSchema,
  );
function result<T>(
...
export async function saveBroadcast(input: SaveBroadcastInput) {
  return result(
    await requestSameOriginMutation(
      "/api/communications/broadcasts/save",
      "POST",
      form(input),
    ),
    broadcastResultSchema,
...
export async function launchBroadcast(input: BroadcastActionInput) {
  return result(
    await requestSameOriginMutation(
      "/api/communications/broadcasts/launch",
      "POST",
      form(input),
    ),
    broadcastResultSchema,
...
export async function pauseBroadcast(input: BroadcastActionInput) {
  return result(
    await requestSameOriginMutation(
      "/api/communications/broadcasts/pause",
      "POST",
      form(input),
    ),
    broadcastResultSchema,
...
export async function resumeBroadcast(input: BroadcastActionInput) {
  return result(
    await requestSameOriginMutation(
      "/api/communications/broadcasts/resume",
      "POST",
      form(input),
    ),
    broadcastResultSchema,
...
export async function cancelBroadcast(input: BroadcastActionInput) {
  return result(
    await requestSameOriginMutation(
      "/api/communications/broadcasts/cancel",
      "POST",
      form(input),
    ),
    broadcastResultSchema,
...
export async function resolveTemplate(input: {
  ...
  return result(
    await requestSameOriginMutation(
      "/api/communications/broadcasts/templates/resolve",
      "POST",
      form(input),
    ),
    templateResultSchema,
