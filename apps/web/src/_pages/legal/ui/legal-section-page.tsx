    ...
    <>
      <header className="max-w-3xl">
        ...
        <p className="mt-4 max-w-[66ch] text-pretty text-base leading-7 text-muted-foreground sm:text-lg">
          Условия, оферты, обработка данных и реквизиты продавца. Каждый
          документ открыт без входа и без оплаты, а у каждой его редакции есть
          постоянный адрес и контрольная сумма.
        </p>
      </header>
      ...
      >
        <h2
          ...
          id="legal-seller-heading"
        >
          Продавец
        </h2>
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          {legalSeller.name}, ИНН {legalSeller.inn}, ОГРНИП {legalSeller.ogrnip}
          . Адрес для письменных обращений, телефон и порядок ответа — в
          документе{" "}
          <Link
            className="underline underline-offset-2"
            href={legalDocumentPath("contacts")}
          >
...
function LegalGroupSection({
  ...
  const entries = LEGAL_NAVIGATION.filter(
