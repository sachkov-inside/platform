...
export const Mobile: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.queryByRole("list", { name: "Практикумы материала" })).not.toBeInTheDocument();
...
export const PlaylistReturn: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const seriesNavigation = within(canvasElement).getByRole("navigation", {
      name: "Навигация по практикуму «Создание Platform Inside»",
    });
    const readerBody = canvasElement.querySelector<HTMLElement>("[data-reader-body]");
    ...
    await expect(within(canvasElement).getByRole("link", { name: "Назад к практикуму" })).toHaveAttribute("href", "/series/platform-inside");
...
export const AccessGuidePurchase: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByRole("heading", { name: "Продолжение входит в практикум" }),
    ).toBeInTheDocument();
    await expect(canvas.getByRole("link", { name: "Купить практикум" })).toHaveAttribute(
      "href",
...
export const AccessNotOffered: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.queryByRole("link", { name: "Купить практикум" })).not.toBeInTheDocument();
  },
};
