 ...
 */
const SeriesLearningContext = createContext<SeriesLearningView>({ kind: "guest" });
...
export function SeriesLearningProvider({
  ...
  return <SeriesLearningContext value={learning}>{children}</SeriesLearningContext>;
}
...
export function SeriesLearningSource({
  ...
  return accountId === null
    ? <SeriesLearningProvider learning={{ kind: "guest" }}>{children}</SeriesLearningProvider>
    : <AccountSeriesLearning key={accountId} accountId={accountId} resolved={reading.resolved} slug={slug}>{children}</AccountSeriesLearning>;
}

function AccountSeriesLearning({ accountId, children, resolved, slug }: {
  readonly accountId: string;
  readonly children: ReactNode;
  readonly resolved: boolean;
  readonly slug: string;
}) {
  const query = useQuery({ queryKey: seriesContinuationQueryKey(accountId, slug), queryFn: () => loadSeriesContinuation(slug), enabled: resolved, retry: false });
  const view = query.isError ? { kind: "unavailable" as const } : query.data;
  const learning: SeriesLearningView = view?.kind === "ready" ? view : view?.kind === "hidden" ? { kind: "guest" } : view?.kind === "unavailable" ? { kind: "unavailable" } : { kind: "loading" };
  return <SeriesLearningProvider learning={learning}>{children}</SeriesLearningProvider>;
}
...
export function PendingSeriesLearning({ children }: { readonly children: ReactNode }) {
  const reading = useMaterialReading();
  return <SeriesLearningProvider learning={reading.accountId === null ? { kind: "guest" } : { kind: "loading" }}>{children}</SeriesLearningProvider>;
}
 ...
 */
export function PendingPurchaseRow({ lockedForGuest, slug }: { readonly lockedForGuest: boolean; readonly slug: string }) {
  const reading = useMaterialReading();
  const expected = rememberedPurchaseRow.get(purchaseRowKey(reading.accountId, slug)) ?? (reading.accountId === null && lockedForGuest);
  return expected ? <div className="mt-4 flex min-h-11 justify-end" data-programme-purchase-row /> : null;
}
...
export function ProgrammeProgress() {
  ...
  return <div className="mt-4 min-h-12 border-t border-border pt-3">
