...
export interface JourneyRun {
  readonly chapter: { readonly header: ReactNode; readonly id: string; readonly name: string } | null;
...
export function SeriesJourneyControls({
  ...
  const [selection, setSelection] = useState(() => ({
    id: (parts.find((entry) => partRows(entry).some((row) => row.slug === requestedMaterial)) ?? parts[0])?.id ?? "programme",
  ...
  const resumeIndex = visible.findIndex((row) => row.slug === continuation?.materialSlug && row.available);
  const restoredIndex = visible.findIndex((row) => row.slug === requestedMaterial);
  ...
  const next = parts.flatMap(partRows).find((row) => row.slug === continuation?.materialSlug && row.available);
  ...
  return <section aria-labelledby="series-materials" className="mt-5 scroll-mt-6 focus:outline-none" ref={routeRef} tabIndex={-1}>
    ...
    {parts.length > 1 ? <p aria-live="polite" className="sr-only">{part?.label}: {part?.kind === "artifacts" ? artifactsInWords(part.count) : formatMaterialCount(visible.length)}</p> : null}
    <div aria-labelledby={part === undefined ? undefined : `series-part-${part.id}`} id={`series-part-panel-${part?.id ?? "programme"}`} role={parts.length > 1 ? "tabpanel" : undefined} tabIndex={parts.length > 1 ? 0 : undefined} className="focus-visible:outline-2 focus-visible:outline-ring">
      {part?.kind === "artifacts" ? part.panel : <>
      {part?.id === "programme" && visible.length === 0 && part.chapterCount === 0 ? <p className="py-10 text-sm leading-6 text-muted-foreground">Программа готовится. Здесь появятся главы и уроки продукта.</p> : null}
      ...
      {offersAccessRecheck ? <Button className="mt-4 h-auto min-h-11 max-w-full whitespace-normal" onClick={() => { router.refresh(); }} variant="outline"><RefreshCw aria-hidden="true" />Повторить проверку доступа</Button> : null}
      <SeriesContinuationProvider materialSlug={next?.slug ?? null}>
        <div className="mt-5 grid gap-6">
          {visibleRuns(part?.kind === "materials" ? part.runs : [], visible.length, count).map((run) => <section aria-labelledby={run.chapter === null ? undefined : `chapter-${run.chapter.id}`} key={run.chapter?.id ?? `open-${String(run.offset)}`}>
            {run.chapter?.header ?? null}
            {run.rows.length === 0 ? null : <ol aria-label={run.chapter === null ? "Материалы продукта" : `Материалы главы «${run.chapter.name}»`} className={cn("grid gap-2", run.chapter === null ? "" : "mt-3")} data-series-order start={run.offset + 1}>
              {run.rows.map((row) => <li onClickCapture={(event) => {
                ...
                if (window.location.pathname === new URL(currentHref, window.location.origin).pathname) window.history.replaceState(window.history.state, "", row.returnHref);
              }} aria-current={next?.slug === row.slug ? "step" : undefined} className="@container/series-entry relative min-w-0 scroll-mt-6 rounded-xl focus-visible:outline-2 focus-visible:outline-ring" data-route-material={row.slug} data-series-ordinal={row.ordinal} key={row.slug} tabIndex={-1}>
                {row.card}
              </li>)}
            </ol>}
          </section>)}
...
function visibleRuns(runs: readonly JourneyRun[], total: number, count: number): readonly JourneyRun[] {
  return runs.flatMap((run) => {
    if (run.rows.length === 0) return run.offset < count || count === total ? [run] : [];
    return run.offset < count ? [{ ...run, rows: run.rows.slice(0, count - run.offset) }] : [];
  });
}
