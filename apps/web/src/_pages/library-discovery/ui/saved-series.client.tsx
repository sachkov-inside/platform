...
export function SavedSeries(props: Props) {
  ...
  return accountId === null ? <LibraryDiscoveryView artifacts={props.artifacts} result={props.result} learning={{ kind: "guest" }} {...(props.returnTarget === undefined ? {} : { returnTarget: props.returnTarget })} /> : <AccountSeries key={accountId} {...props} accountId={accountId} resolved={reading.resolved} />;
}
function AccountSeries({ artifacts, result, accountId, resolved, returnTarget }: Props & { readonly accountId: string; readonly resolved: boolean }) {
  ...
  return <LibraryDiscoveryView artifacts={artifacts} result={result} {...(returnTarget === undefined ? {} : { returnTarget })} learning={view?.kind === "ready" ? view : view?.kind === "hidden" ? { kind: "guest" } : view?.kind === "unavailable" ? { kind: "unavailable" } : { kind: "loading" }} onRetry={() => { void query.refetch(); }} />;
}
