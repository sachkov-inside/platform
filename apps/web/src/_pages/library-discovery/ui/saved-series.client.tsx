...
export function SavedSeries(props: Props) {
  ...
  return accountId === null
    ? <GuideProgrammeView artifacts={props.artifacts} guideOffer={props.guideOffer ?? null} learning={{ kind: "guest" }} result={props.result} />
    : <AccountSeries key={accountId} {...props} accountId={accountId} resolved={reading.resolved} />;
}

function AccountSeries({ artifacts, result, accountId, resolved, guideOffer = null }: Props & { readonly accountId: string; readonly resolved: boolean }) {
