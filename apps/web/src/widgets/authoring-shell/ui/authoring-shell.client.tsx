    ...
    <div className="min-h-svh bg-background text-foreground md:flex md:h-svh md:min-h-0 md:overflow-hidden">
      ...
      <aside aria-label="Редактор" className="hidden w-56 shrink-0 bg-sidebar text-sidebar-foreground md:flex md:flex-col">
        <div className="flex min-h-0 flex-1 flex-col px-3 py-4">
          <Link className="flex min-h-12 items-center gap-3 rounded-xl px-3 text-sidebar-foreground no-underline" href={materialsHref}>
            ...
            <span>
              ...
              <span className="block font-mono text-xs text-sidebar-foreground/60">рабочее пространство</span>
            </span>
          </Link>
...
function AuthoringLink({ current = false, href, icon, label }: { readonly current?: boolean; readonly href: Route; readonly icon: ReactNode; readonly label: string }) {
  return (
    <Link aria-current={current ? "page" : undefined} className={cn("flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-medium text-sidebar-foreground/72 no-underline transition-colors motion-reduce:transition-none", "hover:bg-sidebar-accent hover:text-sidebar-accent-foreground", current && "bg-sidebar-accent text-sidebar-accent-foreground")} href={href}>
...
function MobileLink({ children, current = false, href, label }: { readonly children: ReactNode; readonly current?: boolean; readonly href: Route; readonly label: string }) {
  return (
    <Link aria-current={current ? "page" : undefined} className={cn("flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-lg px-2 text-xs font-medium leading-none text-muted-foreground no-underline", "transition-colors active:bg-muted motion-reduce:transition-none [&_svg]:size-4.5", current && "text-foreground [&_svg]:text-accent")} href={href}>
