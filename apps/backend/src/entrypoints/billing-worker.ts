...
void bootstrap().catch((error: unknown) => reportProcessFailure("billing-worker", error));
async function bootstrap(): Promise<void> {
  const application = await NestFactory.createApplicationContext(BillingWorkerModule.forRoot(), { logger: new StructuredNestLogger() });
  ...
  jobs.on("error", (error: unknown) => reportQueueFailure("billing-worker", error));
  await runWorker({ application, databaseUrl: config.database.url, jobs, process: "billing-worker", readiness: application.get(OperationalReadiness),
    async registerJobs() {
      ...
      await jobs.work(recoveryQueue, observeJob("billing-worker", recoveryQueue, async () => {
      ...
      await jobs.work(renewalQueue, observeJob("billing-worker", renewalQueue, async () => {
      ...
      await jobs.work(noticeQueue, observeJob("billing-worker", noticeQueue, async () => {
      ...
      await jobs.work(communityQueue, observeJob("billing-worker", communityQueue, async () => {
