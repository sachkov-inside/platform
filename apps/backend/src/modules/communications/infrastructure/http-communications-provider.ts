...
const TELEGRAM_CREDENTIAL_URL =
  /\bapi\.telegram\.org\.?(?::[0-9]+)?\/(?:file\/)?bot[^/\s]+/iu;
const errorStatuses = {
  unauthorized: 401,
  forbidden: 403,
  not_found: 404,
...
export class HttpCommunicationsProvider {
  ...
  async execute(request: ProviderRequest): Promise<CommunicationsResult> {
    ...
    try {
      ...
      if (
        response.status !== 200 ||
        !matchesOperation(request, value) ||
        ("template" in value &&
          value.template.botIdentity !== this.config.botIdentity) ||
        ("templates" in value &&
          value.templates.some(
            (template) => template.botIdentity !== this.config?.botIdentity,
          )) ||
        TELEGRAM_CREDENTIAL_URL.test(JSON.stringify(value))
      )
        return communicationsFailure("provider_invalid_response");
...
function matchesOperation(
  ...
  value: Extract<ProviderResponse, { status: "ok" }>,
): boolean {
  switch (request.operation) {
    ...
    case "templates.save":
      return (
        "template" in value &&
        sameId(value.template.templateId, request.payload.templateId)
      );
    case "templates.testSend":
      return "testDeliveryId" in value;
    case "funnels.read":
    case "funnels.save":
    case "funnels.publish":
    case "funnels.lifecycle":
    case "funnels.rollback":
      return (
        "funnel" in value &&
        sameId(value.funnel.funnelId, request.payload.funnelId)
      );
    case "funnels.preview":
      return (
        "preview" in value &&
        sameId(value.preview.funnelId, request.payload.funnelId)
      );
    case "templates.list":
      return "templates" in value;
    case "funnels.list":
      return "funnels" in value;
    case "broadcasts.read":
    case "broadcasts.save":
    case "broadcasts.launch":
    case "broadcasts.lifecycle":
      return (
        "broadcast" in value &&
        sameId(value.broadcast.broadcastId, request.payload.broadcastId)
      );
    case "intro.read":
      return "intro" in value;
    case "intro.save":
      return (
        "intro" in value && sameId(value.intro.introId, request.payload.introId)
      );
    case "statistics.read":
      return "statistics" in value;
    case "deliveries.read":
      return "deliveries" in value;
    case "delivery.resolve":
      return (
        "deliveryId" in value &&
        sameId(value.deliveryId, request.payload.deliveryId) &&
        sameId(value.partId, request.payload.partId) &&
        value.outcome ===
          (request.payload.action === "skip" ? "skipped" : "retry_requested")
      );
    case "broadcasts.list":
      return "broadcasts" in value;
    case "entries.read":
      return "entries" in value;
    case "tracking.resolve":
      return "safeUrl" in value;
    case "tracking.recordHit":
      return (
        "eventId" in value && sameId(value.eventId, request.payload.eventId)
      );
