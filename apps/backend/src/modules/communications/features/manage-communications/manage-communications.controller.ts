...
const problemSchema = z.object({
  ...
  title: z.string(),
  status: z.number().int(),
  code: communicationsFailureSchema.shape.error.shape.code,
});
const templateLookupSchema = z.strictObject({
...
export class ManageCommunicationsController {
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "manageCommunications",
    summary: "Manage Telegram communications as the authenticated Account",
  })
  ...
  async execute(
    ...
    return toHttpResult(
      await this.communications.execute(account.accountId, body),
    );
  }
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "resolveCommunicationsTemplate",
    summary:
  ...
  async resolveTemplate(
    ...
    const reference = templateReferenceSchema.safeParse(
      parsed.success ? parsed.data.reference : undefined,
    );
    if (!parsed.success || !reference.success)
      return toHttpResult({ ok: false, error: { code: "invalid_input" } });
    return toHttpResult(
      await this.communications.execute(account.accountId, {
