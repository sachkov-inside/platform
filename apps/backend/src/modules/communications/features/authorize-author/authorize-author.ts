...
import {
  ...
  type AuthorAuthorizationRequest,
...
export async function authorizeAuthor(
  ...
  if (
    subject.kind === "telegram" &&
    subject.botIdentity !== dependencies.botIdentity
  )
    return denied;
  const result = await dependencies.links.find({
  ...
  if (
    link === null ||
    (subject.kind === "telegram" &&
      link.telegramIdentityRef !== subject.telegramIdentityRef)
  )
    return denied;
