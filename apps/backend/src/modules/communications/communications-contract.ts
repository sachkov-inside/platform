...
import {
  ...
  responseSchema,
  errorSchema,
  type authorizationRequestSchema,
...
export const managementSchemas = requestSchema.options
  .filter(
    (schema) =>
      !["tracking.resolve", "tracking.recordHit", "eligibility.check"].includes(
        schema.shape.operation.value,
      ),
  )
...
export type AuthorAuthorizationRequest = z.infer<
...
const providerSuccessSchema = z.union(
  responseSchema.options.filter(
    (
      schema,
    ): schema is Exclude<
      (typeof responseSchema.options)[number],
      typeof errorSchema
    > => schema !== errorSchema,
  ),
);
export const targetErrorSchema = z.object({
  url: z.url(),
