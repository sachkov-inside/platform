...
export const manageActivationRuleSchema = z
  .strictObject({
    operationId: z.uuid(),
  ...
  .refine(
    (value) =>
      value.value.endsAt === null || value.value.endsAt > value.value.startsAt,
  );
...
export const activationEvidenceSchema = z.strictObject({
  contractVersion: z.literal(ACTIVATION_CONTRACT_VERSION),
  audience: z.literal("inside.platform.subscription-activation"),
  evidenceRef: z.uuid(),
  attemptId: z.uuid(),
  sourceRef: sourceRefSchema,
  identityRef: sourceRefSchema,
...
export interface ActivationBindings {
  find(query: { accountRef: string }): Promise<
    | {
        ok: true;
        link: {
          accountId: string;
...
export const registerSourceSchema = z
  .strictObject({
    ...
    sourcePolicyRef: sourceRefSchema,
    identityRef: sourceRefSchema,
    checkedAt: instantSchema,
    startsAt: instantSchema,
    endsAt: instantSchema.nullable(),
  ...
  .refine((value) =>
    value.origin === "course"
      ? value.endsAt === null
      : value.endsAt !== null && value.endsAt > value.startsAt,
  );
export const sourceEntitlementViewSchema = z.strictObject({
  id: z.uuid(),
  origin: z.enum(["course", "tribute"]),
  sourceRef: sourceRefSchema,
  sourcePolicyRef: sourceRefSchema,
...
export const ownSubscriptionAccessQuerySchema = z.strictObject({
  contractVersion: z.literal(ACTIVATION_CONTRACT_VERSION),
  accountRef: sourceRefSchema,
  identityRef: sourceRefSchema,
  linkRef: z.uuid(),
