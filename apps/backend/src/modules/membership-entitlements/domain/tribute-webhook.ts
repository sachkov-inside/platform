...
const payload = z.strictObject({
  ...
  currency: z.string(),
  user_id: z.int(),
  trb_user_id: z.string(),
  telegram_user_id: z.int().positive(),
  channel_id: z.int(),
  channel_name: z.string(),
  expires_at: timestamp,
  telegram_username: z.string().optional(),
  type: z.enum(["regular", "gift", "trial"]).optional(),
});
export const tributeWebhookSchema = z.discriminatedUnion("name", [
  z.strictObject({
    name: z.literal("new_subscription"),
    created_at: timestamp,
    sent_at: timestamp,
    payload,
  }),
  z.strictObject({
    name: z.literal("renewed_subscription"),
    created_at: timestamp,
    sent_at: timestamp,
    payload: payload.extend({
      type: z.enum(["regular", "gift", "trial"]),
      email: z.string().optional(),
      web_app_link: z.string().optional(),
    }),
  }),
  z.strictObject({
    name: z.literal("cancelled_subscription"),
    created_at: timestamp,
    sent_at: timestamp,
