    ...
    status:
      row.revokedAt !== null
        ...
        : state.observation === "source_ended" &&
            state.mode === "temporary_membership"
          ? "suspended_source"
          : row.accountId === null
            ? "pending_identity"
            : state.mode === "temporary_membership" &&
                (state.observation !== "member" ||
                  state.observedUntil === null ||
                  new Date(state.observedUntil) <= now)
              ? "pending_verification"
              : new Date(state.startsAt) > now
                ? "scheduled"
                : new Date(state.endsAt) <= now
                  ? "expired"
                  : "active",
  });
}
...
export async function projectTributeSource(
  ...
  if (row.accountId !== null && row.accountId !== accountId)
    return {
      ok: false as const,
  ...
  if (row.revokedAt !== null || existing?.revokedAt != null) {
    if (row.revokedAt === null && existing?.revokedAt != null)
      await tx.sourceEntitlement.update({
        where: { id: row.id },
        data: { revokedAt: existing.revokedAt },
      });
    return { ok: true as const, enrollmentId: row.enrollmentId };
  }
  const temporary = state.mode === "temporary_membership";
  const end =
    temporary && state.observedUntil !== null
      ? new Date(
          Math.min(Date.parse(state.endsAt), Date.parse(state.observedUntil)),
        )
  ...
  if (existing === null) {
    const result = await assignEnrollmentInTransaction(
      ...
      actorId,
      {
        ...
        tierRevision: state.tier.revision,
        terms: {
          startsAt: state.startsAt,
          endsAt: safeEnd.toISOString(),
          endPolicy: temporary ? "temporary_membership" : "confirmed_external",
        },
        billingRef: null,
        reason,
      },
    ...
    await tx.subscriptionEnrollment.update({
      ...
      data: {
        startsAt,
        endsAt: end > startsAt ? end : existing.endsAt,
        endPolicy: temporary ? "temporary_membership" : "confirmed_external",
    ...
    await tx.accessGrant.updateMany({
      ...
      data: {
        startsAt,
        validUntil: end > startsAt ? end : existing.endsAt,
        revokedAt: suspended ? now : null,
        revision: { increment: 1 },
        reason,
      },
    });
  ...
  if (!temporary)
    await tx.legacyClassification.updateMany({
      where: { accountId, bridgeEnabled: true },
      data: { bridgeEnabled: false, revision: { increment: 1 } },
    });
