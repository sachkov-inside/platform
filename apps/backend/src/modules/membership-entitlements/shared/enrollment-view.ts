...
export async function enrollmentSourceState(prisma: MembershipEntitlementsPrisma, enrollmentId: string, now: Date) {
  const source = await prisma.sourceEntitlement.findFirst({ where: { enrollmentId, origin: "tribute" } });
  const state = tributeStateSchema.safeParse(source?.tributeState);
  if (!state.success || state.data.mode !== "temporary_membership" || source?.revokedAt != null) return undefined;
  if (state.data.observation === "source_ended") return "suspended_source" as const;
  if (state.data.observation !== "member" || state.data.observedUntil === null || Date.parse(state.data.observedUntil) <= now.getTime()) return "pending_verification" as const;
  return undefined;
}
