...
import type { MembershipAccessPrisma, MembershipEntitlementsPrismaClient } from "../../infrastructure/prisma.js";
...
export async function resolveMembershipForAccess(prisma: MembershipEntitlementsPrismaClient, accountId: AccountId, now: Date, guideIds?: readonly string[], materialId?: string): Promise<MembershipAccessState> {
  return prisma.$transaction(async transaction => {
    ...
    return readMembershipForAccess(transaction, accountId, now, guideIds, materialId);
  });
}
...
async function readMembershipForAccess(prisma: MembershipAccessPrisma, accountId: AccountId, now: Date, guideIds?: readonly string[], materialId?: string): Promise<MembershipAccessState> {
  const result = await resolveAccessCapabilities(prisma, accountId, now, guideIds === undefined && materialId === undefined ? undefined : { guideIds: guideIds ?? [], materialId });
