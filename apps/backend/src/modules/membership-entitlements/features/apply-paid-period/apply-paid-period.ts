  ...
  .object({
    ...
    enrollment: z
      .strictObject({
        ...
        benefitTerms: z.array(grantTermsSchema).min(1).max(100).optional(),
      })
      .optional(),
  })
  .strict();
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    if (enrollmentId === null && command.enrollment === undefined) {
      ...
      if (z.uuid().safeParse(purchaseRef).success) {
        const imported = await transaction.subscriptionEnrollment.findUnique({
          where: {
            origin_sourceRef: {
        ...
        if (imported !== null && imported.accountId !== command.accountId)
          return accessFailure("operation_conflict");
        enrollmentId = imported?.id ?? null;
      }
    }
    if (command.enrollment !== undefined) {
      const paid = command.enrollment;
      const sourceRef = `purchase:${paid.purchaseRef}`;
      const prior = await transaction.subscriptionEnrollment.findUnique({
        where: { origin_sourceRef: { origin: "platform_payment", sourceRef } },
      });
      if (prior !== null && prior.accountId !== command.accountId)
        return accessFailure("operation_conflict");
      const enrollment =
        prior ??
        (await transaction.subscriptionEnrollment.create({
          data: {
            id: randomUUID(),
            accountId: command.accountId,
            tierId: paid.tier.id,
            tierRevision: paid.tier.revision,
            snapshot: paid.tier,
            origin: "platform_payment",
            sourceRef,
            startsAt: new Date(paid.startsAt),
            endsAt: new Date(paid.endsAt),
            endPolicy: "fixed",
            billingRef: paid.billingRef,
            revision: 1,
      ...
      if (command.revoked && enrollment.revokedAt === null) {
        ...
        await transaction.subscriptionEnrollment.update({
    ...
    if (command.enrollment === undefined && enrollmentId !== null) {
      const imported =
        await transaction.subscriptionEnrollment.findUniqueOrThrow({
    ...
    const data = {
      ...
      revokedAt:
        enrollmentRevokedAt ??
        (command.revoked ? now : (existing?.revokedAt ?? null)),
    ...
    for (const terms of command.enrollment?.benefitTerms ?? []) {
      for (const capability of terms.capabilities) {
        ...
        const current = await transaction.accessGrant.findUnique({
          where: { source_sourceRef: { source: "paid", sourceRef } },
        });
        if (current !== null && current.accountId !== command.accountId)
          throw new Error("Paid composition account conflict");
        ...
        const benefit = {
          ...
          startsAt: new Date(terms.startsAt),
          validUntil:
            terms.validUntil === null ? null : new Date(terms.validUntil),
