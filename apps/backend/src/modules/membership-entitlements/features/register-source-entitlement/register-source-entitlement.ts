...
export async function registerSourceEntitlement(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (receipt !== null)
      return fingerprint.recognizes(receipt.fingerprint)
        ? {
            ok: true as const,
            value: sourceEntitlementViewSchema.parse(receipt.result),
          }
    ...
    const existing = await tx.sourceEntitlement.findUnique({
      where: { origin_sourceRef: { origin: command.origin, sourceRef } },
    });
    const row =
      existing ??
      (await tx.sourceEntitlement.create({
        data: {
          id: randomUUID(),
          origin: command.origin,
          sourceRef,
          sourcePolicyRef: command.sourcePolicyRef,
    ...
    const value = sourceEntitlementViewSchema.parse({
      id: row.id,
      origin: row.origin,
      sourceRef: row.sourceRef,
      sourcePolicyRef: row.sourcePolicyRef,
