...
export async function assignEnrollmentInTransaction(
  ...
  await lockAccountAccess(
    tx,
    `enrollment:${command.origin}:${command.sourceRef}`,
  );
  await lockAccountEntitlementChanges(tx, command.accountId);
  const existing = await tx.subscriptionEnrollment.findUnique({
    where: {
      origin_sourceRef: {
  ...
  if (command.origin === "course" && command.courseSource !== undefined) {
    const source = await tx.sourceEntitlement.findUnique({
      where: {
        origin_sourceRef: { origin: "course", sourceRef: command.sourceRef },
      },
    });
    if (
      source !== null &&
      source.accountId !== null &&
      source.accountId !== command.accountId
    )
      return accessFailure("identity_conflict");
  }
  const row =
    existing ??
    (await tx.subscriptionEnrollment.create({
      data: {
        id: randomUUID(),
        accountId: command.accountId,
        tierId: command.tierId,
        tierRevision: command.tierRevision,
        snapshot: tier,
        origin: command.origin,
        sourceRef: command.sourceRef,
  ...
  if (command.origin === "course" && command.courseSource !== undefined) {
    await tx.sourceEntitlement.upsert({
      ...
      create: {
        ...
        origin: "course",
        sourceRef: command.sourceRef,
        sourcePolicyRef: command.courseSource.policyRef,
        identityRef: command.courseSource.verifiedIdentityRef,
        accountId: row.accountId,
        enrollmentId: row.id,
        revision: 1,
        evidence: {
  ...
  await tx.accessReceipt.create({
    data: {
      scope: actorId ?? "activation",
      operationId: command.operationId,
      fingerprint: fingerprint.digest,
      payload: {
        command,
        before: existing === null ? null : enrollmentView(existing, now),
        after: result.value,
      },
      result,
      createdAt: now,
