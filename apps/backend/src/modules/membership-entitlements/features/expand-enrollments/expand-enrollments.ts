...
const storedSchema = z.strictObject({
  command: previewEnrollmentExpansionSchema,
...
function includesPrevious(
  previous: z.infer<typeof tierSnapshotSchema>,
  next: z.infer<typeof tierSnapshotSchema>,
) {
  return (
    previous.benefits.every((value) => next.benefits.includes(value)) &&
    (previous.contentScope.allGuides !== true ||
      next.contentScope.allGuides === true) &&
    previous.contentScope.guideIds.every((id) =>
      scopeIncludesGuide(next.contentScope, id),
    ) &&
    previous.contentScope.materialIds.every((id) =>
      next.contentScope.materialIds.includes(id),
    )
  );
}
export async function previewEnrollmentExpansion(
  ...
  const parsed = previewEnrollmentExpansionSchema.safeParse(input);
...
export async function applyEnrollmentExpansion(
  ...
  return prisma.$transaction(async (tx) => {
    const fingerprint = accessFingerprint({
      action: "expandEnrollments",
      command,
    });
    const receipt = await readAccessReceipt(tx, actorId, command.operationId);
    if (receipt !== null)
      return fingerprint.recognizes(receipt.fingerprint)
        ? appliedSchema.parse(receipt.result)
        : accessFailure("operation_conflict");
    ...
    const initial = await tx.subscriptionEnrollment.findMany({
      where: {
        id: { in: stored.command.targets.map((row) => row.enrollmentId) },
      },
    });
    for (const account of [
      ...new Set(initial.map((row) => row.accountId)),
    ].sort())
      await lockAccountEntitlementChanges(tx, account);
    const rows = await tx.subscriptionEnrollment.findMany({
      where: {
        id: { in: stored.command.targets.map((row) => row.enrollmentId) },
      },
    });
    for (const target of stored.command.targets) {
      ...
      if (
        ...
        return accessFailure("revision_conflict");
      if (
        !includesPrevious(tierSnapshotSchema.parse(row.snapshot), stored.tier)
      )
        return accessFailure("invalid_input");
    }
    for (const row of rows) {
      await tx.subscriptionEnrollment.update({
        where: { id: row.id },
        data: {
          snapshot: stored.tier,
          tierRevision: stored.tier.revision,
      ...
      if (grant === undefined)
        throw new Error("Enrollment has no access grant");
      ...
      const temporary =
        row.origin === "tribute" && row.endPolicy === "temporary_membership";
      const temporaryRevokedAt =
        grants.find((item) => item.revokedAt !== null)?.revokedAt ?? null;
      const temporaryEnds = grants
        .map((item) => item.validUntil)
        .filter((end): end is Date => end !== null);
      const effectiveEnd =
        temporary && temporaryEnds.length > 0
          ? new Date(
              Math.min(
                ...temporaryEnds.map((end) => end.getTime()),
                row.endsAt?.getTime() ?? Infinity,
              ),
            )
          : row.endsAt;
      const previousBenefits = new Set(
        tierSnapshotSchema.parse(row.snapshot).benefits,
      );
    ...
    await tx.accessReceipt.create({
      data: {
        ...
        payload: {
          command,
          before: rows.map((row) => enrollmentView(row, now)),
          tier: stored.tier,
        },
        result,
        createdAt: now,
