...
} from "../accept-evidence/validate-membership-evidence.js";
...
export async function observeTemporaryTribute(
  ...
  for (const id of sourceIds) {
    ...
    if (
      ...
      source.identityRef !== evidence.telegramIdentityRef ||
    ...
    if (
      ...
      continue;
    if (
      state.observationVersion !== null &&
      BigInt(state.observationVersion) >= BigInt(evidence.evidenceVersion)
    )
      continue;
    ...
    const cutoff = Math.min(
      ...
      policy?.temporaryUntil?.getTime() ?? now.getTime(),
    );
    const validUntil = Math.min(
      Date.parse(evidence.validUntil),
      Date.parse(evidence.checkedAt) + MAX_EVIDENCE_VALIDITY_MS,
      cutoff,
    );
    const member =
      evidence.decision === "member" &&
      policy?.enabled === true &&
      validUntil > now.getTime();
    const next = tributeStateSchema.parse({
      ...state,
      observation:
        evidence.decision === "not_member"
          ? "source_ended"
          : member
            ? "member"
            : "observation_stale",
      observedUntil: new Date(validUntil).toISOString(),
      observationVersion: String(evidence.evidenceVersion),
    });
    const saved = await tx.sourceEntitlement.update({
      ...
      data: {
        ...
        evidence: { method: "accepted_membership", evidence },
      },
    });
    const result = await projectTributeSource(
      ...
      null,
      evidence.decision === "not_member"
        ? "Подтверждён выход из прежнего источника Tribute"
        : "Обновлено временное подтверждение Tribute",
      now,
    );
    if (!result.ok)
      throw new Error("Locked temporary source projection failed");
  }
}
