...
export async function activateTributeRegistry(
  tx: MembershipEntitlementsPrisma,
  policyRef: string,
  accountId: string,
  request: z.infer<typeof activationEvidenceSchema>,
  tier: TierSnapshot,
  now: Date,
): Promise<ActivationOutcome> {
  const pending: ActivationOutcome = {
  ...
  if (
    ...
    (source.accountId !== null && source.accountId !== accountId)
  )
    return pending;
  const state = parsed.data;
  if (
    state.tier.id !== tier.id ||
    state.subscriptionId !== policy.subscriptionId
  )
    return pending;
  ...
  if (!existing) {
    const projected = await projectTributeSource(
      ...
      accountId,
      null,
      "Verified Tribute registry activation",
      now,
    );
    if (!projected.ok || projected.enrollmentId === null) return pending;
  }
  const attached = await tx.sourceEntitlement.findUniqueOrThrow({
    where: { id: source.id },
  });
  if (attached.enrollmentId === null) return pending;
  const row = await tx.subscriptionEnrollment.findUniqueOrThrow({
    where: { id: attached.enrollmentId },
  });
  const enrollment = enrollmentView(row, now);
  const sourceState = await enrollmentSourceState(tx, row.id, now);
  if (sourceState !== undefined && enrollment.state !== "revoked")
    enrollment.state = sourceState;
  return {
    contractVersion: ACTIVATION_CONTRACT_VERSION,
    attemptId: request.attemptId,
    state:
