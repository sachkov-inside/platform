...
export async function beginActivation(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const prior = await tx.activationAttempt.findUnique({
      where: { id: command.attemptId },
    });
    if (prior !== null)
      return prior.identityRef === command.identityRef &&
        prior.ruleId === rule.id
        ? {
            ok: true as const,
            value: activationOutcomeSchema.parse(prior.result),
          }
        : accessFailure("operation_conflict");
    const value: ActivationOutcome = {
...
export async function readActivationReceipt(
  prisma: MembershipEntitlementsPrismaClient,
  input: unknown,
) {
  ...
  const receipt = await prisma.accessReceipt.findUnique({
    where: {
      scope_operationId: {
        ...
        operationId: parsed.data.evidenceRef,
      },
    },
  });
...
export async function activateSubscription(
  ...
  if (
    checkedAt > now ||
    now.getTime() - checkedAt.getTime() > sourceProofLifetimeMilliseconds ||
    validUntil <= now ||
  ...
  if (
    linked.link === null ||
    linked.link.telegramIdentityRef !== evidence.identityRef
  )
