...
export async function changeEnrollment(
  ...
  return prisma.$transaction((tx) =>
    changeEnrollmentInTransaction(tx, actorId, command, now),
  );
}
export async function changeEnrollmentInTransaction(
  tx: MembershipEntitlementsPrisma,
  actorId: string,
  command: z.infer<typeof changeEnrollmentSchema>,
  now: Date,
): Promise<EnrollmentResult> {
  ...
  const target = await tx.subscriptionEnrollment.findUnique({
  ...
  if (
    ...
    return accessFailure("invalid_input");
  if (command.action === "restore" && row.revokedAt === null)
    return accessFailure("revision_conflict");
  if (
    row.origin === "tribute" &&
    command.action !== "revoke" &&
    command.terms.endPolicy === "temporary_membership"
  ) {
    const source = await tx.sourceEntitlement.findFirst({
      where: { enrollmentId: row.id, origin: "tribute" },
    });
    ...
    const policy = await tx.tributePolicy.findUnique({
      where: { id: source.sourcePolicyRef },
    });
    if (
      policy?.temporaryUntil == null ||
      !policy.enabled ||
      command.terms.endsAt === null ||
      new Date(command.terms.endsAt) > policy.temporaryUntil
    )
      return accessFailure("invalid_input");
  }
  ...
  const data = {
    ...
    startsAt:
      command.action === "revoke"
        ? row.startsAt
        : new Date(command.terms.startsAt),
    endsAt:
      command.action === "revoke"
        ? row.endsAt
        : command.terms.endsAt === null
          ? null
          : new Date(command.terms.endsAt),
    endPolicy:
      command.action === "revoke" ? row.endPolicy : command.terms.endPolicy,
  };
  const updated = await tx.subscriptionEnrollment.update({
    where: { id: row.id },
    data,
  });
  ...
  if (row.origin === "tribute") {
    ...
    if (source?.tributeState != null) {
      ...
      const next = tributeStateSchema.parse({
        ...
        mode:
          data.endPolicy === "confirmed_external"
            ? "confirmed_period"
      ...
      await tx.sourceEntitlement.update({
        ...
        data: {
          ...
          revision: { increment: 1 },
          checkedAt: now,
          evidence: { method: "owner_enrollment_change", actorId, command },
        },
      });
      if (next.mode === "temporary_membership") {
        const observedUntil =
          next.observedUntil === null ? null : new Date(next.observedUntil);
        const fresh =
          next.observation === "member" &&
          observedUntil !== null &&
          observedUntil > now;
        await tx.accessGrant.updateMany({
          where: { enrollmentId: row.id },
  ...
  if (sourceState !== undefined && value.state !== "revoked")
    value.state = sourceState;
  ...
  await tx.accessChange.create({
    data: {
      ...
      kind: `enrollment_${command.action}`,
      reason: command.reason,
      recordedAt: now,
    },
  });
  await tx.accessReceipt.create({
    data: {
      ...
      payload: { command, before, after: result.value },
      result,
      createdAt: now,
    },
  });
