...
import {
  enrollmentView,
  enrollmentBenefitTerms,
  enrollmentSourceState,
} from "../../shared/enrollment-view.js";
...
} from "../../features/expand-enrollments/expand-enrollments.js";
...
export function assembleAccessGrants(dependencies: AccessGrantsDependencies) {
      ...
      return dependencyFailure(
        {
          ...
          operation: "readClassificationOf",
        },
        error,
  ...
  return Object.freeze({
    lookupRecipient: (actorId: string, identityRef: string) =>
      manage(actorId, "billing:manage", async () => {
        if (!z.string().trim().min(1).max(256).safeParse(identityRef).success)
          return accessFailure("invalid_input");
        const result =
          await dependencies.recipientLinks?.findCurrentByIdentity(identityRef);
        if (result === undefined || !result.ok)
          return accessFailure("unavailable");
        if (
          result.state === "found" &&
          (await accounts.readIdentityForLink(result.recipient.accountId)) ===
            undefined
        )
    ...
    readActivationReceipt: (input: unknown) =>
      readActivationReceipt(prisma, input),
    previewEnrollmentExpansion: (
      actorId: string,
      input: unknown,
      tier: unknown,
    ...
    registerSourceEntitlement: (actorId: string, input: unknown) =>
      manage(actorId, "billing:manage", () =>
        registerSourceEntitlement(prisma, actorId, input, clock()),
      ),
    ...
    listActivationRules: (actorId: string) =>
      manage(actorId, "billing:manage", async () => {
        const rows = await prisma.activationRule.findMany({
          orderBy: { id: "asc" },
        });
        return {
          ok: true as const,
          value: rows.map((row) =>
            activationRuleSchema.parse({
              id: row.id,
              code: row.code,
              name: row.name,
              revision: row.revision,
              tierId: row.tierId,
              tierRevision: row.tierRevision,
    ...
    async readActivationRule(ruleId: string) {
      ...
      const row = await prisma.activationRule.findUnique({
        where: { id: ruleId },
      });
      return row === null
        ? null
      ...
      try {
        ...
        return {
          ...
          value: await Promise.all(
            rows.map(async (row) => {
              const view = enrollmentView(row, now);
              if (row.origin === "tribute" && view.state !== "revoked")
                view.state =
                  (await enrollmentSourceState(prisma, row.id, now)) ??
                  view.state;
              const benefitGrants = await prisma.accessGrant.findMany({
                where: { enrollmentId: row.id },
              });
              return {
                ...view,
                benefitTerms: enrollmentBenefitTerms(benefitGrants),
                ...(dependencies.contentCatalog === undefined
                  ? {}
                  : {
    ...
    async readCompatibilityContentScope() {
      ...
      const row = rows[0];
      if (row === undefined)
        throw new Error("Compatibility scope baseline missing");
      return row.scope;
    },
    readContentCatalog: (actorId: string) =>
      manage(actorId, "billing:manage", async () => {
        if (dependencies.contentCatalog === undefined)
          return accessFailure("unavailable");
        return {
          ok: true as const,
          value: await dependencies.contentCatalog.list(),
        };
      }),
        ...
        return {
          ...
          value: await Promise.all(
            rows.map(async (row) => {
              ...
              const changes = await prisma.accessChange.findMany({
                where: {
                  accountId: targetAccountId,
                  grantId: { in: benefitGrants.map((grant) => grant.id) },
                },
    ...
    async applyPaidPeriod(command: ApplyPaidPeriodCommand) {
      try {
        ...
        return dependencyFailure(
          { module: "membership-entitlements", operation: "applyPaidPeriod" },
          error,
          accessFailure("unavailable"),
        );
      }
    ...
    listGrants: (actorId: string, command: ListAccessGrantsCommand) =>
      manage(actorId, "billing:manage", () =>
        listAccessGrants(prisma, command, clock()),
      ),
        ...
        ids.map((id) => {
          const holders = new Set(
            rows.flatMap((row) =>
              ...
              (row.capabilities.includes("materials") &&
                (row.all_guides === true || (row.guide_ids ?? []).includes(id)))
                ? [row.account_id]
                : [],
            ),
          );
    ...
    async readOwnAccess(targetAccountId: string) {
      try {
        ...
        return dependencyFailure(
          { module: "membership-entitlements", operation: "readOwnAccess" },
          error,
          accessFailure("unavailable"),
        );
      }
