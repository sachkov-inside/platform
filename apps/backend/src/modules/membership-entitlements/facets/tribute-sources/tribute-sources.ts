import { tributeImportReviewSchema, dismissTributeImportSchema } from "../../domain/tribute-source.js";
...
import { applyTributeImportSchema, previewTributeImportSchema, saveTributePolicySchema, tributePolicySchema,
  tributePreviewSchema, tributeApplyResultSchema, tributeStateSchema, type TributeImportRow,
  type TributeState, tributePreviewRowSchema, tributeOperationsViewSchema, reconcileTributeSchema, retryTributeInboxSchema, tributeInboxViewSchema, tributeSourceViewSchema } from "../../domain/tribute-source.js";
...
const storedPreviewSchema = z.strictObject({ command: previewTributeImportSchema,
  rows: z.array(tributePreviewRowSchema), tiers: z.array(tierSnapshotSchema) });
...
export class TributeSources {
  ...
  private async permitted(actorId: string) {
    ...
    const decision = await this.dependencies.accounts.checkPermission({ accountId: actorId, permission: "billing:manage" });
    return decision.ok && decision.allowed;
  }
  async policies() {
    const rows = await this.dependencies.prisma.tributePolicy.findMany({ orderBy: { id: "asc" }, take: 101 });
    if (rows.length > 100) throw new Error("Tribute policy bound exceeded");
    return rows.map(row => tributePolicySchema.parse({ id: row.id, subscriptionId: row.subscriptionId,
      revision: row.revision, enabled: row.enabled, tier: row.tierSnapshot, temporaryUntil: row.temporaryUntil?.toISOString() ?? null }));
  }
  async savePolicy(actorId: string, input: unknown, tierInput: unknown) {
    ...
    const parsed = saveTributePolicySchema.safeParse(input); const tier = tierSnapshotSchema.safeParse(tierInput);
    if (!parsed.success || !tier.success) return accessFailure("invalid_input");
    const command = parsed.data; const now = this.clock();
    if (tier.data.id !== command.tierId || tier.data.revision !== command.tierRevision) return accessFailure("revision_conflict");
    if (command.temporaryUntil !== null && new Date(command.temporaryUntil) <= now) return accessFailure("invalid_input");
    return this.dependencies.prisma.$transaction(async tx => {
      ...
      const collision = await tx.tributePolicy.findUnique({ where: { subscriptionId: command.subscriptionId } });
      if (collision && collision.id !== command.id) return accessFailure("identity_conflict");
      const value = tributePolicySchema.parse({ id: command.id, subscriptionId: command.subscriptionId,
        revision: command.expectedRevision + 1, enabled: command.enabled, tier: tier.data, temporaryUntil: command.temporaryUntil });
  ...
  private async evaluate(tx: MembershipEntitlementsPrisma, row: TributeImportRow, now: Date) {
    ...
    } else if (row.identityRef === null || row.telegramUserId === null || row.verificationRef === null || link === null || !link.ok || link.state === "ambiguous") {
      result.status = "ambiguous"; result.detail = "Нужно подтверждённое сопоставление identity; имя не используется";
    } else if (new Date(row.checkedAt) > now || row.startsAt === null || row.endsAt === null || Date.parse(row.endsAt) <= Date.parse(row.startsAt)
      || (row.mode === "temporary_membership" && (policy.temporaryUntil === null || Date.parse(row.endsAt) > policy.temporaryUntil.getTime()))) {
      result.status = "unknown_term"; result.detail = "Нужны явные подтверждённые границы и разрешение временного режима";
    } else if ((source?.revision ?? 0) !== row.expectedRevision || source?.revokedAt != null || enrollment?.revokedAt != null
      || (prior !== null && (prior.telegramUserId !== row.telegramUserId || prior.subscriptionId !== row.subscriptionId))
      || (source?.accountId != null && source.accountId !== result.accountId)) {
      result.status = "conflict"; result.detail = "Изменилась revision, identity или основание отозвано; требуется отдельное решение";
    } else {
      const collision = await tx.sourceEntitlement.findFirst({ where: { origin: "tribute", AND: [
        { tributeState: { path: ["subscriptionId"], equals: row.subscriptionId } },
        { tributeState: { path: ["telegramUserId"], equals: row.telegramUserId } },
      ], ...(source === null ? {} : { id: { not: source.id } }) } });
      if (collision) { result.status = "conflict"; result.detail = "Внешний получатель уже сопоставлен другой identity"; }
      else if (link.state === "not_found") { result.status = "pending_identity"; result.detail = "Сохранится до подтверждённой регистрации и linking"; }
      else if (source === null) { result.status = "new"; result.detail = "Новое подтверждённое основание"; }
    }
  ...
  async preview(actorId: string, input: unknown) {
    ...
    return this.dependencies.prisma.$transaction(async tx => {
      ...
      for (const row of command.rows) if (row.telegramUserId !== null && command.rows.filter(other => (other.subscriptionId === row.subscriptionId && other.telegramUserId === row.telegramUserId) || (row.identityRef !== null && other.policyRef === row.policyRef && other.identityRef === row.identityRef)).length > 1) {
  ...
  async previewTiers(actorId: string, input: unknown) {
    ...
    if (!parsed.success) return accessFailure("invalid_input");
    const receipt = await this.dependencies.prisma.accessReceipt.findUnique({ where: { scope_operationId: { scope: actorId, operationId: parsed.data.operationId } } });
    // A lost-response replay must return its receipt even after catalog archival.
    if (receipt !== null) return { ok: true as const, value: [] as TierSnapshot[] };
    const row = await this.dependencies.prisma.accessBatchPreview.findUnique({ where: { id: parsed.data.previewRef } });
    if (row === null || row.actorId !== actorId) return accessFailure("not_found");
    const stored = storedPreviewSchema.parse(row.rows);
    return { ok: true as const, value: stored.rows.filter(item => parsed.data.selectedRows.includes(item.rowRef) && item.enrollmentRevision === 0).flatMap(item => item.tier === null ? [] : [item.tier]) };
  }
  async apply(actorId: string, input: unknown) {
    ...
    return this.dependencies.prisma.$transaction(async tx => {
      ...
      const sources = [];
      for (const row of selected) {
        if (row.identityRef === null || row.telegramUserId === null || row.subscriptionId === null || row.verificationRef === null || row.startsAt === null || row.endsAt === null) return accessFailure("invalid_input");
        const evaluated = stored.rows.find(value => value.rowRef === row.rowRef);
        if (evaluated?.tier == null) return accessFailure("invalid_input");
        const sourceRef = sourceIdentityRef("tribute", row.policyRef, row.identityRef);
        const old = await tx.sourceEntitlement.findUnique({ where: { origin_sourceRef: { origin: "tribute", sourceRef } } });
        const before = old?.tributeState == null ? null : tributeStateSchema.parse(old.tributeState);
        const state: TributeState = { subscriptionId: row.subscriptionId, telegramUserId: row.telegramUserId,
          verificationRef: row.verificationRef, mode: row.mode, startsAt: row.startsAt, endsAt: row.endsAt, renewal: row.renewal,
          tier: before?.tier ?? evaluated.tier, policyRevision: evaluated.policyRevision,
          observation: row.mode === "confirmed_period" ? "pending" : before?.observation ?? "pending",
          observedUntil: row.mode === "confirmed_period" ? null : before?.observedUntil ?? null,
          observationVersion: before?.observationVersion ?? null, lastEventAt: row.checkedAt, lastEventFingerprint: null };
        const data = { tributeState: state, checkedAt: new Date(row.checkedAt), evidence: row, revision: (old?.revision ?? 0) + 1 };
        const saved = await tx.sourceEntitlement.upsert({ where: { origin_sourceRef: { origin: "tribute", sourceRef } },
          create: { id: randomUUID(), origin: "tribute", sourceRef, sourcePolicyRef: row.policyRef, identityRef: row.identityRef, ...data }, update: data });
        if (evaluated.accountId !== null) {
          const projected = await projectTributeSource(tx, saved, state, evaluated.accountId, actorId, row.reason, now);
          if (!projected.ok) throw new Error("Binding changed during locked Tribute apply");
        }
        const after = await tx.sourceEntitlement.findUniqueOrThrow({ where: { id: saved.id } });
  ...
  async status(actorId: string, page = 0) {
    if (!z.int().min(0).max(100000).safeParse(page).success) return accessFailure("invalid_input");
    if (!await this.permitted(actorId)) return accessFailure("forbidden");
  ...
  receive(raw: Buffer, input: unknown) { return receiveTribute(this.dependencies.prisma, raw, input, this.clock()); }
  ...
  async reconcile(actorId: string, input: unknown) {
    ...
    return this.dependencies.prisma.$transaction(async tx => {
      ...
      if (command.action === "restore" && source.revokedAt === null && state.observation !== "source_ended") return accessFailure("revision_conflict");
      if (command.action === "retry") {
        await tx.sourceEntitlement.update({ where: { id: source.id }, data: { reconcileAt: null } });
      } else {
        const next = tributeStateSchema.parse({ ...state,
          startsAt: command.confirmedTerms?.startsAt ?? state.startsAt, endsAt: command.confirmedTerms?.endsAt ?? state.endsAt,
          verificationRef: command.confirmedTerms?.verificationRef ?? state.verificationRef,
        ...
        if (source.enrollmentId !== null) {
          const enrollment = await tx.subscriptionEnrollment.findUniqueOrThrow({ where: { id: source.enrollmentId } });
          const changed = await changeEnrollmentInTransaction(tx, actorId, { operationId: randomUUID(), enrollmentId: enrollment.id,
            expectedRevision: enrollment.revision, action: command.action === "restore" && enrollment.revokedAt === null ? "change_term" : command.action,
            terms: { startsAt: next.startsAt, endsAt: next.endsAt, endPolicy: next.mode === "confirmed_period" ? "confirmed_external" : "temporary_membership" }, reason: command.reason }, now);
          if (!changed.ok) throw new Error("Locked Tribute enrollment change failed");
        }
        await tx.sourceEntitlement.update({ where: { id: source.id }, data: { tributeState: next,
          revokedAt: command.action === "revoke" ? source.revokedAt ?? now : null, checkedAt: now,
  ...
  async sweep(eligibleTierIds: readonly string[], limit = 50) {
    const now = this.clock(); const prisma = this.dependencies.prisma;
    const rows = await prisma.sourceEntitlement.findMany({ where: { origin: "tribute", OR: [{ reconcileAt: null }, { reconcileAt: { lte: now } }] },
      orderBy: [{ reconcileAt: { sort: "asc", nulls: "first" } }, { id: "asc" }], take: Math.min(100, Math.max(1, limit)) });
    let attached = 0; let pending = 0;
    for (const candidate of rows) {
      ...
      await prisma.$transaction(async tx => {
        ...
        const current = await this.dependencies.links.readBinding({ accountId: link.recipient.accountId });
        if (!current.ok || current.binding?.accountRef !== link.recipient.accountRef || current.binding.telegramIdentityRef !== row.identityRef
          || current.binding.linkRef !== link.recipient.linkRef || current.binding.linkRevision !== link.recipient.linkRevision) { pending++; return; }
        const fresh = tributeStateSchema.parse(row.tributeState);
        const result = await projectTributeSource(tx, row, fresh, link.recipient.accountId, null, "Сопоставление подтверждённого Tribute после linking", now);
        if (result.ok) attached++; else pending++;
      });
    }
...
function importReviewView(row: { id: string; batchRef: string; revision: number; state: string; pendingRows: unknown; expiresAt: Date; reason: string }) {
  return tributeImportReviewSchema.parse({ previewRef: row.id, batchRef: row.batchRef, revision: row.revision,
    state: row.state, pendingRows: row.pendingRows, expiresAt: row.expiresAt.toISOString(), reason: row.reason });
}
