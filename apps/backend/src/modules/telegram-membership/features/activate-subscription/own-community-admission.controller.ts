...
import { ApiTags, ApiBearerAuth, ApiOkResponse, ApiOperation, ApiResponse } from "@nestjs/swagger";
...
export class OwnCommunityAdmissionController {
  constructor(@Inject(CommunityEntitlements) private readonly community: CommunityEntitlements) {}
  @Get()
  @ApiOperation({ operationId: "currentCommunityAdmission", summary: "Read own current community restriction independently of content access" })
  @ApiOkResponse({ schema: toOpenApiSchema(ownAdmissionSchema) })
  read(@CurrentAccount() current: AuthenticatedAccount) { return this.community.readOwnAdmission(current.accountId); }
}
