...
import type {
  ListBookmarksQuery,
  ListBookmarksResult,
} from "./list-bookmarks.contract.js";
...
export async function listBookmarks(
  ...
  const cursor =
    parsed.data.after === undefined ? null : decodeCursor(parsed.data.after);
  ...
  try {
          ...
          : {
              ...
              OR: [
                { bookmarkedAt: { lt: new Date(cursor.bookmarkedAt) } },
                {
                  ...
                  materialId: { lt: cursor.materialId },
                },
              ],
            },
    ...
    return {
      ...
      value: {
        ...
        nextCursor: hasNext
          ? encodeCursor({
              bookmarkedAt: last.bookmarkedAt.toISOString(),
    ...
    return dependencyFailure(
      { module: "bookmarks", operation: "listBookmarks" },
      error,
...
function decodeCursor(value: string): BookmarkCursor | null {
  try {
    ...
    return parsed.success
      ? {
          bookmarkedAt: parsed.data.bookmarkedAt,
