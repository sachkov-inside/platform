...
import {
  getBookmarkStatesSchema,
  type GetBookmarkStatesResult,
} from "../../features/get-bookmark-states/get-bookmark-states.js";
...
const listQuerySchema = z
  ...
  .strict();
const bookmarkListPageHttpSchema = z
  .object({
...
export class BookmarksController {
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "addMaterialBookmark",
    summary: "Add a Material to the current Account's bookmarks",
  })
  ...
  async add(
    ...
    const result = await this.bookmarks.addBookmark({
      materialId,
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "removeMaterialBookmark",
    summary: "Remove a Material from the current Account's bookmarks",
  })
  @ApiParam({ name: "materialId", schema: toOpenApiSchema(z.uuid()) })
  @ApiOkResponse({ schema: toOpenApiSchema(bookmarkStateSchema) })
  async remove(
    ...
    const result = await this.bookmarks.removeBookmark({
      materialId,
      accountId: current.accountId,
    });
    if (!result.ok) throwBookmarkError(result.error);
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "getMaterialBookmarkStates",
    summary: "Read bookmark states for at most 100 Material IDs",
  })
  ...
  async states(
    ...
    const result = await this.bookmarks.getBookmarkStates({
      ...parsed.data,
      accountId: current.accountId,
    });
    if (!result.ok) throwBookmarkError(result.error);
  ...
  @ApiOperation({
    ...
    summary: "List the current Account's bookmarked Materials, newest first",
  })
...
type BookmarkError =
  ...
  | RemoveBookmarkError
  | Extract<
      GetBookmarkStatesResult | ListBookmarksResult,
      { readonly ok: false }
    >["error"];
...
function throwBookmarkError(error: BookmarkError): never {
  switch (error.code) {
    case "invalid_request":
      throw bookmarkException(400, error);
    case "access_denied":
      throw bookmarkException(403, error);
    case "dependency_unavailable":
      throw bookmarkException(503, error);
    default:
      return assertNever(error);
  }
}

function bookmarkException(
  status: number,
  error: BookmarkError,
): HttpException {
  return problemException(status, error.code, "Bookmark request failed");
}
function assertNever(value: never): never {
  throw new Error(`Unexpected Bookmark error: ${JSON.stringify(value)}`);
}
