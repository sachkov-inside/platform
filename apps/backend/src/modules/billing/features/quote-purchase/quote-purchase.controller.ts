...
@ApiResponse({ status: 409, content: problemDetailsOneOfContent(problemDetailsSchema(409, ["operation_conflict", "quote_changed"]), accountProblemSchema) })
...
@Controller("accounts/current/billing/quote")
export class QuotePurchaseController {
  constructor(@Inject(BillingPricing) private readonly pricing: BillingPricing) {}
  @Post()
  @HttpCode(200)
  @ApiOperation({ operationId: "quoteBillingPurchase", summary: "Save a price quote for a new subscription" })
  @ApiBody({ schema: toOpenApiSchema(quotePurchaseSchema) })
  @ApiOkResponse({ schema: toOpenApiSchema(priceQuoteSchema) })
  async execute(@CurrentAccount() current: AuthenticatedAccount, @Body() input: unknown) {
    const result = await this.pricing.quote(current.accountId, input);
