...
const transitionSchema = z.strictObject({
  ...
  accountId: idSchema,
  state: reservationStateSchema.exclude(["reserved"]),
});
...
type SettleReservationResult = PricingResult<
...
export async function settleReservation(
  ...
  input: SettleReservation,
): Promise<SettleReservationResult> {
  const parsed = transitionSchema.safeParse(input);
  if (!parsed.success) return failure("invalid_request");
  try {
    return await prisma.$transaction(
      async (tx): Promise<SettleReservationResult> => {
        await lockBillingPricing(tx);
        const command = parsed.data;
        const row = await tx.billingPromoReservation.findUnique({
          where: { purchaseRef: command.purchaseRef },
        });
        if (!row || row.accountId !== command.accountId)
          return failure("not_found");
        const current = reservationStateSchema.parse(row.state);
        if (current === command.state)
          return { ok: true, value: { state: current } };
        if (!transitions[current].includes(command.state))
    ...
    return dependencyFailure(
      { module: "billing", operation: "settleReservation" },
      error,
