...
import {
  ...
  methodFlowSchema,
  revokeMethodSchema,
} from "./change-payment-method.contract.js";
...
@ApiResponse({
  status: 409,
  content: problemDetailsOneOfContent(
    problemDetailsSchema(409, ["operation_conflict", "revision_conflict"]),
    accountProblemSchema,
  ),
})
@ApiResponse({
  ...
  content: problemDetailsContent(
    problemDetailsSchema(422, ["method_unavailable"]),
  ),
})
@ApiResponse({
  status: 500,
...
export class ChangePaymentMethodController {
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "changeBillingMethod",
    summary: "Start a proven bank binding session for a new payment method",
  })
  ...
  async change(
    ...
    const result = await this.subscriptions.changeMethod(
      account.accountId,
      input,
    );
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "revokeBillingMethod",
    summary: "Forbid further use of the saved payment method",
  })
  ...
  async revoke(
    ...
    const result = await this.subscriptions.revokeMethod(
      account.accountId,
      input,
    );
    if (!result.ok) throwPaymentError(result.error.code);
