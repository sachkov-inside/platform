...
type DecideRefundCommand = Extract<
  OwnerOperation,
...
export async function decideRefund(
  ...
  try {
    return await prisma.$transaction(async (tx): Promise<OwnerResult> => {
      ...
      if (previous !== null)
        return previous.purchaseRef === command.purchaseRef
          ? await readDecision(tx, previous.purchaseRef, previous.id, command)
          : ownerFailure("operation_conflict");
      ...
      if (purchase.state !== "confirmed" || purchase.paymentId === null)
      ...
      if (command.amountKopecks > totals.refundableKopecks)
        return ownerFailure("unsupported_amount");
      const decisionRef = randomUUID();
      await tx.billingRefundDecision.create({
        data: {
          id: decisionRef,
          ...
          amountKopecks: BigInt(command.amountKopecks),
          basis: command.basis,
          access: refundAccessFor(command.basis),
    ...
    return previous.purchaseRef === command.purchaseRef
      ? await readDecision(prisma, previous.purchaseRef, previous.id, command)
      : ownerFailure("operation_conflict");
  }
}
...
async function readDecision(
  ...
  if (
    value.amountKopecks !== command.amountKopecks ||
    value.basis !== command.basis ||
    value.recurring !== command.recurring ||
