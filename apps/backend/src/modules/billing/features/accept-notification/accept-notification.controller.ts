...
export class AcceptTbankNotificationController {
  ...
  @ApiOperation({
    operationId: "acceptTbankNotification",
    summary: "Durably accept a signed bank notification",
    security: [],
  })
  @ApiBody({ schema: toOpenApiSchema(bankNotificationSchema) })
  @ApiOkResponse({
    content: { "text/plain": { schema: { type: "string", enum: ["OK"] } } },
  })
  @ApiResponse({
    status: 400,
    content: problemDetailsContent(
      problemDetailsSchema(400, ["invalid_notification"]),
    ),
  })
  @ApiResponse({
    status: 503,
