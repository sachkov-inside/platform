...
import {
  refundTotals,
  unsettledRefundStates,
} from "../../shared/refund-amounts.js";
import { refundDecisionViews } from "../read-payments/read-payments.js";

type ExecuteRefundCommand = Extract<
  OwnerOperation,
...
export async function executeRefund(
  ...
  const decision = await prisma.billingRefundDecision.findUnique({
    where: { id: command.decisionRef },
  });
  ...
  const prepared = await prisma.$transaction(
    ...
    > => {
      ...
      const purchase = await tx.billingPurchase.findUniqueOrThrow({
        where: { id: current.purchaseRef },
      });
      if (purchase.paymentId === null || purchase.state !== "confirmed")
        return ownerFailure("state_conflict");
      // Возврат исполняется только тем терминалом и окружением, которые приняли платёж.
      if (
        purchase.environment !== bank.config.environment ||
        purchase.terminalRef !== bank.config.terminalKey
      )
      ...
      if (Number(current.amountKopecks) > totals.refundableKopecks)
      ...
      await tx.billingRefund.create({
        data: {
          ...
          terminalRef: purchase.terminalRef,
          paymentId: purchase.paymentId,
          amountKopecks: current.amountKopecks,
          state: "sent",
          createdAt: now,
          updatedAt: now,
        },
      });
      await tx.billingRefundDecision.update({
        where: { id: current.id },
        data: {
          state: "executing",
...
async function sendRefund(
  ...
  try {
    ...
    observed = await bank.cancel({
      ...
      email: z
        .email()
        .parse(
          bank.openBinding(`${purchase.id}:contact`, contact.emailCiphertext),
        ),
    });
  } catch (error) {
    // Ответ потерян: попытка остаётся неизвестной и сверяется тем же ExternalRequestId.
    reportDependencyFailure(
      { module: "billing", operation: "sendRefund" },
      error,
    );
    await prisma.billingRefund.updateMany({
      where: { id: row.id, state: { in: unsettledRefundStates } },
      data: {
        state: "unknown",
  ...
  const state: RefundState = !succeeded
    ? "failed"
    : refundTerminalStatuses.some((status) => status === observed.Status)
      ? "confirmed"
      : "unknown";
  await prisma.$transaction(async (tx) => {
    const now = dependencies.clock();
    await lockBillingPurchase(tx, row.purchaseRef);
    const attempt = await tx.billingRefund.findUniqueOrThrow({
      where: { id: row.id },
    });
    ...
    const decision = await tx.billingRefundDecision.findUniqueOrThrow({
      where: { id: attempt.decisionRef },
    });
    if (state === "failed") {
      await tx.billingRefundDecision.update({
        where: { id: decision.id },
    ...
    if (decision.recurring === "cancel")
      await cancelRenewal(tx, row.purchaseRef, now);
    if (decision.access === "revoke")
      await revokePaidAccess(tx, row.purchaseRef, decision.id);
    ...
    await recordBillingNotice(
      tx,
      {
        ...
        sourceRef: refundSourceRef(row.id),
        attemptRef: row.purchaseRef,
        title: priceSnapshotSchema.parse(purchase.snapshot).offer.name,
...
async function cancelRenewal(
  ...
  const subscription = await tx.billingSubscription.findUniqueOrThrow({
    where: { id: purchase.subscriptionRef },
  });
  ...
  await advanceSubscription(
    ...
    "renewal_canceled",
    {
      ...
      refundedPurchaseRef: purchaseRef,
    },
    now,
  );
}
 ...
 */
async function revokePaidAccess(
  tx: BillingPrisma,
  purchaseRef: string,
  decisionRef: string,
): Promise<void> {
  ...
  for (const command of commands) {
    ...
    if (previous === undefined || command.revision > previous.revision)
      latest.set(command.periodRef, command);
  }
...
async function readDecision(
  ...
  const value = decisions.find(
    (decision) => decision.decisionRef === decisionRef,
  );
  if (value === undefined) return ownerFailure("not_found");
  return {
    ok: true,
    operationRef: operationId,
    result: { outcome: "refundDecision", value },
  };
}
