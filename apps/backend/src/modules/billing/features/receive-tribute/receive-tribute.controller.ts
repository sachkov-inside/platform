...
export const tributeAcknowledgementSchema = z.strictObject({
  ok: z.literal(true),
  receiptRef: z.uuid(),
  status: z.enum([
    "applied",
...
export class ReceiveTributeController {
  ...
  @ApiResponse({
    ...
    content: problemDetailsContent(
      problemDetailsSchema(401, ["invalid_signature"]),
    ),
  })
  @ApiResponse({
    status: 503,
    content: problemDetailsContent(
      problemDetailsSchema(503, ["tribute_unavailable"]),
    ),
  })
  async receive(
    @Headers("trbt-signature") signature: string | undefined,
    @Req() request: RawBodyRequest<FastifyRequest>,
  ) {
    const config = this.config.tribute;
    if (config === undefined)
      throw new HttpException({ code: "tribute_unavailable" }, 503);
    const raw = request.rawBody;
    const encoded = signature ?? "";
    const validShape =
      config.signatureEncoding === "hex"
        ? /^[a-fA-F0-9]{64}$/u.test(encoded)
        : /^[A-Za-z0-9+/]{43}=$/u.test(encoded);
    if (!raw || !validShape)
      throw new HttpException({ code: "invalid_signature" }, 401);
    ...
    try {
      ...
      return {
        ok: true as const,
        receiptRef: result.value.id,
        status: result.duplicate ? ("duplicate" as const) : result.value.state,
      };
    } catch (error) {
      reportDependencyFailure(
        { module: "billing", operation: "receive" },
        error,
      );
      throw new HttpException({ code: "tribute_unavailable" }, 503);
    }
