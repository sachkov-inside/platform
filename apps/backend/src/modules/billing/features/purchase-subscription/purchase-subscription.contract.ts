...
export type PaymentResult<T> =
  ...
  | {
      readonly ok: false;
      readonly error: { readonly code: PaymentFailureCode };
    };
export function paymentFailure(code: PaymentFailureCode): {
  readonly ok: false;
  readonly error: { readonly code: PaymentFailureCode };
} {
