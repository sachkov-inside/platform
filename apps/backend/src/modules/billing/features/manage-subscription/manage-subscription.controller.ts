...
import {
  ...
  quoteChangeSchema,
  resumeRenewalSchema,
} from "./manage-subscription.contract.js";
...
@ApiResponse({
  status: 404,
  content: problemDetailsContent(problemDetailsSchema(404, ["not_found"])),
})
@ApiResponse({
  status: 409,
  content: problemDetailsOneOfContent(
...
export class ManageSubscriptionController {
  constructor(
    @Inject(BillingSubscriptions)
    private readonly subscriptions: BillingSubscriptions,
    @Inject(ACCESS_GRANTS) private readonly grants: AccessGrants,
  ) {}

  @Get("enrollments")
  @ApiOperation({
    operationId: "currentEnrollments",
    summary:
      "Read own assigned tiers, composition, sources and complete enrollment history",
  })
  @ApiOkResponse({
    schema: toOpenApiSchema(
      z.strictObject({ items: z.array(enrollmentViewSchema) }),
    ),
  })
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "cancelBillingRenewal",
    summary: "Stop future charges and keep the paid term",
  })
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "resumeBillingRenewal",
    summary:
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "quoteBillingChange",
    summary: "Calculate an upgrade top-up or the next period conditions",
  })
  ...
  async quoteChange(
    ...
    const result = await this.subscriptions.quoteChange(
      account.accountId,
      input,
    );
    if (!result.ok) throwPaymentError(result.error.code);
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "changeBillingOption",
    summary: "Accept a calculated option change",
  })
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "cancelBillingChange",
    summary: "Drop a scheduled option change before its attempt is sent",
  })
  ...
  async cancelChange(
    ...
    const result = await this.subscriptions.cancelChange(
      account.accountId,
      input,
    );
    if (!result.ok) throwPaymentError(result.error.code);
