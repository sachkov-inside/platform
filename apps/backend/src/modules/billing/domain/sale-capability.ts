 ...
 */
export function subscriptionSaleConfirmed(terminal: SubscriptionTerminal): boolean {
...
export function saleCapability(terminal: SubscriptionTerminal | undefined, receiptContactConfigured: boolean): SaleCapability {
  const payments = terminal !== undefined && receiptContactConfigured;
  return { payments, subscriptions: payments && subscriptionSaleConfirmed(terminal) };
}
