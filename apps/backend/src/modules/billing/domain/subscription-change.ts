...
export const subscriptionEventKinds = [
...
export const subscriptionEndReasons = [
  "renewal_declined",
...
export const changePlanSchema = z.discriminatedUnion("kind", [
  z.strictObject({
    ...
    topUpKopecks: moneySchema,
    effectiveAt: z.iso.datetime(),
  }),
  z.strictObject({
    kind: z.literal("scheduled"),
    snapshot: priceSnapshotSchema,
    nextPriceKopecks: moneySchema,
    effectiveAt: z.iso.datetime(),
  }),
]);
...
export function upgradeTopUpKopecks(input: {
  readonly targetPriceKopecks: number;
  readonly paidPeriodKopecks: number;
  readonly periodStartsAt: Date;
  readonly paidUntil: Date;
  readonly now: Date;
}): number {
  const total = BigInt(
    input.paidUntil.getTime() - input.periodStartsAt.getTime(),
  );
  ...
  const remaining =
    elapsed <= 0n ? total : elapsed >= total ? 0n : total - elapsed;
  const difference =
    BigInt(input.targetPriceKopecks) - BigInt(input.paidPeriodKopecks);
 ...
 */
export function renewalPriceSnapshot(
  snapshot: unknown,
  ...
  return priceSnapshotSchema.parse({
    offer: current.offer,
    paymentOption: current.paymentOption,
    promotion: null,
    currency: current.currency,
    timezone: current.timezone,
    firstPriceKopecks: current.renewalPriceKopecks,
    renewalPriceKopecks: current.renewalPriceKopecks,
  });
}

/** Условия расчёта: конкретные редакции предложения и варианта с их ценой. */
export function sameChangeConditions(
  left: PriceSnapshot,
  right: PriceSnapshot,
): boolean {
  return (
    left.offer.id === right.offer.id &&
    left.offer.revision === right.offer.revision &&
    left.paymentOption.id === right.paymentOption.id &&
    left.paymentOption.revision === right.paymentOption.revision &&
    left.paymentOption.priceKopecks === right.paymentOption.priceKopecks
  );
}

export const subscriptionViewSchema = z.strictObject({
  subscriptionRef: idSchema,
  ...
  periodAmountKopecks: moneySchema,
  periodIndex: revisionSchema,
  paymentMethod: z
    .strictObject({ methodRef: idSchema, revoked: z.boolean() })
    .nullable(),
  pendingChange: pendingChangeSchema.nullable(),
  pendingMethodChange: z
    .strictObject({ flowRef: idSchema, formUrl: z.url().nullable() })
    .nullable(),
  inFlightPayment: z
    .strictObject({
      attemptRef: idSchema,
