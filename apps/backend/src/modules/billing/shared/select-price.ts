...
import { offerGrantsWithheld, productOfferBreaksOfferTerms, tierLacksComposition } from "./tier-composition.js";
...
export async function selectPrice(tx: BillingPrisma, optionId: string, now: Date, promoCode?: string, options: SelectPriceOptions = {}): Promise<PricingResult<PriceSnapshot, "not_found" | "unsupported_amount">> {
  ...
  if (options.allowUnpublished !== true && (tierLacksComposition(row.offer) || productOfferBreaksOfferTerms(row.offer) || offerGrantsWithheld(row.offer))) return failure("not_found");
