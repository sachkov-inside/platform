...
export async function advanceSubscription(
  ...
  await tx.billingSubscription.update({
    where: { id: row.id },
    data: { ...data, revision, updatedAt: now },
  });
 ...
 */
export async function settleConfirmedAttempt(
  tx: BillingPrisma,
  attempt: AttemptRow,
  paidAt: Date,
): Promise<PaidPeriod> {
  ...
  if (kind === "initial") {
    ...
    const acceptance = subscriptionConsentSchema.parse(
      consentOf(attempt.acceptance, paidAt),
    );
    await tx.billingSubscription.create({
      data: {
        ...
        state: "active",
        revision: 1,
        snapshot: subscriptionSnapshot(snapshot),
        consent: acceptance,
        anchorAt: paidAt,
        anchorMonths: snapshot.paymentOption.months,
        periodIndex: 1,
    ...
    await tx.billingPurchase.update({
      where: { id: attempt.id },
      data: { subscriptionRef, periodIndex: 1 },
    });
    await tx.billingSubscriptionEvent.create({
      data: {
        id: randomUUID(),
        ...
        revision: 1,
        payload: { attemptRef: attempt.id, paidUntil: endsAt.toISOString() },
        occurredAt: paidAt,
        recordedAt: paidAt,
      },
    });
  ...
  if (kind === "renewal") {
    ...
    await advanceSubscription(
      ...
      {
        anchorMonths,
        periodIndex: current.periodIndex + 1,
      ...
      "period_renewed",
      { attemptRef: attempt.id, paidUntil: endsAt.toISOString() },
      paidAt,
    );
  ...
  await advanceSubscription(
    ...
    {
      snapshot: subscriptionSnapshot(snapshot),
      periodAmountKopecks: BigInt(snapshot.paymentOption.priceKopecks),
    },
    "option_upgraded",
    { attemptRef: attempt.id, paymentOptionId: snapshot.paymentOption.id },
    paidAt,
  );
  return {
    subscriptionRef,
    startsAt: paidAt,
    endsAt: current.paidUntil,
    snapshot,
  };
}
...
export async function endSubscription(
  ...
  await tx.billingPurchase.updateMany({
    where: { subscriptionRef, kind: "initial", lifecycleActive: true },
    data: { lifecycleActive: false },
  });
  await advanceSubscription(
    tx,
    current,
    { state: "ended", pendingChange: {} },
    "subscription_ended",
    { reason },
    now,
  );
  ...
  await recordBillingNotice(
    tx,
    {
      ...
      sourceRef: subscriptionEndedSourceRef(subscriptionRef),
      subscriptionRef,
      title: subscriptionSnapshotSchema.parse(current.snapshot).offer.name,
      dueAt: current.paidUntil,
      ...lifecycleWindow(current.paidUntil),
    },
    now,
  );
}
...
function subscriptionSnapshot(snapshot: PriceSnapshot) {
  const { offer, paymentOption, currency, timezone, renewalPriceKopecks } =
    snapshot;
...
function consentOf(acceptance: unknown, acceptedAt: Date) {
  ...
  return {
    evidenceRefs: parsed.command.consentEvidenceRefs,
    documents: parsed.evidence.map((item) => item.document),
