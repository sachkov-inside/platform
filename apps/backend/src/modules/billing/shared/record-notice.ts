...
function unchanged(
  ...
  return (
    row.state === "current" &&
    sameNoticeConditions(noticeConditions(row), occurrence) &&
 ...
 */
export async function recordBillingNotice(
  tx: BillingPrisma,
  occurrence: NoticeOccurrence,
  now: Date,
): Promise<NoticeOutcome> {
  const existing = await tx.billingNotice.findUnique({
    where: {
      kind_sourceRef: {
  ...
  const conditions = {
    ...
    amountKopecks:
      occurrence.amountKopecks === undefined
        ? null
        : BigInt(occurrence.amountKopecks),
  ...
  if (existing)
    ...
    await tx.billingNotice.create({
      data: {
        id: noticeRef,
        accountId: occurrence.accountId,
        kind: occurrence.kind,
        sourceRef: occurrence.sourceRef,
        subscriptionRef: occurrence.subscriptionRef ?? null,
        attemptRef: occurrence.attemptRef ?? null,
        occurredAt: occurrence.occurredAt,
        createdAt: now,
        ...conditions,
      },
    });
  await tx.billingNoticeRevision.create({
    data: {
      noticeRef,
      revision,
 ...
 */
export async function supersedeRenewalReminders(
  tx: BillingPrisma,
  subscriptionRef: string,
  now: Date,
): Promise<number> {
  const { count } = await tx.billingNotice.updateMany({
    where: { subscriptionRef, kind: "renewal_reminder", state: "current" },
