...
const sameDocument = (
  ...
  expected: AcceptedDocument,
): boolean =>
  accepted.kind === expected.kind &&
  accepted.documentId === expected.documentId &&
...
export async function acceptRecurringConsent(
  contact: ConsentReader,
  accountId: string,
  contextRef: string,
  evidenceRefs: readonly string[],
