...
import { tierLacksComposition } from "../../shared/tier-composition.js";
...
export class TributeConvergence {
  ...
  async savePolicy(actorId: string, input: unknown) {
    ...
    return this.prisma.$transaction(async tx => {
      ...
      if (row === null || row.archived || !row.availableForAssignment || tierLacksComposition(row)) return { ok: false as const, error: { code: "not_found" as const } };
  ...
  async apply(actorId: string, input: unknown) {
    return this.prisma.$transaction(async tx => {
      ...
      for (const tier of snapshots.value) {
        ...
        if (current === null || current.archived || !current.availableForAssignment || tierLacksComposition(current)) return { ok: false as const, error: { code: "not_found" as const } };
  ...
  sweep(limit = 50) {
    return this.prisma.$transaction(async tx => {
      await lockPricing(tx);
      const tiers = await tx.billingOffer.findMany({ where: { archived: false, availableForAssignment: true }, select: { id: true, benefits: true, contentScope: true } });
      // Тариф без состава не назначается и сверкой: подтверждённый источник ждёт, пока состав задан.
      return this.sources.sweep(tiers.filter(tier => !tierLacksComposition(tier)).map(tier => z.uuid().parse(tier.id)), limit);
    });
  }
}
