...
import {
  ...
  methodFlowSchema,
  revokeMethodSchema,
  type MethodFlowResult,
} from "../../features/change-payment-method/change-payment-method.contract.js";
import {
  paymentFailure,
  type PaymentResult,
...
const changeReceiptSchema = z.strictObject({
  ...
  attemptRef: z.uuid().nullable(),
});
type SubscriptionRow = Awaited<
  ReturnType<BillingPrisma["billingSubscription"]["findUniqueOrThrow"]>
>;
...
export class BillingSubscriptions {
  ...
  private async currentSubscription(
    ...
    const row = await this.dependencies.prisma.billingSubscription.findFirst({
  ...
  async cancel(
    accountId: string,
    input: unknown,
  ): Promise<PaymentResult<SubscriptionView>> {
    ...
    return this.transition(
      accountId,
      command.operationId,
      commandFingerprint("cancel", command),
      async (tx, row, now) => {
        ...
        const revision = await advanceSubscription(
          tx,
          row,
          { state: "canceled", pendingChange: {} },
          "renewal_canceled",
        ...
        await recordBillingNotice(
          tx,
          {
            kind: "renewal_cancelled",
            accountId,
            sourceRef: renewalCancelledSourceRef(row.id, revision),
            subscriptionRef: row.id,
            title: subscriptionSnapshotSchema.parse(row.snapshot).offer.name,
            dueAt: row.paidUntil,
            ...lifecycleWindow(now),
          },
          now,
  ...
  async resume(
    ...
    return this.transition(
      accountId,
      command.operationId,
      commandFingerprint("resume", command),
      async (tx, row, now) => {
        if (row.revision !== command.expectedRevision)
          return paymentFailure("revision_conflict");
  ...
  async quoteChange(
    accountId: string,
    input: unknown,
  ): Promise<PaymentResult<ChangeQuoteResult>> {
    ...
    try {
      return await this.dependencies.prisma.$transaction(
        async (tx): Promise<PaymentResult<ChangeQuoteResult>> => {
          ...
          const previous = await tx.billingChangeQuote.findUnique({
            where: {
          ...
          if (row.revision !== command.expectedRevision)
            return paymentFailure("revision_conflict");
          const plan = await this.planChange(
            tx,
            row,
      ...
      return dependencyFailure(
        { module: "billing", operation: "quoteChange" },
        error,
        paymentFailure("dependency_unavailable"),
      );
    }
  ...
  async change(
    accountId: string,
    input: unknown,
  ): Promise<PaymentResult<ChangeResult>> {
    ...
    try {
      ...
      accepted ??= await prisma.$transaction(async (tx) => {
        const now = this.clock();
        await lockBillingPricing(tx);
        const quote = await tx.billingChangeQuote.findFirst({
          where: { id: command.changeQuoteRef, accountId },
        });
        ...
        const row = await tx.billingSubscription.findUniqueOrThrow({
          where: { id: quote.subscriptionRef },
        });
        if (
          row.revision !== command.expectedRevision ||
          row.revision !== quote.baseRevision
        )
          throw new CommandFailure("revision_conflict");
        if (plan.kind === "scheduled" && row.state !== "active")
          throw new CommandFailure("revision_conflict");
        ...
        const current = await this.planChange(
          tx,
          row,
        ...
        if (
          current.value.kind !== plan.kind ||
          !sameChangeConditions(current.value.snapshot, plan.snapshot)
        )
        ...
        if (plan.kind === "scheduled") {
          ...
          await advanceSubscription(
            ...
            row,
            {
              pendingChange: {
                ...
                changeQuoteRef: quote.id,
              },
            },
            "change_scheduled",
            {
              paymentOptionId: plan.snapshot.paymentOption.id,
          ...
          await supersedeRenewalReminders(tx, row.id, now);
          const value = {
            subscription: await subscriptionView(
              tx,
          ...
          await tx.billingSubscriptionCommand.create({
            data: {
              accountId,
              operationId: command.operationId,
              fingerprint: digest,
        ...
        await tx.billingPurchase.create({
          data: {
            ...
            amountKopecks: BigInt(plan.topUpKopecks),
            snapshot: plan.snapshot,
            acceptance: {
              changeQuoteRef: quote.id,
              upgradeOfRevision: row.revision,
            },
            ...
            contact: {
              revision: verified.contact.revision,
              verifiedAt: verified.contact.verifiedAt,
        ...
        const value = {
          subscription: await subscriptionView(tx, row),
          attemptRef,
        };
        await tx.billingSubscriptionCommand.create({
          data: {
      ...
      return error instanceof CommandFailure
        ...
        : dependencyFailure(
            { module: "billing", operation: "change" },
            error,
            paymentFailure("dependency_unavailable"),
          );
    }
  ...
  async cancelChange(
    ...
    return this.transition(
      accountId,
      command.operationId,
      commandFingerprint("cancelChange", command),
      async (tx, row, now) => {
        if (row.revision !== command.expectedRevision)
          return paymentFailure("revision_conflict");
        if (!pendingChangeSchema.safeParse(row.pendingChange).success)
          return paymentFailure("not_found");
  ...
  async changeMethod(
    accountId: string,
    input: unknown,
  ): Promise<PaymentResult<MethodFlowResult>> {
    ...
    let flowRef: string;
    try {
      const previous = await prisma.billingPaymentMethodFlow.findUnique({
        where: {
          accountId_operationId: {
            accountId,
            operationId: command.operationId,
      ...
      if (previous)
        return previous.fingerprint === digest
          ? { ok: true, value: flowResult(previous) }
          : paymentFailure("operation_conflict");
      const prepared = await prisma.$transaction(
        async (tx): Promise<PaymentResult<string>> => {
          ...
          await tx.billingPaymentMethodFlow.create({
            data: {
              id,
              subscriptionRef: row.id,
              accountId,
    ...
    try {
      ...
      await prisma.billingPaymentMethodFlow.updateMany({
        where: { id: flowRef, state: "started" },
        data: {
          state: "rejected",
          observedStatus: "session_unavailable",
          updatedAt: this.clock(),
        },
      });
      return paymentFailure("provider_unavailable");
    }
    const row = await prisma.billingPaymentMethodFlow.findUniqueOrThrow({
      where: { id: flowRef },
    });
  ...
  async revokeMethod(
    ...
    return this.transition(
      accountId,
      command.operationId,
      commandFingerprint("revokeMethod", command),
      async (tx, row, now) => {
        if (row.revision !== command.expectedRevision)
          return paymentFailure("revision_conflict");
        if (row.bindingRef !== command.paymentMethodRef)
          return paymentFailure("not_found");
        if (row.bindingRevokedAt !== null)
          return paymentFailure("revision_conflict");
        ...
        await advanceSubscription(
          ...
          "method_revoked",
          { methodRef: command.paymentMethodRef },
          now,
        );
  ...
  async reconcileMethodFlows(
    ...
    try {
      ...
      for (const row of rows) {
        ...
        try {
          observed = await bank.addCardState(row.requestKey);
        } catch (error) {
          reportDependencyFailure(
        ...
        applied += await prisma.$transaction(async (tx) => {
          ...
          const subscription = await tx.billingSubscription.findUniqueOrThrow({
            where: { id: row.subscriptionRef },
          });
          if (subscription.state === "ended") {
            await tx.billingPaymentMethodFlow.update({
              where: { id: row.id },
              data: {
                state: "rejected",
                observedStatus: "subscription_ended",
                updatedAt: now,
              },
            });
            return 0;
          ...
          await tx.billingSubscription.update({
            ...
            data: {
              bindingRef: row.id,
              bindingCiphertext: bank.sealBinding(row.id, rebillId),
  ...
  private async planChange(
    ...
    if (
      ...
      target.revision === current.paymentOption.revision
    )
      return paymentFailure("invalid_request");
    const snapshot: PriceSnapshot = priceSnapshotSchema.parse({
      offer: offerSchema.parse({
        ...
        revision: target.offer.revision,
      ...
      paymentOption: optionSchema.parse({
        id: target.id,
        offerId: target.offerId,
        revision: target.revision,
        months: target.months,
        mode: target.mode,
        priceKopecks: Number(target.priceKopecks),
    ...
    const plan = planSubscriptionChange({
      ...
      targetPriceKopecks: Number(target.priceKopecks),
      paidPeriodKopecks: Number(row.periodAmountKopecks),
    ...
    if (plan.kind === "scheduled")
      return {
        ...
        value: {
          kind: "scheduled",
          snapshot,
          nextPriceKopecks: Number(target.priceKopecks),
          effectiveAt: row.paidUntil.toISOString(),
        },
      };
    ...
    return {
      ...
      value: {
        kind: "upgrade",
        snapshot,
        topUpKopecks: plan.topUpKopecks,
  ...
  private async transition(
    ...
    try {
      const previous = await prisma.billingSubscriptionCommand.findUnique({
        where: { accountId_operationId: { accountId, operationId } },
      });
      if (previous)
        return previous.fingerprint === digest
          ? { ok: true, value: subscriptionViewSchema.parse(previous.result) }
          : paymentFailure("operation_conflict");
      return await prisma.$transaction(
        async (tx): Promise<PaymentResult<SubscriptionView>> => {
          const now = this.clock();
          const existing = await tx.billingSubscriptionCommand.findUnique({
            where: { accountId_operationId: { accountId, operationId } },
          });
          if (existing)
            return existing.fingerprint === digest
              ? {
                  ok: true,
                  value: subscriptionViewSchema.parse(existing.result),
                }
              : paymentFailure("operation_conflict");
          ...
          const value = await subscriptionView(
            tx,
            await tx.billingSubscription.findUniqueOrThrow({
              where: { id: row.id },
            }),
      ...
      return dependencyFailure(
        { module: "billing", operation: "transition" },
        error,
        paymentFailure("dependency_unavailable"),
      );
    }
...
class CommandFailure extends Error {
  constructor(readonly code: Parameters<typeof paymentFailure>[0]) {
    super(code);
  }
}
...
function quoteResult(
  ...
  return changeQuoteResultSchema.parse({
    changeQuoteRef,
    baseRevision,
    plan,
    expiresAt: expiresAt.toISOString(),
...
function flowResult(row: {
  id: string;
  state: string;
  formUrl: string | null;
  appliedBindingRef: string | null;
}): MethodFlowResult {
  return methodFlowSchema.parse({
    flowRef: row.id,
    state: row.state,
    formUrl: row.formUrl,
    methodRef: row.appliedBindingRef,
  });
}
...
async function subscriptionView(
  ...
  const attempt = await tx.billingPurchase.findFirst({
    where: { subscriptionRef: row.id, state: { in: inFlightStates } },
    orderBy: { createdAt: "desc" },
  });
  const flow = await tx.billingPaymentMethodFlow.findFirst({
    where: { subscriptionRef: row.id, state: "started" },
  });
  ...
  return subscriptionViewSchema.parse({
    ...
    pendingMethodChange: flow
      ? { flowRef: flow.id, formUrl: flow.formUrl }
      : null,
    inFlightPayment: attempt
      ? { attemptRef: attempt.id, kind: attempt.kind, state: attempt.state }
      : null,
  });
}
