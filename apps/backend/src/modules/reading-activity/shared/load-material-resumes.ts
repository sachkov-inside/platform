...
export async function loadMaterialResumes(
  dependencies: {
    readonly contentAccess: Pick<ContentAccess, "checkAvailabilityMany">;
    readonly videos: Pick<Videos, "loadProgressMany">;
  },
  subject: Extract<Subject, { kind: "account" }>,
  materials: readonly PublishedMaterialCatalogItemDto[],
) {
  const resumes = new Map<string, ContinueMaterial["resume"]>();
  const videoIds = [
    ...new Set(
      materials.flatMap((item) =>
        item.primaryVideoId === null ? [] : [item.primaryVideoId],
      ),
    ),
  ];
  if (videoIds.length === 0) return resumes;
  try {
    const access = await dependencies.contentAccess.checkAvailabilityMany({
      subject,
      operations: videoIds.map((id) => ({
        itemId: id,
        resource: { kind: "video" as const, videoId: id },
        action: "play" as const,
      })),
      enforcementPoint: "personal_home",
      correlationId: randomUUID(),
    });
    if (!access.ok) return resumes;
    const allowed = access.items
      .filter((item) => item.availability === "available")
      .map((item) => item.itemId);
    const progress = await dependencies.videos.loadProgressMany({
    ...
    for (const material of materials) {
      const saved =
        material.primaryVideoId === null
          ? undefined
          : byId.get(material.primaryVideoId);
      ...
      if (
        ...
        resumes.set(
          material.materialId,
          saved.positionSeconds >= duration
            ? { kind: "reached-end" }
            : { kind: "position", positionSeconds: saved.positionSeconds },
        );
    }
  } catch (error) {
    /* Material access remains usable without a resume claim. */ reportDependencyFailure(
      { module: "reading-activity", operation: "loadMaterialResumes" },
      error,
    );
  }
  return resumes;
