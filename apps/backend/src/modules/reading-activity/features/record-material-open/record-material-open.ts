...
export const recordMaterialOpenSchema = z
  .object({
    materialId: z.uuid(),
...
export type RecordMaterialOpenInput = z.infer<
  typeof recordMaterialOpenSchema
> & { readonly accountId: string };
export type RecordMaterialOpenResult =
  | {
      readonly ok: true;
      readonly value: { readonly openedAt: string; readonly replayed: boolean };
    }
  | {
      readonly ok: false;
      readonly error: {
...
export async function recordMaterialOpen(
  dependencies: {
  ...
  input: RecordMaterialOpenInput,
): Promise<RecordMaterialOpenResult> {
  const parsed = recordMaterialOpenSchema
  ...
  try {
    ...
    const decision = await dependencies.contentAccess.authorize({
      ...
      enforcementPoint: "material_open",
      correlationId: command.commandId,
    });
    return await dependencies.prisma.$transaction(
      async (transaction): Promise<RecordMaterialOpenResult> => {
        ...
        if (receipt !== null) {
          ...
          return {
            ok: true,
            value: {
              ...openReceiptSchema.parse(receipt.outcome),
              replayed: true,
            },
          };
        }
        ...
        const visit = await transaction.readingMaterialVisit.upsert({
          where: { accountId_materialId: pair },
          create: { ...pair, firstOpenedAt: now, lastOpenedAt: now },
    ...
    return dependencyFailure(
      { module: "reading-activity", operation: "recordMaterialOpen" },
      error,
