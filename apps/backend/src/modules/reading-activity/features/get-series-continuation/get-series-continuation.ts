...
export interface SeriesContinuation {
  ...
  readonly continuation: {
    ...
    readonly resume: ContinueMaterial["resume"];
  } | null;
}
...
export type SeriesContinuationResult =
  | { readonly ok: true; readonly value: SeriesContinuation }
  | {
      readonly ok: false;
      readonly error: {
        readonly code:
          "invalid_request" | "dependency_unavailable" | "series_not_found";
      };
    };
const MAX_SERIES_MATERIALS = 10_000;
const querySchema = z
  .object({ account: z.uuid(), slug: z.string().min(1).max(120) })
  .strict();

export async function getSeriesContinuation(
  dependencies: SeriesContinuationDependencies,
  account: string,
  slug: string,
): Promise<SeriesContinuationResult> {
  if (!querySchema.safeParse({ account, slug }).success)
  ...
  try {
    ...
    if (!series.ok)
      return {
        ...
        error: {
          code:
            series.error.code === "discovery_not_found"
              ? "series_not_found"
    ...
    if (!composition.ok)
      return {
        ...
        error: {
          code:
            composition.error.code === "series_not_found"
              ? "series_not_found"
              : "dependency_unavailable",
        },
      };
    ...
    const [states, visited] = await Promise.all([
      dependencies.prisma.readingMaterialState.findMany({
        where: { accountId: account, materialId: { in: ids }, isRead: true },
        select: { materialId: true },
      }),
      dependencies.prisma.readingMaterialVisit.findFirst({
        where: { accountId: account, materialId: { in: ids } },
    ...
    const lastIndex = items.findIndex(
      (item) => item.materialId === visited?.materialId,
    );
    const availableUnread = (item: (typeof items)[number]) =>
      !read.has(item.materialId) && item.availability === "available";
        ...
        continuation:
          next === undefined
            ...
            : {
                ...
                resume: resumes.get(next.materialId) ?? { kind: "start" },
              },
      },
    };
  } catch (error) {
    return dependencyFailure(
      { module: "reading-activity", operation: "getSeriesContinuation" },
      error,
