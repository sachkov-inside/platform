...
export type GetContinueMaterialsResult =
  | { readonly ok: true; readonly value: readonly ContinueMaterial[] }
  | {
      readonly ok: false;
      readonly error: {
...
export async function getContinueMaterials(
  dependencies: {
  ...
  account: string,
): Promise<GetContinueMaterialsResult> {
  ...
  try {
    ...
    for (
      let offset = 0;
      offset < visits.length && candidates.length < CONTINUE_LIMIT;
      offset += MATERIAL_BATCH_SIZE
    ) {
      const batch = visits.slice(offset, offset + MATERIAL_BATCH_SIZE);
      const completed = await dependencies.prisma.readingMaterialState.findMany(
        {
          where: {
            accountId: account,
            isRead: true,
      ...
      const unfinished = batch.filter(
        (visit) => !readIds.has(visit.materialId),
      );
      const content = await readAvailableMaterials(
        dependencies,
        subject,
        unfinished.map((visit) => visit.materialId),
      );
      if (!content.ok) return content;
      const byId = new Map(
        content.value.map((material) => [material.materialId, material]),
      );
      for (const visit of unfinished) {
        const material = byId.get(visit.materialId);
        if (material !== undefined && candidates.length < CONTINUE_LIMIT)
          candidates.push({
            material,
    ...
    const resumes = await loadMaterialResumes(
      dependencies,
      subject,
      candidates.map((candidate) => candidate.material),
    );
    return {
      ok: true,
      value: candidates.map((candidate): ContinueMaterial => ({
        ...candidate,
        resume: resumes.get(candidate.material.materialId) ?? { kind: "start" },
      })),
    };
  } catch (error) {
    return dependencyFailure(
      { module: "reading-activity", operation: "getContinueMaterials" },
      error,
