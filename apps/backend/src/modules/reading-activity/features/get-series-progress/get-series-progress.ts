...
import type {
  ...
  PublishedSeriesCompositionResult,
} from "../../../materials/index.js";

const querySchema = z
  .object({ accountId: z.uuid(), seriesId: z.uuid() })
  .strict();
...
): Promise<GetSeriesProgressResult> {
  ...
  try {
    ...
    const read = await dependencies.prisma.readingMaterialState.count({
      where: {
        ...
        materialId: { in: [...composition.value] },
        isRead: true,
      },
    });
    return {
      ok: true,
      value: {
        seriesId: parsed.data.seriesId.toLowerCase(),
        total,
        read,
        allRead: total > 0 && total === read,
      },
    };
  } catch (error) {
    return dependencyFailure(
      { module: "reading-activity", operation: "getSeriesProgress" },
      error,
