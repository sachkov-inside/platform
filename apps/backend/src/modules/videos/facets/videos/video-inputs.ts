...
export const reconcileInput = z
  .object({ actor: videoAccountIdSchema, videoId: videoIdSchema })
  .strict();
...
export const webhookInput = z
  ...
  .strict();
export const primaryReferenceInput = z
  .object({
    access: videoAccessSchema,
    materialId: videoMaterialIdSchema,
  ...
  .strict();
export const unselectedUploadInput = z
  .object({
    materialId: videoMaterialIdSchema,
...
export const progressIdentityInput = z
  .object({
    accountId: videoAccountIdSchema,
    videoId: videoIdSchema,
  })
  .strict();
export const saveProgressInput = progressIdentityInput
  .extend({
    durationSeconds: z.number().int().positive(),
    positionSeconds: z.number().int().nonnegative(),
  })
  .refine((value) => value.positionSeconds <= value.durationSeconds);
export const progressManyInput = z
  .object({
    accountId: videoAccountIdSchema,
    videoIds: z.array(videoIdSchema).max(100),
  })
  .strict();
