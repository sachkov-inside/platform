  ...
  const provider: VideoProvider = {
    async delete(input) {
      ...
      try {
        response = await request(
          new URL(
            `/v1/videos/${encodeURIComponent(input.id)}`,
            config.apiBaseUrl,
          ),
          {
        ...
        return dependencyFailure(
          { module: "videos", operation: "delete" },
          error,
          {
    ...
    async initUpload(input) {
      const response = await request(
        new URL("/v2/init", config.uploaderBaseUrl),
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 401 || response.status === 403)
        throw new ProviderUploadAuthorizationError();
...
function parseProviderVideo(data: Record<string, unknown>): ProviderVideo {
  ...
  const embedLocator = optionalString(
    data,
    "embed_link",
    "embed_url",
    "play_link",
...
function readString(
  ...
  if (value === null)
    throw new Error(`Kinescope response misses ${keys[0] ?? "field"}`);
  return value;
}
