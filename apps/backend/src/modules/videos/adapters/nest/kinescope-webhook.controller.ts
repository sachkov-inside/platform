...
export class KinescopeWebhookController {
  constructor(
    ...
    private readonly config: Pick<PlatformConfig, "kinescope">,
  ) {}

  @Post("webhook")
  @HttpCode(200)
  @ApiOperation({
    operationId: "acceptKinescopeVideoWebhook",
    summary: "Accept and durably reconcile a Kinescope Video status event",
  })
  ...
  @ApiResponse({
    status: 401,
    content: problemDetailsContent(
      problemDetailsSchema(401, ["invalid_basic_credentials"]),
    ),
  })
  @ApiResponse({
    status: 404,
    content: problemDetailsContent(
      problemDetailsSchema(404, ["video_not_found"]),
    ),
  })
  @ApiResponse({
    status: 409,
    content: problemDetailsContent(
      problemDetailsSchema(409, ["provider_mismatch"]),
    ),
  })
  @ApiResponse({
    status: 503,
  ...
  async webhook(
    ...
    if (
      ...
      throw new HttpException(
        { code: "invalid_basic_credentials", status: 401 },
        401,
      );
    ...
    if (!accepted.ok) {
      switch (accepted.error.code) {
        case "invalid_request":
          throw new HttpException(
            { code: accepted.error.code, status: 400 },
            400,
          );
        case "video_not_found":
          throw new HttpException(
            { code: accepted.error.code, status: 404 },
            404,
          );
        case "provider_mismatch":
          throw new HttpException(
            { code: accepted.error.code, status: 409 },
            409,
          );
        case "dependency_unavailable":
          throw new HttpException(
            { code: accepted.error.code, status: 503, retryable: true },
            503,
          );
        default:
          return assertNever(accepted.error);
      }
    }
...
function assertNever(value: never): never {
  throw new Error(
    `Unexpected Kinescope webhook error: ${JSON.stringify(value)}`,
  );
}
