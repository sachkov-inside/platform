...
export function inboxKey(row: InboxRow) {
  return { scope_messageId: { scope: row.scope, messageId: row.messageId } };
}
...
export async function recordRowFailure(input: {
  ...
  const attempts = Math.min(
    rowAttempts(failure.checkpoint) + 1,
    ROW_RETRY_LIMIT,
  );
  const checkpoint = {
    ...failure.checkpoint,
    attempts,
    reason: ROW_FAILURE_REASON,
  };
  const observed = {
    lane,
    messageId: failure.row.messageId,
    attempts,
    error: loggableFailure(failure.cause),
  };
  const defer = async (
    reason: SweepObservation["reason"],
  ): Promise<SweepObservation> => {
    await prisma.notificationInbox.update({
      where,
      data: {
        checkpoint,
        nextAttemptAt: new Date(now().getTime() + ROW_RETRY_DELAY_MS),
      },
    });
  ...
  try {
    await quarantine(
      lane,
      Buffer.from(failure.row.payload),
      ROW_FAILURE_REASON,
    );
  } catch (error) {
    ...
    return {
      ...(await defer("quarantine_unavailable")),
      quarantineError: loggableFailure(error),
    };
  }
  await prisma.notificationInbox.update({
    where,
    data: { checkpoint, completedAt: now() },
  });
