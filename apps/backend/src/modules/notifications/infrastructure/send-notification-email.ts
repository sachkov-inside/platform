...
export function assembleNotificationEmailSender(
  ...
  return async (message) => {
    try {
      const response = await send({
        ...
        text: message.text,
        messageRef: message.operationId,
      });
      const parsed = z
        .object({
        ...
        .safeParse(response);
      return parsed.success && parsed.data.accepted.includes(message.email)
        ? { state: "sent" }
        : { state: "unknown" };
    } catch (error) {
      const rejected = z
        .object({ responseCode: z.number().int(), command: z.string() })
        .safeParse(error);
      // Only explicit negative SMTP replies prove non-delivery. Timeout/ambiguous failures stay unknown.
      if (
        rejected.success &&
        rejected.data.responseCode >= 400 &&
        rejected.data.responseCode < 500
      )
        return { state: "not_sent", retryAfterMs: 5_000 };
      if (
        rejected.success &&
        rejected.data.responseCode >= 500 &&
        rejected.data.responseCode < 600
      )
        return { state: "failed", reason: "provider_rejected" };
      return dependencyFailure(
        { module: "notifications", operation: "sendEmail" },
        error,
        { state: "unknown" },
      );
    }
