...
export const contentSchema = z.discriminatedUnion("category", [
  z.object({
    ...
    kind: z.literal("material_published"),
  }),
  z.object({
...
export const deliverySchema = z.object({
  contractVersion: z.literal("inside.notification-delivery.v1"),
  operationId: id,
  notificationRef: id,
  deliveryRef: id,
...
export const resultSchema = z.object({
  contractVersion: z.literal("inside.notification-result.v1"),
  messageId: id,
  operationId: id,
  deliveryRef: id,
  commandRevision: revision,
  ...
  attemptRef: id.nullable().optional(),
  receiptRef: id.optional(),
  nextAttemptAt: instant.optional(),
});
export type DeliveryResult = z.infer<typeof resultSchema>;
export const authorizeSchema = z.strictObject({
  contractVersion: z.literal("inside.notification-dispatch.v1"),
  operationId: id,
  deliveryOperationId: id,
  deliveryRef: id,
  commandRevision: revision,
...
export function commandWindow(issuedAt: Date, deadline: Date) {
  ...
  return {
    ...
    notAfter: new Date(
      Math.min(deadline.getTime(), issuedAt.getTime() + COMMAND_LIFETIME_MS),
    ).toISOString(),
  };
}
