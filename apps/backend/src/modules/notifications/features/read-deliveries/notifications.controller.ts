...
import { toOpenApiSchema, problemDetailsContent } from '../../../../infrastructure/http/zod-openapi.js';
...
const errorSchema = z.object({ type: z.string(), title: z.string(), status: z.number().int(), code: z.string(), detail: z.string().optional(), correlationId: z.string().optional() });
...
export class NotificationPreferencesController {
  ...
  @ApiResponse({ status: 401, content: problemDetailsContent(errorSchema) })
  ...
  @ApiResponse({ status: 400, content: problemDetailsContent(errorSchema) })
  @ApiResponse({ status: 401, content: problemDetailsContent(errorSchema) })
  @ApiResponse({ status: 409, content: problemDetailsContent(errorSchema) })
...
export class NotificationOperationsController {
  ...
  @ApiResponse({ status: 200, schema: toOpenApiSchema(z.array(deliveryViewSchema)) })
  @ApiResponse({ status: 401, content: problemDetailsContent(errorSchema) })
  @ApiResponse({ status: 403, content: problemDetailsContent(errorSchema) })
