...
} from "../change-preferences/change-preferences.js";
...
@ApiResponse({
  status: 500,
  content: problemDetailsContent(problemDetailsSchema(500, ["internal_error"])),
})
@PrivateNoStore()
@AcceptedTermsEndpoint()
export class NotificationPreferencesController {
  ...
  @Get("preferences")
  @ApiOperation({
    operationId: "readNotificationPreferences",
    summary: "Read own material notification opt-ins",
  })
  @ApiResponse({ status: 200, schema: toOpenApiSchema(preferenceSchema) })
  @ApiResponse({
    status: 401,
    content: problemDetailsContent(
      problemDetailsSchema(401, ["invalid_proof", "account_not_found"]),
    ),
  })
  read(@CurrentAccount() account: AuthenticatedAccount) {
    return this.notifications.readPreferences(account.accountId);
  }
  @Post("preferences")
  @HttpCode(200)
  @ApiOperation({
    operationId: "changeNotificationPreferences",
    summary: "Change own channel opt-ins with revision and idempotency",
  })
  ...
  async change(
    ...
    const result = await this.notifications.changePreferences(
      account.accountId,
      body,
    );
    if (!result.ok)
      throw new HttpException(
        { code: result.code },
        result.code === "invalid_input" ? 400 : 409,
      );
    return result;
  }
  @Get("deliveries")
  @ApiOperation({
    operationId: "readOwnNotificationDeliveries",
    summary:
  ...
  @ApiResponse({
    status: 200,
    schema: toOpenApiSchema(z.array(deliveryViewSchema)),
  })
  @ApiResponse({
    status: 401,
    content: problemDetailsContent(
  ...
  async deliveries(
    ...
    return this.notifications.readDeliveries(account.accountId, queryId(after));
  }
}
...
export class NotificationOperationsController {
  ...
  @ApiOperation({
    operationId: "readOperatorNotificationDeliveries",
    summary: "Read delivery summaries as current Platform administrator",
  })
  ...
  async read(
    ...
    const result = await this.notifications.readOperatorDeliveries(
      actor.accountId,
      queryId(account) ?? "",
      queryId(after),
    );
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "resolveUnknownNotificationDelivery",
    summary:
  ...
  @ApiResponse({
    status: 200,
    schema: toOpenApiSchema(z.object({ ok: z.literal(true) })),
  })
  @ApiResponse({
    status: 400,
    content: problemDetailsContent(
  ...
  async resolve(
    ...
    const result = await this.notifications.resolveUnknown(
      actor.accountId,
      body,
    );
    if (!result.ok)
