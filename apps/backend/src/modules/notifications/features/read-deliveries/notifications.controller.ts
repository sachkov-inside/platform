...
import { recoverySchema } from './read-deliveries.js';
const errorSchema = z.object({ code: z.string() });
const deliveryView = z.object({ id: z.uuid(), notificationId: z.uuid(), channel: z.enum(['email', 'telegram']), state: z.string(), reason: z.string().nullable(),
  commandRevision: z.number().int(), resultRevision: z.number().int(), recoverySkipped: z.boolean(), updatedAt: z.iso.datetime() });
function own(result: Awaited<ReturnType<Notifications['readDeliveries']>>) { return result.map(row => ({ ...row, updatedAt: row.updatedAt.toISOString() })); }
...
export class NotificationPreferencesController {
  ...
  @ApiResponse({ status: 401, schema: toOpenApiSchema(errorSchema) })
  ...
  @ApiResponse({ status: 400, schema: toOpenApiSchema(errorSchema) })
  @ApiResponse({ status: 401, schema: toOpenApiSchema(errorSchema) })
  @ApiResponse({ status: 409, schema: toOpenApiSchema(errorSchema) })
  ...
  @ApiResponse({ status: 200, schema: toOpenApiSchema(z.array(deliveryView)) })
  @ApiResponse({ status: 401, schema: toOpenApiSchema(errorSchema) })
  async deliveries(@CurrentAccount() account: AuthenticatedAccount, @Query('after') after?: string) { return own(await this.notifications.readDeliveries(account.accountId, after)); }
}
...
export class NotificationOperationsController {
  ...
  @ApiResponse({ status: 200, schema: toOpenApiSchema(z.array(deliveryView)) })
  @ApiResponse({ status: 401, schema: toOpenApiSchema(errorSchema) })
  @ApiResponse({ status: 403, schema: toOpenApiSchema(errorSchema) })
  async read(@CurrentAccount() actor: AuthenticatedAccount, @Param('accountId') account: string, @Query('after') after?: string) {
    const result = await this.notifications.readOperatorDeliveries(actor.accountId, account, after);
    if (!result.ok) throw new HttpException({ code: result.code }, 403);
    return own(result.deliveries);
  }
