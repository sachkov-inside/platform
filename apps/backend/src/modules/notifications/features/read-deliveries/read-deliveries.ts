...
export const deliveryViewSchema = z.object({
  ...
  commandRevision: z.number().int(),
  resultRevision: z.number().int(),
  recoverySkipped: z.boolean(),
...
export async function resolveUnknown(
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    const old = await transaction.notificationRecoveryAudit.findUnique({
      where: { operationId: command.operationId },
    });
    if (old)
      return old.fingerprint === digest
        ? { ok: true as const }
        : { ok: false as const, code: "operation_conflict" as const };
