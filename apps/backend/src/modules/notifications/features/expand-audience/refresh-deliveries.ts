...
export async function refreshDeliveries(deps: NotificationDependencies) {
  const candidates = await deps.prisma.notificationDelivery.findMany({
    where: {
      state: "suppressed",
      recoverySkipped: false,
      nextCommandAt: { lte: deps.now() },
      reason: { in: ["expired", "superseded"] },
    },
    orderBy: [{ nextCommandAt: "asc" }, { id: "asc" }],
    take: 25,
    include: { notification: true },
  });
  for (const candidate of candidates) {
    const facts = await readRefreshFacts(
      deps,
    ...
    await deps.prisma.$transaction(async (transaction) => {
      ...
      const delivery = await transaction.notificationDelivery.findUniqueOrThrow(
        { where: { id: candidate.id }, include: { notification: true } },
      );
      if (
        delivery.state !== "suppressed" ||
        delivery.recoverySkipped ||
        delivery.commandRevision !== candidate.commandRevision
      )
        return;
      await transaction.notificationDelivery.update({
        where: { id: delivery.id },
        data: { nextCommandAt: new Date(deps.now().getTime() + 30_000) },
      });
      const stored = await transaction.notificationCommand.findUniqueOrThrow({
        where: {
          deliveryId_revision: {
            deliveryId: delivery.id,
            revision: delivery.commandRevision,
          },
        },
      });
      const old = deliverySchema.parse(JSON.parse(stored.payload));
      const event = eventSchema.parse(
        JSON.parse(delivery.notification.eventPayload),
      );
      // One reading decides both that the event is still live and what window the replacement gets.
      const deliveryWindow = commandWindow(
        deps.now(),
        new Date(event.notAfter),
      );
      if (!deliveryWindow) {
        await transaction.notificationDelivery.update({
          where: { id: delivery.id },
          data: { nextCommandAt: null },
        });
        return;
      }
      ...
      if (
        source.content.category === "material" &&
        (!(await optedIn(
          transaction,
          delivery.notification.accountId,
          channel,
          new Date(event.occurredAt),
        )) ||
          answer(facts.access) !== "allowed")
      )
        return;
      ...
      const command = {
        ...
        commandRevision: old.commandRevision + 1,
...
async function readRefreshFacts(
  ...
  notification: { readonly eventPayload: string; readonly accountId: string },
  ...
  const current =
    source.status === "fulfilled" && validSource(event.data, source.value)
      ? source.value
      : undefined;
  const access =
    current?.content.category === "material"
      ? await settle(() =>
          deps.sources.canRead(notification.accountId, event.data.sourceRef),
        )
      : undefined;
  const binding =
    channel.success &&
    current &&
    (!access || (access.status === "fulfilled" && access.value === "allowed"))
      ? await settle(() =>
          deps.recipients.binding(notification.accountId, channel.data),
        )
      : undefined;
