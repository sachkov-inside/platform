...
async function persistResult(
  ...
  const effect = await transaction.notificationEmailEffect.findUniqueOrThrow({
  ...
  const result = resultSchema.parse({
    ...
    resultRevision: effect.resultRevision + 1,
    channel: "email",
    recordedAt: now.toISOString(),
    ...outcome,
  });
  await stageNotification(
    transaction.notificationOutbox,
    "emailResult",
    result,
  );
  await transaction.notificationEmailEffect.update({
    where: { deliveryId: command.deliveryRef },
    data: {
      state: result.state,
...
async function replayResult(
  ...
  await transaction.notificationOutbox.update({
    where: { scope_messageId: { scope: "email", messageId: result.messageId } },
    data: { publishedAt: null, nextAttemptAt: new Date() },
  });
}
export async function acceptEmailCommand(
  prisma: NotificationsPrismaClient,
  input: unknown,
  lane: "emailMaterial" | "emailSubscription",
  now: () => Date,
) {
  const { value: command, envelope } = parseWire(lane, input, deliverySchema);
  if (
    Date.parse(command.issuedAt) >= Date.parse(command.notAfter) ||
    Date.parse(command.notAfter) - Date.parse(command.issuedAt) >
      COMMAND_LIFETIME_MS
  )
    return "conflict" as const;
  return prisma.$transaction(async (transaction) => {
    await lockNotification(transaction, `email:${command.deliveryRef}`);
    const inserted = await transaction.notificationEmailInbox.createMany({
      data: {
        operationId: command.operationId,
        deliveryId: command.deliveryRef,
        commandRevision: command.commandRevision,
    ...
    const old = await transaction.notificationEmailInbox.findUnique({
      where: { operationId: command.operationId },
    });
    if (!old || old.digest !== envelope.digest || old.rejected)
      return "conflict" as const;
    const effect = await transaction.notificationEmailEffect.findUnique({
      where: { deliveryId: command.deliveryRef },
    });
    if (inserted.count === 0) {
    ...
    if (effect) {
      ...
      if (
        effect.commandRevision >= command.commandRevision ||
        !["suppressed", "retrying"].includes(effect.state) ||
        (effect.state === "retrying" && effect.attemptRef !== null)
      ) {
        await transaction.notificationEmailInbox.update({
          where: { operationId: command.operationId },
          data: { rejected: true },
        });
        return "conflict" as const;
      ...
      await transaction.notificationEmailEffect.create({
        data: {
          deliveryId: command.deliveryRef,
          operationId: command.operationId,
          commandRevision: command.commandRevision,
    ...
    await persistResult(
      transaction,
      command,
      envelope.digest.slice("sha256:".length),
      now(),
      { state: "accepted" },
    );
...
export async function dispatchEmail(
  ...
  category: "material" | "subscription",
): Promise<boolean> {
  const rows = await deps.prisma.notificationEmailEffect.findMany({
    where: {
      category,
      state: { in: ["accepted", "retrying"] },
      nextAttemptAt: { lte: deps.now() },
    },
    orderBy: [{ nextAttemptAt: "asc" }, { deliveryId: "asc" }],
    take: 25,
  });
  for (const row of rows) {
    const inbox = await deps.prisma.notificationEmailInbox.findUniqueOrThrow({
      where: { operationId: row.operationId },
    });
    const command = deliverySchema.parse(JSON.parse(inbox.payload));
    if (
      command.content.category !== category ||
      command.binding.channel !== "email"
    )
      continue;
    ...
    const request = {
      ...
      deliveryRef: command.deliveryRef,
      commandRevision: command.commandRevision,
      payloadDigest: inbox.digest.slice("sha256:".length),
      attemptRef,
    };
    ...
    const started = await deps.prisma.$transaction(async (transaction) => {
      ...
      if (
        ...
        await persistResult(
          transaction,
          command,
          inbox.digest.slice("sha256:".length),
          deps.now(),
          {
            state: "suppressed",
            reason:
      ...
      await transaction.notificationEmailAttempt.create({
        data: {
          id: attemptRef,
      ...
      await transaction.notificationEmailEffect.update({
        where: { deliveryId: row.deliveryId },
        data: { attemptRef, permitRef: permit.permitRef },
      });
      ...
      await persistResult(
        ...
        inbox.digest.slice("sha256:".length),
        deps.now(),
        { state: "unknown", attemptRef, reason: "interrupted_attempt" },
      );
      return true;
    });
    ...
    const outcome =
      ...
      Date.parse(command.notAfter) <= deps.now().getTime()
        ...
        : await send({
            ...
            dependencyFailure(
              { module: "notifications", operation: "dispatchEmail" },
              error,
    ...
    await deps.prisma.$transaction(async (transaction) => {
      ...
      if (effect.attemptRef !== attemptRef || effect.state !== "unknown")
        throw new Error("email_attempt_conflict");
      const receiptRef = outcome.state === "sent" ? randomUUID() : null;
      await transaction.notificationEmailAttempt.update({
        where: { id: attemptRef },
      ...
      if (outcome.state === "sent" && receiptRef)
        await persistResult(
          ...
          inbox.digest.slice("sha256:".length),
          deps.now(),
          { state: "sent", attemptRef, receiptRef },
        );
        ...
        await persistResult(
          ...
          inbox.digest.slice("sha256:".length),
          deps.now(),
          { state: "failed", attemptRef, reason: outcome.reason },
        );
        ...
        await persistResult(
          ...
          inbox.digest.slice("sha256:".length),
          deps.now(),
          { state: "unknown", attemptRef, reason: "lost_response" },
        );
      else if (outcome.state === "not_sent") {
        const delay = retryDelaysMs[effect.retries];
        const next = new Date(
          deps.now().getTime() + Math.max(delay ?? 0, outcome.retryAfterMs),
        );
        if (
          delay === undefined ||
          next.getTime() >= Date.parse(command.notAfter)
        )
          await persistResult(
            transaction,
            command,
            inbox.digest.slice("sha256:".length),
            deps.now(),
            { state: "failed", attemptRef, reason: "retry_exhausted" },
          );
        else {
          await transaction.notificationEmailEffect.update({
            where: { deliveryId: row.deliveryId },
