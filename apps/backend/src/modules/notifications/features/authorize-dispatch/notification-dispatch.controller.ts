...
export class NotificationDispatchController {
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "authorizeNotificationDispatch",
    summary:
  ...
  async authorize(
    ...
    const versioned = authorizeSchema
      .extend({ contractVersion: z.string() })
      .safeParse(body);
    if (
      versioned.success &&
      versioned.data.contractVersion !== "inside.notification-dispatch.v1"
    ) {
    ...
    if (!parsed.success || Buffer.byteLength(JSON.stringify(body)) > 16 * 1024)
      throw new HttpException({ code: "malformed" }, 400);
    const result = await this.notifications
      .authorizeDispatch("telegram", parsed.data)
      .catch((error: unknown) =>
        dependencyFailure(
          { module: "notifications", operation: "authorizeDispatch" },
          error,
          {
            ...parsed.data,
    ...
    if (result.status === "error")
      throw new HttpException(
        result,
        result.code === "operation_conflict" ? 409 : 503,
      );
    return result;
  }
}
