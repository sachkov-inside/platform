...
} from "../expand-audience/expand-audience.js";
...
export async function authorizeDispatch(
  ...
  return deps.prisma.$transaction(async (transaction) => {
    ...
    await lockNotification(
      transaction,
      `authorize:${channel}:${request.operationId}`,
    );
    const digest = fingerprint(request);
    const old = await transaction.notificationAuthorization.findUnique({
      where: {
        channel_operationId: { channel, operationId: request.operationId },
      },
    });
    if (old)
      return old.digest === digest
        ? dispatchResponseSchema.parse(JSON.parse(old.response))
        : { ...request, status: "error", code: "operation_conflict" };
    ...
    const decide = async (): Promise<DispatchResponse> => {
      ...
      if (
        stored.digest !== request.payloadDigest ||
        stored.revision !== request.commandRevision
      )
        return { ...request, status: "denied", reason: "payload_conflict" };
      if (
        stored.delivery.commandRevision !== stored.revision ||
        stored.delivery.recoverySkipped ||
        ["sent", "failed"].includes(stored.delivery.state)
      )
        return { ...request, status: "denied", reason: "superseded" };
      const command = deliverySchema.parse(JSON.parse(stored.payload));
      if (new Date(command.notAfter) <= deps.now())
        return { ...request, status: "denied", reason: "expired" };
      ...
      if (
        facts?.notificationId !== notification.id ||
        facts.eventPayload !== notification.eventPayload
      )
        return { ...request, status: "error", code: "unavailable" };
      const { source } = facts;
      if (source.status === "unavailable")
        return { ...request, status: "error", code: "unavailable" };
      if (
        !validSource(event, source) ||
        command.sourceEventId !== event.messageId ||
        fingerprint(command.content) !== fingerprint(source.content)
      )
        return { ...request, status: "denied", reason: "superseded" };
      ...
      if (
        ...
        template.templateRef !== command.templateRef ||
        template.templateRevision !== command.templateRevision ||
        (channel === "email" && template.subject !== command.subject)
      )
        return { ...request, status: "denied", reason: "superseded" };
      if (source.content.category === "material") {
        ...
        const { access } = facts;
        if (access === undefined || access === "unavailable")
          return { ...request, status: "error", code: "unavailable" };
        if (access === "denied")
          return { ...request, status: "denied", reason: "access_denied" };
      }
      const { binding } = facts;
      if (!binding || fingerprint(binding) !== fingerprint(command.binding))
        return { ...request, status: "denied", reason: "binding_conflict" };
      const deadline = Math.min(
        deps.now().getTime() + PERMIT_LIFETIME_MS,
        Date.parse(command.notAfter),
        Date.parse(event.notAfter),
      );
      if (deadline <= deps.now().getTime())
        return { ...request, status: "denied", reason: "expired" };
      return {
        ...request,
        status: "allowed",
    ...
    await transaction.notificationAuthorization.create({
      data: {
        channel,
        operationId: request.operationId,
        digest,
        deliveryId: request.deliveryRef,
 ...
 */
async function readDispatchFacts(
  deps: NotificationDependencies,
  channel: Channel,
  request: DeliveryCommandRef,
) {
  ...
  const access =
    source.status === "current" && source.content.category === "material"
      ? await deps.sources.canRead(notification.accountId, event.data.sourceRef)
      : undefined;
  const binding = await deps.recipients.binding(
    notification.accountId,
    channel,
  );
  return {
...
async function findDeliveryCommand(
  prisma: Pick<NotificationsPrisma, "notificationCommand">,
  channel: Channel,
  request: DeliveryCommandRef,
) {
  const stored = await prisma.notificationCommand.findUnique({
    where: { operationId: request.deliveryOperationId },
    include: { delivery: { include: { notification: true } } },
  });
  return stored &&
    stored.deliveryId === request.deliveryRef &&
    stored.delivery.channel === channel
    ? stored
    : null;
}
