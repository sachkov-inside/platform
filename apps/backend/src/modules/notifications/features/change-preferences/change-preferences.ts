...
export const preferenceResultSchema = z.union([
  z.object({ ok: z.literal(true), preferences: preferenceSchema }),
  z.object({
    ok: z.literal(false),
    code: z.enum(["invalid_input", "revision_conflict", "operation_conflict"]),
  }),
]);
export async function readPreferences(
  ...
  return row
    ? { revision: row.revision, email: row.email, telegram: row.telegram }
    : { revision: 0, email: false, telegram: false };
}
export async function changePreferences(
  ...
  input: unknown,
  now: () => Date,
): Promise<z.infer<typeof preferenceResultSchema>> {
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    const previous =
      await transaction.notificationPreferenceRevision.findUnique({
        where: {
    ...
    await transaction.notificationPreferenceRevision.create({
      data: { ...data, operationId: command.operationId, fingerprint: digest },
    });
    return {
      ok: true,
      preferences: {
...
export async function optedIn(
  ...
  const historical = await prisma.notificationPreferenceRevision.findFirst({
    where: { accountId, changedAt: { lt: occurredAt } },
    orderBy: { revision: "desc" },
  });
  return (
    historical?.[channel] === true &&
    (await readPreferences(prisma, accountId))[channel]
  );
}
