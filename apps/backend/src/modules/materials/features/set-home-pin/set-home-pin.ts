  ...
  return async (input) => {
    ...
    return executeAuthoringTransaction<HomePinDto, SetHomePinError>(dependencies.prisma, async (transaction, rollback) => {
      if (command.seriesId !== null) {
        ...
        const series = await transaction.guide.findUnique({ where: { id: command.seriesId }, select: { archivedAt: true } });
        ...
        const count = await transaction.publishedMaterialGuideMembership.count({ where: { seriesId: command.seriesId } });
