...
import {
  ...
  type ReserveSourceGuideOperation,
  type ReorderSourceGuideOperation,
  type UpdateSourceGuideOperation,
  type ValidateSourceGuideOperation,
} from "./import-source-guide.contract.js";

export function assembleReserveSourceGuide(
  dependencies: MaterialAuthoringDependencies,
): ReserveSourceGuideOperation {
  return async (input) => {
    const parsed = parseCommand(
      reserveSourceGuideBodySchema.extend({ actor: accountId }),
      input,
    );
    ...
    try {
      let current = await dependencies.prisma.guide.findUnique({
        where: { sourceId: command.sourceId },
      });
      if (current === null) {
        try {
          current = await dependencies.prisma.guide.create({
            data: {
              id: randomUUID(),
          ...
          current = await dependencies.prisma.guide.findUnique({
            where: { sourceId: command.sourceId },
          });
          if (current === null) {
      ...
      return {
        ...
        error: dependencyFailure(
          { module: "materials", operation: "reserveSourceGuide" },
          error,
...
export function assembleValidateSourceGuide(
  dependencies: MaterialAuthoringDependencies,
): ValidateSourceGuideOperation {
  return async (input) => {
    const parsed = parseCommand(
      validateSourceGuideBodySchema.extend({ actor: accountId }),
      input,
    );
    if (!parsed.ok) return parsed;
    const authorized = await authorizeManager(
...
export function assembleUpdateSourceGuide(
  dependencies: MaterialAuthoringDependencies,
): UpdateSourceGuideOperation {
  return async (input) => {
    const parsed = parseCommand(
      updateSourceGuideBodySchema.extend({ actor: accountId }),
      input,
    );
    if (!parsed.ok) return parsed;
...
export function assembleReorderSourceGuide(
  dependencies: MaterialAuthoringDependencies,
): ReorderSourceGuideOperation {
  return async (input) => {
    const parsed = parseCommand(
      reorderSourceGuideBodySchema.extend({ actor: accountId }),
      input,
    );
    if (!parsed.ok) return parsed;
    const { sourceId, ...command } = parsed.value;
