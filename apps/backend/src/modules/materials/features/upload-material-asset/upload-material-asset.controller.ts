...
export class UploadMaterialAssetController {
  ...
  @ApiOperation({
    operationId: "uploadMaterialAsset",
    summary: "Upload and finalize an immutable Material asset",
  })
  @ApiParam({ name: "materialId", schema: toOpenApiSchema(materialIdSchema) })
  @ApiHeader({
    name: "idempotency-key",
    required: true,
    schema: toOpenApiSchema(assetIdempotencyKeySchema),
  })
  ...
  @ApiResponse({
    ...
    content: problemDetailsContent(
      uploadProblemSchema(400, ["invalid_upload"]),
    ),
  })
  @ApiResponse({
    status: 401,
    content: problemDetailsContent(
      uploadProblemSchema(401, [
        "account_not_found",
  ...
  @ApiResponse({
    status: 403,
    content: problemDetailsContent(uploadProblemSchema(403, ["forbidden"])),
  })
  @ApiResponse({
    status: 404,
    content: problemDetailsContent(
      uploadProblemSchema(404, ["material_not_found"]),
    ),
  })
  @ApiResponse({
    status: 409,
    content: problemDetailsContent(
      uploadProblemSchema(409, [
  ...
  @ApiResponse({
    status: 500,
    content: problemDetailsContent(
      uploadProblemSchema(500, ["internal_error"]),
    ),
  })
  @ApiResponse({
    status: 503,
    content: problemDetailsContent(
      uploadProblemSchema(503, ["dependency_unavailable"]),
    ),
  })
  async upload(
    ...
    try {
      const part = await request.file({
        limits: {
          fields: 4,
          fileSize: MATERIAL_ASSET_LIMITS.fileBytes,
          files: 1,
        },
      });
    ...
    const parsedIdempotencyKey =
      assetIdempotencyKeySchema.safeParse(idempotencyKey);
    if (
...
function field(file: MultipartFile, name: string): string | undefined {
  ...
  if (value === undefined || Array.isArray(value) || value.type !== "field")
    return undefined;
  return String(value.value);
}

function throwAssetUploadError(
  error: Extract<UploadMaterialAssetForAuthoringResult, { ok: false }>["error"],
): never {
  switch (error.code) {
    case "dependency_unavailable":
      throw problemException(
        503,
        error.code,
        "Material asset dependency is unavailable",
      );
    case "forbidden":
      throw problemException(
        403,
        error.code,
        "Material asset upload is forbidden",
      );
    ...
    case "upload_in_progress":
      throw problemException(
        409,
        error.code,
        "Material asset upload conflicts with an existing request",
      );
    ...
    case "size_mismatch":
      throw problemException(
        413,
        error.code,
        "Uploaded file exceeds or does not match its declared size",
      );
    ...
    case "unsupported_image_type":
      throw problemException(
        422,
        error.code,
        "Uploaded bytes are not an accepted Material asset",
      );
  }
}
