...
const validationQuerySchema = z
  .object({ expectedContentVersion: z.coerce.number().int().positive() })
  .strict();
...
export class ValidateMaterialController {
  ...
  @ApiOperation({
    operationId: "validateCurrentMaterial",
    summary: "Validate the current Material for publication",
  })
  @ApiParam({ name: "materialId", schema: toOpenApiSchema(materialIdSchema) })
  @ApiQuery({
    name: "expectedContentVersion",
    schema: toOpenApiSchema(contentVersionSchema),
  })
  ...
  async validate(
    ...
    const result = await this.authoring.validateMaterial({
      actor: account.accountId,
      materialId,
      expectedContentVersion: query.expectedContentVersion,
    });
