...
import { idempotencyKeySchema, materialMutationReceiptSchema, parseMaterialAuthoringBody, throwMaterialAuthoringError } from "../../adapters/nest/material-authoring-http.js";
...
export class ImportSourceMaterialController {
  constructor(@Inject(MATERIAL_AUTHORING) private readonly authoring: MaterialAuthoring, @Inject(PLATFORM_CONFIG) private readonly config: PlatformConfig) {}
  ...
  async validate(@CurrentAccount() account: AuthenticatedAccount, @Body() input: unknown) {
    ...
    const result = await this.authoring.validateSourceContent({ actor: account.accountId, ...body });
    if (!result.ok) throwMaterialAuthoringError(result.error);
    return result.value;
  }

  @Post("reserve")
  @ApiOperation({ operationId: "reserveSourceMaterial", summary: "Reserve a stable authoring source identity without publishing" })
  ...
  async reserve(@CurrentAccount() account: AuthenticatedAccount, @Body() input: unknown) {
    ...
    const result = await this.authoring.reserveSourceMaterial({ actor: account.accountId, source: body.source });
    if (!result.ok) throwMaterialAuthoringError(result.error);
    return result.value;
  }

  @Post("apply")
  @ApiOperation({ operationId: "applySourceMaterial", summary: "Apply one selected source Material with optimistic version checking" })
  ...
  @ApiOkResponse({ schema: toOpenApiSchema(materialMutationReceiptSchema) })
  @ApiMaterialAuthoringErrors(400, 401, 403, 404, 409, 422, 500, 503)
  async apply(@CurrentAccount() account: AuthenticatedAccount, @Headers("idempotency-key") key: string | undefined, @Body() input: unknown) {
    const body = parseMaterialAuthoringBody(applySourceBodySchema, input);
    const result = await this.authoring.applySourceMaterial({ actor: account.accountId, idempotencyKey: key ?? "", ...body });
    if (!result.ok) throwMaterialAuthoringError(result.error);
    return result.value;
  }
}
