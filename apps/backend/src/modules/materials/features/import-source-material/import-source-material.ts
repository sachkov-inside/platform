...
import { accountId, idempotencyKeySchema, parseCommand } from "../../shared/command-validation.js";
...
const receiptSchema = z.object({
  id: z.uuid(), contentVersion: z.coerce.number().int().positive(),
  publicationState: z.enum(["draft", "published", "unpublished"]),
  publishedAt: z.date().nullable(),
});

export function assembleReserveSourceMaterial(dependencies: MaterialAuthoringDependencies): ReserveSourceOperation {
  return async (input) => {
    ...
    async function existing() {
      const row = await dependencies.prisma.material.findUnique({ where: { sourceId: source.id } });
      if (row === null) return null;
      const current = receiptSchema.parse(row);
      return { ok: true as const, value: {
        materialId: current.id, contentVersion: current.contentVersion,
        publicationState: current.publicationState, publishedAt: current.publishedAt?.toISOString() ?? null,
      } };
    }
    try {
      const found = await existing();
      if (found !== null) return found;
      const created = await assembleCreateDraft(dependencies, source)({
        actor,
        idempotencyKey: `source:${createHash("sha256").update(source.id).digest("hex")}`,
        metadata: { title: null, summary: null, access: "free", difficulty: null, outcomes: [], topicId: null, formatId: null, tagIds: [], seriesIds: [] },
...
export function assembleApplySourceMaterial(dependencies: MaterialAuthoringDependencies): ApplySourceOperation {
  return async (input) => {
    const parsed = parseCommand(applyCommand, input);
    if (!parsed.ok) return parsed;
    const { source, videoChapters, ...command } = parsed.value;
    if (command.publicationState === "published" && source.showInFeed && command.metadata.seriesIds.length === 0 && command.metadata.access !== "free") {
      return { ok: false, error: { code: "invalid_reference", issues: [{ code: "standalone_feed_material_must_be_free", path: "/metadata/access" }] } };
    }
    return assembleSaveMaterial(dependencies, source)({
      ...command,
...
export function assembleValidateSourceContent(dependencies: MaterialAuthoringDependencies): ValidateSourceOperation {
  return async (input) => {
    const parsed = parseCommand(validateSourceBodySchema.extend({ actor: accountId }), input);
    if (!parsed.ok) return parsed;
    ...
    const body = dependencies.materialBodyOperations.accept(command.body, { assignMissingNodeIds: true });
    if (!body.ok) return body;
    if (command.publicationState === "published") {
      const metadata = selection.value.materialize(command.metadata.seriesIds.map((seriesId, index) => ({ seriesId, ordinal: index + 1 })), "import-preview");
