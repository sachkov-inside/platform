...
function onlyKnownTerms(value: string): boolean {
  return [...value.matchAll(placeholders)].every(([, name]) =>
    (guidePageTerms as readonly string[]).includes(name ?? ""),
  );
}
...
const text = (max: number) =>
  z
    ...
    .refine(
      onlyKnownTerms,
      `Only {${guidePageTerms.join("}, {")}} may be substituted`,
    );
...
const list = <T extends z.ZodType>(item: T, min = 1) =>
  z.array(item).min(min).max(GUIDE_PAGE_ITEMS_MAX);
...
export const guidePageSchema = z
  ...
  .refine(
    (page) =>
      ...
      GUIDE_PAGE_BYTES_MAX,
    {
      message: `A page description must stay under ${String(GUIDE_PAGE_BYTES_MAX)} bytes`,
    },
  );
...
export function readStoredGuidePage(
  value: unknown,
): GuidePage | null | "invalid" {
