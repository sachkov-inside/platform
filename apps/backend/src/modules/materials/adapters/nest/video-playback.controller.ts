...
export class VideoPlaybackController {
  constructor(
    @Inject(VIDEO_PLAYBACK) private readonly playback: VideoPlayback,
  ) {}

  @Post(":materialId/videos/:videoId/playback")
  @HttpCode(200)
  @ApiOperation({
    operationId: "createVideoPlaybackSession",
    summary: "Authorize and create a short-lived Video playback session",
  })
  ...
  @ApiResponse({
    ...
    content: problemDetailsContent(
      problemDetailsSchema(403, ["access_denied", "video_mismatch"]),
    ),
  })
  @ApiResponse({
    status: 409,
    content: problemDetailsOneOfContent(
      problemDetailsSchema(409, ["video_not_ready"]),
      accountProblemSchema,
    ),
  })
  @ApiResponse({
...
export class VideoProgressController {
  ...
  @HttpCode(204)
  @ApiOperation({
    operationId: "saveVideoPlaybackProgress",
    summary: "Save coarse Account progress for one Video",
  })
  @ApiNoContentResponse()
...
function throwPlaybackError(error: PlaybackError): never {
  switch (error.code) {
    case "invalid_request":
      throw playbackException(400, error.code);
    case "access_denied":
    case "video_mismatch":
      throw playbackException(403, error.code);
    case "video_not_ready":
      throw playbackException(409, error.code);
    case "dependency_unavailable":
      throw playbackException(503, error.code);
    default:
      return assertNever(error);
  }
}

function playbackException(
  status: number,
