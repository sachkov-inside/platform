...
export class KinescopeVideoAuthorizationController {
  constructor(
    @Inject(VIDEO_PLAYBACK)
    private readonly playback: Pick<VideoPlayback, "authorizeProvider">,
  ...
  @HttpCode(200)
  @ApiOperation({
    operationId: "authorizeKinescopeVideoPlayback",
    summary: "Repeat the member access decision for a Kinescope DRM request",
  })
  ...
  async authorize(
    ...
    if (
      !(await this.playback.authorizeProvider({
        providerVideoId: parsed.data.id,
