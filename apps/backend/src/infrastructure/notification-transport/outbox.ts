...
export interface NotificationOutboxTable {
  ...
  findFirst(args: {
    where: { lane: string; publishedAt: null; nextAttemptAt: { lte: Date } };
...
export async function stageNotification(
  ...
  if (row.digest !== envelope.digest || row.lane !== lane)
    throw new Error("notification_operation_conflict");
}
const RELAY_RETRY_INITIAL_MS = 1_000;
const RELAY_RETRY_MAX_MS = 60_000;
export function assembleNotificationOutbox(
  table: NotificationOutboxTable,
  allowed: readonly NotificationLane[],
  now: () => Date = () => new Date(),
): NotificationOutbox {
  return {
    async relay(lane, publish) {
      ...
      const row = await table.findFirst({
        where: { lane, publishedAt: null, nextAttemptAt: { lte: now() } },
        orderBy: [{ createdAt: "asc" }, { messageId: "asc" }],
      });
      if (!row) return false;
      const where = {
      ...
      try {
        ...
        if (
          ...
          envelope.messageId !== row.messageId
        )
          throw new Error("outbox_integrity_failure");
        await publish(envelope);
        // A crash here deliberately republishes the same immutable ID after restart.
        await table.update({
          where,
          data: { publishedAt: now(), lastFailure: null },
        });
      } catch (error) {
        await table.update({
          where,
          data: {
            ...
            nextAttemptAt: new Date(
              now().getTime() +
                Math.min(
                  RELAY_RETRY_MAX_MS,
                  RELAY_RETRY_INITIAL_MS * 2 ** Math.min(row.attempts, 6),
                ),
            ),
          },
        });
