...
export const lanes = {
  billing: {
    exchange: "inside.events.billing.v1",
    key: "billing.notice-ready",
    queue: "platform.notifications.billing.v1",
    publisher: "billing",
    consumer: "notifications",
    version: "inside.notification-event.v1",
  },
  materials: {
    exchange: "inside.events.materials.v1",
    key: "material.published",
    queue: "platform.notifications.materials.v1",
    publisher: "materials",
    consumer: "notifications",
    version: "inside.notification-event.v1",
  },
  telegramSubscription: {
    ...
    key: "subscription",
    queue: "telegram.notifications.subscription.v1",
    publisher: "notifications",
    consumer: "telegram",
    version: "inside.notification-delivery.v1",
  },
  telegramMaterial: {
    exchange: "inside.notifications.telegram.v1",
    key: "material",
    queue: "telegram.notifications.material.v1",
    publisher: "notifications",
    consumer: "telegram",
    version: "inside.notification-delivery.v1",
  },
  emailSubscription: {
    ...
    key: "subscription",
    queue: "email.notifications.subscription.v1",
    publisher: "notifications",
    consumer: "email",
    version: "inside.notification-delivery.v1",
  },
  emailMaterial: {
    exchange: "inside.notifications.email.v1",
    key: "material",
    queue: "email.notifications.material.v1",
    publisher: "notifications",
    consumer: "email",
    version: "inside.notification-delivery.v1",
  },
  telegramResult: {
    exchange: "inside.results.telegram.v1",
    key: "delivery.result",
    queue: "platform.notification-results.telegram.v1",
    publisher: "telegram",
    consumer: "notifications",
    version: "inside.notification-result.v1",
  },
  emailResult: {
    exchange: "inside.results.email.v1",
    key: "delivery.result",
    queue: "platform.notification-results.email.v1",
    publisher: "email",
    consumer: "notifications",
    version: "inside.notification-result.v1",
  },
...
export type NotificationPrincipal =
  (typeof lanes)[NotificationLane]["publisher"];
export const notificationLaneSchema = z.custom<NotificationLane>(
  (value) => typeof value === "string" && Object.hasOwn(lanes, value),
);
...
function canonical(value: Json, key = ""): Json {
  if (typeof value === "string")
    return uuidFields.has(key) ? value.toLowerCase() : value;
...
export function encodeNotification(
  ...
  if (
    body.contractVersion !== route.version ||
    (route.version === "inside.notification-event.v1" &&
      body.eventType !== route.key) ||
    (route.version === "inside.notification-delivery.v1" &&
      (body.content?.category !== route.key ||
        body.binding?.channel !== channel)) ||
    (route.version === "inside.notification-result.v1" &&
      body.channel !== channel)
  )
    throw new Error("route_mismatch");
  const payload = JSON.stringify(canonical(z.json().parse(input)));
  if (Buffer.byteLength(payload) > NOTIFICATION_MESSAGE_MAX_BYTES)
  ...
  return {
    ...
    payload,
    digest: digestNotificationPayload(payload),
  };
}
