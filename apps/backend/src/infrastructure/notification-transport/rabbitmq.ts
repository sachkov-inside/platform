...
export async function connectNotificationBroker(
...
export async function publishNotification(
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      timer = setTimeout(
        () => reject(new Error("publisher_confirm_timeout")),
        BROKER_CONFIRM_TIMEOUT_MS,
      );
      channel.once("close", () =>
        reject(new Error("publisher_channel_closed")),
      );
...
export function decodeNotification(
  ...
  if (message.content.length > NOTIFICATION_MESSAGE_MAX_BYTES)
    throw new Error("message_too_large");
  if (
    message.fields.exchange !== route.exchange ||
    message.fields.routingKey !== route.key ||
    message.properties.contentType !== "application/json" ||
    message.properties.type !== route.version ||
  ...
  if (message.properties.messageId !== envelope.messageId)
    throw new Error("message_id_mismatch");
  return envelope;
}
export async function consumeNotificationLane(
  connection: ChannelModel,
  lane: NotificationLane,
  receiver: NotificationReceiver,
  prefetch: number,
): Promise<{ stop(): Promise<void>; failed: Promise<void> }> {
  ...
  channel.on("close", () => {
    if (!stopping) fail(new Error("consumer_channel_closed"));
  });
  ...
  const consumer = await channel.consume(
    ...
    (message) => {
      if (!message) {
        fail(new Error("consumer_cancelled"));
        return;
      }
      const task = (async () => {
        let envelope: NotificationEnvelope;
        try {
          envelope = decodeNotification(lane, message);
        } catch {
          await receiver.quarantine(
            lane,
            message.content,
            "invalid_transport_or_payload",
          );
          channel.ack(message);
          return;
        }
        ...
        if (result === "conflict")
          await receiver.quarantine(lane, message.content, "payload_conflict");
