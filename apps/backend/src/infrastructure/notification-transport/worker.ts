...
export function assembleNotificationWorker(input: {
  ...
  billing: NotificationOutbox;
  materials: NotificationOutbox;
  ...
  async function sweep(lane: NotificationLane, connection: ChannelModel) {
    const source =
      lane === "billing"
        ...
        : lane === "materials"
          ? input.materials
          : input.transport.outbox;
    while (!abort.signal.aborted) {
      try {
        await source.relay(lane, (envelope) =>
          publishNotification(connection, envelope),
        );
      } catch (error) {
        input.report({
      ...
      await delay(RELAY_SWEEP_MS, undefined, { signal: abort.signal }).catch(
        () => undefined,
      );
    }
  }
    ...
    async start() {
      try {
        for (const principal of [
          ...
          connection.on("close", () => {
            if (!stopping) fail(new Error("notification_broker_disconnected"));
          });
        }
        for (const lane of Object.keys(lanes).map((key) =>
          notificationLaneSchema.parse(key),
        )) {
          ...
          if (consumer) {
            const handle = await consumeNotificationLane(
              consumer,
              lane,
              input.transport,
              input.config.prefetch,
            );
            ...
            void handle.failed.catch((error: unknown) => {
              ...
              input.report({
                status: "operator_attention",
                reason: "notification_consumer_stopped",
                lane,
                error: loggableFailure(error),
              });
              fail(
                error instanceof Error
                  ? error
                  : new Error("notification_consumer_stopped"),
              );
            });
          }
        }
        if (input.processInbox)
          tasks.push(
            (async () => {
              while (!abort.signal.aborted) {
                ...
                await (input.runInboxSweep ?? ((sweep) => sweep()))(
                  async () => {
                    try {
                      for (const observation of (await input.processInbox?.()) ??
                        [])
                        input.report({
                          status: "operator_attention",
                          ...observation,
                      ...
                      input.report({
                        status: "operator_attention",
                        reason: "inbox_sweep_failed",
                        error: loggableFailure(error),
                      });
                    }
                  },
                );
                await delay(RELAY_SWEEP_MS, undefined, {
                  signal: abort.signal,
                }).catch(() => undefined);
              }
            })(),
            ...
            while (!abort.signal.aborted) {
              ...
              try {
                ...
                input.report({
                  status:
                    observation.oldest &&
                    Date.now() - observation.oldest.getTime() > BACKLOG_ALERT_MS
                      ? "operator_attention"
              ...
              await delay(OBSERVATION_INTERVAL_MS, undefined, {
                signal: abort.signal,
              }).catch(() => undefined);
            }
          })(),
        ...
        for (const task of tasks)
          void task.catch((error: unknown) => {
            ...
            fail(
              error instanceof Error
                ? error
                : new Error("notification_worker_failed"),
            );
          });
      } catch (error) {
        ...
        input.report({
          status: "operator_attention",
          reason: "notification_worker_start_failed",
          error: loggableFailure(error),
        });
        await this.stop();
      ...
      try {
        await Promise.race([
          (async () => {
            ...
            await Promise.allSettled(
              [...connections.values()].map((connection) => connection.close()),
            );
          })(),
