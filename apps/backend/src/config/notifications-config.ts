...
export const brokerUrlSchema = z.url().refine((value) => {
  ...
  return (
    ["amqp:", "amqps:"].includes(url.protocol) &&
...
export function oneBrokerEnvironment(urls: readonly string[]): boolean {
  ...
  return (
    new Set(parsed.map((url) => url.username)).size === parsed.length &&
    new Set(parsed.map((url) => `${url.protocol}//${url.host}${url.pathname}`))
      .size === 1
  );
}
export const notificationsConfigSchema = z
  .object({
    urls: z
      .object({
        billing: brokerUrlSchema,
        materials: brokerUrlSchema,
        notifications: brokerUrlSchema,
        email: brokerUrlSchema,
      })
      .strict(),
    caFile: z.string().min(1).optional(),
    prefetch: z.coerce.number().int().min(1).max(32).default(4),
    quarantineCapacity: z.coerce
      .number()
      .int()
      .min(1)
      .max(100_000)
  ...
  .refine(
    (value) => oneBrokerEnvironment(Object.values(value.urls)),
...
export function parseNotificationsConfig(
  ...
  if (
    ...
    throw new Error("Notifications require AMQPS in production");
  return parsed.data;
}
