...
const tbankTerminalSchema = z.strictObject({
  ...
  recurringCardConfirmed: z.boolean(),
  cardOnlyHostedConfirmed: z.boolean(),
  minimumKopecks: z.int().positive(),
  maximumKopecks: z.int().positive(),
  returnUrl: httpsUrl,
  notificationUrl: httpsUrl,
  // Смена карты требует отдельно подтверждённой проверки: CheckType=NO не возвращает RebillId.
  cardBinding: z
    .strictObject({
      confirmed: z.literal(true),
      checkType: z.enum(["3DS", "3DSHOLD", "HOLD"]),
    })
    .optional(),
...
const withinLimits = (value: {
  minimumKopecks: number;
  maximumKopecks: number;
...
export const tbankRuntimeSchema = tbankTerminalSchema
  .extend({
    ...
    endpoints: bankEndpointsSchema,
    returnUrl: cleanUrl,
    notificationUrl: cleanUrl,
  })
  .refine(withinLimits)
  .refine(
    (value) =>
      ...
      [
        ...
        value.endpoints.apiBaseUrl,
        ...value.endpoints.formOrigins,
 ...
 */
export function parseBankContour(
...
export function parseTbankConfig(
  ...
  try {
    terminal = tbankConfigSchema.parse(JSON.parse(value));
  } catch {
    throw new Error(
...
const DEFAULT_LOCAL_BANK_NOTIFICATION_URL =
  "http://127.0.0.1:3001/billing/tbank/notification";
const DEFAULT_LOCAL_BANK_RETURN_URL =
  "http://127.0.0.1:3000/subscription/return";
/**
...
export function localTbankConfig(environment: NodeJS.ProcessEnv): TbankConfig {
  try {
    return tbankRuntimeSchema.parse({
      environment: "local",
      terminalKey: "INSIDELOCALDOUBLE",
      password: "inside-local-bank-double-password",
    ...
    throw new Error(
      "Invalid TBANK_TEST_* stand address; use an absolute http or https URL without credentials",
    );
  }
}
