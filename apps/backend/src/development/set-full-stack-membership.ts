...
async function main() {
  ...
  if (
    ...
    throw new Error("Expected an isolated local full-stack Membership fixture");
  const prisma = createPrismaClient(config.database.url);
  try {
    const account = await prisma.account.findUniqueOrThrow({
      where: {
        logtoIssuer_logtoSubject: {
          logtoIssuer: config.identity.issuer,
          logtoSubject: "fullstack-member",
        },
      },
      select: { id: true },
    });
    ...
    const result = await membership.acceptEvidence({
      ...
      deliveryId: `full-stack-home-${now.toISOString()}`,
      source: "link_time",
      evidence: {
        checkedAt: now.toISOString(),
        contractVersion: "inside.membership-evidence.v1",
        decision,
        evidenceRef: "full-stack-home-evidence",
        evidenceVersion: now.getTime(),
        ...
        validUntil: new Date(
          now.getTime() + MEMBERSHIP_FIXTURE_LIFETIME_MS,
        ).toISOString(),
      },
    });
    if (!result.ok || result.outcome !== "applied")
      throw new Error("Membership fixture was not applied");
    const material = await prisma.material.findUniqueOrThrow({
      where: { slug: "developer-pipeline-bez-poteri-konteksta" },
      select: { id: true },
    });
    const visit = await prisma.readingMaterialVisit.findUnique({
      where: {
        accountId_materialId: {
          accountId: account.id,
          materialId: material.id,
        },
      },
      select: { firstOpenedAt: true, lastOpenedAt: true },
    });
    process.stdout.write(
      JSON.stringify({
        kind: (await membership.resolveForAccess(accountId(account.id))).kind,
        visit,
      }),
    );
  } finally {
...
void main().catch(() => {
  console.error("Full-stack Membership transition failed");
  process.exitCode = 1;
});
