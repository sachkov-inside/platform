...
export const refundOutcomes = {
  accepted: "Банк возвращает",
  declined: "Банк отказывает",
} as const;
...
export const operationNotFoundOutcome: BankOutcome = {
  status: "UNKNOWN",
  success: false,
  errorCode: "7",
};
export const declinedBindingOutcome: BankOutcome = {
  status: "REJECTED",
  success: false,
  errorCode: "3005",
};

export const isKnownOutcome = <T extends object>(
  table: T,
  value: string,
): value is Extract<keyof T, string> => Object.hasOwn(table, value);
...
const ledgerSchema = z.object({
  ...
  chargeOutcome: z.enum(chargeOutcomes),
  refundOutcome: z.enum(["accepted", "declined"]),
});
...
export type OrderRecord = Omit<StoredOrder, "refunds"> & {
...
export function loadBankLedger(path: string | undefined): BankLedger {
  ...
  console.warn(
    JSON.stringify({ process: "bank-double", ledger: "unreadable", path }),
  );
  return emptyLedger();
}

export function saveBankLedger(
