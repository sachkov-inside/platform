...
export function indexPage(view: StandView): string {
  const orders = view.orders
    .map(
      (
        order,
      ) => `<tr><td><a href="/pay/${order.paymentId}">${escapeHtml(order.description)}</a></td>
    <td>${money(order.amount)}</td><td>${escapeHtml(order.status)}</td><td>${escapeHtml(order.email)}</td>
    <td>${order.refundedKopecks === 0 ? "—" : money(order.refundedKopecks)}</td></tr>`,
    )
    .join("");
  const sessions = view.bindings
    .map(
      (
        session,
      ) => `<tr><td><a href="/card/${session.requestKey}">${session.requestKey}</a></td>
    <td>${escapeHtml(session.status)}</td></tr>`,
    )
    .join("");
  return `<h1>Двойник банка Inside</h1>
    ...
    <form method="post" action="/control">
      ...
      <label>Следующее списание
        <select name="chargeOutcome">${options(
          chargeOutcomes.map(
            (key) => [key, paymentOutcomes[key].label] as const,
          ),
          view.chargeOutcome,
        )}</select></label>
      <label>Возврат
...
export function paymentPage(order: OrderRecord): string {
  const buttons = Object.entries(paymentOutcomes)
    .map(
      ([key, outcome]) =>
        `<button type="submit" name="outcome" value="${key}">${outcome.label}</button>`,
    )
    .join("");
...
export function bindingPage(session: BindingRecord): string {
  const buttons = Object.entries(bindingOutcomes)
    .map(
      ([key, outcome]) =>
        `<button type="submit" name="outcome" value="${key}">${outcome.label}</button>`,
    )
