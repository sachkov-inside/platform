...
import {
  ...
  missingPage,
  paymentPage,
} from "./bank-double-pages.js";
import {
  bankReference,
  bindingOutcomes,
  declinedBindingOutcome,
  isChargeOutcome,
  isKnownOutcome,
  loadBankLedger,
  paymentOutcomes,
...
const chargeSchema = z
  .object({
    ...
    RebillId: z.union([z.string(), z.number()]),
  })
  .loose();
const paymentReferenceSchema = z
  .object({ PaymentId: z.union([z.string(), z.number()]) })
  .loose();
...
const addCardSchema = z
  .object({ CustomerKey: z.string().min(1), CheckType: z.string().min(1) })
  .loose();
const requestKeySchema = z.object({ RequestKey: z.string().min(1) }).loose();

export function createLocalBankDouble(
  ...
  const orders = new Map<string, OrderRecord>(
    stored.orders.map((order) => [
      order.orderId,
      { ...order, refunds: new Map(order.refunds) },
    ]),
  );
  const paymentIndex = new Map<string, string>(
    [...orders.values()].map((order) => [order.paymentId, order.orderId]),
  );
  const bindings = new Map<string, BindingRecord>(
    stored.bindings.map((session) => [session.requestKey, session]),
  );
  ...
  async function announce(
    ...
    await notify(order.notificationUrl, {
      ...body,
      Token: tbankToken(body, config.password),
    });
  }
  ...
  async function api(
    ...
    if (method === "Init") {
      ...
      const order: OrderRecord = {
        ...
        paymentId,
        amount: input.Amount,
        description: input.Description ?? "Покупка",
        email: input.Receipt?.Email ?? "",
        recurrent: input.Recurrent === "Y",
    ...
    if (method === "GetAddCardState") {
      ...
      if (!session)
        return Response.json({
          Success: operationNotFoundOutcome.success,
          ErrorCode: operationNotFoundOutcome.errorCode,
      ...
      return Response.json({
        ...
        ...(session.rebillId === undefined
          ? {}
          : { RebillId: session.rebillId }),
      });
    }
  ...
  async function cancel(
    ...
    const known = orderByPayment(input.PaymentId);
    if (!known) return Response.json(notFound(input.PaymentId));
    const answer = (refund: RefundRecord) =>
      Response.json({
        TerminalKey: config.terminalKey,
        ...
        OriginalAmount: known.amount,
        NewAmount: refund.newAmount,
      });
    ...
    const settled: RefundRecord = {
      status:
        known.refundedKopecks >= known.amount ? "REFUNDED" : "PARTIAL_REFUNDED",
      success: true,
      errorCode: "0",
      newAmount: known.amount - known.refundedKopecks,
    };
  ...
  const notFound = (paymentId: string | number): Record<string, unknown> => ({
    ...
    Status: operationNotFoundOutcome.status,
    Success: operationNotFoundOutcome.success,
    ErrorCode: operationNotFoundOutcome.errorCode,
  ...
  const paymentForm = (order: OrderRecord) =>
    `${formOrigin}/pay/${order.paymentId}`;
  ...
  async function handle(request: Request): Promise<Response> {
    ...
    if (request.method === "POST" && path === "/control") {
      ...
      if (isKnownOutcome(refundOutcomes, chosenRefund))
        refundOutcome = chosenRefund;
      persist();
      return redirect("/");
    }
    if (path.startsWith("/pay/")) {
      ...
      else if (isKnownOutcome(paymentOutcomes, outcome))
        await settle(known, outcome);
    ...
    if (path.startsWith("/card/")) {
      ...
      const outcome = await formValue(request, "outcome");
      if (!isKnownOutcome(bindingOutcomes, outcome))
        return html(missingPage("Неизвестный исход"), 400);
