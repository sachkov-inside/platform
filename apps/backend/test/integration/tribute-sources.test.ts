...
import { assembleAccessGrants, assembleMembershipEntitlements, TributeSources } from "../../src/modules/membership-entitlements/index.js";
...
import { SubscriptionActivation, TributeConvergence } from "../../src/modules/billing/index.js";
...
describe("Tribute source production facets and signed HTTP with PostgreSQL", () => {
  let db: TestDatabase; let owner: string; let sources: TributeSources; let convergence: TributeConvergence;
  let grants: ReturnType<typeof assembleAccessGrants>; let membership: ReturnType<typeof assembleMembershipEntitlements>;
  let http: NestFastifyApplication; let now = new Date("2030-01-01T00:00:00.000Z"); let subscriptionSequence = 625000;
  const key = "synthetic-tribute-key-local-625";
  beforeAll(async () => {
    db = await createMigratedTestDatabase();
    owner = (await bootstrapOwnerAccount(db.prisma, { issuer: "https://tribute.example.test", subject: "owner" }, "platform:admin")).accountId;
    const accounts = assembleAccounts({ prisma: db.prisma, emailFingerprintKey: "synthetic-tribute-account-fingerprint-key-625" });
    const links = new TelegramAccountLinks(db.prisma);
    sources = new TributeSources({ prisma: db.prisma, accounts, links, clock: () => now });
    ...
    membership = assembleMembershipEntitlements({ prisma: db.prisma, recipientLinks: links, clock: () => now,
      workshopEntitlements: assembleWorkshopEntitlements({ prisma: db.prisma, clock: () => now }) });
    @Module({ controllers: [ReceiveTributeController, SubscriptionActivationController], providers: [
      { provide: TributeConvergence, useValue: convergence },
      { provide: SubscriptionActivation, useValue: new SubscriptionActivation({ prisma: db.prisma, grants, bindings: links, readAdmission: () => Promise.resolve({ state: "checking", admissionRestriction: null }) }) },
      { provide: PLATFORM_CONFIG, useValue: parsePlatformConfig({ NODE_ENV: "test", TRIBUTE_API_KEY: key, TRIBUTE_SIGNATURE_ENCODING: "hex", TELEGRAM_ACTIVATION_INGRESS_SECRET: "synthetic-activation-625" }) },
      { provide: APP_FILTER, useClass: ProblemDetailsFilter }, { provide: APP_INTERCEPTOR, useClass: HttpCachePolicyInterceptor },
    ] })
    ...
    class FixtureModule {}
    http = await NestFactory.create<NestFastifyApplication>(FixtureModule, new FastifyAdapter(), { logger: false, rawBody: true });
  ...
  async function setup(mode: "confirmed_period" | "temporary_membership" = "confirmed_period") {
    ...
    value(await convergence.savePolicy(owner, { operationId: randomUUID(), expectedRevision: 0, id: policyRef, subscriptionId, enabled: true,
      tierId: tier.id, tierRevision: 1, temporaryUntil: mode === "temporary_membership" ? "2030-02-01T00:00:00.000Z" : null, reason: "Подтверждённый источник" }));
    const row = { rowRef: randomUUID(), policyRef, subscriptionId, identityRef, telegramUserId: String(subscriptionId), verificationRef: randomUUID(),
      checkedAt: now.toISOString(), mode, startsAt: now.toISOString(), endsAt: "2030-02-01T00:00:00.000Z", renewal: "enabled" as const,
      expectedRevision: 0, reason: "Подтверждённые даты и identity" };
    return { row, tier, guideId };
  }
  async function link(identityRef: string) {
    const id = randomUUID();
    await db.prisma.account.create({ data: { id, logtoIssuer: "https://tribute.example.test", logtoSubject: id } });
    const principal = await linkTelegramAccount(db.prisma, { accountId: id, identityRef, now });
    expect(await membership.bindPrincipal({ accountId: accountId(id), principalRef: principal })).toMatchObject({ ok: true });
    return { id, principal };
  }
  async function apply(row: Awaited<ReturnType<typeof setup>>["row"]) {
    ...
    const command = { operationId: randomUUID(), previewRef: preview.previewRef, selectedRows: [row.rowRef] };
    return { command, result: value(await convergence.apply(owner, command)) };
  }
  function event(row: Awaited<ReturnType<typeof setup>>["row"], name = "renewed_subscription", expiresAt = "2030-03-01T00:00:00.000Z") {
    return { name, created_at: now.toISOString(), sent_at: now.toISOString(), payload: {
      subscription_name: "Подписка", subscription_id: row.subscriptionId, period_id: 1, period: "monthly", price: 10000, amount: 9000, currency: "rub",
      user_id: 1, trb_user_id: `T-${row.telegramUserId}`, telegram_user_id: Number(row.telegramUserId), channel_id: 10, channel_name: "Источник",
      expires_at: expiresAt, type: "regular", ...(name === "cancelled_subscription" ? { cancel_reason: "" } : {}),
    } };
  }
  async function send(input: unknown, signature?: string) {
    const raw = JSON.stringify(input, null, 2);
    const response = await http.inject({ method: "POST", url: "/integrations/tribute/v1/webhook", payload: raw,
      headers: { "content-type": "application/json", "trbt-signature": signature ?? createHmac("sha256", key).update(raw).digest("hex") } });
  ...
  test("9/11 pending import, verified linking and recoverable sweep preserve one source and exact receipt", async () => {
    ...
    expect(value(await convergence.apply(owner, imported.command))).toEqual(imported.result);
    expect(await db.prisma.subscriptionEnrollment.count({ where: { accountId: customer.id } })).toBe(1);
  ...
  test("10 preview never infers identity or periods; changed revisions and archived catalog fail closed", async () => {
    ...
    expect(await convergence.apply(owner, { operationId: randomUUID(), previewRef: unknown.previewRef, selectedRows: [context.row.rowRef] })).toMatchObject({ ok: false });
    const ambiguous = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [{ ...context.row, identityRef: null, verificationRef: null }] }));
    expect(ambiguous.rows[0]?.status).toBe("ambiguous");
    const duplicate = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [context.row, { ...context.row, rowRef: randomUUID() }] }));
    expect(duplicate.rows.every(row => row.status === "conflict")).toBe(true);
    const preview = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [context.row] }));
    await db.prisma.billingOffer.update({ where: { id: context.tier.id }, data: { archived: true } });
    expect(await convergence.apply(owner, { operationId: randomUUID(), previewRef: preview.previewRef, selectedRows: [context.row.rowRef] })).toMatchObject({ ok: false, error: { code: "not_found" } });
  });
  test("12/13/14 official signed inbox deduplicates transport retries, preserves cancel remainder and holds unordered facts", async () => {
    const context = await setup(); const customer = await link(context.row.identityRef); await apply(context.row);
    now = new Date("2030-01-02T00:00:00.000Z"); const renewal = event(context.row);
    expect((await send(renewal, "bad")).statusCode).toBe(401);
    expect((await send(renewal, createHmac("sha256", "other-key").update(JSON.stringify(renewal, null, 2)).digest("hex"))).statusCode).toBe(401);
    const missing = await http.inject({ method: "POST", url: "/integrations/tribute/v1/webhook", payload: renewal });
    expect(missing.statusCode).toBe(401);
    const signatureBeforeTamper = createHmac("sha256", key).update(JSON.stringify(renewal, null, 2)).digest("hex");
    expect((await send({ ...renewal, sent_at: "2030-01-02T00:00:01.000Z" }, signatureBeforeTamper)).statusCode).toBe(401);
    const accepted = await send(renewal); expect(accepted.json()).toMatchObject({ ok: true, status: "applied" });
    now = new Date("2030-01-03T00:00:00.000Z");
    expect((await send({ ...renewal, sent_at: now.toISOString() })).json()).toMatchObject({ status: "duplicate", receiptRef: z.object({ receiptRef: z.uuid() }).parse(accepted.json()).receiptRef });
    expect((await send(event(context.row, "cancelled_subscription"))).json()).toMatchObject({ status: "applied" });
    expect((await send({ ...renewal, payload: { ...renewal.payload, expires_at: "2030-04-01T00:00:00.000Z" } })).json()).toMatchObject({ status: "pending_reconciliation" });
    expect((await send({ ...event(context.row), created_at: "2030-01-01T12:00:00.000Z" })).json()).toMatchObject({ status: "pending_reconciliation" });
    for (const type of ["trial", "gift", undefined]) {
      const draft = event(context.row, "new_subscription");
      expect((await send({ ...draft, payload: { ...draft.payload, type } })).json()).toMatchObject({ status: "pending_reconciliation" });
    }
    expect((await send({ ...event(context.row), payload: { ...event(context.row).payload, subscription_id: 999999 } })).json()).toMatchObject({ status: "pending_reconciliation" });
    const enrollment = await db.prisma.subscriptionEnrollment.findFirstOrThrow({ where: { accountId: customer.id } });
  ...
  test("35 explicit revoke survives import and renewal; owner restore changes the same source", async () => {
    const context = await setup(); const customer = await link(context.row.identityRef); const imported = await apply(context.row);
    const source = imported.result.sources[0]; if (!source?.enrollmentId) throw new Error("Expected linked source");
    value(await grants.changeEnrollment(owner, { operationId: randomUUID(), enrollmentId: source.enrollmentId, expectedRevision: 1,
      action: "revoke", terms: { startsAt: context.row.startsAt, endsAt: context.row.endsAt, endPolicy: "confirmed_external" }, reason: "Owner revoked exact source" }));
    now = new Date("2030-01-02T00:00:00.000Z");
    expect((await send(event(context.row))).json()).toMatchObject({ status: "pending_reconciliation" });
    const current = await db.prisma.sourceEntitlement.findUniqueOrThrow({ where: { id: source.id } });
    const preview = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [{ ...context.row, expectedRevision: current.revision }] }));
    expect(preview.rows[0]?.status).toBe("conflict");
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).not.toMatchObject({ kind: "active" });
    const restored = value(await convergence.reconcile(owner, { operationId: randomUUID(), sourceId: source.id, expectedRevision: current.revision,
      action: "restore", confirmedTerms: { startsAt: context.row.startsAt, endsAt: context.row.endsAt, verificationRef: "owner-confirmed-period" }, reason: "Explicit confirmed restoration" }));
    expect(restored).toMatchObject({ id: source.id, enrollmentId: source.enrollmentId, status: "active", revoked: false });
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).toMatchObject({ kind: "active" });
    expect(await db.prisma.subscriptionEnrollment.count({ where: { accountId: customer.id } })).toBe(1);
  });
  test("36 temporary TTL restores only stale source; accepted left is latched across subsequent member", async () => {
    const context = await setup("temporary_membership"); const customer = await link(context.row.identityRef); const imported = await apply(context.row);
    const source = imported.result.sources[0]; if (!source) throw new Error("Expected temporary source");
    expect(source.status).toBe("pending_verification");
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).not.toMatchObject({ kind: "active" });
    async function observe(decision: "member" | "not_member", version: number) {
      return membership.acceptEvidence({ accountId: accountId(customer.id), deliveryId: randomUUID(), source: "member_status_event", evidence: {
        contractVersion: "inside.membership-evidence.v1", principalRef: customer.principal, telegramIdentityRef: context.row.identityRef,
        ...
        decision, reasonCode: decision === "member" ? "chat_member" : "chat_not_member",
      } });
    }
    expect(await observe("member", 1)).toMatchObject({ ok: true, outcome: "applied" });
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).toMatchObject({ kind: "active" });
    ...
    expect(value(await grants.readOwnEnrollments(customer.id))[0]?.state).toBe("suspended_source");
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).not.toMatchObject({ kind: "active" });
    const enrollment = await db.prisma.subscriptionEnrollment.findUniqueOrThrow({ where: { id: source.enrollmentId ?? "" } });
    const changed = value(await grants.changeEnrollment(owner, { operationId: randomUUID(), enrollmentId: enrollment.id,
      expectedRevision: enrollment.revision, action: "change_term", reason: "Temporary term correction is not membership evidence",
      terms: { startsAt: context.row.startsAt, endsAt: context.row.endsAt, endPolicy: "temporary_membership" } }));
    expect(changed.state).toBe("suspended_source");
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).not.toMatchObject({ kind: "active" });
    const current = await db.prisma.sourceEntitlement.findUniqueOrThrow({ where: { id: source.id } });
    const confirmed = { ...context.row, mode: "confirmed_period" as const, expectedRevision: current.revision, checkedAt: now.toISOString() };
    await apply(confirmed);
    expect(await membership.resolveForAccess(accountId(customer.id), [context.guideId])).toMatchObject({ kind: "active" });
    expect(value(await grants.readOwnEnrollments(customer.id))[0]?.endPolicy).toBe("confirmed_external");
  });

  test("preview rejects same-account relinking and parallel apply cannot duplicate a source", async () => {
    const context = await setup(); const customer = await link(context.row.identityRef);
    const preview = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [context.row] }));
    await db.prisma.telegramLinkTransaction.updateMany({ where: { accountId: customer.id }, data: { status: "expired" } });
    await linkTelegramAccount(db.prisma, { accountId: customer.id, identityRef: context.row.identityRef, now });
    expect(await convergence.apply(owner, { operationId: randomUUID(), previewRef: preview.previewRef, selectedRows: [context.row.rowRef] }))
      .toMatchObject({ ok: false, error: { code: "revision_conflict" } });
    const fresh = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [context.row] }));
    const command = { operationId: randomUUID(), previewRef: fresh.previewRef, selectedRows: [context.row.rowRef] };
    const results = await Promise.all([convergence.apply(owner, command), convergence.apply(owner, command)]);
  ...
  test("23 Tribute activation uses only registry, exact binding and finite source; forwarded links and member proof grant nothing", async () => {
    const context = await setup(); const imported = await apply(context.row);
    const rule = { id: randomUUID(), code: randomUUID(), name: "Tribute", tierId: context.tier.id, tierRevision: 1,
      sourceRef: context.row.policyRef, verificationMode: "tribute_registry", published: true, startsAt: now.toISOString(), endsAt: null };
    expect(await grants.manageActivationRule(owner, { operationId: randomUUID(), value: rule, reason: "Verified registry rule" })).toMatchObject({ ok: true });
    async function request(path: string, payload: object, credential = "synthetic-activation-625") {
      const response = await http.inject({ method: "POST", url: `/integrations/telegram/v1/subscription-activation/${path}`,
        headers: { authorization: `Bearer ${credential}` }, payload });
      expect(response.headers["cache-control"]).toBe("private, no-store");
      return response;
    }
    const attemptId = randomUUID(); const contractVersion = "inside.subscription-activation.v1";
    const begin = { contractVersion, attemptId, code: rule.code, identityRef: context.row.identityRef };
    expect((await request("attempts", begin)).json()).toMatchObject({ ok: true, value: { state: "needs_account", rule: { verificationMode: "tribute_registry" } } });
    expect(await db.prisma.sourceEntitlement.findUnique({ where: { id: imported.result.sources[0]?.id ?? "" } })).toMatchObject({ accountId: null });
    const customer = await link(context.row.identityRef);
    const snapshot = await new TelegramAccountLinks(db.prisma).readBinding({ accountId: customer.id });
    if (!snapshot.ok || !snapshot.binding) throw new Error("Missing binding");
    const evidence = { contractVersion, audience: "inside.platform.subscription-activation", evidenceRef: randomUUID(), attemptId,
      sourceRef: context.row.policyRef, identityRef: context.row.identityRef, accountRef: customer.principal,
      linkRef: snapshot.binding.linkRef, linkRevision: snapshot.binding.linkRevision, ruleId: rule.id, ruleRevision: 1,
      checkedAt: now.toISOString(), validUntil: "2030-01-01T00:04:00.000Z", decision: "registry_lookup" };
    expect((await request("evidence", evidence, "other-source-key")).statusCode).toBe(401);
    expect((await request("evidence", { ...evidence, decision: "member" })).json()).toMatchObject({ ok: false, error: { code: "source_not_confirmed" } });
    await db.prisma.$executeRaw`ALTER TABLE membership_entitlements.tribute_policies RENAME TO tribute_policies_fault625`;
    try {
      const unavailable = await request("evidence", evidence);
      expect(unavailable.statusCode).toBe(200);
      expect(unavailable.json()).toEqual({ ok: false, error: { code: "unavailable" } });
      expect(await db.prisma.accessReceipt.count({ where: { scope: "source-evidence", operationId: evidence.evidenceRef } })).toBe(0);
    } finally { await db.prisma.$executeRaw`ALTER TABLE membership_entitlements.tribute_policies_fault625 RENAME TO tribute_policies`; }
    const accepted = activationResponseSchema.parse((await request("evidence", evidence)).json());
    expect(accepted).toMatchObject({ ok: true, value: { state: "active", enrollment: { origin: "tribute", endsAt: context.row.endsAt, endPolicy: "confirmed_external" } } });
    expect((await request("evidence", evidence)).json()).toEqual(accepted);
    const second = activationResponseSchema.parse((await request("evidence", { ...evidence, evidenceRef: randomUUID() })).json());
    if (!accepted.ok || accepted.value.enrollment === null) throw new Error("Expected active receipt");
    expect(second).toMatchObject({ ok: true, value: { state: "already_active", enrollment: { id: accepted.value.enrollment.id, revision: accepted.value.enrollment.revision } } });
    const strangerIdentity = randomUUID(); const stranger = await link(strangerIdentity); const strangerAttempt = randomUUID();
    await request("attempts", { ...begin, identityRef: strangerIdentity, attemptId: strangerAttempt });
    const strangerBinding = await new TelegramAccountLinks(db.prisma).readBinding({ accountId: stranger.id });
    if (!strangerBinding.ok || !strangerBinding.binding) throw new Error("Missing stranger binding");
    expect((await request("evidence", { ...evidence, evidenceRef: randomUUID(), attemptId: strangerAttempt,
      identityRef: strangerIdentity, accountRef: stranger.principal, linkRef: strangerBinding.binding.linkRef, linkRevision: strangerBinding.binding.linkRevision })).json())
      .toMatchObject({ ok: true, value: { state: "pending_review", enrollment: null } });
    expect(await db.prisma.accessGrant.count({ where: { accountId: stranger.id } })).toBe(0);
    expect(await db.prisma.billingPurchase.count({ where: { accountId: customer.id } })).toBe(0);
  });
  test("unknown import rows survive preview expiry until an audited owner disposition", async () => {
    const context = await setup();
    const before = value(await convergence.status(owner)).metrics.unresolvedImports;
    const preview = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: "unknown-period-review", rows: [{ ...context.row, endsAt: null }] }));
    now = new Date(now.getTime() + 900_000);
    expect(value(await convergence.status(owner)).metrics.unresolvedImports).toBe(before + 1);
    expect(await convergence.apply(owner, { operationId: randomUUID(), previewRef: preview.previewRef, selectedRows: [context.row.rowRef] })).toMatchObject({ ok: false, error: { code: "preview_expired" } });
    const command = { operationId: randomUUID(), previewRef: preview.previewRef, expectedRevision: 1, reason: "Confirmed excluded row; no verified entitlement" };
    expect(value(await convergence.dismissImport(owner, command)).state).toBe("dismissed");
    expect(value(await convergence.dismissImport(owner, command)).revision).toBe(2);
    expect(value(await convergence.status(owner)).metrics.unresolvedImports).toBe(before);
    const collision = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [context.row, { ...context.row, rowRef: randomUUID(), telegramUserId: String(Number(context.row.telegramUserId) + 90000) }] }));
    expect(collision.rows.map(row => row.status)).toEqual(["conflict", "conflict"]);
  });
  test("audited inbox rejection resolves a conflict without inventing a period or reviving rejected events", async () => {
    const context = await setup(); now = new Date("2030-01-02T00:00:00.000Z");
    const first = z.object({ receiptRef: z.uuid() }).parse((await send(event(context.row))).json());
    const second = z.object({ receiptRef: z.uuid() }).parse((await send(event(context.row, "renewed_subscription", "2030-04-01T00:00:00.000Z"))).json());
    await apply(context.row);
    const rejected = await db.prisma.tributeInbox.findUniqueOrThrow({ where: { id: second.receiptRef } });
    const command = { operationId: randomUUID(), inboxId: rejected.id, expectedRevision: rejected.revision, action: "reject", reason: "Provider confirmed this conflicting delivery is incorrect" };
    expect(value(await convergence.retryEvent(owner, command)).state).toBe("rejected");
    expect(value(await convergence.retryEvent(owner, command)).state).toBe("rejected");
    const remaining = await db.prisma.tributeInbox.findUniqueOrThrow({ where: { id: first.receiptRef } });
    expect(value(await convergence.retryEvent(owner, { operationId: randomUUID(), inboxId: remaining.id, expectedRevision: remaining.revision, action: "retry", reason: "Verified retained provider event" })).state).toBe("applied");
    expect(value(await convergence.retryEvent(owner, { operationId: randomUUID(), inboxId: rejected.id, expectedRevision: rejected.revision + 1, action: "retry", reason: "Rejected event stays terminal" })).state).toBe("rejected");
    const source = await db.prisma.sourceEntitlement.findFirstOrThrow({ where: { identityRef: context.row.identityRef } });
    expect(z.object({ endsAt: z.string() }).parse(source.tributeState).endsAt).toBe("2030-03-01T00:00:00.000Z");
  });
  test("pending source keeps its promised tier; changing policy cannot bypass archive, while existing terms remain updateable", async () => {
    ...
    const preview = value(await convergence.preview(owner, { operationId: randomUUID(), batchRef: randomUUID(), rows: [currentRow] }));
    expect(preview.rows[0]?.tier?.id).toBe(context.tier.id);
    expect(await convergence.apply(owner, { operationId: randomUUID(), previewRef: preview.previewRef, selectedRows: [currentRow.rowRef] })).toMatchObject({ ok: false, error: { code: "not_found" } });
    await db.prisma.billingOffer.update({ where: { id: context.tier.id }, data: { archived: false } });
    const customer = await link(context.row.identityRef); await convergence.sweep();
    await db.prisma.billingOffer.update({ where: { id: context.tier.id }, data: { archived: true } });
    const attached = await db.prisma.sourceEntitlement.findUniqueOrThrow({ where: { id: source.id } });
    await apply({ ...context.row, expectedRevision: attached.revision, endsAt: "2030-03-01T00:00:00.000Z" });
    expect(value(await grants.readOwnEnrollments(customer.id))[0]).toMatchObject({ tier: { id: context.tier.id }, endsAt: "2030-03-01T00:00:00.000Z" });
  });
});
