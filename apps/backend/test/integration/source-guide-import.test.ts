...
const materialSource = {
  id: "inside-content:guide-import-lesson",
  path: "lesson.md",
...
describe("authoring source Guide completion", () => {
  ...
  test("keeps an imported Guide out of ordinary archive and renames it through its source", async () => {
    ...
    const updated = await authoring.updateSourceGuide({
      actor,
      sourceId: guideSource,
      collectionId: guide.id,
      expectedVersion: guide.version,
      name: "Переименованный продукт",
      summary: guide.summary,
      source: { slug: guide.slug, presentation: "default", page: null },
    });
    ...
    expect(
      await authoring.updateSourceGuide({
        actor,
  ...
  test("stores the product page only through its source and keeps the product when its address changes", async () => {
    ...
    expect(
      await authoring.updateContentCollection({
        ...editorRequest,
        kind: "guide",
        source: { slug: "page-guide", presentation: "ai-first-process", page },
      }),
    ).toMatchObject({ ok: false, error: { code: "forbidden" } });
    for (const invalid of [
      ...
      const outcome = await authoring.updateSourceGuide({
        ...request,
    ...
    const imported = await authoring.updateSourceGuide({
      ...request,
      source: { slug: "page-guide", presentation: "ai-first-process", page },
    });
    ...
    const repeated = await authoring.updateSourceGuide({
      ...request,
      source: {
        slug: "page-guide",
        presentation: "ai-first-process",
    ...
    const moved = await authoring.updateSourceGuide({
      ...request,
      expectedVersion: imported.value.version,
      source: {
        slug: "page-guide-renamed",
        presentation: "ai-first-process",
        page,
      },
    });
    ...
    expect(listed.value.find((item) => item.id === guideId)).toMatchObject({
      page: null,
      pageRejected: true,
    });
    const current = listed.value.find((item) => item.id === guideId);
    if (current === undefined)
      throw new Error("Expected the imported Guide in the list");
    const repaired = await authoring.updateSourceGuide({
      ...request,
    ...
    expect(
      await authoring.reserveSourceGuide({
        actor,
    ...
    const other = await reserveGuide(
      "inside-content:other-page-guide",
      "other-page-guide",
    );
    expect(
      await authoring.updateSourceGuide({
        actor,
        sourceId: "inside-content:other-page-guide",
  ...
  test("changes covers only for records owned by the same source", async () => {
    ...
    const guide = await reserveGuide(
      "inside-content:cover-guide",
      "cover-guide",
    );
    expect(
      await covers.changeImported(
  ...
  test("imports artifacts only into the Guide owned by the declared source", async () => {
    const artifacts = assembleGuideArtifacts({
      authorPolicy,
      objectStorage,
      prisma: database.prisma,
    });
    const guide = await reserveGuide(
      "inside-content:artifact-guide",
      "artifact-guide",
    );
    ...
    expect(
      await artifacts.applyAuthoringImport({
        ...
        artifacts: [artifact],
        guideId: guide.id,
        guideSourceId: "inside-content:other",
      }),
    ).toMatchObject({ ok: false, error: { code: "forbidden" } });
    expect(
      await artifacts.applyAuthoringImport({
        ...
        artifacts: [artifact],
        guideId: guide.id,
        guideSourceId: "inside-content:artifact-guide",
      }),
    ).toMatchObject({
      ok: true,
      value: {
