...
describe("Subscription Enrollment with real PostgreSQL", () => {
  ...
  beforeAll(async () => {
    ...
    const accounts = assembleAccounts({
      prisma: db.prisma,
      emailFingerprintKey: "synthetic-enrollment-fingerprint-key",
    });
  ...
  test("course repeat is one origin, revoke survives retries, restore retains identity and first start", async () => {
    ...
    const command = {
      ...
      accountId: target,
      origin: "course",
      sourceRef: `course:${target}`,
      tierId: tier.id,
      tierRevision: 1,
      terms,
      billingRef: null,
      reason: "Verified course",
    };
    const [a, b] = await Promise.all([
      grants.assignEnrollment(owner, command, tier),
      grants.assignEnrollment(owner, command, tier),
    ]);
    expect(a).toEqual(b);
    ...
    expect(
      await db.prisma.billingPurchase.count({ where: { accountId: target } }),
    ).toBe(0);
    expect(
      await grants.assignEnrollment(
        owner,
        { ...command, operationId: randomUUID(), accountId: await customer() },
        tier,
      ),
  ...
  test("scope cannot open another product or community, exact interval needs no worker", async () => {
    ...
    expect(
      await membership.resolveForAccess(accountId(target), [], material),
    ).toMatchObject({ kind: "active" });
    expect(
      await membership.resolveForAccess(accountId(target), [randomUUID()]),
    ).not.toMatchObject({ kind: "active" });
    expect(await grants.resolveCapabilities(target)).toMatchObject({
      ok: true,
      capabilities: [{ capability: "materials", validUntil: end }],
    });
    now = new Date(end);
    expect(
      await membership.resolveForAccess(accountId(target), [guide]),
  ...
  test("paid scope expansion preserves source revisions and refund revokes partial and late fulfillment", async () => {
    ...
    const enrollment = {
      purchaseRef,
      billingRef: randomUUID(),
      tier,
      startsAt: terms.startsAt,
      endsAt: paidTerms.validUntil,
    };
    const command = {
      eventRef: randomUUID(),
      periodRef: `${purchaseRef}:materials`,
      accountId: target,
      revision: 1,
      revoked: false,
      terms: paidTerms,
    ...
    const row = await db.prisma.subscriptionEnrollment.findFirstOrThrow({
      where: { accountId: target },
    });
    const expanded = {
      ...tier,
      ...
      contentScope: { ...tier.contentScope, materialIds: [randomUUID()] },
    };
    const preview = value(
      await grants.previewEnrollmentExpansion(
        owner,
        {
          ...
          tierRevision: 2,
          targets: [
            { enrollmentId: row.id, expectedRevision: 1, tierRevision: 1 },
          ],
          reason: "Approved cohort expansion",
        },
        expanded,
      ),
    );
    ...
    expect(
      await db.prisma.accessGrant.count({
        where: { accountId: target, capabilities: { has: "community" } },
      }),
    ).toBe(0);
    expect(
      await grants.applyPaidPeriod({
        ...command,
        eventRef: randomUUID(),
        periodRef: `${purchaseRef}:community`,
        terms: {
          ...paidTerms,
    ...
    expect(
      await db.prisma.accessGrant.count({
        where: { accountId: target, revokedAt: null },
      }),
    ).toBe(0);
    expect(
      ...
      {
        validUntil: new Date("2030-01-15T00:00:00.000Z"),
        contentScope: expanded.contentScope,
      },
    ]);
    expect(
      await grants.applyPaidPeriod({
        ...
        revision: 3,
        revoked: false,
        terms: { ...paidTerms, capabilities: ["community"] },
      }),
    ).toMatchObject({ ok: true });
    expect(
      await db.prisma.accessGrant.count({
    ...
    const second = await customer(),
      secondPurchase = randomUUID();
    expect(
      await grants.applyPaidPeriod({
        ...command,
        eventRef: randomUUID(),
        accountId: second,
        periodRef: `${secondPurchase}:materials`,
        enrollment: {
          ...enrollment,
          purchaseRef: secondPurchase,
          benefitTerms: [
            paidTerms,
            {
              ...paidTerms,
    ...
    expect(
      await db.prisma.accessGrant.count({ where: { accountId: second } }),
    ).toBe(2);
    const ownerView = value(await grants.listEnrollments(owner, second));
    expect(ownerView[0]?.benefitTerms).toEqual(
    ...
    expect(
      await membership.resolveManyForAccess?.(accountId(second), [
        { guideIds: tier.contentScope.guideIds },
        { guideIds: [randomUUID()] },
      ]),
    ).toMatchObject([{ kind: "active" }, { kind: "required" }]);
  });
  test("Tribute cannot be granted or changed to an indefinite period", async () => {
    ...
    const command = {
      ...
      terms,
      billingRef: null,
      reason: "Verified external membership",
    };
    expect(await grants.assignEnrollment(owner, command, tier)).toMatchObject({
      ok: false,
    ...
    expect(
      await grants.changeEnrollment(owner, {
        ...
        reason: "Cannot create temporary audience through term change",
      }),
    ...
    expect(
      await grants.changeEnrollment(owner, {
        ...
        action: "change_term",
        terms,
        reason: "Cannot erase external bound",
      }),
    ).toMatchObject({ ok: false, error: { code: "invalid_input" } });
  });
  ...
  test("recipient lookup is authorized, current and unambiguous; relink cannot assign another identity", async () => {
    ...
    expect(await lookup()).toMatchObject({
      ok: true,
      state: "found",
      recipient: { accountId: target, identityRef, linkRevision: 1 },
    });
    expect(await grants.lookupRecipient(target, identityRef)).toMatchObject({
      ok: false,
    ...
    await db.prisma.telegramAccountLinkState.create({
      data: {
        ...
        linkRef: randomUUID(),
        revision: 1,
        principalRef: `account:${duplicate}`,
        identityRef,
        updatedAt: now,
      },
    });
    ...
    await db.prisma.telegramAccountLinkState.update({
      where: { accountId: duplicate },
      data: { identityRef: null, principalRef: null, revision: 2 },
    });
    const tier = {
      id: randomUUID(),
      revision: 1,
    ...
    const command = {
      ...
      terms,
      billingRef: null,
      reason: "Verified owner selection",
    };
    const assigned = await grants.assignEnrollment(owner, command, tier);
    expect(assigned).toMatchObject({ ok: true });
    await db.prisma.telegramAccountLinkState.update({
      where: { accountId: target },
      data: { identityRef: "relinked-identity", revision: 2 },
    });
    expect(await lookup()).toMatchObject({ ok: true, state: "not_found" });
    expect(await grants.assignEnrollment(owner, command, tier)).toEqual(
      assigned,
    );
    expect(
      await grants.assignEnrollment(
        owner,
        { ...command, operationId: randomUUID(), sourceRef: randomUUID() },
        tier,
      ),
    ).toMatchObject({ ok: false, error: { code: "identity_conflict" } });
    expect(
