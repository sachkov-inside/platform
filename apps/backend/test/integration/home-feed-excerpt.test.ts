...
describe("Home feed public note excerpts", () => {
  ...
  test("returns current free text, bounds long notes and removes text when access closes", async () => {
    ...
    const created = await authoring.createDraft({
      actor,
      idempotencyKey: "feed-note-draft",
      metadata,
      body: noteBody("Short complete note"),
    });
    ...
    const publish = async (
      ...
      const result = await authoring.saveMaterial({
        actor,
        idempotencyKey: key,
    ...
    const read = async () => {
      const result = await listPublishedMaterials(
        ...
        contentAccess,
        emptyCatalogVideos,
        { subject: anonymousSubject, q: "Feed excerpt safety", first: 24 },
      );
      if (!result.ok) throw new Error(result.error.code);
      return result.value.items.find(
        (item) => item.materialId === created.value.materialId,
      );
    };
    await publish("Short complete note", "free", "feed-note-short");
    expect(await read()).toMatchObject({
      noteExcerpt: { text: "Short complete note", truncated: false },
    });
    await publish(
      "Документация",
      "free",
    ...
    expect(await read()).toMatchObject({
      noteExcerpt: { linkUrl: "https://example.com/new-docs" },
    });
