...
function noteBody(text: string, href?: string) {
  return { schemaVersion: 1 as const, doc: { type: "doc", content: [{ type: "paragraph", attrs: { nodeId: "74000000-0000-4000-8000-000000000099" }, content: [{ type: "text", text, ...(href === undefined ? {} : { marks: [{ type: "link", attrs: { href } }] }) }] }] } };
}
...
describe("Home feed public note excerpts", () => {
  ...
  test("returns current free text, bounds long notes and removes text when access closes", async () => {
    ...
    await publish("Документация", "free", "feed-note-link", "https://example.com/docs");
    expect(await read()).toMatchObject({ noteExcerpt: { text: "Документация", linkUrl: "https://example.com/docs" } });
    await publish("Новая документация", "free", "feed-note-new-link", "https://example.com/new-docs");
    expect(await read()).toMatchObject({ noteExcerpt: { linkUrl: "https://example.com/new-docs" } });
    ...
    await publish("Private note secret", "membership", "feed-note-closed", "https://example.com/private-link");
