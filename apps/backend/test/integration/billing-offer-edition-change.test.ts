...
const currentOffer = syntheticConsentDocument("terms", {
  version: "purchase-v3",
  appliesTo: ["one_time"],
});
const acceptanceSchema = z.object({
  evidence: z.array(z.object({ document: z.object({ version: z.string() }) })),
});
...
describe("one-time offer edition change (real PostgreSQL and real facets; synthetic bank and email only)", () => {
  ...
  const contactOn = (documents: readonly LegalDocument[]) =>
    new BillingContact({
      prisma: db.prisma,
      protection,
      documents,
  ...
  async function scenario() {
    ...
    const start = await before.start(buyer, {
    ...
    const paymentsOn = (contact: BillingContact) =>
      new BillingPayments({
        prisma: db.prisma,
        bank,
        contact,
    ...
    const command = (
      quoteRef: string,
      consentEvidenceRefs: readonly string[],
    ) => ({
      operationId: randomUUID(),
      quoteRef,
      contactRevision: 1,
      consentEvidenceRefs: [...consentEvidenceRefs],
      acknowledgeExistingAccess: false,
    });
    const acceptedVersions = async (purchaseRef: string) =>
      acceptanceSchema
        .parse(
          (
  ...
  test("расчёт с согласием на прежнюю редакцию не оплачивается, пока покупатель не примет действующую", async () => {
    ...
    expect(
      code(
        await payments.purchase(
          s.buyer,
          s.command(quoteRef, earlier.evidenceRefs),
        ),
      ),
    ...
    expect(await s.accept(s.after, quoteRef, previousOffer)).toMatchObject({
      ok: false,
    ...
    const purchase = value(
      await payments.purchase(
        s.buyer,
        s.command(quoteRef, current.evidenceRefs),
      ),
    );
    expect(await s.acceptedVersions(purchase.purchaseRef)).toEqual([
      "purchase-v3",
    ]);
  });
  ...
  test("покупка, переданная в банк на прежней редакции, подтверждается на ней и открывает доступ", async () => {
    ...
    const purchase = value(
      await s
        .paymentsOn(s.before)
        .purchase(s.buyer, s.command(quoteRef, earlier.evidenceRefs)),
    );

    // Новая редакция вводится, пока банк ещё не подтвердил оплату.
    const payments = s.paymentsOn(s.after);
    expect(await payments.notification(s.confirmed())).toMatchObject({
      ok: true,
    });
    expect(await payments.recover()).toMatchObject({ ok: true });

    expect(
      value(await payments.status(s.buyer, purchase.purchaseRef)),
    ...
    expect(granted.map((grant) => grant.capabilities)).toEqual([
      [s.capability],
    ]);
  });
});
