...
const validateResponse = ajv.compile({
  $ref: `${schema.$id}#/definitions/activationResponse`,
});
const validateBinding = ajv.compile({
  $ref: `${schema.$id}#/definitions/bindingResponse`,
});
const validateEvidence = ajv.compile({
  $ref: `${schema.$id}#/definitions/evidence`,
});
describe("course activation HTTP authority with real PostgreSQL", () => {
  ...
  beforeAll(async () => {
    ...
    owner = (
      await bootstrapOwnerAccount(
        db.prisma,
        { issuer: "https://activation.example.test", subject: "owner" },
        "platform:admin",
      )
    ).accountId;
    const accounts = assembleAccounts({
      prisma: db.prisma,
      emailFingerprintKey: "activation-test-fingerprint-secret",
    });
    ...
    const service = new SubscriptionActivation({
      prisma: db.prisma,
      grants,
      ...
      providers: [
        ...
        { provide: SubscriptionActivation, useValue: service },
        {
          ...
          useValue: parsePlatformConfig({
            ...
            TELEGRAM_EVIDENCE_INGRESS_SECRET: otherAuthority,
  ...
  async function send(
    ...
    if (response.statusCode === 200 && path === "binding")
      expect(
        validateBinding(response.json()),
        JSON.stringify(validateBinding.errors),
      ).toBe(true);
    else if (response.statusCode === 200 && path !== "own-access")
      expect(
  ...
  async function linkedSnapshot(identityRef: string) {
    ...
    if (!result.ok || result.value.state !== "linked")
      throw new Error("Expected current binding");
  ...
  async function setup() {
    ...
    const policy = `course:${id}`;
    await db.prisma.account.create({
      data: {
        id,
        logtoIssuer: "https://activation.example.test",
        logtoSubject: id,
      },
    });
    ...
    expect(
      await grants.manageActivationRule(owner, {
        ...
        value: rule,
        reason: "Confirmed course source",
      }),
  ...
  test("pending source does not grant; two links and owner assignment share one durable source", async () => {
    ...
    expect(
      await grants.registerSourceEntitlement(owner, {
        operationId: randomUUID(),
        origin: "course",
        sourcePolicyRef: context.policy,
        identityRef: context.identityRef,
        checkedAt: now.toISOString(),
        startsAt: now.toISOString(),
        endsAt: null,
        reason: "Verified purchase before Account binding",
      }),
    ).toMatchObject({ ok: true });
    expect(
      await db.prisma.accessGrant.count({ where: { accountId: context.id } }),
    ).toBe(0);
    const begin = {
      contractVersion: version,
      attemptId,
    ...
    expect((await send("attempts", begin, "wrong-secret")).statusCode).toBe(
      401,
    );
    expect((await send("attempts", begin)).json()).toMatchObject({
      ok: true,
      value: { state: "needs_account" },
    });
    const accountRef = await linkTelegramAccount(db.prisma, {
      accountId: context.id,
      identityRef: context.identityRef,
      now,
    });
    const binding = await new TelegramAccountLinks(db.prisma).readBinding({
      accountId: context.id,
    });
    if (!binding.ok || binding.binding === null)
    ...
    const evidence = {
      ...
      identityRef: context.identityRef,
      accountRef,
      linkRef: binding.binding.linkRef,
      linkRevision: binding.binding.linkRevision,
    ...
    ).toMatchObject({ value: { state: "rejected" } });
    expect(
      (
        await send("evidence", {
          ...evidence,
    ...
    const source = await db.prisma.sourceEntitlement.findUniqueOrThrow({
      where: {
        origin_sourceRef: {
          origin: "course",
          sourceRef: courseSourceRef(context.policy, context.identityRef),
        },
      },
    });
    ...
    expect(
      await grants.assignEnrollment(
        owner,
        {
          ...
          courseSource: {
            policyRef: context.policy,
            verifiedIdentityRef: context.identityRef,
          },
          tierId: context.tier.id,
          tierRevision: 1,
          terms,
          billingRef: null,
          reason: "Owner repeats confirmed course",
        },
        row.snapshot,
      ),
    ).toMatchObject({ ok: true, value: { id: row.id } });
    expect(
    ...
    expect(
      await grants.manageActivationRule(owner, {
        operationId: randomUUID(),
        value: rule2,
        reason: "Second link same course",
      }),
    ).toMatchObject({ ok: true });
    ...
    expect(
      ...
      value: {
        ...
        enrollment: { id: row.id, state: "revoked" },
      },
    });
    expect(
      await db.prisma.subscriptionEnrollment.count({
    ...
    const own = await send("own-access", {
      contractVersion: version,
      accountRef,
      identityRef: context.identityRef,
      linkRef: binding.binding.linkRef,
    ...
    const validateOwn = ajv.compile({
      $ref: `${schema.$id}#/definitions/ownAccessResponse`,
    });
    expect(validateOwn(own.json()), JSON.stringify(validateOwn.errors)).toBe(
      true,
    );
    expect(own.json()).toMatchObject({
      ok: true,
      value: { admission: { state: "checking" } },
    });
    ...
    expect(
      (await send("evidence", { ...evidence, decision: "not_member" })).json(),
    ).toMatchObject({ ok: false, error: { code: "operation_conflict" } });
    await grants.manageActivationRule(owner, {
      operationId: randomUUID(),
      expectedRevision: 1,
      value: { ...rule2, published: false },
      reason: "Pause",
    });
    expect(
      (
    ...
    ).toMatchObject({ ok: false, error: { code: "policy_paused" } });
  });
  test("published rule explicitly rebinds to a new tier revision and retains the same activation link", async () => {
    ...
    await db.prisma.billingOffer.update({
      where: { id: context.tier.id },
      data: { name: "Новое название", revision: 2 },
    });
    const attemptId = randomUUID();
    const begin = {
      contractVersion: version,
    ...
    expect((await send("evidence", evidence)).json()).toMatchObject({
      ok: false,
    ...
    const repair = {
      operationId: randomUUID(),
      expectedRevision: 1,
      value: { ...context.rule, tierRevision: 2 },
      reason: "Explicit owner rebind",
    };
    expect(await grants.manageActivationRule(owner, repair)).toMatchObject({
      ok: true,
      value: {
        revision: 2,
        code: context.rule.code,
    ...
    expect(
      (await send("attempts", { ...begin, attemptId: nextAttempt })).json(),
    ).toMatchObject({ ok: true, value: { rule: { revision: 2 } } });
    expect(
      (
        await send("evidence", {
          ...evidence,
      ...
      value: {
        state: "active",
        enrollment: { tier: { revision: 2, name: "Новое название" } },
      },
    });
  });
  ...
  test("binding lookup requires the separate authority and returns only the exact current wire identity", async () => {
    ...
    for (const credential of [
      ...
      "invalid-key",
      otherAuthority,
      "test-linking-authority",
    ])
      expect((await send("binding", query, credential)).statusCode).toBe(401);
    for (const input of [
      { identityRef: context.identityRef },
      { ...query, contractVersion: "inside.subscription-activation.v2" },
      { ...query, identityRef: "" },
      { ...query, identityRef: "x".repeat(257) },
      { ...query, identityRef: 123 },
      { ...query, accountId: context.id },
      { ...query, username: "synthetic-name" },
    ])
      expect((await send("binding", input)).json()).toEqual({
        ok: false,
        error: { code: "invalid_input" },
      });
    expect(await lookup(context.identityRef)).toEqual({
      ok: true,
      value: { contractVersion: version, state: "unlinked" },
    });
    const accountRef = await linkTelegramAccount(db.prisma, {
      accountId: context.id,
      identityRef: context.identityRef,
    ...
    const current = await db.prisma.telegramAccountLinkState.findUniqueOrThrow({
      where: { accountId: context.id },
    });
    const browserTransaction =
      await db.prisma.telegramLinkTransaction.findFirstOrThrow({
        where: { accountId: context.id, status: "linked" },
      });
    const binding = await linkedSnapshot(context.identityRef);
    expect(binding).toEqual({
      accountRef,
      identityRef: context.identityRef,
      linkRef: current.linkRef,
      linkRevision: current.revision,
    });
    ...
    await db.prisma.telegramLinkTransaction.update({
      where: { linkRef: browserTransaction.linkRef },
      data: { status: "expired" },
    });
    expect(
      await db.prisma.telegramAccountLinkState.findUniqueOrThrow({
        where: { accountId: context.id },
      }),
    ).toMatchObject({ identityRef: null, principalRef: null });
    expect(await lookup(context.identityRef)).toEqual({
      ok: true,
      value: { contractVersion: version, state: "unlinked" },
    });
  ...
  test("binding lookup fails closed on ambiguity, malformed stored refs and PostgreSQL read failure", async () => {
    ...
    await db.prisma.telegramLinkTransaction.updateMany({
      where: { accountId: duplicate.id, status: "linked" },
      data: { status: "expired" },
    });
    ...
    await db.prisma
      .$executeRaw`alter table telegram_membership.account_link_states rename to unavailable_link_states`;
    try {
      expect(await lookup(context.identityRef)).toEqual({
        ok: false,
        error: { code: "unavailable" },
      ...
      await db.prisma
        .$executeRaw`alter table telegram_membership.unavailable_link_states rename to account_link_states`;
    }
    expect(
      await db.prisma.accessGrant.count({ where: { accountId: context.id } }),
    ).toBe(0);
  ...
  test("relink after HTTP lookup rejects stale evidence and own-access; refreshed snapshot works and replay stays durable", async () => {
    ...
    const evidence = {
      ...
      attemptId,
      sourceRef: context.policy,
      identityRef: oldBinding.identityRef,
      accountRef: oldBinding.accountRef,
      linkRef: oldBinding.linkRef,
      linkRevision: oldBinding.linkRevision,
      ruleId: context.rule.id,
      ruleRevision: 1,
      checkedAt: now.toISOString(),
      validUntil: "2030-01-01T00:04:00.000Z",
    ...
    expect(
      (
        await send("own-access", { contractVersion: version, ...oldBinding })
      ).json(),
    ...
    expect(
      (
        await send("own-access", { contractVersion: version, ...current })
      ).json(),
    ).toMatchObject({
      ok: true,
      value: { enrollments: [{ state: "active" }] },
    });
    await db.prisma.telegramLinkTransaction.updateMany({
      where: { accountId: context.id, status: "linked" },
      data: { status: "expired" },
    });
    expect(await lookup(context.identityRef)).toMatchObject({
      ok: true,
      value: { state: "unlinked" },
    });
    expect(
      (
        await send("own-access", { contractVersion: version, ...current })
    ...
    expect((await send("evidence", freshEvidence)).json()).toEqual(
      granted.json(),
    );
    expect(
      await db.prisma.subscriptionEnrollment.count({
