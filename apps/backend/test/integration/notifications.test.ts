...
import {
  ...
  type AudienceLane,
} from "../../src/modules/notifications/features/expand-audience/expand-audience.js";
...
import {
  dispatchEmail,
  acceptEmailCommand,
} from "../../src/modules/notifications/features/dispatch-email/dispatch-email.js";
...
describe("Notifications persistence and delivery (real PostgreSQL; synthetic source/provider facts)", () => {
  ...
  async function scenario(
    ...
    await database.prisma.account.create({
      data: {
        ...
        logtoSubject: actor,
        createdAt: new Date(instant.getTime() - 60_000),
      },
    });
    let sentCode = "";
    const billing = new BillingContact({
    ...
    const verify = async (revision = 0) => {
      const started = await billing.start(actor, {
        operationId: randomUUID(),
        expectedRevision: revision,
        email: `member-${revision}-${actor}@example.test`,
      });
      if (!started.ok) throw new Error(started.error.code);
      expect(
        await billing.confirm(actor, {
          operationId: randomUUID(),
          challengeRef: started.challengeRef,
          code: sentCode,
        }),
    ...
    const event: NotificationEvent = {
      ...
      sourceRevision: 1,
      occurredAt: new Date(instant.getTime() + 10_000).toISOString(),
      notAfter: new Date(
        instant.getTime() +
          10_000 +
          (category === "material" ? 86_400_000 : 3_600_000),
      ).toISOString(),
      eventType:
        category === "material" ? "material.published" : "billing.notice-ready",
      ...(category === "subscription"
        ? { accountRef: actor, kind: "payment_succeeded" }
        : {}),
    };
    const fact: NotificationSource = {
      status: "current",
      event,
      content:
        category === "material"
          ? { category, kind: "material_published" }
          : { category, kind: "payment_succeeded" },
      accountId: category === "material" ? null : actor,
      title: "Проверяемое сообщение",
      readerPath: "/materials/example",
      ...(category === "subscription" ? { amountMinor: 200000 } : {}),
    };
    ...
    const deps: NotificationDependencies = {
      ...
      sources: {
        resolve: (candidate) =>
          Promise.resolve(
            candidate.occurrenceRef === event.occurrenceRef
              ? source
              : { status: "unavailable" },
          ),
        canRead: () => Promise.resolve(access),
      },
      recipients: {
        exists: (id) => contacts.exists(id),
        enumerate: (query) => contacts.enumerate(query),
        binding: (id, channel) =>
          channel === "email" ? contacts.binding(id) : Promise.resolve(null),
        email: (binding) => contacts.email(binding),
      },
    };
    const accounts = assembleAccounts({
      prisma: database.prisma,
      emailFingerprintKey: "notification-test-key-long-enough",
    });
    const app = new Notifications(deps, accounts);
    const advance = (ms: number) => {
      instant = new Date(instant.getTime() + ms);
    };
    const lane: AudienceLane =
      category === "material" ? "materials" : "billing";
    const receive = async () => {
      advance(10_000);
      await app.acceptEvent(encodeNotification(lane, event));
      await database.prisma.notificationInbox.update({
        where: { scope_messageId: { scope: lane, messageId: event.messageId } },
        data: { nextAttemptAt: now() },
      });
    };
    const publish = async () => {
      await receive();
      await expandAudience(deps, lane, quarantined);
    };
    // Разбор берёт самую раннюю ожидающую строку дорожки, поэтому сценарий остаётся один на дорожке.
    const receiveAlone = async () => {
      await database.prisma.notificationInbox.deleteMany({
        where: { lane, completedAt: null },
      });
      await receive();
    };
    const commands = async () =>
      database.prisma.notificationCommand.findMany({
        where: {
          delivery: {
            notification: {
    ...
    const command = async () => {
      const row = (await commands()).at(-1);
      if (!row) throw new Error("Missing command");
      return { row, value: deliverySchema.parse(JSON.parse(row.payload)) };
    };
    const admit = async () => {
      const { value } = await command();
      await app.acceptEvent(
        encodeNotification(
          category === "material" ? "emailMaterial" : "emailSubscription",
          value,
        ),
      );
      return value;
    ...
    return {
      ...
      command,
      admit,
      source: (value: NotificationSource) => {
        source = value;
      },
      access: (value: typeof access) => {
        access = value;
      },
    };
  }
  ...
  function onPool(
    ...
    const ownConnection = async <Answer>(answer: Promise<Answer>) => {
    ...
    return {
      ...
      prisma,
      sources: {
        resolve: (event) => ownConnection(s.deps.sources.resolve(event)),
        canRead: (account, sourceRef) =>
          ownConnection(s.deps.sources.canRead(account, sourceRef)),
      },
      recipients: {
        exists: (id) => contacts.exists(id),
        enumerate: (query) => contacts.enumerate(query),
        binding: (id, channel) =>
          channel === "email"
            ? contacts.binding(id)
            : ownConnection(Promise.resolve(null)),
        email: (binding) => contacts.email(binding),
      },
    };
  }
  /** Expands the lane until nothing progresses: a material audience spans several batches. */
  async function drain(deps: NotificationDependencies, lane: AudienceLane) {
  ...
  test("preferences default off; atomic concurrent revisions, immutable replay and account isolation", async () => {
    ...
    expect(await s.app.readPreferences(s.actor)).toEqual({
      revision: 0,
      email: false,
      telegram: false,
    });
    ...
    const race = await Promise.all(
      Array.from({ length: 8 }, () => s.app.changePreferences(s.actor, input)),
    );
    expect(
      race.every((result) => result.ok && result.preferences.revision === 1),
    ).toBe(true);
    expect(
      await s.app.changePreferences(s.actor, { ...input, telegram: true }),
    ).toMatchObject({ ok: false, code: "operation_conflict" });
    const competing = await Promise.all(
      [true, false].map((email) =>
        s.app.changePreferences(s.actor, {
          ...input,
          operationId: randomUUID(),
          expectedRevision: 1,
          email,
    ...
    expect(await s.app.readPreferences(randomUUID())).toEqual({
      revision: 0,
      email: false,
      telegram: false,
    });
    expect(
      await s.app.changePreferences(s.actor, {
        ...input,
  ...
  test("billing occurrence and channel dedupe; concurrent provider workers commit started before one send", async () => {
    ...
    expect(
      await Promise.all(
        Array.from({ length: 8 }, () => s.app.acceptEvent(envelope)),
      ),
    ).toEqual(Array(8).fill("duplicate"));
    ...
    const send = async () => {
      sends += 1;
      expect(
        await database.prisma.notificationEmailEffect.findUnique({
          where: { deliveryId: c.deliveryRef },
        }),
    ...
    await Promise.all(
      Array.from({ length: 6 }, () =>
        dispatchEmail(s.deps, send, "subscription"),
      ),
    );
    expect(sends).toBe(1);
    expect(await project(s.app, c.deliveryRef)).toMatchObject({
      state: "sent",
    });
    const restarted = new Notifications(
      s.deps,
      assembleAccounts({
        prisma: database.prisma,
        emailFingerprintKey: "notification-test-key-long-enough",
    ...
    expect(
      await restarted.acceptEvent(encodeNotification("emailSubscription", c)),
    ).toBe("duplicate");
    await dispatchEmail(s.deps, send, "subscription");
    expect(sends).toBe(1);
    expect(
      (await s.app.readDeliveries(s.actor)).find(
        (row) => row.id === c.deliveryRef,
      )?.state,
    ).toBe("sent");
  });
  test("material audience excludes late opt-in; current opt-out and access are checked again", async () => {
    ...
    s.access("denied");
    expect(
      await s.app.authorizeDispatch("email", request(c, row.digest)),
    ).toMatchObject({ status: "denied", reason: "access_denied" });
    s.access("allowed");
    await s.app.changePreferences(s.actor, {
      operationId: randomUUID(),
      expectedRevision: 1,
    ...
    expect(
      await s.app.authorizeDispatch("email", request(c, row.digest)),
    ).toMatchObject({ status: "denied", reason: "preference_disabled" });
    const late = await scenario("material");
    await late.publish();
    await late.app.changePreferences(late.actor, {
      operationId: randomUUID(),
      expectedRevision: 0,
      email: true,
      telegram: false,
  ...
  test("authorization reads source and contact while its transaction holds the whole pool", async () => {
    ...
    const decided = await withExhaustedPool(database, (prisma) =>
      ...
      ).authorizeDispatch("email", request(c, row.digest)),
    );
  ...
  test("audience expansion reads source, audience, access and contact while its transaction holds the whole pool", async () => {
    for (const category of ["subscription", "material"] as const) {
      ...
      await s.receiveAlone();
      const expansions = await withExhaustedPool(database, (prisma) =>
        drain(onPool(s, prisma), s.lane),
      );
  ...
  test("a failed read the expansion never uses does not fail the row", async () => {
    ...
    const deps: NotificationDependencies = {
      ...
      recipients: {
        ...
        binding: (id, channel) =>
          channel === "telegram"
            ? Promise.reject(new Error("notification_binding_unavailable"))
            : s.deps.recipients.binding(id, channel),
      },
    };
    expect(
      (await drain(deps, s.lane)).filter((expansion) => expansion.observation),
    ).toEqual([]);
    expect(await s.commands()).toHaveLength(1);
  });
  test("audience facts read before another expansion moved the row are not applied", async () => {
    ...
    await s.receiveAlone();
    const key = {
      scope_messageId: { scope: s.lane, messageId: s.event.messageId },
    };
    const moved = { after: randomUUID(), recipients: 0, attempts: 0 };
    const moving: NotificationDependencies["prisma"] = {
      ...s.deps.prisma,
      $transaction: async (operation) => {
        await database.prisma.notificationInbox.update({
          where: key,
          data: { checkpoint: moved },
        });
        return s.deps.prisma.$transaction(operation);
      },
    };
    expect(
      await expandAudience({ ...s.deps, prisma: moving }, s.lane, quarantined),
    ).toEqual({ progressed: false });
    expect(
      await database.prisma.notificationInbox.findUniqueOrThrow({ where: key }),
    ).toMatchObject({ completedAt: null, checkpoint: moved });
    expect(await s.commands()).toHaveLength(0);
    // The next expansion reads the facts of the moved row again.
    expect(await expandAudience(s.deps, s.lane, quarantined)).toEqual({
      progressed: true,
    });
    expect(await s.commands()).toHaveLength(1);
  });
  test("command refresh reads source, access and contact while its transaction holds the whole pool", async () => {
    ...
    await database.prisma.notificationInbox.deleteMany({
      where: { lane: s.lane, completedAt: null },
    });
    await s.publish();
    const c = await s.admit();
    s.advance(600_001);
    ...
    const effect = () =>
      database.prisma.notificationEmailEffect.findUniqueOrThrow({
        where: { deliveryId: c.deliveryRef },
      });
    for (
      let call = 0;
      call < 25 && (await effect()).state === "accepted";
      call += 1
    )
      await dispatchEmail(
        s.deps,
    ...
    await withExhaustedPool(database, (prisma) =>
      refreshDeliveries(onPool(s, prisma)),
    );
  ...
  test("authorization binds channel, digest, attempt, source and contact; replay does not extend permit", async () => {
    ...
    await s.publish();
    const { value: c, row } = await s.command();
    const r = request(c, row.digest);
    expect(await s.app.authorizeDispatch("telegram", r)).toMatchObject({
      status: "denied",
      reason: "not_found",
    });
    ...
    expect(
      await s.app.authorizeDispatch("email", {
        ...r,
        attemptRef: randomUUID(),
      }),
    ).toMatchObject({ status: "error", code: "operation_conflict" });
    ...
    expect(
      await s.app.authorizeDispatch("email", request(c, row.digest)),
    ).toMatchObject({ status: "denied", reason: "binding_conflict" });
  });
  test("unknown blocks restart/revision/recipient bypass; operator skip remains unknown with audit", async () => {
    ...
    await dispatchEmail(
      ...
      () => {
        sends += 1;
        return Promise.reject(new Error("lost SMTP response"));
      },
      "subscription",
    );
    ...
    expect(sends).toBe(1);
    expect(
      await acceptEmailCommand(
        database.prisma,
        { ...c, operationId: randomUUID(), commandRevision: 2 },
        "emailSubscription",
        s.deps.now,
      ),
    ).toBe("conflict");
    ...
    expect(await s.app.resolveUnknown(s.actor, recovery)).toMatchObject({
      ok: false,
      code: "forbidden",
    });
    await database.prisma.accountPermission.create({
      data: { accountId: s.actor, permission: "platform:admin" },
    });
    ...
    ).toMatchObject({ state: "unknown", recoverySkipped: true });
    expect(
      await database.prisma.notificationRecoveryAudit.count({
        where: { deliveryId: c.deliveryRef },
      }),
    ).toBe(1);
    ...
    const sent = {
      ...unknown,
      messageId: randomUUID(),
      resultRevision: unknown.resultRevision + 1,
      state: "sent",
  ...
  test("explicit not-sent rejection retries with new attempt and fresh permit, same command", async () => {
    ...
    await dispatchEmail(
      s.deps,
      () => Promise.resolve({ state: "not_sent", retryAfterMs: 5_000 }),
      "subscription",
    );
    const retry = await project(s.app, c.deliveryRef);
    expect(retry.state).toBe("retrying");
    s.advance(5_000);
    await dispatchEmail(
      s.deps,
      () => Promise.resolve({ state: "sent" }),
    ...
    const sent = await project(s.app, c.deliveryRef);
    expect(sent.state).toBe("sent");
    expect(sent.attemptRef).not.toBe(retry.attemptRef);
    expect(
      await database.prisma.notificationEmailAttempt.count({
        where: { deliveryId: c.deliveryRef },
      }),
    ).toBe(2);
    expect(await s.commands()).toHaveLength(1);
  ...
  test("expired command is suppressed without I/O and may get a new immutable revision only after proof", async () => {
    ...
    await refreshDeliveries({
      ...s.deps,
      now: distinctClock(() => s.deps.now().getTime()),
    });
    expect(await s.commands()).toHaveLength(2);
    const next = await s.admit();
    expect(next.commandRevision).toBe(2);
    expect(next.deliveryRef).toBe(c.deliveryRef);
    expect(
      Date.parse(next.notAfter) - Date.parse(next.issuedAt),
    ).toBeLessThanOrEqual(COMMAND_LIFETIME_MS);
    await dispatchEmail(
      s.deps,
      () => {
        sends += 1;
  ...
  test("result correlation rejects foreign channel/attempt and conflicting revision, preserves late lower result", async () => {
    ...
    const unknown = await project(s.app, c.deliveryRef);
    expect(await s.app.acceptDeliveryResult("email", unknown)).toBe(
      "duplicate",
    );
    expect(
      await s.app.acceptDeliveryResult("email", {
        ...unknown,
        messageId: randomUUID(),
      }),
    ).toBe("operation_conflict");
    expect(
      await s.app.acceptDeliveryResult("telegram", {
        ...unknown,
        channel: "telegram",
        messageId: randomUUID(),
        resultRevision: 100,
      }),
    ).toBe("correlation_conflict");
    expect(
      await s.app.acceptDeliveryResult("email", {
        ...unknown,
        messageId: randomUUID(),
    ...
    const outbox = await database.prisma.notificationOutbox.findMany({
      where: { scope: "email" },
    });
    const prior = outbox
      .map((row) => resultSchema.parse(JSON.parse(row.payload)))
      .find(
        (row) => row.deliveryRef === c.deliveryRef && row.state === "accepted",
      );
  ...
  test("late retrying projection cannot suppress a retry proven not_sent by the email ledger", async () => {
    ...
    await dispatchEmail(
      s.deps,
      () => Promise.resolve({ state: "not_sent", retryAfterMs: 1_000 }),
      "subscription",
    );
    const rows = await database.prisma.notificationOutbox.findMany({
      where: { scope: "email" },
    });
    const unknown = rows
      .map((row) => resultSchema.parse(JSON.parse(row.payload)))
      .find(
        (result) =>
          result.deliveryRef === c.deliveryRef && result.state === "unknown",
      );
    expect(unknown).toBeDefined();
    await s.app.acceptDeliveryResult("email", unknown);
    s.advance(1_000);
    let sends = 0;
    await dispatchEmail(
      s.deps,
    ...
    const retrying = rows
      ...
      .find(
        (result) =>
          result.deliveryRef === c.deliveryRef && result.state === "retrying",
      );
    expect(await s.app.acceptDeliveryResult("email", retrying)).toBe(
      "accepted",
    );
    expect(await s.app.acceptDeliveryResult("email", sent)).toBe("accepted");
    expect(
  ...
  test("pause after started commit cannot send with an expired permit", async () => {
    ...
    await s.publish();
    const c = await s.admit();
    const pausingPrisma: NotificationDependencies["prisma"] = {
      ...s.deps.prisma,
      $transaction: async (operation) => {
        const result = await s.deps.prisma.$transaction(operation);
        const effect = await s.deps.prisma.notificationEmailEffect.findUnique({
          where: { deliveryId: c.deliveryRef },
        });
    ...
    expect(sends).toBe(0);
    expect(
      await database.prisma.notificationEmailAttempt.findFirst({
        where: { deliveryId: c.deliveryRef },
      }),
  ...
  test("expired source tombstones cannot starve a still-current command refresh", async () => {
    ...
    for (let index = 0; index < 30; index += 1) {
      ...
      await database.prisma.notification.create({
        data: {
          id: notificationId,
          kind: expired.eventType,
          occurrenceRef: expired.occurrenceRef,
          accountId: s.actor,
          eventPayload: JSON.stringify(expired),
          sourceRevision: 1,
          createdAt: s.deps.now(),
        },
      });
      await database.prisma.notificationDelivery.create({
        data: {
          id: deliveryId,
          notificationId,
          channel: "email",
          ...
          state: "suppressed",
          reason: "expired",
          nextCommandAt: new Date(s.deps.now().getTime() - 1_000),
          updatedAt: s.deps.now(),
        },
      });
      await database.prisma.notificationCommand.create({
        data: {
          operationId: copy.operationId,
          deliveryId,
          revision: 1,
          payload: envelope.payload,
          digest: envelope.digest.slice("sha256:".length),
          createdAt: s.deps.now(),
        },
      });
    }
    ...
    expect(
      await database.prisma.notificationDelivery.count({
        where: {
          notification: { accountId: s.actor },
          state: "suppressed",
          nextCommandAt: null,
        },
      }),
  ...
  test("service HTTP preserves correlated JSON errors and fails closed for wrong credentials/version", async () => {
    const s = await scenario();
    await s.publish();
    const { value, row } = await s.command();
    const credential = "synthetic-dedicated-telegram-secret-436";
    const config = parsePlatformConfig({
      NODE_ENV: "test",
      NOTIFICATIONS_PLATFORM_ORIGIN: s.deps.origin,
      NOTIFICATIONS_TELEGRAM_SECRET: credential,
    });
    @Module({
      controllers: [NotificationDispatchController],
      providers: [
        { provide: Notifications, useValue: s.app },
    ...
    try {
      ...
      expect(unauthorized.json<unknown>()).toEqual({ code: "unauthorized" });
      const unknown = await http.inject({
        ...
        payload: {
          ...payload,
          contractVersion: "inside.notification-dispatch.v2",
        },
      });
      expect(unknown.statusCode).toBe(422);
      expect(unknown.json<unknown>()).toMatchObject({
        ...payload,
        status: "error",
      ...
      expect(denied.json<unknown>()).toMatchObject({
        status: "denied",
        reason: "not_found",
      });
      const conflict = await http.inject({
        ...
        url: "/internal/notifications/dispatch/authorize",
        headers: { authorization: `Bearer ${credential}` },
        payload: { ...payload, attemptRef: randomUUID() },
      });
      expect(conflict.statusCode).toBe(409);
      expect(conflict.headers["content-type"]).toContain("application/json");
      expect(conflict.json<unknown>()).toMatchObject({
  ...
  test("SIGKILL after durable started cannot open another SMTP attempt after restart", async () => {
    ...
    const command = await s.admit();
    const child = fork(
      new URL("./fixtures/notification-email-crash.ts", import.meta.url),
      [],
      {
        execArgv: ["--import", "tsx"],
        ...
        env: {
          ...process.env,
          NOTIFICATION_CRASH_FIXTURE: JSON.stringify({
            databaseUrl: database.url,
            actor: s.actor,
            now: s.deps.now().toISOString(),
            event: s.event,
            fact: s.fact,
          }),
        },
    ...
    try {
      const message: unknown = await once(child, "message");
      expect(message).toEqual(expect.arrayContaining(["started"]));
      child.kill("SIGKILL");
  ...
  test("audience checkpoint survives batches and excludes Accounts created after the occurrence", async () => {
    ...
    for (const actor of existing)
      await s.app.changePreferences(actor, {
        operationId: randomUUID(),
        expectedRevision: 0,
        email: true,
        telegram: false,
    ...
    await s.app.changePreferences(late, {
      operationId: randomUUID(),
      expectedRevision: 0,
      email: true,
      telegram: false,
    ...
    for (let batch = 0; batch < 10; batch += 1)
      if (
        !(await expandAudience(restarted, "materials", quarantined)).progressed
      )
        break;
    const audience = await database.prisma.notification.findMany({
      where: { occurrenceRef: s.event.occurrenceRef },
      select: { accountId: true },
    });
    expect(
      existing.every((id) => audience.some((row) => row.accountId === id)),
    ).toBe(true);
    expect(audience.some((row) => row.accountId === late)).toBe(false);
    const total = audience.length;
    await s.app.acceptEvent(encodeNotification("materials", s.event));
    await expandAudience(restarted, "materials", quarantined);
    expect(
      await database.prisma.notification.count({
        where: { occurrenceRef: s.event.occurrenceRef },
      }),
    ).toBe(total);
  });
  test("строка без настроенной доставки ждёт настройки, а не роняет разбор", async () => {
    const s = await scenario();
    const key = {
      scope_messageId: { scope: "billing", messageId: s.event.messageId },
    };
    // Разбор берёт самую раннюю ожидающую строку дорожки, поэтому сценарий остаётся один на дорожке.
    await database.prisma.notificationInbox.deleteMany({
      where: { lane: "billing", completedAt: null },
    });
    s.advance(10_000);
    await s.app.acceptEvent(encodeNotification("billing", s.event));
    await database.prisma.notificationInbox.update({
      where: key,
      data: { nextAttemptAt: s.deps.now() },
    });
    ...
    const expansion = await expandAudience(
      { ...s.deps, origin: undefined },
      "billing",
      quarantined,
    );
    expect(expansion.observation).toMatchObject({
      lane: "billing",
      reason: "delivery_not_configured",
    });
    const row = await database.prisma.notificationInbox.findUniqueOrThrow({
      where: key,
    });
  ...
  test("необрабатываемая строка получает повторные попытки, затем карантин, и разбор продолжается", async () => {
    ...
    const record: QuarantineNotification = (lane, _bytes, reason) => {
      quarantine.push({ lane, reason });
      return Promise.resolve();
    };
    for (const attempt of [1, 2]) {
      ...
      expect(String(expansion.observation?.error)).toContain(
        "notification_link_invalid",
      );
      const pending = await database.prisma.notificationInbox.findUniqueOrThrow(
        { where: key },
      );
      expect(pending.completedAt).toBeNull();
      expect(pending.nextAttemptAt.getTime()).toBeGreaterThan(
        s.deps.now().getTime(),
      );
      s.advance(30_000);
    }
    ...
    expect(quarantine).toEqual([
      { lane: "billing", reason: "unprocessable_notification" },
    ]);
    const done = await database.prisma.notificationInbox.findUniqueOrThrow({
      where: key,
    });
    ...
    await database.prisma.notificationInbox.update({
      where: {
        scope_messageId: { scope: "billing", messageId: next.messageId },
      },
      data: { nextAttemptAt: s.deps.now() },
    });
    expect((await expandAudience(s.deps, "billing", record)).progressed).toBe(
      true,
    );
    const healthy = await database.prisma.notificationInbox.findUniqueOrThrow({
      where: {
        scope_messageId: { scope: "billing", messageId: next.messageId },
      },
    });
  ...
  test("переполненный карантин отводит строку в сторону, а круг разбора доходит до писем", async () => {
    ...
    const refused: QuarantineNotification = () =>
      Promise.reject(new Error("notification_quarantine_full"));
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      if (attempt > 1) s.advance(30_000);
      const expansion = await expandAudience(s.deps, "billing", refused);
      expect(expansion.observation?.reason).toBe(
        attempt < 3 ? "row_retry" : "quarantine_unavailable",
      );
    }
    ...
    const stalled = await database.prisma.notificationInbox.findUniqueOrThrow({
      where: key,
    });
    expect(stalled.completedAt).toBeNull();
    expect(stalled.nextAttemptAt.getTime()).toBeGreaterThan(
      s.deps.now().getTime(),
    );
    ...
    expect(
      observations.some((observation) => observation.lane === "billing"),
    ).toBe(true);
  });
  test("unavailable and invalid source facts never authorize an event, template links stay on Platform", async () => {
    ...
    expect(() =>
      renderNotification(
        { ...s.fact, readerPath: "https://evil.example/path" },
        standOrigin,
      ),
    ).toThrow("notification_link_invalid");
  });
});
