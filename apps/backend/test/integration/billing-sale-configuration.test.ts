...
const allMethods = { recurringCardConfirmed: false, cardOnlyHostedConfirmed: false };
const cardOnly = { recurringCardConfirmed: true, cardOnlyHostedConfirmed: true };
const contact = { encryptionKey: Buffer.alloc(32, 7).toString("base64"), smtpHost: "127.0.0.1", smtpPort: 587, from: "inside@example.test", localInsecure: true };
 ...
 * Проверка читает весь каталог, поэтому у каждого случая своя база.
 */
describe("sale configuration at process start (real PostgreSQL)", () => {
  async function catalog() {
    ...
    const pricing = new BillingPricing({ prisma: db.prisma, sale: { payments: true, subscriptions: true },
      accounts: assembleAccounts({ prisma: db.prisma, emailFingerprintKey: "sale-configuration-test-key-000000000" }) });
    ...
    const check = (tbank?: Terminal, billingContact?: typeof contact) => pricing.assertSaleConfigured({ tbank, billingContact });
    return { db, offer, check };
  }

  test("nothing on sale starts without payment configuration; a published offer without a live option sells nothing", async () => {
    const { db, offer, check } = await catalog();
    try {
  ...
  test("one-time sale needs the terminal and receipt contact but not the card-only confirmations", async () => {
    ...
    try {
      ...
      await expect(check()).rejects.toThrow("Sale is enabled in the billing catalog, but TBANK_CONFIG_JSON is not configured");
      await expect(check(allMethods)).rejects.toThrow("Sale is enabled in the billing catalog, but BILLING_CONTACT_* is not configured");
  ...
  test("subscription sale on a terminal without both confirmations refuses to start", async () => {
    ...
    try {
      ...
      await expect(check(allMethods, contact))
        .rejects.toThrow("Subscription sale is enabled in the billing catalog, but the terminal does not confirm recurringCardConfirmed and cardOnlyHostedConfirmed");
      await expect(check({ ...cardOnly, cardOnlyHostedConfirmed: false }, contact)).rejects.toThrow("Subscription sale");
