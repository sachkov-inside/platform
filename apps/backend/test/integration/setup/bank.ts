...
export class BankFixture {
  ...
  private respond(
    ...
    if (url.endsWith("/Init")) {
      ...
      if (this.failInit)
        throw new Error("Synthetic Init timeout after bank acceptance");
    ...
    if (url.endsWith("/Charge")) {
      ...
      if (this.failCharge)
        throw new Error("Synthetic Charge timeout after bank acceptance");
      entry[1].status = this.chargeOutcome;
      return Response.json(this.event(entry[0]));
    }
    // Подтверждённый возврат. Отказ и потерянный ответ банка на возврат принадлежат
    // `billing-operations`: своего двойника с этими исходами он уже держит.
    ...
    if (url.endsWith("/GetAddCardState")) {
      ...
      return Response.json({
        ...
        Success: this.binding.success,
        ErrorCode: this.binding.success ? "0" : "3000",
        ...(this.binding.rebillId === undefined
          ? {}
          : { RebillId: this.binding.rebillId }),
      });
    }
