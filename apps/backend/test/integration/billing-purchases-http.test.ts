...
const terminal = syntheticTbankConfig({
  environment: "demo",
  terminalKey: "SYNTHETICPURCHASES",
...
describe("Billing purchases HTTP", () => {
  ...
  test("a buyer reads the payment they started, and nobody else's", async () => {
    ...
    const strangerToken = await signToken(
      "purchase-stranger-001",
    ...
    const unknown = await server.inject({
      method: "GET",
      url: `/accounts/current/billing/purchases/${randomUUID()}`,
      headers,
    });
    ...
    expect(
      (
        await server.inject({
          method: "GET",
          url: `/accounts/current/billing/purchases/not-a-uuid`,
          headers,
        })
      ).statusCode,
    ).toBe(404);
  ...
  async function startedPurchase(
    buyer: string,
    owner: string,
  ): Promise<string> {
    ...
    if (code === undefined)
      throw new Error("the synthetic contact never received its code");
    succeeds(
      ...
      pressedPaymentButton(
        {
          ...
          documents: syntheticConsentDocuments.map((document) => ({
            kind: document.kind,
    ...
    const payments = new BillingPayments({
      prisma: database.prisma,
      bank: new BankFixture(terminal).client(),
      contact,
      grants,
      clock: () => now,
    });
