...
function asRefundDecision(result: OwnerResult) {
  if (!result.ok) throw new Error(result.error.code);
  if (result.result.outcome !== "refundDecision")
    throw new Error(`Unexpected outcome ${result.result.outcome}`);
  return result.result.value;
}
function asRefunds(result: OwnerResult) {
  if (!result.ok) throw new Error(result.error.code);
  if (result.result.outcome !== "refunds")
    throw new Error(`Unexpected outcome ${result.result.outcome}`);
...
function standUrl(value: string | null | undefined): string {
  if (typeof value !== "string" || value.length === 0)
    throw new Error("Expected a stand URL");
  return value;
}
...
describe("локальная продажа через двойника банка (реальный PostgreSQL, двойник стенда)", () => {
  ...
  beforeAll(async () => {
    ...
    for (const permission of ["platform:admin", "billing:manage"])
      await db.prisma.accountPermission.create({
        data: { accountId: owner, permission },
      });
    accounts = assembleAccounts({
      prisma: db.prisma,
      emailFingerprintKey: "synthetic-stand-fingerprint-key-000000",
    });
  ...
  async function scenario() {
    ...
    const control = async (values: Record<string, string>) =>
      double.handle(
        new Request(`${formOrigin}/control`, {
          method: "POST",
    ...
    async function beginPurchase(
        ...
        pressedPaymentButton(
          {
            ...
            documents: documents
              .filter((document) => accepted.includes(document.kind))
              .map((document) => ({
                kind: document.kind,
      ...
      return {
        ...
        paymentUrl: standUrl(purchase.paymentUrl),
      };
    }
    ...
    const purchaseRow = (purchaseRef: string) =>
      db.prisma.billingPurchase.findUniqueOrThrow({
  ...
  test("оплата на форме стенда выдаёт права, сохраняет привязку и не удваивает выдачу", async () => {
    ...
    ).toMatchObject({ state: "active", paidUntil: "2030-02-28T10:00:00.000Z" });
    ...
    expect(
      await db.prisma.accessGrant.count({
        where: { accountId: s.buyer, revokedAt: null },
      }),
    ).toBe(1);
    expect(
      await db.prisma.billingPurchase.count({
        where: { accountId: s.buyer, state: "confirmed" },
      }),
    ).toBe(1);
  });
  ...
  test("руководство продаётся тем же контуром и не просит сохранить карту", async () => {
    ...
    expect(
      ...
      result: {
        outcome: "payment",
        value: { kind: "one_time", environment: "local" },
      },
    });
  ...
  test("продление списывает по сохранённой карте, а отказ банка закрывает подписку", async () => {
    ...
    expect(
      value(await s.subscriptions.read(s.buyer)).subscription,
    ).toMatchObject({ state: "active", paidUntil: "2030-03-31T10:00:00.000Z" });
    expect(
      s.delivered.filter((item) => item.status === "CONFIRMED"),
    ).toHaveLength(2);
    ...
    expect(
      await db.prisma.billingPurchase.count({
        where: { accountId: s.buyer, kind: "renewal", state: "failed" },
      }),
    ).toBe(1);
  });
  ...
  test("смена карты применяется только после подтверждения на форме привязки", async () => {
    ...
    expect(after?.paymentMethod?.methodRef).not.toBe(
      before?.paymentMethod?.methodRef,
    );
  });
  ...
  test("возврат проходит частью и целиком, повтор сверки не возвращает дважды", async () => {
    ...
    const partial = asRefundDecision(
      await s.operations.execute(owner, {
        ...
        basis: "compensation",
        recurring: "keep",
        reason: "Стенд: частичный возврат",
      }),
    );
    const partialExecuted = asRefundDecision(
      await s.operations.execute(owner, {
        operation: "refunds.execute",
        operationId: randomUUID(),
        decisionRef: partial.decisionRef,
        expectedRevision: 1,
      }),
    );
    expect(partialExecuted).toMatchObject({
      state: "executed",
      attempt: {
        state: "confirmed",
        amountKopecks: 40_000,
        observedStatus: "PARTIAL_REFUNDED",
        errorCode: "0",
      },
    });

    const rest = asRefundDecision(
      await s.operations.execute(owner, {
        operation: "refunds.decide",
        operationId: randomUUID(),
        purchaseRef,
        ...
        basis: "withdrawal",
        recurring: "cancel",
        reason: "Стенд: возврат остатка",
      }),
    );
    const restExecuted = asRefundDecision(
      await s.operations.execute(owner, {
        operation: "refunds.execute",
        operationId: randomUUID(),
        decisionRef: rest.decisionRef,
        expectedRevision: 1,
      }),
    );
    expect(restExecuted).toMatchObject({
      state: "executed",
      attempt: {
        state: "confirmed",
        amountKopecks: 60_000,
    ...
    expect(await s.operations.reconcileRefunds()).toMatchObject({
      inspected: 0,
      settled: 0,
    });
    value(await s.payments.recover());
