...
test("preview, transaction rollback, preserved legacy scope and late fulfillment exclude new products", async () => {
  ...
  try {
    const migrationIndex = platformMigrations.findIndex(
      (item) => item.name === "0063_subscription_enrollments",
    );
    ...
    await db.prisma.account.create({
      data: {
        id: owner,
        logtoIssuer: "https://migration.example.test",
        logtoSubject: owner,
      },
    });
    ...
    await pool.query(
      `INSERT INTO membership_entitlements.access_grants(id, account_id, source, source_ref, capabilities, starts_at, valid_until, revision, reason) VALUES ($1,$2,'manual','historical',ARRAY['materials','support','reviews'],'2030-01-01',NULL,1,'Historical promise')`,
      [grant, owner],
    );
    const before = (
      await pool.query(
        "SELECT * FROM membership_entitlements.access_grants WHERE id=$1",
        [grant],
      )
    ).rows;
    ...
    expect(
      (
        await pool.query(
          "SELECT id FROM materials.series WHERE archived_at IS NULL ORDER BY id",
        )
      ).rows,
    ).toEqual([{ id: included }]);
    ...
    expect(
      (
        await pool.query(
          "SELECT to_regclass('membership_entitlements.subscription_enrollments') AS table",
        )
      ).rows,
    ).toEqual([{ table: null }]);
    expect(
    ...
    expect(
      await db.prisma.accessGrant.findUnique({ where: { id: grant } }),
    ).toMatchObject({
      id: grant,
      capabilities: ["materials", "support", "reviews"],
    ...
    expect(
      await membership.resolveForAccess(accountId(owner), [excluded]),
