...
describe("one-time guide purchase (real PostgreSQL and real facets; synthetic bank and email only)", () => {
  ...
  async function scenario(
    ...
    const runtime = new BillingPayments({
      prisma: db.prisma,
      bank,
      contact,
      grants,
    ...
    const notify = (state: string, extra = {}) => {
      const body = event(state, extra);
    ...
    async function command(
      accepted: readonly Consent[] = ["terms"],
        ...
        pressedPaymentButton(
          {
            ...
            documents: documents
              .filter((document) =>
                accepted.some((kind) => kind === document.kind),
              )
              .map((document) => ({
                kind: document.kind,
    ...
    async function buy(
      ...
      const purchase = value(
        await runtime.purchase(
          buyer,
          await command(accepted, acknowledgeExistingAccess),
        ),
      );
      expect(await runtime.notification(notify("CONFIRMED"))).toMatchObject({
        ok: true,
      });
  ...
  test("оплаченное руководство открывается навсегда и не заводит подписку", async () => {
    ...
    expect(
      value(await s.runtime.status(s.buyer, purchase.purchaseRef)),
    ).toMatchObject({
      state: "confirmed",
      access: "ready",
      periodEndsAt: null,
    ...
    const granted = await db.prisma.accessGrant.findMany({
      where: { accountId: s.buyer, capabilities: { has: s.capability } },
    });
  ...
  test("новая разовая покупка Guide бессрочна даже при старом ограниченном варианте", async () => {
    ...
    expect(
      (
        await db.prisma.accessGrant.findFirst({
          where: {
            accountId: silent.buyer,
            capabilities: { has: silent.capability },
          },
        })
      )?.validUntil,
    ).toBeNull();
  ...
  test("сопровождение в предложении продукта живёт шесть месяцев с покупки и не бывает бессрочным", async () => {
    ...
    expect(later.capabilities.map((item) => item.capability)).toEqual([
      "community",
      s.capability,
    ]);
    ...
    await db.prisma.billingOffer.update({
      where: { id: s.offerId },
      data: { benefitPeriods: [{ capability: s.capability, months: null }] },
    });
    expect(
      code(
  ...
  test("разовая покупка не принимает согласие на списания и требует оферту", async () => {
    ...
    expect(
      code(
        await s.runtime.purchase(
          s.buyer,
          await s.command(["terms", "recurring"]),
        ),
      ),
    ).toBe("consent_required");
    expect(
      code(
        await s.runtime.purchase(s.buyer, await s.command(["personal_data"])),
      ),
    ).toBe("consent_required");
    expect(
    ...
    expect(
      value(await s.runtime.purchase(s.buyer, await s.command(["terms"]))),
    ).toMatchObject({ state: "pending" });
  });

  test("неизвестная старая классификация не мешает купить руководство", async () => {
    const s = await scenario();
    expect(await grants.readLegacyClassification(s.buyer)).toMatchObject({
      ok: true,
      classification: "unknown",
      recurringAllowed: false,
    });
    expect(
      value(await s.runtime.purchase(s.buyer, await s.command())),
    ).toMatchObject({ state: "pending" });
  });
  ...
  test("повторная покупка честно сообщает о существующем праве и не удваивает его", async () => {
    ...
    expect(code(await s.runtime.purchase(s.buyer, await s.command()))).toBe(
      "existing_access",
    );
    ...
    expect(
      resolved.capabilities.filter((item) => item.capability === s.capability),
    ).toEqual([{ capability: s.capability, validUntil: null }]);
    expect(
      await db.prisma.billingSubscription.count({
        where: { accountId: s.buyer },
      }),
  ...
  test("выключенное предложение не продаёт руководство и не отзывает уже купленное", async () => {
    ...
    expect(
      value(
        await pricing.offers({ mode: "one_time", capability: s.capability }),
      ).items,
    ).toEqual([]);
    expect(
      code(
  ...
  test("подписка не мешает купить руководство ни до, ни после неё", async () => {
    ...
    const subscriptionOffer = randomUUID(),
      subscriptionOption = randomUUID();
    value(
      await pricing.manage(owner, {
        operationId: randomUUID(),
        operation: "offers.save",
      ...
      pressedPaymentButton(
        {
          ...
          contextRef: quote.quoteRef,
          documents: documents
            .filter((item) => item.kind !== "personal_data")
            .map((item) => ({
              kind: item.kind,
    ...
    expect(
      value(
        await s.runtime.purchase(s.buyer, {
          operationId: randomUUID(),
          quoteRef: quote.quoteRef,
          contactRevision: 1,
          consentEvidenceRefs: consent.evidenceRefs,
    ...
    expect(await s.runtime.notification(s.notify("CONFIRMED"))).toMatchObject({
      ok: true,
    });
    const subscription = await db.prisma.billingSubscription.findFirstOrThrow({
      where: { accountId: s.buyer },
    });
    ...
    const first = value(
      await another.runtime.purchase(another.buyer, await another.command()),
    );
  ...
  test("форма со всеми способами продаёт руководство без подтверждений «только карта», но не подписку", async () => {
    ...
    const allMethods = syntheticTbankConfig({
      ...config,
