...
describe("local development seed for the owner's stand", () => {
  ...
  test(
    "keeps demo Materials for the editor, off the reader and unpublished across runs",
    async () => {
      await seedLocalDevelopment(database.prisma, { demo: "hidden" });
      const seeded = await database.prisma.material.findMany({
        select: { id: true, publicationState: true, contentVersion: true },
      });
      expect(seeded.length).toBeGreaterThan(0);
      expect(
        seeded.filter((material) => material.publicationState === "published"),
      ).toEqual([]);
      ...
      expect(
        await database.prisma.material.count({
          where: { firstPublishedAt: { not: null } },
        }),
      ).toBe(0);
      // A product without readable Materials is not sold on the owner's stand.
      expect(
        await database.prisma.billingOffer.count({
          where: { published: true },
        }),
      ).toBe(0);

      await seedLocalDevelopment(database.prisma, { demo: "hidden" });
      const repeated = await database.prisma.material.findMany({
        select: { id: true, publicationState: true, contentVersion: true },
      });
