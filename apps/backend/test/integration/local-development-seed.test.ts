 ...
 * за 12–15.09.2026 шёл не меньше чем в 1,7 раза медленнее замера: тело из двух засевов оборвалось
 * там на сроке Vitest по умолчанию, 5 с, против 2,97 с в замере. Насколько медленнее он был на
 * самом деле, не измерено: 1,7 — нижняя граница, и запасы ниже посчитаны от неё.
 */
/**
 * Создание базы с миграциями: 1,0 с в CI и не меньше 1,7 с на медленном раннере; запас почти
 * шестикратный.
 */
 ...
 * 4,4 с на медленном раннере, запас больше чем трёхкратный. Проверка получает этот бюджет на каждый
