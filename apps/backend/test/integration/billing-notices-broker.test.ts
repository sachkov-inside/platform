...
const config = syntheticTbankConfig({
  environment: "demo",
  terminalKey: "SYNTHETICBROKER",
  password: "synthetic-test-password",
  bindingEncryptionKey: Buffer.alloc(32, 63).toString("base64"),
...
test("подтверждённая оплата доходит до обоих каналов через реальные RabbitMQ и PostgreSQL", async () => {
  ...
  const started = await contact.start(buyer, {
    operationId: randomUUID(),
    email: "broker-proof@example.test",
    expectedRevision: 0,
  });
  ...
  expect(
    await contact.confirm(buyer, {
      ...
      code: codes.get(started.challengeRef),
    }),
  ).toMatchObject({ ok: true });
  ...
  value(
    await pricing.manage(owner, {
      ...
      value: { id: optionId, offerId, months: 1, priceKopecks: 350_000 },
    }),
  );
  value(
    await pricing.manage(owner, {
  ...
  expect(
    await payments.notification(bank.notify(purchase.purchaseRef, "CONFIRMED")),
  ).toMatchObject({ ok: true });
  ...
  try {
    ...
    await eventually(async () => {
      expect(
        await database.prisma.billingNotificationOutbox.count({
      ...
      expect(
        await queueDepth(
          admin,
          "inside-test",
          lanes.telegramSubscription.queue,
        ),
      ).toBe(1);
      expect(
    ...
    expect(
      await database.prisma.billingPurchase.findUniqueOrThrow({
        where: { id: purchase.purchaseRef },
      }),
    ).toMatchObject({ state: "confirmed" });
    expect(
      await database.prisma.billingNotice.count({
        where: {
          accountId: buyer,
