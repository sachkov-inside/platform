...
describe("Personal Home on PostgreSQL", () => {
  ...
  async function material(
    ...
    const metadata = {
      ...
      summary: "Continue test",
      topicId,
      formatId: withVideo ? videoFormatId : formatId,
      access,
      tagIds: [],
      difficulty: null,
      outcomes: [],
      seriesIds: memberships,
    };
    const body = representativeDocument("Personal Home text.");
    const draft = await materials.authoring.createDraft({
      actor,
    ...
    const saved = await materials.authoring.saveMaterial({
      actor,
      materialId: draft.value.materialId,
      expectedContentVersion: draft.value.contentVersion,
      idempotencyKey: randomUUID(),
      metadata,
      body,
  ...
  function open(
    accountId: string,
    item: { materialId: string; contentVersion: number },
  ) {
    return home.recordOpen({
      accountId,
  ...
  test("records only authorized current bodies; replay does not move recency or mark completion", async () => {
    ...
    const first = await home.recordOpen(command);
    expect(first).toMatchObject({ ok: true, value: { replayed: false } });
    await open(accountId, item);
    const visit = await database.prisma.readingMaterialVisit.findUniqueOrThrow({
      where: {
        accountId_materialId: { accountId, materialId: item.materialId },
      },
    });
    expect(await home.recordOpen(command)).toMatchObject({
      ok: true,
      value: { replayed: true },
    });
    expect(
      await database.prisma.readingMaterialVisit.findUniqueOrThrow({
        where: {
          accountId_materialId: { accountId, materialId: item.materialId },
        },
      }),
    ...
    ).toEqual({ ok: false, error: { code: "command_conflict" } });
    expect(
      await home.recordOpen({
        ...command,
        commandId: randomUUID(),
    ...
    ).toEqual({ ok: false, error: { code: "access_changed" } });
    expect(
      await database.prisma.readingEvent.count({ where: { accountId } }),
    ).toBe(0);
    expect(
      await database.prisma.readingMaterialState.count({
        where: { accountId },
      }),
    ).toBe(0);
    expect(await home.getContinue(accountId)).toMatchObject({
      ok: true,
      value: [
        {
    ...
    expect(await home.getContinue(randomUUID())).toEqual({
      ok: true,
      value: [],
    });
  });
  test("concurrent visible-open retries commit one visit and one receipt across connections", async () => {
    ...
    try {
      const other = new PersonalHome({
        composition: new PublishedSeriesComposition(database.prisma),
        reader: materials.publishedMaterialReader,
        prisma: second,
        materialContent: materials.materialContent,
        selection: new PublishedMaterialSelection(second),
        contentAccess,
        videos,
      });
      const command = {
      ...
      const results = await Promise.all([
        home.recordOpen(command),
        other.recordOpen(command),
      ]);
      ...
      expect(
        await database.prisma.readingMaterialVisit.count({
          where: { accountId },
        }),
      ).toBe(1);
      expect(
        await database.prisma.readingCommand.count({ where: { accountId } }),
      ).toBe(1);
    } finally {
  ...
  test("marking excludes opened content; unmarking restores only existing opens", async () => {
    ...
    expect(await home.getContinue(accountId)).toMatchObject({
      ok: true,
      value: [{ material: { materialId: opened.materialId } }],
    });
  });
  test("expiry hides protected cards without removing history and rejoin restores them", async () => {
    ...
    try {
      ...
      expect(await home.getContinue(accountId)).toMatchObject({
        ok: true,
        value: [{ material: { materialId: item.materialId } }],
      });
      membershipActive = false;
      expect(await home.getContinue(accountId)).toEqual({
        ok: true,
        value: [],
      });
      expect(
  ...
  test("video resume belongs to Videos and an unavailable adapter leaves text cards usable", async () => {
    const accountId = randomUUID();
    const item = await material("free", true);
    ...
    expect(
      await videos.loadProgressMany({ accountId, videoIds: [item.videoId] }),
    ).toMatchObject({
      ok: true,
      value: [{ videoId: item.videoId, positionSeconds: 123 }],
    });
    expect(
      await videos.loadProgressMany({
    ...
    expect(await home.getContinue(accountId)).toMatchObject({
      ok: true,
      value: [{ resume: { kind: "position", positionSeconds: 123 } }],
    });
    await videos.saveProgress({
      accountId,
      videoId: item.videoId,
    ...
    expect(await home.getContinue(accountId)).toMatchObject({
      ok: true,
      value: [{ resume: { kind: "reached-end" } }],
    });
    const unavailable = makeHome({
      ...videos,
      loadProgressMany: () =>
        Promise.resolve({
          ok: false,
    ...
    expect(await unavailable.getContinue(accountId)).toMatchObject({
      ok: true,
      value: [{ resume: { kind: "start" } }],
    });
    expect(
      await database.prisma.readingEvent.count({ where: { accountId } }),
    ).toBe(0);
  ...
  test("content updates preserve visits and replacement Video starts with its own progress", async () => {
    ...
    const replacement = await videos.attachExisting({
      actor,
      materialId: item.materialId,
      access: "free",
      providerVideoId: randomUUID(),
    ...
    const updated = await materials.authoring.saveMaterial({
      actor,
      materialId: item.materialId,
      expectedContentVersion: item.contentVersion,
      idempotencyKey: randomUUID(),
      metadata: { ...item.metadata, title: "Updated video material" },
      body: representativeDocument("Updated body"),
      primaryVideoId: replacement.value.videoId,
      publicationState: "published",
    });
    ...
    expect(await open(accountId, item)).toEqual({
      ok: false,
      error: { code: "access_changed" },
    });
    expect(await open(accountId, updated.value)).toMatchObject({ ok: true });
    expect(
      await videos.loadProgress({ accountId, videoId: item.videoId }),
    ).toEqual({ ok: true, value: { positionSeconds: 123 } });
  });
  test("bounds the projection to six, excludes unpublished and rolls back a failed receipt", async () => {
    const accountId = randomUUID();
    for (let index = 0; index < 8; index++)
      await open(accountId, await material());
    const result = await home.getContinue(accountId);
    expect(result.ok && result.value).toHaveLength(6);
    if (!result.ok || result.value[0] === undefined)
    ...
    expect(
      filtered.ok &&
        filtered.value.some(
          (entry) => entry.material.materialId === item.materialId,
        ),
    ).toBe(false);
    ...
    try {
      await pool.query(
        "CREATE FUNCTION reading_activity.reject_open_receipt() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'test receipt failure'; END $$; CREATE TRIGGER reject_open_receipt BEFORE INSERT ON reading_activity.commands FOR EACH ROW EXECUTE FUNCTION reading_activity.reject_open_receipt();",
      );
      expect(await open(accountId, rejected)).toEqual({
        ok: false,
        error: { code: "dependency_unavailable" },
      });
      expect(
        await database.prisma.readingMaterialVisit.count({
          where: { accountId, materialId: rejected.materialId },
        }),
      ).toBe(0);
    } finally {
      await pool.query(
        "DROP TRIGGER reject_open_receipt ON reading_activity.commands; DROP FUNCTION reading_activity.reject_open_receipt();",
      );
      await pool.end();
    }
  });
  ...
  test("learning Home keeps completed visits as series history, highlights author order and excludes all-read series", async () => {
    ...
    expect(await home.getSeries(accountId, collection.slug)).toMatchObject({
      ok: true,
      value: { total: 2, read: 0, continuation: null },
    });
    ...
    expect(current).toMatchObject({
      ...
      value: {
        video: null,
        series: { collection: { id: collection.id }, total: 2, read: 1 },
      },
    });
    if (!current.ok || current.value.series === null)
      throw new Error("Missing series");
    expect(current.value.series.continuation?.materialSlug).toBe(
      current.value.series.collection.previewItems.find(
        (item) => item.materialId === second.materialId,
      )?.slug,
    );
    expect(await home.getLearning(randomUUID())).toEqual({
      ok: true,
      value: { video: null, series: null },
    });
    await reading.setReadingState({
    ...
    expect(await home.getLearning(accountId)).toMatchObject({
      ok: true,
  ...
  test("learning video searches beyond six text visits and rejects ended, completed or replaced Video progress", async () => {
    const accountId = randomUUID();
    const video = await material("free", true);
    ...
    expect(await home.getLearning(accountId)).toMatchObject({
      ...
      value: {
        video: {
          material: { materialId: video.materialId },
          resume: { kind: "position", positionSeconds: 123 },
        },
        series: null,
      },
    });
    ...
    await materials.authoring.saveMaterial({
      actor,
      materialId: video.materialId,
      expectedContentVersion: video.contentVersion,
      idempotencyKey: randomUUID(),
  ...
  test("series continuation follows reorder and publication while counting all published manual marks", async () => {
    ...
    await materials.authoring.reorderSeries({
      ...
      orderedMaterialIds: [second.materialId, first.materialId],
    });
    const result = await home.getSeries(accountId, collection.slug);
    if (!result.ok) throw new Error(result.error.code);
    expect(result.value.collection.previewItems[0]?.materialId).toBe(
      second.materialId,
    );
    expect(result.value.continuation?.materialSlug).toBe(
      result.value.collection.previewItems[1]?.slug,
    );
    await materials.authoring.transitionPublication({
      actor,
      materialId: second.materialId,
      expectedContentVersion: second.contentVersion,
      idempotencyKey: randomUUID(),
      publicationState: "unpublished",
    });
    expect(await home.getSeries(accountId, collection.slug)).toMatchObject({
  ...
  test("series resumes the latest visit, advances after completion, skips locked items and wraps to unfinished entries", async () => {
    ...
    await database.prisma.readingMaterialVisit.update({
      where: {
        accountId_materialId: { accountId, materialId: first.materialId },
      },
      data: { firstOpenedAt: new Date(0), lastOpenedAt: new Date(0) },
    });
    const slug = async (item: { materialId: string }) => {
      const result = await materials.authoring.loadMaterial({
        actor,
        materialId: item.materialId,
      });
      if (!result.ok) throw new Error(result.error.code);
    ...
    expect(await home.getSeries(accountId, collection.slug)).toMatchObject({
      ...
      value: {
        ...
        total: 4,
        continuation: {
          materialSlug: await slug(second),
          resume: { kind: "position", positionSeconds: 123 },
        },
      },
    });
    ...
    expect(await home.getSeries(accountId, collection.slug)).toMatchObject({
      ...
      value: {
        read: 1,
        total: 4,
        continuation: { materialSlug: await slug(last) },
      },
    });
    ...
    expect(await home.getSeries(accountId, collection.slug)).toMatchObject({
      ...
      value: {
        read: 2,
        total: 4,
        continuation: { materialSlug: await slug(first) },
      },
    });
  });
  test("series history survives expiry and resumes after rejoining; Videos failure still allows series continuation", async () => {
    ...
    try {
      ...
      expect(await home.getLearning(accountId)).toMatchObject({
        ok: true,
        value: { series: { collection: { id: collection.id } } },
      });
      membershipActive = false;
      expect(await home.getLearning(accountId)).toEqual({
        ok: true,
      ...
      expect(await home.getSeries(accountId, collection.slug)).toMatchObject({
        ok: true,
        value: { total: 1, continuation: null },
      });
      expect(
        await database.prisma.readingMaterialVisit.count({
          where: { accountId },
      ...
      const unavailable = makeHome({
        ...videos,
        loadProgressMany: () => Promise.reject(new Error("offline")),
        loadReadyDurations: () => Promise.reject(new Error("offline")),
      });
      expect(await unavailable.getLearning(accountId)).toMatchObject({
        ok: true,
        value: { series: { continuation: { resume: { kind: "start" } } } },
      });
    } finally {
      membershipActive = false;
    }
