...
const config = syntheticTbankConfig({
  ...
  notificationUrl: `${origin}/billing/tbank/notification`,
...
describe("служебные сообщения подписки (реальные PostgreSQL и фасеты; синтетические банк и каналы)", () => {
  ...
  async function scenario(options: { readonly telegram?: boolean } = {}) {
          ...
          binding: async (account, channel) => {
            ...
            return result.binding?.telegramIdentityRef &&
              ...
              ? {
                  channel: "telegram",
                  ...result.binding,
                  accountRef: result.binding.accountRef,
                  telegramIdentityRef: result.binding.telegramIdentityRef,
                }
              : null;
          },
        },
    ...
    const deliverEmail: SendNotificationEmail = (message) => {
      sent.push(message);
    ...
    async function pendingEvents() {
      const events = await db.prisma.billingNoticeRevision.findMany({
        where: { notice: { accountId: buyer } },
        select: { messageId: true },
      });
      return db.prisma.notificationInbox.count({
        where: {
          messageId: { in: events.map((row) => row.messageId) },
          completedAt: null,
        },
      });
    }
    ...
    async function pump(send?: SendNotificationEmail) {
      for (let pass = 0; pass < 24; pass += 1) {
        let moved = false;
        while (
          await relay.relay("billing", (envelope) =>
            notifications.acceptEvent(envelope).then(() => undefined),
          )
        )
          moved = true;
        ...
        for (const lane of [
          ...
          while (
            await notifications.transport.outbox.relay(lane, (envelope) => {
              if (lane === "telegramSubscription") {
                telegramCommands.push(JSON.parse(envelope.payload));
                return Promise.resolve();
              }
    ...
    async function commandFor(kind: string) {
      const rows = await db.prisma.notificationCommand.findMany({
        where: { delivery: { notification: { accountId: buyer } } },
        orderBy: { revision: "asc" },
      });
      const found = rows
        .map((row) => ({
          row,
          command: deliverySchema.parse(JSON.parse(row.payload)),
        }))
        .find((entry) => entry.command.content.kind === kind);
    ...
    const noticesOf = (kind: string) =>
      db.prisma.billingNotice.findMany({
        where: { accountId: buyer, kind },
    ...
    const delivery = (channel: "email" | "telegram") =>
      db.prisma.notificationDelivery.findFirstOrThrow({
        where: { ...own, channel },
      });
    const staged = async () => {
      const revisions = await db.prisma.billingNoticeRevision.findMany({
        where: { notice: { accountId: buyer } },
        select: { messageId: true },
      });
      return db.prisma.billingNotificationOutbox.count({
        where: { messageId: { in: revisions.map((row) => row.messageId) } },
      });
    };
    return {
      buyer,
  ...
  test("подтверждённая оплата даёт один повод и одно письмо; повтор банка не создаёт второй", async () => {
    ...
    expect(await s.noticesOf("payment_succeeded")).toMatchObject([
      {
        state: "current",
        revision: 1,
    ...
    expect(
      await s.payments.notification(s.bank.notify(purchaseRef, "CONFIRMED")),
    ).toMatchObject({ ok: true });
    ...
    expect(s.sent[0]).toMatchObject({
      subject: "Оплата Inside подтверждена",
      email: `${s.buyer}@example.test`,
    });
    ...
    expect(s.sent).toHaveLength(1);
    expect(
      await db.prisma.notification.count({ where: { accountId: s.buyer } }),
    ).toBe(1);
    expect(await s.cabinet()).toMatchObject({
      notices: [
        { kind: "payment_succeeded", state: "current", amountKopecks: 100_000 },
      ],
    });
  });
  ...
  test("служебное сообщение идёт в оба канала и не требует согласия на новые материалы", async () => {
    ...
    expect(await s.notifications.readPreferences(s.buyer)).toEqual({
      revision: 0,
      email: false,
      telegram: false,
    });
    ...
    expect(s.telegramCommands[0]).toMatchObject({
      content: { category: "subscription", kind: "payment_succeeded" },
      binding: {
        channel: "telegram",
        telegramIdentityRef: `identity-${s.buyer}`,
      },
    });
    const deliveries = await s.notifications.readDeliveries(s.buyer);
    expect(deliveries.map((row) => row.channel).sort()).toEqual([
      "email",
      "telegram",
    ]);
    ...
    expect(deliveries.find((row) => row.channel === "email")).toMatchObject({
      state: "sent",
    });
    expect(deliveries.find((row) => row.channel === "telegram")).toMatchObject({
      state: "accepted",
    });
  });
  ...
  test("напоминание появляется за три дня с принятой датой и суммой и не повторяется", async () => {
    ...
    expect(value(await s.notices.scheduleReminders())).toMatchObject({
      created: 1,
      refreshed: 0,
      superseded: 0,
    });
    ...
    const reminder = s.sent.find(
      (message) => message.subject === "Скоро продление подписки Inside",
    );
  ...
  test("календарь доходит до каждой подписки, а не перебирает одни и те же", async () => {
    ...
    await s.buy();
    s.at("2030-02-26T10:00:00Z");
    const original = await db.prisma.billingSubscription.findFirstOrThrow({
    ...
    expect(
      await db.prisma.billingNotice.count({
        where: {
          ...
          state: "current",
          accountId: { in: [s.buyer, ...neighbours] },
        },
      }),
    ).toBe(3);
  });
  ...
  test("отмена до отправки закрывает напоминание и сообщает об отмене продления", async () => {
    ...
    expect(await s.noticesOf("renewal_reminder")).toMatchObject([
      { state: "superseded" },
    ]);
    expect(
      await s.notifications.authorizeDispatch("email", {
    ...
    await s.pump(s.deliverEmail);
    expect(s.sent.map((message) => message.subject)).toEqual([
      "Продление Inside отменено",
    ]);
    expect(
      await db.prisma.notificationDelivery.findUniqueOrThrow({
  ...
  test("изменение условий выпускает следующую revision того же повода без второго сообщения", async () => {
    ...
    value(await s.notices.scheduleReminders());
    await s.prepare();
    expect((await s.commandFor("renewal_reminder")).command.text).toContain(
      "Сумма: 1000.00 ₽.",
    );
    const first = await db.prisma.billingNoticeRevision.findFirstOrThrow({
      where: { notice: { accountId: s.buyer, kind: "renewal_reminder" } },
    ...
    expect(notice).toMatchObject({
      revision: 2,
      state: "current",
      amountKopecks: 900_000n,
      title: "Материалы на год",
    });
    // Прежняя revision больше не актуальна, но повод остаётся одним и тем же.
    expect(await s.notices.resolveNotice(first.payload)).toEqual({
      status: "superseded",
    });
    await s.pump(s.deliverEmail);
    const reminders = s.sent.filter(
      (message) => message.subject === "Скоро продление подписки Inside",
    );
    expect(reminders).toHaveLength(1);
    expect(reminders[0]?.text).toContain("Сумма: 9000.00 ₽.");
    expect(
      await db.prisma.notification.count({
        where: { occurrenceRef: notice.id },
      }),
    ).toBe(1);
    ...
    expect(await s.delivery("email")).toMatchObject({
      state: "sent",
      commandRevision: 2,
    });
    expect(await s.delivery("telegram")).toMatchObject({
      state: "no_channel",
      commandRevision: 0,
    });
    expect(
      await db.prisma.notificationCommand.count({
        where: { delivery: { notification: { occurrenceRef: notice.id } } },
      }),
    ).toBe(2);
  });
  ...
  test("конец оплаченного срока сообщается по своему моменту, чужой повод не отправляется", async () => {
    ...
    expect(await s.noticesOf("access_expired")).toMatchObject([
      { state: "current", occurredAt: new Date("2030-02-28T10:00:00Z") },
    ]);
    await s.pump(s.deliverEmail);
    expect(s.sent.map((message) => message.subject)).toContain(
      "Доступ Inside закончился",
    );
    ...
    expect(await s.notices.resolveNotice(foreign)).toEqual({
      status: "superseded",
    });
    ...
    const accepted = await db.prisma.notificationInbox.findFirstOrThrow({
      where: { messageId: foreign.messageId },
    });
