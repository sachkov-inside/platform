...
} from "../../src/modules/communications/facets/tracking-visits/tracking-visits.js";
...
const config = {
  endpoint: "https://telegram.test/integrations/platform/v1/communications",
  secret: "synthetic-secret",
  authorizationSecret: "synthetic-authorization",
...
test("safe target only, opaque tokens and invalid provider destinations never create events", async () => {
  expect(await visits().resolve({ token: "bad", traffic: "unknown" })).toEqual({
    kind: "invalid",
  });
  for (const value of [
...
test("provider outage retains durable backlog and live age; a new instance retries the same event", async () => {
  const first = visits();
  expect(await first.resolve({ token, traffic: "unknown" })).toEqual({
    kind: "resolved",
    safeUrl: target,
  });
  outage = true;
  ...
  expect(await first.backlog()).toEqual({
    kind: "ready",
    pending: 1,
...
test("concurrent pumps claim once; lost ACK retries idempotently, forwarded links count independent hits", async () => {
  ...
  expect(
    await database.prisma.communicationTrackingHit.count({
      where: { traffic: "known_automation", deliveredAt: { not: null } },
    }),
  ).toBe(1);
});
...
test("crash after claim releases through expiry without changing event identity", async () => {
  await visits().resolve({ token, traffic: "unknown" });
  await database.prisma.communicationTrackingHit.updateMany({
    data: { claimId: randomUUID(), availableAt: new Date(+now + 60_000) },
  });
  await visits().deliverPending();
...
test("a PostgreSQL write failure does not break resolved navigation and is observable", async () => {
  ...
  await database.prisma.$transaction(async (tx) => {
    ...
    const result = await new TrackingVisits(
      tx,
      provider,
      origin,
      reportFailure,
