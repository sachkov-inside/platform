...
test("real RabbitMQ event → audience → email inbox/effect → result outage/recovery; both categories and ACL", async () => {
  ...
  const event = (category: "subscription" | "material"): NotificationEvent => ({
    ...
    eventType:
      category === "subscription"
        ? "billing.notice-ready"
        : "material.published",
    occurredAt: instant.toISOString(),
    notAfter: new Date(
      instant.getTime() + (category === "material" ? 86_400_000 : 3_600_000),
    ).toISOString(),
    ...(category === "subscription"
      ? { accountRef: actor, kind: "payment_succeeded" }
      : {}),
  });
  const billing = event("subscription");
  ...
  await database.prisma.billingContact.create({
    data: {
      ...
      revision: 1,
      verifiedAt: before,
      emailCiphertext: protection.seal(actor, "broker-proof@example.test"),
    },
  });
  const contacts = new NotificationAccounts(database.prisma, protection);
  const source = (candidate: NotificationEvent): NotificationSource => ({
    status: "current",
    event: candidate,
    accountId: candidate.eventType === "material.published" ? null : actor,
    content:
      candidate.eventType === "material.published"
        ? { category: "material", kind: "material_published" }
        : { category: "subscription", kind: "payment_succeeded" },
    title: "Synthetic source for real transport proof",
    readerPath: "/materials/broker-proof",
  });
  ...
  const app = new Notifications(
    {
      ...
      sources: {
        resolve: (candidate) =>
          Promise.resolve(
            [billing.occurrenceRef, material.occurrenceRef].includes(
              candidate.occurrenceRef,
            )
              ? source(candidate)
              : { status: "unavailable" },
          ),
    ...
    assembleAccounts({
      prisma: database.prisma,
      emailFingerprintKey: "notification-broker-fixture-key-at-least-32",
    }),
  );
  // Consent predates the publication. The preference facet is independently covered against PostgreSQL.
  await database.prisma.notificationPreference.create({
    data: {
      accountId: actor,
      revision: 1,
      email: true,
  ...
  await database.prisma.notificationPreferenceRevision.create({
    data: {
      accountId: actor,
      revision: 1,
      email: true,
  ...
  const emailPermission = topology.permissions.find(
    (permission) => permission.user === "local-email",
  );
  ...
  try {
    await expect(
      publishNotification(invalid, encodeNotification("billing", billing)),
    ).rejects.toThrow();
    await database.prisma.$transaction(async (transaction) => {
      await stageBillingNotification(transaction, billing);
    });
    await database.prisma.$transaction(async (transaction) => {
      await stageMaterialsNotification(transaction, material);
    });
    await worker.start();
    ...
    await publishNotification(
      publisher,
      encodeNotification("billing", billing),
    );
    ...
    await eventually(async () => {
      expect(await queueDepth(admin, "inside-test", lanes.billing.queue)).toBe(
        0,
      );
    }, barrierBudgetMs);
    expect(sends).toBe(2);
