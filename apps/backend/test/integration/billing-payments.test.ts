...
describe("subscription payment recovery (real PostgreSQL and real facets; synthetic bank and email only)", () => {
  ...
  beforeAll(async () => {
    ...
    accounts = assembleAccounts({
      prisma: db.prisma,
      emailFingerprintKey: "synthetic-billing-fingerprint-key-000000",
    });
  ...
  async function scenario(
    benefits = ["materials"],
    benefitPeriods?: { capability: string; months: number | null }[],
  ) {
    ...
    const classification = await grants.classifyLegacy(owner, {
    ...
    value(
      await pricing.manage(owner, {
        ...
        value: { id: optionId, offerId, months: 1, priceKopecks: 200_000 },
      }),
    );
    value(
      await pricing.manage(owner, {
    ...
    const command = {
      ...
      contactRevision: 1,
      consentEvidenceRefs: consent.evidenceRefs,
    ...
    const event = (status: string, extra = {}) => ({
    ...
    const bank = new Tbank(config, async (url, init) => {
      ...
      const body = z
        .object({
          ...
          Token: z.string(),
          Receipt: z.object({ Email: z.string() }).optional(),
        })
        .loose()
      ...
      if (url.endsWith("/Init")) {
        ...
        const row = await db.prisma.billingPurchase.findUniqueOrThrow({
          where: { id: orderId },
        });
        expect(row.state).toBe("sent");
        expect(
          await db.prisma.billingPromoReservation.findUnique({
            where: { purchaseRef: orderId },
          }),
        ...
        if (failInit)
          throw new Error("Synthetic timeout after bank acceptance");
    ...
    const runtime = (projector = grants) =>
      new BillingPayments({
        prisma: db.prisma,
        bank,
        contact,
    ...
    const notify = (status: string, extra = {}) => {
      const body = event(status, extra);
  ...
  test("two tabs and concurrent CONFIRMED create one payment and one set of grants; return status is not payment evidence", async () => {
    const s = await scenario(
      ...
      [{ capability: "support", months: 2 }],
    );
    const runtime = s.runtime();
    const [first, second] = await Promise.all([
      runtime.purchase(s.buyer, s.command),
      runtime.purchase(s.buyer, { ...s.command, operationId: randomUUID() }),
    ]);
    ...
    expect(
      await runtime.notification(
        s.notify("AUTHORIZED", { RebillId: "synthetic-saved-method" }),
      ),
    ).toMatchObject({ ok: true });
    expect(
    ...
    expect(
      value(await runtime.status(s.buyer, purchase.purchaseRef)).state,
    ).toBe("authorized");
    expect(
      (
        await Promise.all(
          Array.from({ length: 6 }, () =>
            runtime.notification(s.notify("CONFIRMED")),
          ),
        )
    ...
    expect(
      await db.prisma.billingPaymentEvent.count({
        where: { purchaseRef: purchase.purchaseRef },
      }),
    ).toBe(1);
    expect(
      value(await runtime.status(s.buyer, purchase.purchaseRef)),
    ).toMatchObject({
      state: "confirmed",
      access: "preparing",
    ...
    expect(
      saved
        .find((grant) => grant.capabilities.includes("support"))
        ?.validUntil?.toISOString(),
    ).toBe("2030-03-31T10:00:00.000Z");
    expect(
      value(await runtime.status(s.buyer, purchase.purchaseRef)).access,
    ).toBe("ready");
    ...
    expect(
      (
        await db.prisma.billingPurchase.findUniqueOrThrow({
          where: { id: purchase.purchaseRef },
        })
      ).bindingCiphertext,
    ).not.toContain("synthetic-saved-method");
    expect(await runtime.status(owner, purchase.purchaseRef)).toMatchObject({
  ...
  test("unknown Init survives restart and cannot be retried; CheckOrder confirms once", async () => {
    ...
    expect(
      value(await s.runtime().purchase(s.buyer, s.command)).purchaseRef,
    ).toBe(purchase.purchaseRef);
    expect(s.requests()).toBe(1);
    s.outcome("CONFIRMED");
    expect(await s.runtime().reconcile(purchase.purchaseRef)).toMatchObject({
      ok: true,
    });
  ...
  test.each(["notification", "reconciliation"] as const)(
    "first verified %s starts a full period and late replays keep its original bounds",
    async (source) => {
      const s = await scenario();
      const runtime = s.runtime();
      const purchase = value(await runtime.purchase(s.buyer, s.command));
      ...
      if (source === "notification")
        ...
        s.outcome("CONFIRMED");
        expect(await runtime.reconcile(purchase.purchaseRef)).toMatchObject({
          ok: true,
        });
      }
      const bounds = {
        confirmedAt: "2030-02-02T14:15:00.000Z",
        periodEndsAt: "2030-03-02T14:15:00.000Z",
      };
      expect(
        value(await runtime.status(s.buyer, purchase.purchaseRef)),
      ...
      await Promise.all([
        runtime.notification(s.notify("CONFIRMED")),
        runtime.reconcile(purchase.purchaseRef),
        runtime.recover(),
      ]);
      expect(
        value(await runtime.status(s.buyer, purchase.purchaseRef)),
      ).toMatchObject({ ...bounds, access: "ready" });
  ...
  test("signature and terminal/order/payment/amount matching precede dedupe", async () => {
    ...
    for (const extra of [
      ...
      expect(
        await runtime.notification(s.notify("CONFIRMED", extra)),
    ...
    expect(await runtime.notification(s.notify("CONFIRMED"))).toMatchObject({
      ok: true,
    });
    expect(
      await runtime.notification(s.notify("CONFIRMED", { Amount: 1 })),
    ).toMatchObject({ ok: false });
  });
  ...
  test("fulfillment failure replays without bank I/O; saved conditions survive offer archive and PostgreSQL rejects mutation", async () => {
    ...
    expect(
      value(await failing.status(s.buyer, purchase.purchaseRef)).access,
    ).toBe("preparing");
    value(
      await pricing.manage(owner, {
        operationId: randomUUID(),
    ...
    expect(
      value(await runtime.status(s.buyer, purchase.purchaseRef)).snapshot,
    ).toEqual(s.quote.snapshot);
    await expect(
      db.prisma.billingPurchase.update({
        where: { id: purchase.purchaseRef },
        data: { snapshot: {} },
      }),
    ).rejects.toThrow();
    await expect(
      db.prisma.billingPaymentEvent.deleteMany({
        where: { purchaseRef: purchase.purchaseRef },
      }),
    ).rejects.toThrow();
    now = new Date(now.getTime() + 60_000);
    await s.runtime().recover();
    await s.runtime().recover();
    expect(
      await db.prisma.accessGrant.count({ where: { accountId: s.buyer } }),
    ).toBe(1);
  ...
  test("SIGKILL after durable Init admission and after confirmation recovers without another charge", async () => {
    const s = await scenario();
    const child = fork(
      new URL("./fixtures/billing-payment-crash.ts", import.meta.url),
      [],
      {
        execArgv: ["--import", "tsx"],
        ...
        env: {
          ...process.env,
          BILLING_CRASH_FIXTURE: JSON.stringify({
            databaseUrl: db.url,
            config,
            documents,
            buyer: s.buyer,
    ...
    try {
      ...
      purchaseRef = z
        .tuple([z.object({ purchaseRef: z.uuid() })])
        .rest(z.unknown())
        .parse(messages)[0].purchaseRef;
      child.kill("SIGKILL");
    ...
    const payment = {
      TerminalKey: config.terminalKey,
      OrderId: purchaseRef,
      PaymentId: "crash-bank-payment",
      Amount: 200_000,
      Status: "CONFIRMED",
      Success: true,
      ErrorCode: "0",
    };
    const confirming = fork(
      new URL("./fixtures/billing-payment-crash.ts", import.meta.url),
      [],
      {
        execArgv: ["--import", "tsx"],
        ...
        env: {
          ...
          BILLING_CRASH_FIXTURE: JSON.stringify({
            ...
            notification: {
              ...payment,
              Token: tbankToken(payment, config.password),
            },
          }),
        },
      },
    ...
    try {
      ...
      confirming.kill("SIGKILL");
      await once(confirming, "exit");
    } finally {
      if (confirming.exitCode === null && confirming.signalCode === null)
        confirming.kill("SIGKILL");
    }
    expect(value(await s.runtime().status(s.buyer, purchaseRef)).access).toBe(
      "preparing",
    );
    await s.runtime().recover();
    await s.runtime().recover();
  ...
  test("HTTP callback acknowledges only durable valid notifications as plain text", async () => {
    const s = await scenario();
    const payments = s.runtime();
    await payments.purchase(s.buyer, s.command);
    @Module({
      controllers: [AcceptTbankNotificationController],
      providers: [{ provide: BillingPayments, useValue: payments }],
    })
    ...
    try {
      const success = await http.inject({
        method: "POST",
        url: "/billing/tbank/notification",
        payload: s.notify("CONFIRMED"),
      });
      expect(success.statusCode).toBe(200);
      expect(success.body).toBe("OK");
      ...
      const invalid = await http.inject({
        method: "POST",
        url: "/billing/tbank/notification",
        payload: { ...s.notify("CONFIRMED"), Token: "bad" },
      });
      expect(invalid.statusCode).toBe(400);
      expect(invalid.headers["content-type"]).toContain(
        "application/problem+json",
      );
      expect(invalid.json<unknown>()).toMatchObject({
  ...
  test("a permanently rejected fulfillment does not starve later purchases", async () => {
    ...
    const firstPurchase = value(
      await first.runtime().purchase(first.buyer, first.command),
    );
    const secondPurchase = value(
      await second.runtime().purchase(second.buyer, second.command),
    );
    await first.runtime().notification(first.notify("CONFIRMED"));
    await second.runtime().notification(second.notify("CONFIRMED"));
    await db.prisma.billingFulfillment.updateMany({
      where: { purchaseRef: firstPurchase.purchaseRef },
      data: { nextAttemptAt: new Date("2029-01-01T00:00:00Z") },
    });
    const rejecting = first.runtime({
      ...grants,
      applyPaidPeriod: (input) =>
        input.accountId === first.buyer
          ? Promise.resolve({ ok: false, error: { code: "not_found" } })
          : grants.applyPaidPeriod(input),
    });
    await rejecting.recover(1);
    expect(
      value(
        await first.runtime().status(first.buyer, firstPurchase.purchaseRef),
      ).access,
    ).toBe("preparing");
    ...
    expect(
      value(
        await second.runtime().status(second.buyer, secondPurchase.purchaseRef),
      ).access,
    ).toBe("ready");
  });
  ...
  test("receipt result is separate from confirmation and cannot create access", async () => {
    ...
    expect(
      await runtime.notification(
        s.notify("RECEIPT", { Success: false, ErrorCode: "receipt-error" }),
      ),
    ).toMatchObject({ ok: true });
    expect(
      value(await runtime.status(s.buyer, purchase.purchaseRef)),
    ).toMatchObject({ fiscalization: "failed", access: "awaiting_payment" });
    await runtime.notification(s.notify("RECEIPT"));
    expect(
      value(await runtime.status(s.buyer, purchase.purchaseRef)),
    ).toMatchObject({ fiscalization: "confirmed", access: "awaiting_payment" });
    await runtime.notification(s.notify("CONFIRMED"));
    await runtime.notification(
      s.notify("AUTHORIZED", { RebillId: "late-saved-method" }),
    );
    expect(
      (
        await db.prisma.billingPurchase.findUniqueOrThrow({
