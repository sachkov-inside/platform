...
const boundary = async () => {
  process.send?.(crashWorkerSignals.boundary);
  await new Promise(() => undefined);
};
if (config.phase.includes("confirm")) {
  await assembleNotificationOutbox(prisma.billingNotificationOutbox, [
    "billing",
  ]).relay("billing", async (envelope) => {
