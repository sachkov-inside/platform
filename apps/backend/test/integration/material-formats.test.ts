...
test("a clean production database offers and saves the three domain formats without a development seed", async () => {
  ...
  expect(references).toMatchObject({
    ...
    value: {
      formats: [
        { name: "Видео", archived: false },
        { name: "Гайд", archived: false },
        { name: "Заметка", archived: false },
      ],
    },
  });
  ...
  for (const format of references.value.formats) {
    const created = await authoring.createDraft({
      actor,
      idempotencyKey: `domain-format-${format.id}`,
      metadata: {
        title: format.name,
        summary: null,
        access: "free",
    ...
    expect(
      await authoring.saveMaterial({
        ...
        idempotencyKey: `save-format-${format.id}`,
    ...
    expect(
      ...
      ok: true,
      value: { metadata: { formatId: format.id, title: format.name } },
    });
  }
});

test.each(["podcast", "38900000-0000-4000-8000-000000000001"])(
  ...
  async (formatId) => {
    ...
    expect(
      await authoring.createDraft({
        actor,
        idempotencyKey: `unsupported-${formatId}`,
        metadata: {
          title: "Unsupported",
          summary: null,
          access: "free",
...
test.each(["video", "guide", "note", "text"])(
  "migrates legacy %s references without changing Material content",
  async (legacySlug) => {
    const legacy = await createTestDatabase();
    try {
      const migrationIndex = platformMigrations.findIndex(
        ({ name }) => name === "0035_domain_material_formats",
      );
      ...
      await legacy.prisma.$executeRaw(
        Prisma.sql`insert into materials.formats (id, slug, name) values (${formatId}::uuid, ${legacySlug}, 'Legacy format')`,
      );
      await insertLegacyMaterial(legacy, materialId, formatId);
      await migrateToLatest(legacy.url);
      expect(
        await legacy.prisma.material.findUnique({ where: { id: materialId } }),
      ).toMatchObject({
      ...
      expect(
        await legacy.prisma.$queryRaw(
          Prisma.sql`select to_regclass('materials.formats')::text as relation`,
        ),
...
test("refuses unknown referenced legacy formats and preserves the database for explicit conversion", async () => {
  ...
  try {
    ...
    await legacy.prisma.$executeRaw(
      Prisma.sql`insert into materials.formats (id, slug, name) values (${formatId}::uuid, 'podcast', 'Podcast')`,
    );
    await insertLegacyMaterial(legacy, materialId, formatId);
    await expect(migrateToLatest(legacy.url)).rejects.toThrow(
      "Unsupported legacy material format",
    );
    // Схема осталась дореформенной, поэтому строка читается её же колонками.
    expect(
      await legacy.prisma.$queryRaw(
        Prisma.sql`select format_id::text as "formatId" from materials.materials where id = ${materialId}::uuid`,
      ),
    ).toEqual([{ formatId }]);
    expect(
      await legacy.prisma.$queryRaw(
        Prisma.sql`select slug from materials.formats`,
      ),
    ).toEqual([{ slug: "podcast" }]);
  } finally {
    await legacy.dispose();
  }
});
