...
import {
  quotePurchaseSchema,
  priceQuoteSchema,
} from "../src/modules/billing/features/quote-purchase/quote-purchase.js";
...
import {
  ...
  discountedPrice,
  moneySchema,
} from "../src/modules/billing/domain/pricing.js";
...
describe("Billing pricing rules", () => {
  test("OpenAPI preserves UUID and benefit types through normalization", () => {
    expect(toOpenApiSchema(quotePurchaseSchema)).toMatchObject({
      properties: {
        operationId: { type: "string", format: "uuid" },
        paymentOptionId: { type: "string", format: "uuid" },
      },
    });
    expect(toOpenApiSchema(priceQuoteSchema)).toMatchObject({
      properties: {
        quoteRef: { type: "string", format: "uuid" },
        snapshot: {
          properties: {
            offer: {
              properties: {
                benefits: {
                  ...
                  items: {
                    anyOf: [
                      {
                        type: "string",
                        enum: ["materials", "community", "reviews", "support"],
                      },
                      { type: "string" },
                    ],
                  },
  ...
  test("rounds half up exactly even at the safe-integer ceiling", () => {
    ...
    expect(discountedPrice(Number.MAX_SAFE_INTEGER, 1)).toBe(
      8_917_127_262_193_581,
    );
    expect(discountedPrice(Number.MAX_SAFE_INTEGER, 0)).toBe(
  ...
  test("promotion uses a half-open instant interval, both scopes and exact trimmed code", () => {
    ...
    const promotion = {
      ...
      percent: 10,
      code: "CODE",
      startsAt: "2026-09-08T15:00:00+03:00",
      endsAt: "2026-09-08T16:00:00+03:00",
      offerIds: [option.offerId],
      paymentOptionIds: [option.id],
      usageLimit: null,
      archived: false,
    };
    expect(
    ...
    expect(
      manageCatalogSchema.safeParse({
        ...
        value: {
          id: randomUUID(),
          name: "Inside",
          benefits: ["materials", "materials"],
        },
      }).success,
    ).toBe(false);
  });
  test("maps price changes to visible conflict and unsupported amount to validation failure", () => {
    expect(() => throwPricingError({ code: "quote_changed" })).toThrow(
      expect.objectContaining({ status: 409 }),
    );
    expect(() => throwPricingError({ code: "unsupported_amount" })).toThrow(
