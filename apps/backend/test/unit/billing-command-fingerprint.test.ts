...
describe("billing command fingerprint", () => {
  test("ignores key order and absent optional fields, but binds every value", () => {
    ...
    expect(commandFingerprint("refunds.decide", command)).toBe(
      commandFingerprint("refunds.decide", {
        basis: "compensation",
        amountKopecks: 100_000,
    ...
    expect(
      commandFingerprint("refunds.decide", { ...command, reason: undefined }),
    ...
    expect(
      commandFingerprint("refunds.decide", { ...command, basis: "withdrawal" }),
    ).not.toBe(commandFingerprint("refunds.decide", command));
    // Одна нагрузка в разных операциях остаётся разными отпечатками.
    expect(commandFingerprint("refunds.execute", command)).not.toBe(
      commandFingerprint("refunds.decide", command),
    );
  });
  test("keeps array order significant", () => {
    expect(
      commandFingerprint("grants.applyBatch", { confirmedRows: ["a", "b"] }),
    ).not.toBe(
      commandFingerprint("grants.applyBatch", { confirmedRows: ["b", "a"] }),
    );
  });
});
