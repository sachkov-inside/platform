...
describe("billing command fingerprint", () => {
  test("ignores key order and absent optional fields, but binds every value", () => {
    const command = { operationId: "b7c1f0e2-0000-4000-8000-000000000001", amountKopecks: 100_000, basis: "compensation" };
    expect(commandFingerprint("refunds.decide", command))
      .toBe(commandFingerprint("refunds.decide", { basis: "compensation", amountKopecks: 100_000, operationId: command.operationId }));
    ...
    expect(commandFingerprint("refunds.decide", { ...command, basis: "withdrawal" }))
      .not.toBe(commandFingerprint("refunds.decide", command));
