...
const url = (
  user: string,
  options: { vhost?: string; protocol?: string; secret?: string } = {},
) =>
...
const environment = (
  ...
  NOTIFICATIONS_BROKER_URLS: JSON.stringify({
    billing: url("platform-billing", {
      secret: "with%2Fslash%40and-at-000000",
    }),
    materials: url("platform-materials"),
    notifications: url("platform-notifications"),
    email: url("platform-email"),
    ...overrides,
  }),
...
function hashMatches(passwordHash: string, candidate: string): boolean {
  ...
  return (
    raw.length === 36 &&
    createHash("sha256")
      .update(raw.subarray(0, 4))
      .update(candidate)
      .digest()
      .equals(raw.subarray(4))
  );
}
...
describe("production notification broker definitions", () => {
  it("imports one environment vhost with five scoped principals and no plaintext credential", () => {
    ...
    const users = new Map(
      definitions.users.map((user) => [user.name, user.password_hash]),
    );
    ...
    expect(
      hashMatches(
        users.get("platform-billing") ?? "",
        "with/slash@and-at-000000",
      ),
    ).toBe(true);
    for (const user of [
      "platform-materials",
    ...
    expect(JSON.stringify(definitions)).not.toMatch(
      /synthetic-broker-password|with%2Fslash|and-at-000000/u,
    );
    expect(
      definitions.permissions.find(
        (permission) => permission.user === "telegram",
      ),
    ).toEqual({
      user: "telegram",
      vhost: "inside-production",
      configure: "^$",
    ...
    expect(
      definitions.queues.every(
        (queue) =>
          queue.vhost === "inside-production" &&
          queue.arguments["x-queue-type"] === "quorum",
      ),
    ).toBe(true);
    // Соль новая при каждом выпуске определений: одинаковые пароли не дают одинаковых хэшей.
    expect(
      notificationBrokerDefinitions(environment()).users[0]?.password_hash,
    ).not.toBe(definitions.users[0]?.password_hash);
  });
  ...
  it("refuses a shared principal, another vhost, plaintext AMQP or a missing Telegram principal without echoing credentials", () => {
    ...
    for (const refused of refusals) {
      let message = "";
      try {
        notificationBrokerDefinitions(refused);
      } catch (error) {
