...
import { z } from "zod";
const activationCodecs: Record<string, z.ZodType> = {
  begin: beginActivationSchema,
  evidence: activationEvidenceSchema,
  ownAccessQuery: ownSubscriptionAccessQuerySchema,
  activationResponse: activationResponseSchema,
...
const communityCodecs: Record<string, z.ZodType> = {
  ...
  communityResponse: z.union([communityResultSchema, communityErrorSchema]),
  authorizationRequest: dispatchAuthorizeSchema,
  authorizationResponse: z.union([dispatchResultSchema, dispatchErrorSchema]),
};
  ...
  for (const bundle of [
    {
      ...
      codecs: activationCodecs,
    },
    {
      schema: communitySchema,
      fixtures: communityFixtures,
      codecs: communityCodecs,
    },
  ]) {
    ...
    for (const fixture of bundle.fixtures)
      test(fixture.name, () => {
        ...
        expect(
          bundle.codecs[fixture.definition]?.safeParse(fixture.value).success,
        ).toBe(fixture.valid);
      });
  }
});
