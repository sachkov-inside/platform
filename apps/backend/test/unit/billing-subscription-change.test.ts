...
describe("календарный anchor продления", () => {
  test("Jan 31 переходит в конец февраля и снова в 31 марта от исходного anchor", () => {
    ...
    expect(subscriptionPeriodEnd(anchor, 3).toISOString()).toBe(
  ...
  test("продление от прежнего конца потеряло бы исходное число", () => {
    ...
    expect(shifted.toISOString()).not.toBe(
      subscriptionPeriodEnd(anchor, 2).toISOString(),
    );
  });
});
...
describe("доплата за повышение варианта", () => {
  ...
  test("оплаченные 1 000 ₽ и переход на 3 500 ₽ в середине срока дают 1 250 ₽", () => {
    expect(
      upgradeTopUpKopecks({
        targetPriceKopecks: 350_000,
        paidPeriodKopecks: 100_000,
        periodStartsAt,
        paidUntil,
        now: middle,
      }),
  ...
  test("половина копейки округляется вверх один раз на итоговой доплате", () => {
    expect(
      upgradeTopUpKopecks({
        targetPriceKopecks: 103,
        paidPeriodKopecks: 100,
        periodStartsAt,
        paidUntil,
        now: middle,
      }),
...
describe("выбор между повышением и следующим периодом", () => {
  const period = {
    periodStartsAt: new Date("2030-01-01T00:00:00Z"),
    paidUntil: new Date("2030-01-31T00:00:00Z"),
    now: new Date("2030-01-16T00:00:00Z"),
  };
