...
const config = syntheticTbankConfig({
  ...
  minimumKopecks: 100,
  maximumKopecks: 1000000,
  returnUrl: "https://example.test/account",
  notificationUrl: "https://example.test/billing/tbank/notification",
...
describe("concrete bank boundary", () => {
  test("signature excludes nested receipt/DATA and Token, but binds all scalar fields", () => {
    ...
    expect(token).not.toBe(
      tbankToken({ ...payload, Success: false }, "password"),
    );
  });
  test("hosted redirects reject hostile hosts, credentials, fragments and non-HTTPS", () => {
    ...
    expect(validatedPaymentUrl("https://pay.tbank.ru/new/test", origins)).toBe(
      "https://pay.tbank.ru/new/test",
    );
    expect(
      validatedPaymentUrl("https://securepay.tinkoff.ru/order/test", origins),
    ).toBe("https://securepay.tinkoff.ru/order/test");
    for (const url of [
      "https://securepay.tinkoff.ru.evil.test/a",
      "https://evil.test/",
      "http://securepay.tinkoff.ru/",
      "https://user@securepay.tinkoff.ru/",
      "https://securepay.tinkoff.ru/#fake",
      "https://securepay.tinkoff.ru:8443/",
    ])
    ...
    expect(() =>
      validatedPaymentUrl("https://securepay.tinkoff.ru/order/test", [
        "http://127.0.0.1:8090",
      ]),
    ).toThrow();
  });
  test("binding ciphertext is scoped to its order, environment, terminal and key", () => {
    ...
    expect(() =>
      new Tbank({ ...config, environment: "production" }).openBinding(
        "order-a",
        ciphertext,
      ),
    ).toThrow();
    expect(() =>
      new Tbank({ ...config, terminalKey: "another" }).openBinding(
        "order-a",
        ciphertext,
      ),
    ).toThrow();
    expect(() =>
      new Tbank({
        ...config,
        bindingEncryptionKey: Buffer.alloc(32, 1).toString("base64"),
      }).openBinding("order-a", ciphertext),
    ).toThrow();
  });
  test("pinned bank root matches the documented fingerprint and a missing file fails loudly", () => {
    ...
    const root = fileURLToPath(
      new URL(
        "../../../../infra/tls/russian-trusted-root-ca.pem",
        import.meta.url,
      ),
    );
    // Отпечаток записан в docs/runbooks/runtime-configuration.md; подмена корня ломает этот тест.
    expect(new X509Certificate(readFileSync(root)).fingerprint256).toBe(
      bankRootFingerprint,
    );
  ...
  test("months clamp the Moscow local date while preserving the original anchor", () => {
    ...
    expect(subscriptionPeriodEnd(anchor, 1).toISOString()).toBe(
      "2032-02-28T22:15:12.345Z",
    );
    expect(subscriptionPeriodEnd(anchor, 2).toISOString()).toBe(
      "2032-03-30T22:15:12.345Z",
    );
