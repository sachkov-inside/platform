...
describe("vendored communications contract", () => {
  for (const fixture of fixtures) {
    test(fixture.name, () => {
      ...
      const valid = ajv.compile({
        $ref: `${schema.$id}#/definitions/${fixture.definition}`,
      });
  ...
  test("success contracts exclude provider and application failures", () => {
    expect(
      communicationsSuccessSchema.safeParse({
        ok: true,
        value: {
  ...
  test("public management refuses actors, service operations and destination overrides", () => {
    ...
    expect(
      managementRequestSchema.safeParse({
        ...request,
        actor: { accountRef: "forged" },
      }).success,
    ).toBe(false);
    expect(
      managementRequestSchema.safeParse({
        ...request,
        payload: { ...request.payload, chatId: "foreign" },
      }).success,
    ).toBe(false);
    expect(
      managementRequestSchema.safeParse({
  ...
  test("management timing survives parsing and remains closed to invalid fields", () => {
    const fixture = fixtures.find(
      (f) => f.name === "broadcasts.save-elapsed-timing",
    );
    ...
    expect(
      managementRequestSchema.safeParse({
        ...input,
        payload: { ...parsed.payload, sendAfterSeconds: 1 },
      }).success,
    ).toBe(false);
  });
});
