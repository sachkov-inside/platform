...
describe("таблица сценариев доступа", () => {
  test("описывает каждую клетку «что открывается × основание», каждый переход и сценарий публикации", () => {
    ...
    expect(Object.keys(accessScenarioTable.transitions)).toHaveLength(
      accessTransitions.length,
    );
  ...
  test("расхождение ожидания с наблюдаемым доступом называется по имени клетки", () => {
    expect(
      compareAccessObservation(
        divergentExpectation.id,
        divergentExpectation.expectation,
        divergentExpectation.observation,
      ),
    ).toBe(
      "product-material/withdrawal-refund expected open (lifetime) but observed locked",
    );
    expect(
      compareAccessObservation(
        "support/one-time-purchase",
        { outcome: "open", term: "six-months" },
        { outcome: "open", term: "lifetime" },
      ),
    ).toBe(
      "support/one-time-purchase expected open (six-months) but observed open (lifetime)",
    );
    expect(
      compareAccessObservation(
        "community-chat/expired-or-revoked",
        { outcome: "entry-closed" },
        { outcome: "closed" },
      ),
    ).toBe(
      "community-chat/expired-or-revoked expected entry-closed but observed closed",
    );
    expect(
      compareAccessObservation(
        "cabinet/withdrawal-refund",
        { outcome: "shown", shows: "purchase-history" },
        { outcome: "shown", shows: "nothing" },
      ),
    ).toBe(
      "cabinet/withdrawal-refund expected shown (purchase-history) but observed shown (nothing)",
    );
    expect(
      compareAccessObservation(
        "mcp/guest",
        { outcome: "not-applicable", because: "Нет Account" },
        { outcome: "closed" },
      ),
    ).toBe(
      "mcp/guest is not applicable (Нет Account) but was observed as closed",
    );
    expect(
      compareAccessObservation(
        "support/tier-via-course",
        { outcome: "by-tier" },
        { outcome: "by-tier" },
      ),
    ).toBeNull();
