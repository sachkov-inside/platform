...
const request = requestSchema.parse({
  ...
  expectedRevision: 0,
  actor: { accountRef: "synthetic-owner" },
...
const success = {
  ...
  template: {
    ...
    revision: 1,
    botIdentity: "synthetic-bot",
    content: { type: "text", text: "Synthetic", entities: [], buttons: [] },
  },
};
...
test.each([
  ...
  [
    501,
    { contractVersion: "inside-communications-v1", status: "not_implemented" },
    "not_implemented",
  ],
  [
    503,
  ...
  [
    ...
    {
      ...success,
      template: { ...success.template, botIdentity: "foreign-bot" },
    },
    ...
    200,
    {
      ...
      template: {
        ...
        content: {
          ...
          text: "https://api.telegram.org/file/botSECRET/document",
        },
      },
    },
    "provider_invalid_response",
  ...
  [
    200,
    {
      contractVersion: "inside-communications-v1",
      status: "ok",
      testDeliveryId: "33333333-3333-4333-8333-333333333333",
    },
    "provider_invalid_response",
  ],
] as const)(
  "maps status %s without fictitious success",
  async (status, body, code) => {
    const provider = new HttpCommunicationsProvider(config, () =>
...
test("an uncertain transport is not retried automatically", async () => {
  ...
  await expect(
    new HttpCommunicationsProvider(config, fetcher).execute(request),
  ).resolves.toEqual({ ok: false, error: { code: "provider_unavailable" } });
  expect(fetcher).toHaveBeenCalledTimes(1);
  await expect(
    new HttpCommunicationsProvider(undefined, fetcher).execute(request),
  ).resolves.toEqual({ ok: false, error: { code: "provider_unavailable" } });
  expect(fetcher).toHaveBeenCalledTimes(1);
});
...
test("configuration is opt-in, complete, credential-free in URLs and fails closed", () => {
  expect(
    parsePlatformConfig({ NODE_ENV: "test" }).communications,
  ).toBeUndefined();
  expect(() =>
    parsePlatformConfig({
      NODE_ENV: "test",
      TELEGRAM_COMMUNICATIONS_ENDPOINT: "https://example.test",
    }),
  ).toThrow();
});
...
test("ordinary mentions of the Telegram API domain remain author content", async () => {
  const value = {
    ...
    template: {
      ...
      content: {
        ...success.template.content,
        text: "Запрос отправляется на api.telegram.org",
      },
    },
  };
  const provider = new HttpCommunicationsProvider(config, () =>
...
test("canonical UUID response spelling does not change the caller's retry envelope", async () => {
  ...
  const input = requestSchema.parse({
    ...request,
    payload: { templateId: id.toUpperCase() },
  });
  const value = {
    ...success,
    template: { ...success.template, templateId: id },
  };
  const fetcher = vi
    .fn<typeof fetch>()
    .mockImplementation(() => Promise.resolve(Response.json(value)));
  await expect(
    new HttpCommunicationsProvider(config, fetcher).execute(input),
...
test("saved post lists enforce bot identity for every returned snapshot", async () => {
  ...
  for (const botIdentity of ["synthetic-bot", "foreign-bot"]) {
    const value = {
      contractVersion: "inside-communications-v1",
      status: "ok",
      templates: [{ ...success.template, botIdentity }],
      nextCursor: null,
    };
    const provider = new HttpCommunicationsProvider(config, () =>
      Promise.resolve(Response.json(value)),
    );
    expect((await provider.execute(input)).ok).toBe(
      botIdentity === "synthetic-bot",
    );
  }
});
