...
const directory = fileURLToPath(
  new URL(
    "../../../docs/contracts/subscription-activation-v1/",
    import.meta.url,
  ),
);
...
const schema = {
  ...
  $id: "https://sachkov-inside.github.io/platform/contracts/subscription-activation-v1.schema.json",
  definitions,
};
const output = JSON.stringify(schema, null, 2) + "\n";
await mkdir(directory, { recursive: true });
if (process.argv.includes("--check")) {
  if ((await readFile(directory + "schema.json", "utf8")) !== output)
    throw new Error(
      "Subscription activation schema drift; run contracts:generate",
    );
