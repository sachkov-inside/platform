...
const issuer = "https://enrollment.smoke.test/oidc",
  audience = "https://enrollment.smoke.test/api";
const keyPair = await generateKeyPair("ES384");
const provider = Fastify();
provider.get("/jwks", async () => ({
  keys: [
    { ...(await exportJWK(keyPair.publicKey)), alg: "ES384", kid: "smoke-key" },
  ],
}));
...
await prisma.telegramAccountLinkState.create({
  data: {
    accountId: owner.accountId,
    linkRef: "62000000-0000-4000-8000-000000000702",
    revision: 1,
    principalRef: "synthetic-owner-account",
    identityRef: "synthetic-owner-624",
    updatedAt: new Date(),
  },
});
...
await writeFile(
  ...
  JSON.stringify({
    ...
    token,
    providerUrl,
    ENROLLMENT_OWNER_ID: owner.accountId,
  }),
);
let stopping = false;
async function stop() {
