...
if (checkOnly) {
  ...
  if (committed === undefined) {
    throw new Error(
      `MCP tool surface is missing at ${surfacePath}. Run \`pnpm mcp:generate\`.`,
    );
  }
  if (committed !== generated) {
    throw new Error(
      drift(surfacePath, parseToolSurface(committed, surfacePath), registered),
    );
  }
  process.stdout.write(
    `MCP tool surface is up to date: ${String(registered.length)} tools.\n`,
  );
} else {
  await mkdir(path.dirname(surfacePath), { recursive: true });
  await writeFile(surfacePath, generated);
  process.stdout.write(
    `Wrote ${surfacePath}: ${String(registered.length)} tools\n`,
  );
}
