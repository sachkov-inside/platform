...
let savedPost = {
  ...
  content: {
    type: "text" as const,
    text: "Тестовая рассылка browser parity",
    entities: [{ type: "bold" as const, offset: 0, length: 8 }],
    buttons: [{ text: "Открыть", url: "https://inside.test/material", row: 0 }],
  },
};
...
provider.post("/integrations/platform/v1/communications", (request, reply) => {
  ...
  if (command.operation === "templates.list")
    return { ...version, templates: [savedPost], nextCursor: null };
  if (command.operation === "templates.read")
    return { ...version, template: savedPost };
  if (command.operation === "templates.save") {
    if (command.expectedRevision !== savedPost.revision)
      return reply.code(409).send({ ...version, status: "revision_conflict" });
    const candidate = {
      ...savedPost,
      content: command.payload.content,
      revision: savedPost.revision + 1,
    };
    ...
    savedPost = {
      ...
      content: {
        ...
        entities: savedPost.content.entities,
        buttons: candidate.content.buttons.map((b) => ({
          ...b,
          row: b.row ?? 0,
        })),
      },
