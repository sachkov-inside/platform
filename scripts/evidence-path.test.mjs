...
test("an explicit update writes only into the folder of its own issue", () => {
  ...
  assert.match(
    evidenceDirectory("issue-529", environment),
    /docs\/evidence\/issue-529$/u,
  );
  // Прогон снимает и соседние сценарии: их свидетельства по-прежнему уходят в артефакты.
  assert.match(
    evidenceDirectory("issue-122", environment),
    /ci-artifacts\/evidence\/issue-122$/u,
  );
  assert.match(
    evidenceDirectory("issue-5290", environment),
    /ci-artifacts\/evidence\/issue-5290$/u,
  );
});
