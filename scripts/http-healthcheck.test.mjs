...
async function probe(service, url, environment = {}) {
  const child = spawn(
    process.execPath,
    ["--import", loopbackFixture, command, service],
    {
      env: {
        ...process.env,
        ...expectedEnvironment,
        ...environment,
        HEALTHCHECK_TEST_URL: url,
      },
...
for (const [service, path, port] of [
  ...
  test(`${service}: live HTTP command succeeds with exact readiness and Host`, async (t) => {
    ...
    const { url } = await serverFor(t, (request, response) => {
      requests += 1;
      assert.equal(
        request.headers["x-probe-url"],
        `http://127.0.0.1:${port}${path}`,
      );
      if (service === "mcp")
        assert.equal(request.headers.host, "mcp.example.invalid:8443");
...
test("safe failures reject HTTP, JSON, missing fields and wrong releases", async (t) => {
  ...
  const cases = [
    ...
    [
      ...
      JSON.stringify({
        ...ready("api"),
        release: { release: "v1", sourceSha: "3".repeat(40) },
      }),
      "source SHA mismatch",
    ],
  ];
  ...
  for (current of cases) {
    assert.deepEqual(await probe("api", url), {
      code: 1,
      stderr: `readiness: ${current[2]}\n`,
    });
  }
  assert.equal(
    requests,
    cases.length,
    "one captured response, no redirects or retries",
  );
  current = [200, JSON.stringify({ ...ready("web"), dependencies: {} })];
  assert.deepEqual(await probe("web", url), {
    code: 1,
    stderr: "readiness: invalid readiness report\n",
  });
});
...
test("invalid expected identity and MCP URL fail before any request", async (t) => {
  ...
  for (const name of ["PLATFORM_RELEASE_VERSION", "PLATFORM_SOURCE_SHA"]) {
    for (const value of [undefined, "", " ", "secret"]) {
      assert.deepEqual(await probe("api", url, { [name]: value }), {
        code: 1,
        stderr: "readiness: invalid expected release identity\n",
      });
    }
  }
  for (const value of [undefined, "", "secret", "file:///secret"]) {
    assert.deepEqual(await probe("mcp", url, { MCP_SERVER_URL: value }), {
      code: 1,
      stderr: "readiness: invalid MCP server URL\n",
    });
  }
...
test("connection refusal fails with a safe reason", async (t) => {
  ...
  assert.deepEqual(await probe("api", url), {
    code: 1,
    stderr: "readiness: connection failed\n",
  });
});
...
for (const sendHeaders of [false, true]) {
  test(`timeout bounds a stalled ${sendHeaders ? "body" : "response"}`, async (t) => {
    ...
    assert.deepEqual(await probe("api", url), {
      code: 1,
      stderr: "readiness: request timed out\n",
    });
    assert.ok(
      performance.now() - started < 4_500,
      "command exits before Docker's 5s timeout",
    );
  });
}
