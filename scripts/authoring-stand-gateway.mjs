...
const storedPatSchema = z.object({
  endpoint: z.string(),
  email: z.string(),
  userId: z.string(),
...
export function forwardedPath(host, url, origin, fetchSite) {
  ...
  if (
    normalized !== path ||
    !parsed.pathname.startsWith("/authoring/") ||
    /%2e|\/\//iu.test(path)
  )
    return null;
  return normalized;
...
export function tokenCache(exchange, now = () => Date.now()) {
  ...
  return async () => {
    if (
      current === undefined ||
      current.expiresAt - tokenRefreshMarginMs <= now()
    ) {
...
async function ownerPersonalAccessToken(settings, email, { renew }) {
  ...
  if (
    !renew &&
    stored?.endpoint === settings.LOGTO_ENDPOINT &&
    stored.email === email
  )
    return stored.value;
  const bootstrap = await import("./identity-proof-bootstrap.mjs");
  const secret = await bootstrap.retry(() =>
    Promise.resolve(bootstrap.readSeededManagementSecret()),
  );
  const api = bootstrap.createManagementApi(
    await bootstrap.fetchManagementAccessToken(secret),
  );
  const users = z
    .array(z.object({ id: z.string(), primaryEmail: z.string().nullable() }))
  ...
  if (existing.some((token) => token.name === patName))
    await api(
      `/users/${userId}/personal-access-tokens/${encodeURIComponent(patName)}`,
      { method: "DELETE" },
    );
  const created = z.object({ value: z.string().min(1) }).parse(
    await api(`/users/${userId}/personal-access-tokens`, {
      method: "POST",
      body: { name: patName },
    }),
  );
  await writePrivate(patPath, {
...
async function main() {
  ...
  if (!email)
    throw new Error(
      "Usage: pnpm authoring:stand-gateway --owner-email OWNER_EMAIL",
    );
  ...
  const settings = settingsSchema.parse(
    parseEnv(
      await readFile(
        ...
        throw new Error(
          "Start the stand with pnpm local:stand first: it configures the authoring client",
        );
      }),
    ),
  );
  ...
  const exchange = async (pat) => {
    const response = await fetch(`${settings.LOGTO_ENDPOINT}/oidc/token`, {
      ...
      body: new URLSearchParams({
        grant_type: exchangeType,
        subject_token: pat,
        subject_token_type: patType,
        resource: settings.LOGTO_AUDIENCE,
      }),
  ...
  const accessToken = tokenCache(async () => {
    ...
    if (!next.ok)
      throw new Error(
        `Stand token exchange failed: ${String(next.status)} ${next.detail}`,
      );
    return next.token;
  });
  const environment = await fetch(
    `${apiOrigin}/authoring/import/materials/environment`,
    { headers: { authorization: `Bearer ${await accessToken()}` } },
  );
  if (!environment.ok)
    throw new Error(
      `Stand API refused the owner token (${String(environment.status)}); run the owner bootstrap for ${email}`,
    );
  if (
    z.object({ mode: z.string() }).parse(await environment.json()).mode !==
    "development"
  )
    throw new Error("The authoring gateway serves only a development stand");
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      response.writeHead(502, { "content-type": "application/json" }).end(
        JSON.stringify({
          title: "Authoring gateway failure",
          detail: String(error),
        }),
      );
    }
  ...
  server.listen(Number(gateway.port), gateway.hostname, () => {
    process.stdout.write(
      `Authoring gateway for the stand: ${gateway.origin} (owner ${email}). Stop with Ctrl+C.\n`,
    );
  });
