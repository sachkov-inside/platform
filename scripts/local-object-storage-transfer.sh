...
if [[ -z "$project" ]]; then
  echo "docker compose config did not name the Compose project" >&2
  exit 1
fi
...
if docker container inspect "$transfer_container" >/dev/null 2>&1 ||
  docker volume inspect "$snapshot_volume" >/dev/null 2>&1; then
  echo "$transfer_container or $snapshot_volume is left from an interrupted run; remove it first" >&2
  exit 1
fi
...
cleanup() {
  if [[ "$created_container" == true ]]; then docker rm --force "$transfer_container" >/dev/null || true; fi
  if [[ "$created_snapshot" == true ]]; then docker volume rm "$snapshot_volume" >/dev/null || true; fi
  if [[ "$started_storage" == true ]]; then docker compose stop object-storage >/dev/null || true; fi
}
trap cleanup EXIT
...
timeout "$readiness_budget_seconds" bash -c 'until mc ready former >/dev/null 2>&1; do sleep 1; done'
...
manifest() {
  local alias="$1" bucket="$2" path stat size content_type metadata rest pairs checksum
  mc find "$alias/$bucket" | while IFS= read -r path; do
    ...
    metadata="$(printf '%s\n' "${pairs[@]}" | sort | paste -sd ' ')"
    checksum="$(mc cat "$path" | sha256sum | cut -d ' ' -f 1)"
    printf '%s\t%s\t%s\t%s\t%s\n' "${path#"$alias"/}" "$size" "$checksum" "$content_type" "$metadata"
  done | sort
}
...
for bucket in "${buckets[@]}"; do
  ...
  # Mirror never overwrites; an object it refuses is reported by the comparison below.
  mc mirror --quiet "former/$bucket" "rustfs/$bucket" >/dev/null ||
    echo "mc mirror reported errors for $bucket; comparing what arrived" >&2
  ...
  if [[ -n "$absent" ]]; then
    ...
    differing=$((differing + absent_count))
  fi
done

if ((differing > 0)); then
  echo "Transfer is incomplete: $differing objects are missing or different; the former volume is unchanged" >&2
  exit 1
fi
