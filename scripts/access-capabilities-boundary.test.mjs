...
const checker = fileURLToPath(
  new URL("check-access-capabilities-boundary.mjs", import.meta.url),
);
...
test("a second copy of the vocabulary fails, wherever an application declares it", () => {
  ...
  assert.notEqual(
    result.status,
    0,
    "the checker accepted a second copy of the vocabulary",
  );
  assert.match(
    result.output,
    /accessComposition belongs to @inside\/access-capabilities/u,
  );
  assert.match(
    result.output,
