...
if (negative !== undefined && !["unknown_period", "nonpaid"].includes(negative)) throw new Error("Unsupported negative fixture");
...
try {
  ...
  let tier = tiers.items.find(row => row.availableForAssignment && !row.archived && row.tier.benefits.includes("materials"));
  if (!tier) throw new Error("Prepared local tier required");
  const tierId = randomUUID();
  await command("offers/save", { operationId: randomUUID(), value: { id: tierId, name: `Tribute acceptance ${String(user)}`,
    benefits: tier.tier.benefits, contentScope: tier.tier.contentScope, availableForAssignment: true } });
  tier = { ...tier, tier: { ...tier.tier, id: tierId, revision: 1, name: `Tribute acceptance ${String(user)}` } };
  ...
  await buyer.request.post(`${telegram}/proof/source`, { data: { user: String(user), source: negative ? "member" : "left", community: "not_member" } });
  ...
  const preview = await command("tribute/preview", { operationId: randomUUID(), batchRef: `batch-${String(user)}`, rows: [{ ...row, ...(negative ? { endsAt: null } : {}) }] });
  expect(preview.value.rows[0].status).toBe(negative ? "unknown_term" : "pending_identity");
  ...
  if (negative === "nonpaid") {
    const trial = { name: "new_subscription", created_at: new Date().toISOString(), sent_at: new Date().toISOString(), payload: {
      subscription_name: "Synthetic trial", subscription_id: user, period_id: 1, period: "monthly", price: 10000, amount: 0, currency: "rub",
      user_id: user, trb_user_id: `T-${String(user)}`, telegram_user_id: user, channel_id: 625, channel_name: "Synthetic", expires_at: endsAt, type: "trial" } };
    const raw = JSON.stringify(trial);
    const response = await buyer.request.post("http://127.0.0.1:3601/integrations/tribute/v1/webhook", { headers: { "content-type": "application/json", "trbt-signature": createHmac("sha256", process.env.TRIBUTE_API_KEY).update(raw).digest("hex") }, data: raw });
    expect(response.status()).toBe(200); expect((await response.json()).status).toBe("pending_reconciliation");
  }
  ...
  if (negative) {
    await expect.poll(async () => (await sql.query("SELECT count(*)::int AS count FROM membership_entitlements.access_receipts WHERE scope='source-evidence' AND payload->>'identityRef'=$1 AND payload->>'decision'='registry_lookup'", [identityRef])).rows[0].count, { timeout: 90000 }).toBeGreaterThan(0);
    expect(await ownEnrollments()).toHaveLength(0);
    expect((await sql.query("SELECT count(*)::int AS count FROM billing.purchases WHERE account_id IN (SELECT account_id FROM membership_entitlements.activation_attempts WHERE identity_ref=$1)", [identityRef])).rows[0].count).toBe(0);
    await send("/access");
    await expect.poll(async () => (await messages()).some(message => /проверк|подтвержд/iu.test(message.text))).toBe(true);
    await page.goto(`${web}/account/subscription`);
    await page.screenshot({ path: resolve(output, "tribute-pending.png"), fullPage: true });
    report.evidence.push("tribute-pending.png");
    report.scenarios.push(`PASS actual registry activation with source member and ${negative} creates no Enrollment or billing charge`);
  } else {
  ...
  report.scenarios.push("PASS Tribute registry grant opens the protected included material through real Reader/BFF/Platform");
  const includedGuide = tier.tier.contentScope?.guideIds[0];
  if (!includedGuide) throw new Error("Acceptance tier must name an included Guide");
  const metadata = (await sql.query("SELECT topic_id FROM materials.materials WHERE slug=$1", ["developer-pipeline-bez-poteri-konteksta"])).rows[0];
  const materialFields = { access: "membership", difficulty: "unassigned", formatId: "guide", topicId: metadata.topic_id,
    title: `Tribute625 new included material ${String(user)}`, summary: "Synthetic new material in the already included Guide",
    seriesIds: JSON.stringify([includedGuide]), document: JSON.stringify({ type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "New Guide material after the original confirmed enrollment." }] }] }) };
  const createdResponse = await owner.request.post(`${web}/api/authoring/materials`, { headers: { origin: web }, multipart: { ...materialFields, submissionId: randomUUID() } });
  expect(createdResponse.status()).toBe(200);
  const created = await createdResponse.json(); expect(created.kind).toBe("created");
  const publishedResponse = await owner.request.put(`${web}/api/authoring/materials`, { headers: { origin: web }, multipart: {
    ...materialFields, submissionId: randomUUID(), materialId: created.draft.materialId,
    expectedContentVersion: String(created.draft.contentVersion), publicationState: "published" } });
  expect(publishedResponse.status()).toBe(200); expect((await publishedResponse.json()).kind).toBe("saved");
  const material = (await sql.query("SELECT slug FROM materials.materials WHERE id=$1", [created.draft.materialId])).rows[0];
  await page.goto(`${web}/materials/${material.slug}`);
  await expect(page.locator("[data-reader-body]:visible")).toBeVisible();
  expect((await ownEnrollments()).find(item => item.id === first.id)).toEqual(first);
  report.scenarios.push("PASS newly published material inside the included Guide opens without replacing or extending the existing Enrollment");
  const secondHolder = (await (await owner.request.get(`${web}/auth/status`)).json()).accountId;
  const unselected = await command("enrollments/assign", { operationId: randomUUID(), accountId: secondHolder, origin: "manual",
    sourceRef: `unselected625-${String(user)}`, tierId, tierRevision: 1,
    terms: { startsAt, endsAt, endPolicy: "fixed" }, billingRef: null, reason: "Synthetic unselected cohort comparator" });
  const secondBefore = (await command("enrollments/list", { operationId: randomUUID(), accountId: secondHolder })).items.find(item => item.id === unselected.value.id);
  const guideResponse = await owner.request.post(`${web}/api/authoring/collections`, { headers: { origin: web }, multipart: {
    kind: "series", name: `New Guide ${String(user)}`, slug: `new-guide-${String(user)}`, summary: "Synthetic catalog expansion acceptance" } });
  expect(guideResponse.status()).toBe(200); const newGuide = (await guideResponse.json()).collection;
  const newFields = { ...materialFields, title: `New excluded Guide material ${String(user)}`, seriesIds: JSON.stringify([newGuide.id]) };
  const newDraftResponse = await owner.request.post(`${web}/api/authoring/materials`, { headers: { origin: web }, multipart: { ...newFields, submissionId: randomUUID() } });
  expect(newDraftResponse.status()).toBe(200); const newDraft = (await newDraftResponse.json()).draft;
  const newPublished = await owner.request.put(`${web}/api/authoring/materials`, { headers: { origin: web }, multipart: { ...newFields,
    submissionId: randomUUID(), materialId: newDraft.materialId, expectedContentVersion: String(newDraft.contentVersion), publicationState: "published" } });
  expect(newPublished.status()).toBe(200); expect((await newPublished.json()).kind).toBe("saved");
  const newMaterial = (await sql.query("SELECT slug FROM materials.materials WHERE id=$1", [newDraft.materialId])).rows[0];
  await page.goto(`${web}/materials/${newMaterial.slug}`);
  await expect(page.locator('[data-material-reader-state="access-required"]:visible')).toBeVisible();
  await command("offers/save", { operationId: randomUUID(), expectedRevision: 1, value: { id: tierId, name: "Changed next cohort",
    benefits: tier.tier.benefits, contentScope: { ...tier.tier.contentScope, guideIds: [...tier.tier.contentScope.guideIds, newGuide.id] }, availableForAssignment: true } });
  expect((await ownEnrollments()).find(item => item.id === first.id)).toEqual(first);
  await page.reload(); await expect(page.locator('[data-material-reader-state="access-required"]:visible')).toBeVisible();
  const expansion = await command("enrollments/preview-expansion", { operationId: randomUUID(), tierId, tierRevision: 2,
    targets: [{ enrollmentId: first.id, expectedRevision: first.revision, tierRevision: 1 }], reason: "Synthetic explicitly selected existing cohort" });
  await command("enrollments/apply-expansion", { operationId: randomUUID(), previewRef: expansion.value.previewRef });
  const secondAfter = await command("enrollments/list", { operationId: randomUUID(), accountId: secondHolder });
  expect(secondAfter.items.find(item => item.id === unselected.value.id)).toEqual(secondBefore);
  const expanded = (await ownEnrollments()).find(item => item.id === first.id);
  expect(expanded).toMatchObject({ id: first.id, startsAt: first.startsAt, endsAt: first.endsAt });
  await page.reload(); await expect(page.locator("[data-reader-body]:visible")).toBeVisible();
  report.scenarios.push("PASS new Guide stays excluded after Offer edit; explicit selected-cohort expansion opens it without changing dates or the unselected holder");
  await send("/access");
  await expect.poll(async () => (await messages()).some(message => message.text.includes("Tribute"))).toBe(true);
  let invite;
  await expect.poll(async () => {
    await send("/community");
    invite = (await messages()).map(message => /https:\/\/t\.me\/\+\S+/u.exec(message.text)?.[0]).find(Boolean);
    return Boolean(invite);
  }, { timeout: 150000, intervals: [5000] }).toBe(true);
  await update({ chat_join_request: { chat: { id: -1000000000000, type: "supergroup" }, from, user_chat_id: user,
    date: Math.floor(Date.now()/1000), invite_link: { invite_link: invite, creator: { id: 1234, is_bot: true, first_name: "SyntheticBot" }, creates_join_request: true, is_primary: false, is_revoked: false } } });
  await expect.poll(async () => (await (await buyer.request.get(`${telegram}/proof/state`)).json()).members[String(user)], { timeout: 30000 }).toBe("member");
  report.scenarios.push("PASS finite Tribute source authorizes intended community join through actual consumer and Platform dispatch permit");
  await send(`/start a_${report.code}`);
  ...
  const status = await command("tribute/status", { operationId: randomUUID(), page: 0 });
  const source = status.value.sources.find(item => item.identityRef === identityRef);
  expect(source).toBeDefined();
  const revoked = await command("tribute/reconcile", { operationId: randomUUID(), sourceId: source.id,
    expectedRevision: source.revision, action: "revoke", reason: "Synthetic exact-source revoke acceptance" });
  expect((await ownEnrollments()).find(item => item.id === first.id).state).toBe("revoked");
  await send(`/start a_${report.code}`);
  const blockedPreview = await command("tribute/preview", { operationId: randomUUID(), batchRef: `revoked-${String(user)}`,
    rows: [{ ...row, checkedAt: new Date().toISOString(), endsAt: expiresAt, expectedRevision: revoked.value.revision }] });
  expect(blockedPreview.value.rows[0].status).toBe("conflict");
  const afterRevoke = { ...event, created_at: new Date().toISOString(), sent_at: new Date().toISOString() };
  expect((await tributeEvent(afterRevoke)).status).toBe("pending_reconciliation");
  expect((await ownEnrollments()).find(item => item.id === first.id).state).toBe("revoked");
  const restored = await command("tribute/reconcile", { operationId: randomUUID(), sourceId: source.id,
    expectedRevision: revoked.value.revision, action: "restore", reason: "Synthetic explicit restoration after verified paid period",
    confirmedTerms: { startsAt, endsAt: expiresAt, verificationRef: `restore-${String(user)}` } });
  expect(restored.value.id).toBe(source.id);
  expect((await ownEnrollments()).filter(item => item.origin === "tribute")).toHaveLength(1);
  expect((await ownEnrollments()).find(item => item.id === first.id)).toMatchObject({ state: "active", endsAt: expiresAt });
  report.scenarios.push("PASS actual owner revoke survives bot start/import/signed renewal; explicit restore keeps same source and Enrollment");
  await ownerPage.reload();
  const columns = ["rowRef", "policyRef", "subscriptionId", "identityRef", "telegramUserId", "verificationRef", "checkedAt", "mode", "startsAt", "endsAt", "renewal", "expectedRevision", "reason"];
  const csvRow = { ...row, rowRef: `ui-${String(user)}`, endsAt: expiresAt, renewal: "stopped", checkedAt: new Date().toISOString(), expectedRevision: restored.value.revision };
  await ownerPage.getByLabel("Название сверки", { exact: true }).fill(`ui-recovery-${String(user)}`);
  await ownerPage.getByLabel("Содержимое реестра", { exact: true }).fill([columns.join(","), columns.map(key => csvRow[key]).join(",")].join("\n"));
  await ownerPage.getByRole("button", { name: "Проверить без применения", exact: true }).click();
  await expect(ownerPage.getByRole("heading", { name: "3. Проверить и применить выбранное" })).toBeVisible();
  await ownerPage.getByRole("checkbox", { name: new RegExp(`ui-${String(user)}`, "u") }).check();
  await ownerPage.route("**/api/authoring/billing/tribute/apply", async route => {
    const committed = await route.fetch(); expect(committed.status()).toBe(200);
    await route.abort("failed");
  }, { times: 1 });
  await ownerPage.getByRole("button", { name: "Применить выбранные строки", exact: true }).click();
  await expect(ownerPage.getByRole("button", { name: "Восстановить результат применения", exact: true })).toBeEnabled();
  await ownerPage.getByRole("button", { name: "Проверить без применения", exact: true }).click();
  await expect(ownerPage.getByText("Сначала восстановите результат предыдущего применения.", { exact: true })).toBeVisible();
  await ownerPage.getByRole("button", { name: "Восстановить результат применения", exact: true }).click();
  await expect(ownerPage.getByText(/Импорт применён\. Строк: 1/u)).toBeVisible();
  expect((await ownEnrollments()).filter(item => item.origin === "tribute")).toHaveLength(1);
  report.scenarios.push("PASS owner CSV preview/select/apply commits through real BFF and restores lost HTTP response without replacing command");
